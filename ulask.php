...
if (mb_strpos($_SERVER['SCRIPT_NAME'], "ulask.php")  !== FALSE) {
...
function hae_skannattu_lasku($kasittele_seuraava = "") {
  ...
  foreach ($files as $file) {
    ...
    mysqli_data_seek($kesken_chk_res, 0);

    while ($kesken_chk_row = mysqli_fetch_assoc($kesken_chk_res)) {
    ...
    if ($kasittele_seuraava != "") {
      ...
      $file_basename = mb_substr($timestamp.mb_substr(($micro * 10), 0, 1), 2);
...
if ($tee == 'VIIVA') {
  ...
  if (mb_strlen($nimi) == 54 and ( mb_substr($nimi, 0, 1) == '2' or mb_substr($nimi, 0, 1) == '4' or mb_substr($nimi, 0, 1) == '5') ) {
    $versio = mb_substr($nimi, 0, 1);
    $tee2  = "";

    if ($versio == '2') {
      $tilino = mb_substr($nimi, 1, 14);
      $summa  = mb_substr($nimi, 15, 8) / 100;
      $viite  = ltrim(mb_substr($nimi, 23, 20), "0"); // etunollat pois
      $erv    = mb_substr($nimi, 43, 2);
      $erk    = mb_substr($nimi, 45, 2);
      $erp    = mb_substr($nimi, 47, 2);
    }
    elseif ($versio == '4') {
      $tilino  = mb_substr($nimi, 1, 16);
      $summa  = mb_substr($nimi, 17, 8) / 100;
      $viite   = ltrim(mb_substr($nimi, 28, 20), "0"); // etunollat pois
      $erv   = mb_substr($nimi, 48, 2);
      $erk   = mb_substr($nimi, 50, 2);
      $erp   = mb_substr($nimi, 52, 2);
    }
    elseif ($versio == '5') {
      $tilino  = mb_substr($nimi, 1, 16);
      $summa  = mb_substr($nimi, 17, 8) / 100;
      $viite   = "RF".mb_substr($nimi, 25, 2).ltrim(mb_substr($nimi, 27, 21), "0"); // Tarkistenumeron jälkeen tulevat täytenollat pois
      $erv   = mb_substr($nimi, 48, 2);
      $erk   = mb_substr($nimi, 50, 2);
      $erp   = mb_substr($nimi, 52, 2);
    }
...
if ($tee == 'I') {
  ...
  for ($k=1; $k<=3; $k++) {
    if (is_uploaded_file($_FILES['userfile'.$k]['tmp_name'])) {
      ...
      if (mb_strtoupper($ext) == "JPEG") $ext = "jpg";

      // extensio pitää olla oikein
      if (mb_strtoupper($ext) != "JPG" and mb_strtoupper($ext) != "PNG" and mb_strtoupper($ext) != "GIF" and mb_strtoupper($ext) != "PDF") {
      ...
      $result = pupe_query($query);
      $varirow = mysqli_fetch_row($result);
  ...
  if (isset($toitilinumero)) {
    ...
    $toitilinumero = trim(mb_strtoupper($toitilinumero));
    $toiswift = trim(mb_strtoupper($toiswift));

    if ($trow['ultilno_maa'] != "") $ibanmaa = $trow['ultilno_maa'];
    else $ibanmaa = $trow['maa'];

    // Kotimainen tilinumero
    if (isset($toitilinumero) and mb_strtoupper($trow['maa']) == 'FI') {
  ...
  if (mb_strlen($viite) == 0 and mb_strlen($viesti) == 0 and mb_strlen($toimittajan_laskunumero) == 0) {
    $errormsg .= "<font class='error'>".t("Anna viite, viesti tai laskunumero")."</font><br>";
    $tee = 'E';
  }

  if (mb_strlen($viite) > 0 and mb_substr($viite, 0, 2) != "RF" and tarkista_viite($viite) === FALSE) {
    $errormsg .= "<font class='error'>".t("Viite on väärin")."</font><br>";
    $tee = 'E';
  }

  if (mb_strlen($viite) > 0 and mb_substr($viite, 0, 2) == "RF" and tarkista_rfviite($viite) === FALSE) {
    $errormsg .= "<font class='error'>".t("RF-Viite on väärin")."</font><br>";
    $tee = 'E';
  }

  if (mb_strlen($viite) > 0 and mb_strlen($viesti) > 0) {
    $errormsg .= "<font class='error'>".t("Viitettä ja viestiä ei voi antaa yhtaikaa")."</font><br>";
    $tee = 'E';
  }

  // Tällöin ei tarvitse erikseen syöttää summaa
  if ($maara == 2 and mb_strlen($isumma[1]) == 0) {
    $isumma[1] = $summa;
  }
  ...
  if ((is_array($trow) and mb_strtoupper($trow['maa']) == mb_strtoupper($yhtiorow['maa'])) or (!is_array($trow) and $tyyppi == mb_strtoupper($yhtiorow['maa']))) {
    $ohjeitapankille = '';
  }
  else {
    if (mb_strlen($ohjeitapankille) > 350) {
  ...
  for ($i = 1; $i < $maara; $i++) {
    ...
    if (mb_strlen($itili[$i]) > 0) {
      ...
      $ivirhe[$i]  = $virhe;
      $iulos[$i]    = $ulos;
      if (!isset($yleissumma)) $yleissumma = (float)$turvasumma; // MUOKKAUS: isset()
  ...
  if (mb_strlen($trow['ytunnus']) == 0) {
    $errormsg .= "<font class='error'>".t("Ytunnus puuttuu")."</font><br>";
    $tee = 'E';
  }

  if (mb_strlen($trow['nimi']) == 0) {
  ...
  elseif (mb_strtoupper($yhtiorow['maa']) == 'SE' and $trow['tilinumero'] != '') {
  ...
  if ($trow['ultilno'] != "") {
    ...
    // Vaaditaan isot kirjaimet
    $trow['ultilno'] = mb_strtoupper($trow['ultilno']);
    $trow['swift']   = mb_strtoupper($trow['swift']);
...
if ($tee == 'P' or $tee == 'E') {
  ...
  $tarkrow = mysqli_fetch_assoc($tarkres);
  ...
  if ($toimittajaid > 0) {
    ...
    if ($tee == 'P') {
      ...
      for ($tuuraaja_i = 1; $tuuraaja_i < 6; $tuuraaja_i++) {
        $query = "SELECT if (tuuraaja != '', tuuraaja, kuka) kuka
                  ...
                  AND kuka    != ''";
        $result = pupe_query($query);
        $hyvak_row = mysqli_fetch_assoc($result);
    ...
    if ($errormsg != '') {
      ...
      if (mysqli_num_rows($vikatlaskutres) > 0) {
        ...
        while ($vikatlaskutrow = mysqli_fetch_assoc($vikatlaskutres)) {
          echo "<tr>";
    ...
    if ($tyyppi != mb_strtoupper($yhtiorow['maa'])) { // MUOKKAUS: Konvertoidaan postitp uppercase
      ...
      while ($vrow = mysqli_fetch_assoc($vresult)) {
        $sel = "";
        if (mb_strtoupper($trow["maa"]) == mb_strtoupper($vrow["koodi"])) {
          $sel = "selected";
        }
        echo "<option value = '".mb_strtoupper($vrow["koodi"])."' $sel>".t($vrow["nimi"])."</option>";
      }
  ...
  if (mysqli_num_rows($alhire) >= 1) {
    ...
    while ($vrow = mysqli_fetch_assoc($alhire)) {
      $sel = "";
  ...
  if (laskun_hyvaksyjia()) {
    ...
    for ($i=1; $i<6; $i++) {
      ...
      if (!mysqli_data_seek($vresult, 0)) {
        echo "mysqli_data_seek failed!";
        exit;
      }
  ...
  // Hoidetaan oletukset!
  for ($i = 1; $i < 50; $i++) {
    if ($i == 1 and mb_strlen($itili[$i]) == 0) {
      $itili[$i] = $oltil;
    }
    if ($i == 1 and mb_strlen($ikohde[$i]) == 0) {
      $ikohde[$i] = $olkohde;
    }
    if ($i == 1 and mb_strlen($iprojekti[$i]) == 0) {
      $iprojekti[$i] = $olprojekti;
    }
    if ($i == 1 and mb_strlen($ikustp[$i]) == 0) {
      $ikustp[$i] = $olkustp;
    }
    if (mb_strlen($ivero[$i]) == 0) {
      if (mb_strtoupper($trow['maa']) == mb_strtoupper($yhtiorow['maa'])) {
...
if ($tee == 'I') {
  ...
  if ($yhtiorow["ostolaskujen_kurssipaiva"] == 0 or mb_strtoupper($yhtiorow["valkoodi"]) == mb_strtoupper($valkoodi)) {
  ...
  if (mb_strlen($hyvak[5]) > 0) {
    $hyvaksyja_nyt=$hyvak[5];
    $tila = "H";
  }
  if (mb_strlen($hyvak[4]) > 0) {
    $hyvaksyja_nyt=$hyvak[4];
    $tila = "H";
  }
  if (mb_strlen($hyvak[3]) > 0) {
    $hyvaksyja_nyt=$hyvak[3];
    $tila = "H";
  }
  if (mb_strlen($hyvak[2]) > 0) {
    $hyvaksyja_nyt=$hyvak[2];
    $tila = "H";
  }
  if (mb_strlen($hyvak[1]) > 0) {
  ...
  if ($kpexport == 1 or mb_strtoupper($yhtiorow['maa']) != 'FI') {
    ...
    $tositenrorow = mysqli_fetch_assoc($tresult);
  ...
  if ($kuva1 or $kuva2 or $kuva3) {
    ...
    $tunnarit = mb_substr($tunnarit, 0, -1);
  ...
  if ($kopioi > 0) {
    ...
    if (mysqli_num_rows($result) != 1) {
      ...
      if (mysqli_num_rows($tilresult) == 0) {
      ...
      for ($kopio=1; $kopio<=$kopioi; $kopio++) {
        ...
        while ($tiliointirow = mysqli_fetch_assoc($tilresult)) {

          $query = "INSERT INTO tiliointi SET ";

          for ($i=0; $i<mysqli_num_fields($tilresult); $i++) {

            if (mysqli_field_name($tilresult, $i) != 'tunnus') {

              if (mysqli_field_name($tilresult, $i) == 'tapvm') {
                $query .= "tapvm ='$kopiotpv-$kopiotpk-$kopiotpp',";
              }
              elseif (mysqli_field_name($tilresult, $i) == 'ltunnus') {
                $query .= "ltunnus ='$kopiotunnus',";
              }
              elseif (mysqli_field_name($tilresult, $i) == 'aputunnus') {
                if ($tiliointirow['aputunnus'] != 0) {
                  $query .= "aputunnus ='$kopiotiltunnus',";
                }
              }
              else {
                $query .= mysqli_field_name($tilresult, $i) . "='" . $tiliointirow[mysqli_field_name($tilresult, $i)] . "',";
              }
            }
          }
...
if ($tee == "") {
  ...
  echo "<td>
    <select name='tyyppi'>
    <option value =".mb_strtoupper($yhtiorow['maa']).">".t("Kotimaa")."
