...
function ftp_rmfiles($ftp_stream, $directory, $nodel = "", $nodelpict = "") {
  ...
  foreach ($list as $current) {
    ...
    if ($statusnext === true) {
      $currentfolder = mb_substr($current, 0, -1);
      $statusnext = false;
      $statusprev = true;
      continue;
    }
  ...
  foreach ($files as $file) {
    ...
    if (mb_stripos($file, "eipoisteta") === FALSE) {
      if ($nodelpict != '') {
        if (mb_strpos($file, $nodelpict) === FALSE) {
...
if ($tee == "aja") {
  ...
  if ($ftpkuvahost=='' or $ftpkuvauser=='' or $ftpkuvapass=='' or $ftpkuvapath=='' or $ftpmuupath == '') {
    ...
    if (mb_substr($ftpkuvapath, -1) != "/") {
      $ftpkuvapath .= "/";
    }

    if (mb_substr($ftpmuupath, -1) != "/") {
      $ftpmuupath .= "/";
    }
