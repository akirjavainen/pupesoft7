...
class import {
    ...
    if (isset($ob['value'])) {
      ...
      foreach ($ob as $key => $value) {
        if ($key !== 'itype') {
          ...
          $new[mb_substr($key, 1)] = $value;
        }
      }
      $r = $this->pdf->add_raw_object($new);
    }
    echo "Object $r is now colorspace $name<br>\n";
    $this->pdf->builddata['colorspaces'][mb_substr($name, 1)] = $r;
  }
  ...
  function extract_font($oldname, $ob) {
    /* $r is the library id once the font is created.  This ID # is
         ...
         */
    $r = $this->pdf->new_font(mb_substr($ob['/BaseFont'], 1));
  ...
  function extract_obj($id) {
    ...
    if (isset($temp['stream']) && $temp['stream']->decompressed) {
      //            $temp['stream']->d = '[STRIPPED]'; // kommentoin t\E4m\E4n, koska php 5:ll\E4 joudutaan t\E4h\E4n haaraan joka tuhoaa pdf importin. jos t\E4t\E4 ei tehd\E4 niin homma ok. go figure! -joni
    }
    print_r($temp);
    echo "</pre>\n";
  }
  ...
  function extract_dictionary() {
    ...
    while (true) {
      ...
      if (mb_substr($d->d, $d->l, 1) == '[') {
        echo "Analyzing array '" . mb_substr($d->d, $d->l, 15) .
          "...' for [$l]<br>\n";
        $r[$l] = $d->get_array();
        $label = false;
        continue;
  ...
  function extract_stream($meta) {
    ...
    if ($t == 'stream') {
      ...
      $stream = mb_substr($d->d, $start, $length);
      $d->l += 1 + $length;
...
class StreamHandler {
  ...
  public function __construct($stream, $decompress) {
    $this->l = 0;
    ...
    $this->d .= "\n";
    $this->length = mb_strlen($this->d);
  }

  public function StreamHandler($stream, $decompress) {
    self::__construct($stream, $decompress);
  }
  ...
  function get_array() {
    ...
    if (count($as) > 0 && mb_strlen(trim($as[0])) > 0) {
      $r = array_merge($r, $as);
    }
    return $r;
  }
  ...
  function skip_whitespace($forward = true) {
    while ($this->is_whitespace()) {
      ...
      if ($this->l == 0 || $this->l == (mb_strlen($this->d) - 1)) break;
    }
    return $this->l;
  }
