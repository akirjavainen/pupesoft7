...
if (isset($tuoteno_array)) {
  ...
  $muutparametrit = mb_substr($muutparametrit, 0, -1); // vika pilkku pois
...
if ($tee == "selaa" and isset($ehdotusnappi)) {
  ...
  if (!empty($varastot)) {
    ...
    $lisa_varastot = mb_substr($lisa_varastot, 0, -1); // vika pilkku pois
  ...
  $result = pupe_query($query);
  $erorow = mysqli_fetch_array($result);
  ...
  while ($row = mysqli_fetch_array($res)) {
    ...
    $result   = pupe_query($query);
    $summarow = mysqli_fetch_array($result);
    ...
    if ($maittain != "") {
      ...
      echo "<td align='right'>";
      while ($maarow = mysqli_fetch_array($result)) {
...
if ($tee == "" or !isset($ehdotusnappi)) {
  ...
  while ($varow = mysqli_fetch_array($vtresult)) {
    $sel = '';
