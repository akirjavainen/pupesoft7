...
if (isset($tee_lataa)) {
  ...
  if ($tee != '' and $asiakas > 0 and isset($ajahinnasto)) {
    ...
    $kurssi = mysqli_fetch_assoc($asres);
    ...
    if (mysqli_num_rows($rresult) == 0) {
      ...
      $_asiakashinnasto_row = mysqli_fetch_assoc($_asiakashinnasto_res);
      ...
      $bar = new ProgressBar();
      $elements = mysqli_num_rows($rresult); // total number of elements to process
      ...
      foreach ($rivit as $rrow) {
        ...
        if (isset($GLOBALS['eta_yhtio']) and $GLOBALS['eta_yhtio'] != '' and $GLOBALS['koti_yhtio'] == $kukarow['yhtio']) {
          ...
          $alehinrrow = mysqli_fetch_assoc($tres_eta);
