...
if ($tee == "SYNK") {
  ...
  if (mb_substr($table, 0, 9) == "avainsana") {
    if (mb_strpos($yhtiorow["synkronoi"], mb_substr($table, 0, 9)) === false) {
      echo "VIRHE: Pyydettyä taulua $table ei voida synkronoida, sitä ei ole määritelty!";
      exit;
    }

    $table = mb_substr($table, 0, 9);
    ...
    if ($regs[3] != "") {
      ...
      $lajit = mb_substr($lajit, 0, -1);

      $lajit .= ")";
    }
  }
  else {
    if (mb_strpos($yhtiorow["synkronoi"], $table) === false or $table == "") {
      echo "VIRHE: Pyydettyä taulua $table ei voida synkronoida, sitä ei ole määritelty!";
      exit;
    }
    $lajit = "";
  ...
  $kohderow = mysqli_fetch_array($kohderes);

  if (mb_strlen($kohderow["yhtiot"]) == 0) {
  ...
  foreach ($indeksi as $pakollinen) {
    $group .= mb_strtolower($pakollinen).",";
  }

  $group = mb_substr($group, 0, -1);
  ...
  while ($aburow = mysqli_fetch_array($abures)) {
    ...
    while ($aburow1 = mysqli_fetch_assoc($abures1)) {
