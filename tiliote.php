...
$forceta = $php_cli; // MUOKKAUS: salli CLI-interface
...
if ($ok == 1) {
  ...
  if (mb_substr($tietue, 0, 9) == "<SOAP-ENV" or mb_substr($tietue, 0, 5) == "<?xml" or mb_substr($tietue, 0, 9) == "<Invoice>") {
    // Finvoice verkkolasku
    fclose($fd);

    require "verkkolasku-in.php" ;
  }
  elseif (mb_substr($tietue, 5, 12) == "Tilivaluutan") {
    // luetaanko kursseja

    lue_kurssit($filenimi, $fd);
    fclose($fd);
  }
  elseif (mb_substr($tietue, 0, 7) == "VK01000") {
