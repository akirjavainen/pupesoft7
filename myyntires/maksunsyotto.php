...
if ($tee == "SYOTTO") {
  ...
  if (mb_substr($ytunnus, 0, 1) == "£") {
    $query = "SELECT nimi, liitostunnus
              FROM lasku
              WHERE ytunnus = '".mb_substr($ytunnus, 1)."'
  ...
  if ($row = mysqli_fetch_assoc($result)) {
    ...
    $asiakasstr = mb_substr($row["nimi"], 0, 12);
  ...
  $query = "INSERT into lasku
            ...
            luontiaika = now()";
  $result = pupe_query($query);
  $ltunnus = mysqli_insert_id($GLOBALS["masterlink"]);
  ...
  $result = pupe_query($query);
  $ttunnus = mysqli_insert_id($GLOBALS["masterlink"]);
  ...
  $query = "INSERT INTO suoritus SET
            ...
            kurssi         = '$tilikurssi'";
  $result = pupe_query($query);
  $suoritus_tunnus = mysqli_insert_id($GLOBALS["masterlink"]);
...
if (!empty($viite_haku) and $tee == "ETSI") {
  ...
  if ($asiakas = mysqli_fetch_assoc($result)) {
...
if (!empty($asiakasid) and $tee == "ETSI") {
  ...
  if ($ytunnus == "" and mb_stripos($_SERVER['HTTP_USER_AGENT'], "MSIE") !== FALSE or mb_stripos($_SERVER['HTTP_USER_AGENT'], "EXPLORER") !== FALSE or !is_numeric($asiakasid)) {
    $ytunnus = $asiakasid;
    $asiakasid = "";
  }
...
if ($ytunnus != '' and $tee == "") {
  ...
  while ($apurow = mysqli_fetch_assoc($kustpvr)) {
