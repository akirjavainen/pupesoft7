...
if (mb_strpos($_SERVER['SCRIPT_NAME'], "manuaalinen_suoritusten_kohdistus")  !== FALSE) {
...
if ($tila == "vaihdasuorituksentili") {
  ...
  if (mysqli_num_rows($result) == 1) {
    ...
    $suoritus = mysqli_fetch_assoc($result);
    ...
    if (mysqli_affected_rows($link) == 0 or $vaihdasuorituksentiliointitunnuksia != "") {
      ...
      if ($rivi = mysqli_fetch_assoc($result)) {
        ...
        if (mysqli_num_rows($result) == 0) {
          ...
          while ($tilioinnit = mysqli_fetch_assoc($result)) {
            echo "<tr>";
            $chk="";
...
if ($tila == 'tee_kohdistus') {
  ...
  if (mysqli_num_rows($result) == 0) {
    ...
    if ($osasuoritus == 1) {
      ...
      if (mb_strtoupper($suoritus['valkoodi']) != mb_strtoupper($yhtiorow['valkoodi'])) {
      ...
      $jarow = mysqli_fetch_assoc($jaresult);
    ...
    $errorrow = mysqli_fetch_assoc($result);
    ...
    if ($osasuoritus != 1 and $pyoristys_virhe_ok != 1) {
      ...
      if ($laskutunnukset != 0 or $laskutunnuksetkale != 0) {
        ...
        $tskrow = mysqli_fetch_assoc($tskres);
...
if ($tila == 'tee_kohdistus') {
  ...
  $query = "SELECT *
            ...
            and korjattu    = ''";
  $result = pupe_query($query);
  $apurow = mysqli_fetch_assoc($result);
  ...
  if ($osasuoritus == 1) {
    ...
    if (mb_strtoupper($suoritus["valkoodi"]) != mb_strtoupper($yhtiorow['valkoodi'])) $suoritussumma = round($suoritussummaval * $suoritus["kurssi"], 2);
    ...
    $korko_row = mysqli_fetch_assoc($result_korko);
    ...
    if (mb_strtoupper($suoritus["valkoodi"]) != mb_strtoupper($yhtiorow['valkoodi'])) $suoritussumma = round($suoritussummaval * $lasku["vienti_kurssi"], 2);
    ...
    if (mb_strtoupper($suoritus["valkoodi"]) != mb_strtoupper($yhtiorow['valkoodi'])) {
      ...
      if (abs($valuuttaero) >= 0.01) {
        ...
        if ($lasku["yhtio_toimipaikka"] != '') {
          ...
          $ytprow = mysqli_fetch_assoc($ytpres);
    ...
    $tiliointi1 = mysqli_fetch_assoc($result);
    ...
    $tiliointi2 = mysqli_fetch_assoc($result);
    ...
    if ($laskutunnukset != 0) {
      ...
      if (mysqli_num_rows($result) != count($lasku_tunnukset)) {
        echo "<font class='error'>".t("Joku laskuista katosi (joku maksoi sen sinua ennen?)")." '".mysqli_num_rows($result)."' '".count($lasku_tunnukset)."'</font><br>";
      ...
      while ($lasku = mysqli_fetch_assoc($result)) {
        $laskut[] = $lasku;
    ...
    if ($laskutunnuksetkale != 0) {
      ...
      if (mysqli_num_rows($result) != count($lasku_tunnukset_kale)) {
        echo "<font class='error'>".t("Joku laskuista katosi (joku maksoi sen sinua ennen?)")." '".mysqli_num_rows($result)."' '".count($lasku_tunnukset_kale)."'</font><br>";
        exit;
      }
    ...
    if ($kaatosumma != 0 and $pyoristys_virhe_ok == 1) {
      ...
      if (mysqli_num_rows($result) == 0) {
        ...
        $kohdistuslasku = mysqli_fetch_assoc($result);
      }
    }
    ...
    if (is_array($laskut) and count($laskut) > 0) {
	  ...
	  // MUOKKAUS: puuttuneet lainausmerkit:
        ...
        if ($lasku["summa"] != $lasku["alkup_summa"]) {
          ...
          $uusin = mysqli_fetch_assoc($uusinresult);
...
if ($tila == 'suorituksenvalinta') {
  ...
  if (mysqli_num_rows($result) > 1) {
    ...
    while ($suoritus = mysqli_fetch_assoc($result)) {
      ...
      if (mysqli_num_rows($result)==$r) echo "checked";
      $r++;

      echo "></td>";

      for ($i=0; $i<mysqli_num_fields($result)-2; $i++) {
        echo "<td>".$suoritus[mysqli_field_name($result, $i)]."</td>";
      }
...
if ($tila == 'kohdistaminen' and (int) $suoritus_tunnus > 0) {
  ...
  if (mysqli_num_rows($result) == 0) {
    ...
    if ($ytunnusrow=mysqli_fetch_assoc($result)) {
    ...
    for ($i = 0; $i < mysqli_num_fields($result)-1; $i++) {
      echo "<th><a href='$PHP_SELF?suoritus_tunnus=$suoritus_tunnus&asiakas_tunnus=$asiakas_tunnus&asiakas_nimi=$asiakas_nimi&tila=$tila&ojarj=".$i.$ulisa."&lopetus=$lopetus'>" . t(mysqli_field_name($result, $i))."</a></th>";
    }
    ...
    while ($maksurow = mysqli_fetch_assoc($result)) {
      ...
      $maararow = mysqli_fetch_assoc($cresult);
...
if ($tila == '') {
  ...
  while ($vrow = mysqli_fetch_assoc($vresult)) {
    ...
    echo "<option value = '".mb_strtoupper($vrow["maa"])."' $sel>".t($vrow["maa"])."</option>";
  }
  ...
  while ($asiakas = mysqli_fetch_assoc($result)) {
    ...
    $lasku = mysqli_fetch_assoc($lresult);
