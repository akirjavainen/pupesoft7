...
if ($tee == "uusi_ekirjekierros") {
  ...
  while ($uek_row = mysqli_fetch_assoc($uek_res)) {
...
if ((isset($tee_hae) and $tee_hae != "") or (isset($tee_kaikki) and $tee_kaikki != "") or (isset($tee_kierros) and $tee_kierros != "")) {
  ...
  if (mysqli_num_rows($res) > 0) {
    ...
    $ktunnus = mysqli_fetch_assoc($res);
    ...
    while ($row = mysqli_fetch_assoc($res)) {
      ...
      $karhuttu = mysqli_fetch_assoc($ka_res);
      ...
      $tunnukset = mysqli_fetch_assoc($la_res);
      ...
      if ($yhtiorow['karhuerapvm'] > 0) {
        $paiva = mb_substr($row["pvm"], 8, 2);
        $kuu   = mb_substr($row["pvm"], 5, 2);
        $year  = mb_substr($row["pvm"], 0, 4);
