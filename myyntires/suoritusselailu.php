...
if ($tila == 'poistasuoritus' or $tila == 'siirrasuoritus' or $tila == "siirrasuoritus_tilille") {
  ...
  while ($suoritus_row = mysqli_fetch_assoc($suoritus_res)) {
    ...
    // saamiset ei löydy
    if (mysqli_num_rows($tiliointi1_res) !== 1) {
      ...
      $tiliointi1_row = mysqli_fetch_assoc($tiliointi1_res);
...
if ($tila == 'tulostakuitti') {
  ...
  if (mysqli_num_rows($result) == 0) {
    ...
    $kirjoitinrow = mysqli_fetch_assoc($result);
...
if ($tila == 'tarkenna') {
  ...
  foreach ($haku as $key => $value) {
    ...
    $siivottu = preg_replace('/\b(oy|ab)\b/i', '', mb_strtolower($value));
