...
if (!empty($tee) and $tee == 'TULOSTA_EMAIL_LASKUT' and !empty($laskunrot)) {
  ...
  if (mb_strpos($laskunrot, ",") !== FALSE) echo t("Laskut lähetettiin osoitteeseen"), ": {$asiakasemail}";
...
if ($tee == "") {
  ...
  function laskeMaksunopeus($tunnukset, $yhtio) {
    ...
    $laskut = mysqli_fetch_array($result);
  ...
  if ($tila == 'tee_raportti') {
    ...
    if (mysqli_num_rows($result) > 0) {
      ...
      if ($alatila == "T") {
        ...
        $result = pupe_query($query);
        $asiakasrow2 = mysqli_fetch_array($result);
      ...
      // Kaatotilin saldo
      if ($savalkoodi != "") {
        $savalkoodi = sanitize_string($savalkoodi);
      ...
      if (mysqli_num_rows($kaatoresult) > 1) {
        $riveja = mysqli_num_rows($kaatoresult) + 1;
      }
      else {
        $riveja = 1;
        if (mysqli_num_rows($kaatoresult) != 0) {
          $kaato = mysqli_fetch_array($kaatoresult);
          mysqli_data_seek($kaatoresult, 0);

          if (mb_strtoupper($yhtiorow['valkoodi']) != mb_strtoupper($kaato['valkoodi'])) {
            $riveja = 2;
          }
        }
      }
      ...
      if (mysqli_num_rows($kaatoresult) > 1) { // Valuuttasummia
        $kotisumma = 0;
        while ($kaato = mysqli_fetch_array($kaatoresult)) {
      ...
      if (mysqli_num_rows($result) > 1) {
        $riveja = mysqli_num_rows($result) + 1;
      }
      else {
        $riveja = 1;
        if (mysqli_num_rows($result) != 0) {
          $kok = mysqli_fetch_array($result);
          mysqli_data_seek($result, 0);
          if (mb_strtoupper($yhtiorow['valkoodi']) != mb_strtoupper($kok['valkoodi'])) {
            $riveja = 2;
          }
        }
      }
      ...
      if (mysqli_num_rows($result) > 1) { // Valuuttasummia
        $kotisumma = 0;

        while ($kok = mysqli_fetch_array($result)) {
      ...
      $amrow = mysqli_fetch_assoc($amres);
      ...
      if (mysqli_num_rows($aasres) > 1) {

        echo "<tr><th>".t("Valuutta").":</th><td><select name='savalkoodi' onchange='submit();'>";
        echo "<option value = ''>".t("Kaikki")."</option>";

        while ($aasrow = mysqli_fetch_array($aasres)) {
          $sel="";
          if ($aasrow["valuutat"] == mb_strtoupper($savalkoodi)) {
            $sel = "selected";
          }
      ...
      if (mysqli_num_rows($result) > 0) {
        ...
        while ($maksurow = mysqli_fetch_array($result)) {
          ...
          if (mb_strtoupper($yhtiorow['valkoodi']) != mb_strtoupper($maksurow['valkoodi'])) {
...
function hae_maksusuoritukset($maksurow, $linkki) {
  ...
  if ($ratiro["rahatilit"] != "") {
    ...
    $ressukka = mysqli_fetch_assoc($keijo);
    ...
    if ($maksurow["mapvm"] == "0000-00-00" or mysqli_num_rows($lasktilitre) > 1) {
      ...
      $alku = mysqli_fetch_array($alku_res, 0);

      if (!$alku) {
        $alku = '3000-01-01';
      }

      while ($lasktilitro = mysqli_fetch_array($lasktilitre)) {
