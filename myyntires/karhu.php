...
if (mb_strpos($_SERVER['SCRIPT_NAME'], "karhu.php")  !== FALSE) {
...
if ($tee == "ALOITAKARHUAMINEN") {
  ...
  if ($ktunnus != 0) {
    ...
    if (mysqli_num_rows($result) == 1) {
      $factoringrow = mysqli_fetch_assoc($result);
      ...
      $maksuehdotrow = mysqli_fetch_assoc($result);
  ...
  if (mysqli_num_rows($result) > 0) {
    $konsrow = mysqli_fetch_assoc($result);
  ...
  elseif (isset($_POST['ytunnus_spec']) and ! empty($_POST['ytunnus_spec'])) {
    $asiakaslisa = sprintf(" and asiakas.ytunnus = '%s' ", sanitize_string($_POST['ytunnus_spec']));
  }
  ...
  if (mysqli_num_rows($result) > 0) {
    ...
    while ($karhuttavarow = mysqli_fetch_assoc($result)) {
...
if ($tee == 'KARHUA') {
  ...
  $asiakastiedot = mysqli_fetch_assoc($result);
  ...
  if ($asiakastiedot["kieli"] != "" and mb_strtoupper($asiakastiedot["kieli"]) != mb_strtoupper($yhtiorow["maa"])) {
  ...
  $sel1 = $sel2 = $sel3 = '';
  if ($max >= 2 and mysqli_num_rows($res) > 2) {
    $sel3 = 'selected';
  }
  elseif ($max >= 1 and mysqli_num_rows($res) > 1) {
    $sel2 = 'selected';
  }
  else {
    $sel1 = 'selected';
  ...
  while ($viesti = mysqli_fetch_assoc($res)) {
  ...
  $lirow = mysqli_fetch_assoc($lires);
  ...
  $kaato = mysqli_fetch_assoc($summaresult);
  ...
  while ($lasku = mysqli_fetch_assoc($result)) {
    ...
    if ($lasku["karhuttu"] >= 1 and $asiakastiedot['myyjanro'] != 0) {
      ...
      if (mysqli_num_rows($res) > 0) {
        ...
        $myyjatiedot = mysqli_fetch_assoc($res);
      }
    }
...
if ($tee == "") {
  ...
  if (mysqli_num_rows($meapu) > 0) {
    ...
    while ($row = mysqli_fetch_assoc($meapu)) {
