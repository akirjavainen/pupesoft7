...
if (!function_exists("uusi_karhukierros")) {
  function uusi_karhukierros($yhtio) {
    ...
    if (!mysqli_num_rows($result)) {
      ...
      $result = pupe_query($query);
      $uusid = mysqli_insert_id($GLOBALS["masterlink"]);
    }
    else {
    ...
    if (isset($_POST['ekirje_laheta']) === false) {
      ...
      if ($yhtiorow['maa'] != $asiakastiedot['maa']) {
        $query = sprintf(
          ...
          sanitize_string($asiakastiedot['maa'])
        );

        $maa_result = pupe_query($query);
        $maa_nimi = mysqli_fetch_assoc($maa_result);
  ...
  function loppu($firstpage, $summa, $korko) {
    ...
    */
    ...
    if (isset($maksuehtotiedot["factoring_id"])) {
      ...
      $factoringrow = mysqli_fetch_assoc($fac_result);
...
if ($nayta_pdf == 1 and $karhutunnus != '') {
  $karhutunnus = sanitize_string($karhutunnus);
...
$maksuehtotiedot = mysqli_fetch_assoc($maksuehtoresult);
...
if (!isset($karhuviesti)) {
  ...
  $avg = floor(($r/mysqli_num_rows($res))+1);
...
$res = pupe_query($query);
$viestit = mysqli_fetch_assoc($res);
...
while ($row = mysqli_fetch_assoc($result)) {
  ...
  if ($tee_pdf == 'tulosta_karhu') {
    ...
    $lasktilitro = mysqli_fetch_assoc($lasktilitre);
...
if ($yhtiorow["verkkolasku_lah"] == "maventa" and $_REQUEST['maventa_laheta'] == 'Lähetä Maventaan') {
  ...
  $laskurow = mysqli_fetch_assoc($result_temp);
