...
if (mb_strtolower($toim) == 'oletusvarasto') {
...
if (mysqli_num_rows($invaste_tuotepaikat_result) > 0) {
  ...
  while ($invaste_tuotepaikat_row = mysqli_fetch_assoc($invaste_tuotepaikat_result)) {
...
if ($yhtiorow['kerayserat'] == 'K') {
  ...
  $cnt_chk_row = mysqli_fetch_assoc($cnt_chk_res);
...
if ($tee == 'TULOSTA' and isset($tulosta)) {
  ...
  elseif (!empty($status)) {
    $rajauslisatuote .= " and tuote.status = '".sanitize_string($status)."' ";
  }
  ...
  function il_topmyydyt($top, $where, $kutsujoinlisa, $rajauslisa, $rajauslisatuote, $invaamatta, $extra) {
    ...
    while ($tuotrow = mysqli_fetch_assoc($tuotresult)) {
  ...
  if ($sub_ehto1 or $sub_ehto2) {
    $ressulisa = $reservipaikka != '' ? "varaston_hyllypaikat.reservipaikka = '".sanitize_string($reservipaikka)."' AND " : "";
    $vyohykelisa = $keraysvyohyke != '' ? "varaston_hyllypaikat.keraysvyohyke = '".sanitize_string($keraysvyohyke)."' AND " : "";
  ...
  if ($_tuote_chk) {
    ...
    if ($ahyllyalue != '' and $lhyllyalue != '') {
      ...
      $apaikka = mb_strtoupper(sprintf("%-05s", $ahyllyalue)).mb_strtoupper(sprintf("%05s", $ahyllynro)).mb_strtoupper(sprintf("%05s", $ahyllyvali)).mb_strtoupper(sprintf("%05s", $ahyllytaso));
      $lpaikka = mb_strtoupper(sprintf("%-05s", $lhyllyalue)).mb_strtoupper(sprintf("%05s", $lhyllynro)).mb_strtoupper(sprintf("%05s", $lhyllyvali)).mb_strtoupper(sprintf("%05s", $lhyllytaso));
...
if ($tee == 'TULOSTA' and isset($tulosta)) {
  if (mysqli_num_rows($saldoresult) > 0 ) {
    ...
    if ($yhtiorow['laaja_inventointilista'] != "") {
      $kokonaissivumaara = ceil(mysqli_num_rows($saldoresult) / 16);
    }
    else {
      $kokonaissivumaara = ceil(mysqli_num_rows($saldoresult) / 17);
    }
    ...
    if ($ei_inventointi == "") {
      ...
      $_vapaa_teksti = sanitize_string($vapaa_teksti);
    ...
    while ($tuoterow = mysqli_fetch_assoc($saldoresult)) {
      ...
      $prow    = mysqli_fetch_assoc($result1);
      ...
      if ($yhtiorow['laaja_inventointilista'] != "") {
        ...
        $prn .= sprintf('%-5.5s', mb_str_pad($rivinro, 5, " ", STR_PAD_LEFT));
      }
      else {
