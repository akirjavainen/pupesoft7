...
if ($tee == '') {
  ...
  if (trim($kuka_poimi) != '') {
    $lisa .= " and (lasku.maksaja like '%".sanitize_string((string) $kuka_poimi)."%' or kuka.nimi like '%".sanitize_string((string) $kuka_poimi)."%') ";
  }
...
if ($tee == 'selaa') {
  ...
  if (mysqli_num_rows($result) > 0) {
    ...
    $eka_row = mysqli_fetch_assoc($result);
    ...
    if (mb_strtoupper($yhtiorow['maa']) == 'EE' and mb_substr($eka_row['yriti_iban'], 0, 2) == "EE") {
