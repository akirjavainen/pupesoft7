...
if ($tila == 'parametrit') {
  ...
  if (mysqli_num_rows($result) > 0) {
    $uusiyhtiorow = mysqli_fetch_array($result);
...
if ($tila == 'ulkonako') {
  if ($fromyhtio != '') {
    ...
    $yht_row = mysqli_fetch_assoc($yht_res);
...
if ($tila == 'profiilit') {
  if (is_array($profiilit)) {
    foreach ($profiilit as $prof) {
      ...
      while ($trow = mysqli_fetch_array($pres)) {
...
if ($tila == 'kayttaja') {
  ...
  $query = "INSERT into kuka SET
            ...
            profiilit = '$profile'";
  $result = pupe_query($query);
  $kuka_id = mysqli_insert_id($GLOBALS["masterlink"]);
...
if ($tila == 'tili') {
  if ($fromyhtio != '') {
    ...
    while ($row = mysqli_fetch_array($kukar)) {
...
if ($tila == 'varasto') {
  if ($varasto != '') {
    $varasto = sanitize_string($varasto);
...
if ($tila == 'parametrit') {
  ...
  while ($uusiyhtiorow = mysqli_fetch_array($result)) {
...
if ($tila == 'menut') {
  ...
  if (mysqli_num_rows($result) > 0) {
    ...
    while ($profiilirow=mysqli_fetch_array($result)) {
