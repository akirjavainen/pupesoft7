...
if (isset($_FILES['userfile']) and is_uploaded_file($_FILES['userfile']['tmp_name']) === TRUE and $tee == 'YHDISTA_TIEDOSTOSTA') {
  ...
  for ($y = 0; $y < count($excelrivit); $y++) {
    for ($h = 0; $h < count($headers); $h++) {
      $taulunrivit[$y][mb_strtoupper(trim($headers[$h]))] = $excelrivit[$y][$h];
    }
  }
  ...
  foreach ($taulunrivit as $r) {
    ...
    if ( $tunnus = hae_asiakastunnus($tunnukset) ) {
      ...
      if ( mb_strtoupper($r['JATA_TAMA']) != 'X' ) {
...
if ( ( !isset($jataminut) and !isset($yhdista) ) and (!isset($_FILES['userfile']) or is_uploaded_file($_FILES['userfile']['tmp_name']) === false ) ) {
  ...
  for ($i = 1; $i < mysqli_num_fields($result)-1; $i++) { // HAKUKENTÄT
    echo "<th><a href='$PHP_SELF?ojarj=".mysqli_field_name($result, $i).$ulisa."'>" . t(mysqli_field_name($result, $i)) . "</a>";

    if  (mysqli_field_len($result, $i)>20) $size='20';
    elseif  (mysqli_field_len($result, $i)<=20)  $size='10';
    else  $size='10';
  ...
  while ($trow = mysqli_fetch_array($result)) { // tiedot
  ...
  foreach ($yhdista as $haettava) {
    ...
    if (mysqli_num_rows($asresult) == 1) {
      ...
      if (mysqli_num_rows($hresult) == 0) {
        ...
        while ($ahrow = mysqli_fetch_assoc($hresult)) {
      ...
      if (mysqli_num_rows($hresult) == 0) {
        ...
        while ($alrow = mysqli_fetch_assoc($hresult)) {
      ...
      if (mysqli_num_rows($hresult) == 0) {
        ...
        while ($ahrow = mysqli_fetch_assoc($hresult)) {
          ...
          $ahrow['filename'] = sanitize_string($ahrow['filename']);
          ...
          if ($ahy == 0) {
            $ahinsert = "INSERT INTO liitetiedostot SET
                         ...
                         liitos          = '$ahrow[liitos]',
                         liitostunnus    = '$jrow[tunnus]',
                         data            = '".sanitize_string($ahrow["data"])."',
                         selite          = '$ahrow[selite]',
                         kieli           = '$ahrow[kieli]',
