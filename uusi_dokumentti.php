...
echo "<font class='head'>".t('Lisää hyväksyttävä dokumentti')."</font><hr><br>";
...
if (!empty($tee) and $tee == "lisaa_tiedosto" and !empty($sub_button)) {
  ...
  if (is_uploaded_file($_FILES['userfile1']['tmp_name']) === TRUE) {
    ...
    if ($_FILES['userfile1']['size'] == 0) {
      $virhe = "<font class='error'><br>".t("Tiedosto on tyhjä")."!</font>";
    }

    if (!$file = fopen($_FILES['userfile1']['tmp_name'], "r")) {
      $virhe = "<font class='error'>".t("VIRHE: Tiedoston avaus epäonnistui!")."</font>";
    }
  }
  else {
...
echo "<tr><th class='ptop'>".t("Hyväksyjät")."</th><td>";
