...
if ($tee == 'UUSI') {
  ...
  if (mysqli_num_rows($resyh) > 0) {
    $monta = mysqli_fetch_assoc($resyh);
  ...
  if (mysqli_num_rows($reskuka) > 0 and $jatka != 1 and $kopsaakuka == "JOO") {
    $monta = mysqli_fetch_assoc($reskuka);
  ...
  if (strlen($ktunnus) > 0 and $jatka != 1) {
    ...
    $oletus_profiili = sanitize_string(trim($oletus_profiili));
    ...
    if (count($profiilit) > 0 and $profiilit[0] !='') {
      ...
      foreach ($profiilit as $prof) {
        ...
        while ($trow = mysqli_fetch_assoc($pres)) {
          ...
          $tarkesultrow = mysqli_fetch_assoc($tarkesult);

          if (mysqli_num_rows($tarkesult) == 0) {
...
$result = pupe_query($query);
$selkukarow = mysqli_fetch_assoc($result);
  ...
  if (strlen($firname) > 0 and isset($submit_button)) {
    ...
    if ($selkuka != "KOPSAAUUSI") {
      ...
      if ($toim != 'extranet') {
        ...
        while ($hyvaksyja_row = mysqli_fetch_assoc($hyvaksyja_result)) {
        ...
        if (mysqli_num_rows($avainsana_result) > 0) {
          ...
          while ($terminaalialue_row = mysqli_fetch_assoc($avainsana_result)) {
      ...
      if ($toim != 'extranet' and $yhtiorow['pakkaamolokerot'] != '') {
        ...
        while ($pakkaamorow = mysqli_fetch_assoc($pakkaamores)) {
          $sel = '';
      ...
      if ($toim != 'extranet') {
        ...
        while ($kuittitulostin_rivi = mysqli_fetch_assoc($kuittitulostin_result)) {
          $sel = '';
        ...
        $kassalippaat = array();
        while ($kassalipas = mysqli_fetch_assoc($kassalipasres)) {
      ...
      while ($tt_row = mysqli_fetch_array($tt_result)) {
...
if ($tee == "") {
  ...
  while ($kurow = mysqli_fetch_assoc($kukares)) {
