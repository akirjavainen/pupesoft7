    ...
    foreach ($pulkkarit as $pulkkari) {
      ...
      if ($updated != NULL) {
        if (strtotime($updated) > $updatedtime) {
          ...
          $title   = $pulkkari->title;

          $newfeature = (mb_substr(trim($title), 0, 1) == "*") ? 1 : 0;

          $pulkkari_ser = sanitize_string(serialize($pulkkari));
          ...
          foreach ($filet as $file) {
            ...
            if (mb_substr($filename, -4) == ".php"
              and mb_strpos($filename, "tulostakopio.php") === FALSE
              and mb_strpos($filename, "yllapito.php") === FALSE
            ) {
              ...
              while ($menurow = mysqli_fetch_assoc($menures)) {
                $filetarr[] = array($menurow["sovellus"], $menurow["nimi"], $menurow["nimitys"]);
              }
            }
          }

          $filet_ser = sanitize_string(serialize($filetarr));
...
if (mysqli_num_rows($vetores)) {

  $vedot = array();
  $taveto_hash = "";
  $taveto_hash_pupenext = "";

  while ($vetorow = mysqli_fetch_assoc($vetores)) {
    $vedot[] = $vetorow;
  }
  ...
  foreach ($vedot as $i => $veto) {
    ...
    if ($veto == "HEAD") {
      ...
      $pulrow = mysqli_fetch_assoc($pulres);
    ...
    if ($pull_ids != "") {
      foreach (explode(",", $pull_ids) as $pull) {
        ...
        $title = $pulkkaridata->title;
        $body  = $pulkkaridata->body;
