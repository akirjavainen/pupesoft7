...
if (!isset($thumb_size_x, $thumb_size_y, $normaali_size_x, $normaali_size_y)) {
  echo "<font class='error'>".t("Kuvakokoja ei ole määritelty. Ei voida jatkaa!")."</font>";
  exit;
}
...
if ($yhtiorow['kuvapankki_polku'] == '') {
  echo "<font class='error'>".t("Kuvapankkia ei ole määritelty. Ei voida jatkaa!")."</font>";
  exit;
}
...
function listdir($start_dir = '.') {
  ...
  if (is_dir($start_dir)) {
    ...
    while (($file = readdir($fh)) !== false) {
      if (strcmp($file, '.') == 0 or strcmp($file, '..') == 0 or mb_substr($file, 0, 1) == ".") {
        continue;
      }
...
function konvertoi($ykoko, $xkoko, $type, $taulu, $kuva, $dirri, $upfile1) {
  ...
  // tmpfile
  $upfilesgh = mb_strtolower("/tmp/$nimi"."1.".$ext);
  ...
  if ($error != 0) {
    ...
    if ($copy_boob === FALSE) {
      echo t("Kopiointi epäonnistui")." {$upfilesgh} {$uusnimi} {$upfile1} <br>";
      $upfileall = "";
    }
    else {
      $upfileall = "$uusnimi";
...
$alkupituus = mb_strlen($dirri) + 1;

if (!is_writable($dirri)) {
  echo "<font class='error'>";
  echo t("Kuvapankkiin (%s) ei ole määritelty kirjoitusoikeutta. Ei voida jatkaa!", "", $dirri);
  echo "</font>";
  echo "<br>";
  exit;
}
...
if ($tee == 'GO') {
  ...
  if (isset($kasittele_kuvat) and $kasittele_kuvat == '1') {
    ...
    echo "<br>";
    echo "<font class='message'>".t("Käsitellään konvertoitavia kuvia").":</font>";
    echo "<br><br>";

    foreach ($files as $file) {

      $polku = mb_substr($file, $alkupituus);
      list($taulu, $toiminto, $kuva) = explode("/", $polku, 3);

      if (mb_strtolower($taulu) != 'tuote') {
      ...
      if ($size !== FALSE and $toiminto == 'kasittele' and $mtype == "image") {
        ...
        echo " &raquo; ";
        echo "<font class='error'>".t("Virhe! Voidaan käsitellä vain kuvia!")."<br>";
      }
  ...
  echo "<font class='message'>".t("Päivitetään tuotekuvat järjestelmään").":</font>";
  echo "<br><br>";
  ...
  foreach ($files as $file) {
    ...
    if ($koko !== FALSE) {
      if ($toiminto == 'thumb' and (($kork > $thumb_size_y and $thumb_size_y > 0) or ($leve > $thumb_size_x and $thumb_size_x > 0))) {
        ...
        if ($thumbi != "") {
          ...
          echo " &raquo; ";
          echo "<font class='error'>";
          echo t("Ohitetaan thumb-kuva, koska resoluutio %d x %d on liian suuri ja skaalaus epäonnistui!", "", $leve, $kork);
          echo "</font>";
          echo "<br>";
          continue;
        }
      ...
      if ($toiminto == 'normaali' and (($kork > $normaali_size_y and $normaali_size_y > 0) or ($leve > $normaali_size_x and $normaali_size_x > 0))) {
        ...
        if ($normi != "") {
          ...
          echo " &raquo; ";
          echo "<font class='error'>";
          echo t("Ohitetaan normaali-kuva, koska resoluutio %d x %d on liian suuri ja skaalaus epäonnistui!", "", $leve, $kork);
          echo "</font>";
          echo "<br>";
          continue;
        }
    ...
    // pitää kattoo onko nimessä häshsiä, otetaan järjestys vikan hashin jälkene
    if (mb_strpos($kuva, "#") !== false) {
      $kuva = explode("#", $kuva);
    ...
    if (mb_strpos($kuva, "%") !== FALSE) {
      ...
      $mihin = mb_strpos($kuva, "%");
      $kuvanalku = mb_substr($kuva, 0, $mihin);

      //kyseessä on käyttöturvatiedot ja tuotekortti
      if (mb_strpos($kuva, "%ktt") !== FALSE) {
        $mistakieli = mb_strpos($kuva, "%ktt") + 4;
        $mikakieli = mb_substr($kuva, $mistakieli, 2);

        if (mb_strpos($mikakieli, "fi") !== FALSE or
          mb_strpos($mikakieli, "se") !== FALSE or
          mb_strpos($mikakieli, "en") !== FALSE or
          mb_strpos($mikakieli, "ru") !== FALSE or
          mb_strpos($mikakieli, "ee") !== FALSE or
          mb_strpos($mikakieli, "no") !== FALSE or
          mb_strpos($mikakieli, "de") !== FALSE) {
      ...
      elseif (mb_strpos($kuva, "%tko") !== FALSE) {
        $mistakieli = mb_strpos($kuva, "%tko" ) + 4;
        $mikakieli = mb_substr($kuva, $mistakieli, 2);
    ...
    if (file_exists($file)) {
      ...
      $paketti = mysqli_fetch_array($result);
      ...
      if ($filesize == 0) {
        echo " &raquo; ";
        echo "<font class='error'>";
        echo t("Ohitetaan kuva, koska tiedosto on tyhjä!");
        echo "</font>";
        echo "<br>";
        continue;
      }
      ...
      if ($data === FALSE) {
        echo " &raquo; ";
        echo "<font class='error'>";
        echo t("Ohitetaan kuva, koska tiedoston luku epäonnistui!");
        echo "</font>";
        echo "<br>";
        continue;
      }
      ...
      if (!isset($apuresult)) {
        $mihin   = mb_strpos($kuva, ".$ext");
        $tuoteno = mb_substr($kuva, 0, "$mihin");

        if (mb_strpos($tuoteno, "=") !== false and empty($kayttotarkoitus_custom)) {
      ...
      if (mysqli_num_rows($apuresult) > 0) {
        ...
        echo " &raquo; ";
        echo t("Lisättiin liitetiedosto tuotteelle");
        echo " ";

        // lisätään file
        while ($apurow = mysqli_fetch_array($apuresult)) {

          $kuvaselite = mb_strtolower($ext) == 'pdf' ? 'Liitetiedosto' : 'Tuotekuva';
          $kayttotarkoitus = "MU";
          ...
          if (trim($kayttotarkoitus_custom) != '') {
            $kayttotarkoitus = mb_strtoupper($kayttotarkoitus_custom);
          }
        ...
        echo " &raquo; ";
        echo "<font class='error'>";
        echo t("Ohitetaan kuva, koska kuvalle ei löytynyt tuotetta!");
        echo "</font>";
        echo "<br>";
      }
    }
...
if ($tee == 'DUMPPAA') {
  ...
  if (!is_writable($dirri."/tuote")) {
    die(t("Kuvapankkiin/%s ei ole määritelty kirjoitusoikeutta. Ei voida jatkaa!", "", 'tuote')."<br>");
  }
  ...
  while ($row = mysqli_fetch_assoc($result)) {
    ...
    if (!is_writable($kokohak)) {
      echo "<font class='error'>";
      echo t("Hakemistolle %s ei ole määritelty kirjoitusoikeutta. Ei voida tallentaa kuvaa!", "", $kokonimi);
      echo "</font>";
      echo "<br>";
      continue;
    }
    ...
    if (!empty($row["tuoteno"])) {
      ...
      if (file_put_contents($kokonimi, $row["data"]) !== false) {
        ...
        echo "<font class='error'>";
        echo t("Tiedoston %s kirjoitus epäonnistui!", "", $kokonimi);
        echo "</font>";
        echo "<br>";
      }
    }
  ...
  if ($dellattuja > 0) {
    echo "<br>";
    echo "<font class='message'>";
    echo t("Poistettiin %d kuvaa järjestelmästä", "", $dellattuja);
    echo "</font>";
    echo "<br>";
  }
...
if ($lukuthumbit + $lukunormit + $lukupainot + $lukumuut + $lukutconvertit + $lukutuoteinfo + $lukuktt == 0) {
  ...
  echo "<tr>";
  echo "<td>".t("Poistetaanko järjestelmästä")."</td>";
