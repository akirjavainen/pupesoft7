...
if ((isset($tulosta) or isset($tulostakopio)) and $otsikkonro > 0) {
  ...
  if (isset($tulostakopio)) {
    ...
    if ($varasto == 0) {
      ...
      $tmp_varasto = mysqli_fetch_assoc($tempr);
    ...
    if (!$rahtikirja_ilman_asiakasta) {
      ...
      $asiakasrow = mysqli_fetch_assoc($asres);
  ...
  $query = "SELECT nimi, nimitark, osoite, postino, postitp, maa
            ...
            AND tunnus  = '$varasto'";
  $tempr = pupe_query($query);
  $postirow_varasto = mysqli_fetch_assoc($tempr);
  ...
  $rahtikirjanrostring = sanitize_string(serialize($osoitelappurow));
  ...
  $toitarow = mysqli_fetch_assoc($result);
...
if (!$asiakasid and !$rahtikirja_ilman_asiakasta) {
  ...
  if (mysqli_num_rows($kirres) > 0) {
    ...
    while ($rakir_row = mysqli_fetch_assoc($kirres)) {
      ...
      if ($rakir_row['rahtikirjanro'] != '') {
        ...
        while ($kirow = mysqli_fetch_assoc($kirre)) {
...
if ($asiakasid or $rahtikirja_ilman_asiakasta) {
  ...
  while ($vrow=mysqli_fetch_array($vresult)) {
    $sel="";
    if (mb_strtoupper($asiakasrow['toim_maa']) == mb_strtoupper($vrow[0])) {
      $sel = "SELECTED";
    }
...
function pupe_rahtikirja_insert($data) {
  ...
  foreach ($data as $key => &$val) {
    $val = sanitize_string($val, $GLOBALS['link']);
  }
...
function pupe_rahtikirja_fetch($otsikkonro) {
  ...
  while ($rahtikirja = mysqli_fetch_assoc($result)) {
...
function pupe_varasto_fetch_all() {
  $query = sprintf("  SELECT tunnus, nimitys
            ...
            ORDER BY tyyppi, nimitys", sanitize_string($GLOBALS['kukarow']['yhtio']));
...
function pupe_rahtisopimus($merahti, $toimitustapa, $ytunnus = null) {
  if ($merahti) {
    ...
    $res = pupe_query($query);
    $merahti = mysqli_fetch_assoc($res);
  ...
  if (mysqli_num_rows($res) === 1) {
    $sopimus = mysqli_fetch_assoc($res);
