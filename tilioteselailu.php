...
if ($tee == 'T' and (int) $kuitattava_tiliotedata_tunnus > 0) {
  ...
  $kuitetaan_row = mysqli_fetch_assoc($kuitetaan_result);
...
if ($tee == 'Z' or $tiliote == 'Z') {
  ...
  if (mysqli_num_rows($result) != 1) {
    ...
    $yritirow = mysqli_fetch_array($result);
    $tee = 'T';
...
if ($tee == 'X' or $tee == 'XX' or $tee == "XS" or $tee == "XXS") {
  ...
  if (mysqli_num_rows($tiliotedataresult) == 0) {
    $tee = '';
  }
  else {
    $tee = 'T';
    $tiliotedatarow = mysqli_fetch_array($tiliotedataresult);
...
if ($tee == 'S') {
  ...
  $tilioterivimaara  = mysqli_num_rows($tiliotedataresult);
  ...
  if ($tilioterivimaara == 0) {
    ...
    $meteidrow = mysqli_fetch_assoc($meteidres);

    while ($tiliotedatarow = mysqli_fetch_array($tiliotedataresult)) {
...
if ($tee == '') {
  ...
  while ($yritirow = mysqli_fetch_array($result)) {
    $chk = "";
