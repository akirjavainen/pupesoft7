...
if (!empty($laajahaku)) {
  ...
  if (mysqli_num_rows($tyomkentta_res)) {
    $tyom_kentat_array = array();

    while ($al_row = mysqli_fetch_assoc($tyomkentta_res)) {
...
if ($tee == 'etsi' and !empty($hakusubmit)) {
  ...
  foreach ($tyom_kentat_array as $selite => $selitetark) {
    if (!empty(${$selite})) {
      if (mb_strtoupper($tyom_kentat_tyyppi[$selite]) == "DROPDOWN") {
  ...
  if (mysqli_num_rows($sresult) > 0) {
    ...
    while ($row = mysqli_fetch_array($sresult)) {
      ...
      echo "<tr>
          ...
          <td valign='top'>".tv1dateconv(mb_substr($row["luontiaika"], 0, 10))."</td>
