...
if ($tyostatus_muutos != '' and $tyomaarayksen_tunnus != '') {
  $tyostatus_muutos = sanitize_string($tyostatus_muutos);
...
if ($tyojono_muutos != '' and $tyomaarayksen_tunnus != '') {
  $tyojono_muutos = sanitize_string($tyojono_muutos);
  $tyomaarayksen_tunnus = (int) $tyomaarayksen_tunnus;
...
while ($prioriteetti_row = mysqli_fetch_assoc($prioriteetti_result)) {
...
while ($tyojono_row = mysqli_fetch_assoc($tyojono_result)) {
...
while ($tyostatus_row = mysqli_fetch_assoc($tyostatus_result)) {
...
if (trim($konserni) != '') {
  ...
  $konsernit = " lasku.yhtio in (".mb_substr($konsernit, 0, -1).") ";
}
else {
...
if ($linkkihaku != "") {
  $linkkihaku = sanitize_string(trim($_GET['linkkihaku']));
...
while ($vrow = mysqli_fetch_assoc($vresult)) {
  ...
  if ($vrow["tyojono"] != "" and $toim != 'TYOMAARAYS_ASENTAJA') {
    ...
    $ankkuri_pp = (mb_strlen($ankkuri_pp) == 2 and mb_substr($ankkuri_pp, 0, 1) == 0) ? mb_substr($ankkuri_pp, 1, 1) : $ankkuri_pp;
    $ankkuri_kk = (mb_strlen($ankkuri_kk) == 2 and mb_substr($ankkuri_kk, 0, 1) == 0) ? mb_substr($ankkuri_kk, 1, 1) : $ankkuri_kk;
  ...
  if ($toim != 'TYOMAARAYS_ASENTAJA') {
    ...
    if (mysqli_num_rows($tyojono_result) > 0) {
      ...
      while ($tyojono_row = mysqli_fetch_assoc($tyojono_result)) {
        ...
        if ($vrow['tyojono'] == $tyojono_row['selitetark']) {
          $sel = 'SELECTED';
          $jonon_nimi = mb_strtolower($tyojono_row['selitetark']);
        }
      ...
      if ($yhtiorow['laiterekisteri_kaytossa'] != '' and isset($tyojonotyyppi) and $tyojonotyyppi == $jonon_nimi) {
        ...
        $kvrivi = mysqli_fetch_array($kvresult);
