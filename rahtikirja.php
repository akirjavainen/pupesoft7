...
if ($tee == 'add' and $id == 'dummy' and $mista == 'rahtikirja-tulostus.php') {
  ...
  $pakkaustieto_rakir_row = mysqli_fetch_assoc($pakkaustieto_rakir_res);
  ...
  for ($i = 0; $i < count($pakkaus); $i++) {
    if (($kollit[$i] != '' or mb_strpos($kilot[$i], "/") !== FALSE) and ($kilot[$i] != '' or $kuutiot[$i] != '' or $lavametri[$i] != '') and $subnappi != '') {
      $tutkimus++;
    }
  }
  ...
  if ($tutkimus > 0) {
    ...
    if ($kollit[$i] == '' and mb_strpos($kilot[$i], "/") === FALSE) {
...
if (is_array($erikoispakkaus) and count($erikoispakkaus) > 0) {
  ...
  $toimitustapa_row = mysqli_fetch_assoc($toimitustapa_res);
...
if ($id > 0 and $tunnukset != "") {
  ...
  $tarkistus_row = mysqli_fetch_assoc($tarkres);
...
if ($id > 0 and $tunnukset != "") {
  ...
  $vakrow = mysqli_fetch_assoc($vakresult);
  ...
  if ($vakrow['vaktuotteet'] != '') {
    ...
    if ($toimitustapa_row['vak_kielto'] != '' and $toimitustapa_row['vak_kielto'] != 'K') {
      ...
      $toimitustapa = sanitize_string($toimitustapa_row['vak_kielto']);
      $tee = '';
    }
...
if ($toimitustapa != "") {
  ...
  if (mysqli_num_rows($vak_toim_result) > 0 and $marow["jv"] != "") {
...
if ($tee == "lisaatilaus") {
  ...
  while ($row = mysqli_fetch_assoc($tilre)) {
...
if ($tee == 'add' and isset($muutos) and $muutos == 'yes') {
  ...
  if ($yhtiorow['kerayserat'] == 'K' and mb_strpos($tunnukset, ',') !== FALSE) {
  ...
  if (mysqli_num_rows($result) != count(explode(',', $tunnuslisa))) {
...
if ($tee == 'add') {
  ...
  if ($tutkimus > 0 and $tunnukset != "") {
    ...
    if (isset($muutos) and $muutos == 'yes') {
      ...
      $pakrow = mysqli_fetch_assoc($pakres);
    ...
    if (!function_exists("lisaarivi")) {
      function lisaarivi($otunnus, $tuoteno, $kpl, $hinta = "") {
        ...
        $laskurow = mysqli_fetch_assoc($rhire);
      ...
      if ($tutkimus > 0) {
        ...
        if (($kilot[$i] != '' or $kollit[$i] != '' or $kuutiot[$i] != '' or $lavametri[$i] != '') and $subnappi != '') {
          ...
          if ($rakirno == '') {
            ...
            $result = pupe_query($query);
            $rakirow = mysqli_fetch_assoc($result);
          ...
          foreach ($tilaukset as $otsikkonro) {
            ...
            if (mysqli_num_rows($pakres) == 1) {
              $pakrow = mysqli_fetch_assoc($pakres);
              $laskurow = mysqli_fetch_assoc($tilre);
  ...
  if ($apu > 0) {
    ...
    if ($yhtiorow["splittauskielto"] == "" and mb_strpos($tunnukset, ',') !== false and $yhtiorow['pakkaamolokerot'] != '' and $yhtiorow['kerayserat'] == '') {
    ...
    if (mysqli_num_rows($result) == 1) {
      ...
      $row = $toimitustaparow = mysqli_fetch_assoc($result);
      ...
      $laskurow = mysqli_fetch_assoc($lasresult);
    ...
    if ($rakirsyotto_lahete_tulostin != '' and $komento != "" and $lahetekpl > 0) {
      ...
      $asiakas_chk_row = mysqli_fetch_assoc($asiakas_chk_res);
    ...
    if ($rakirsyotto_oslapp_tulostin != "" and $oslapp != '' and $oslappkpl > 0) {
      ...
      if ($toimitustaparow['osoitelappu'] == 'oslap_mg' and $yhtiorow['kerayserat'] == 'K') {
        ...
        foreach ($osoitelaput as $index => $laskutunnus) {
          ...
          $laskurow = mysqli_fetch_assoc($osresult);
...
if (($toim == 'lisaa' or $toim == 'lisaa_siirto') and $id == 0 and (string) $id != 'dummy') {
  ...
  if (is_numeric($etsi) and $etsi != '') {
    ...
    $vanhatrow = mysqli_fetch_assoc($vanhatre);
    ...
    if ($vanhatrow['vanhatunnus'] == 0) {
      ...
      $etsirow = mysqli_fetch_assoc($etsire);
  ...
  if (mysqli_num_rows($tilre) > 0) {
    ...
    while ($row = mysqli_fetch_assoc($tilre)) {
      ...
      if ($yhtiorow["splittauskielto"] == "" and $yhtiorow['pakkaamolokerot'] != '') {
        ...
        $vanhat_row = mysqli_fetch_assoc($vanhat_res);
      }
      ...
      if ((isset($vanhat_row) and $vanhat_row['kpl'] == $row['tunnukset_lkm']) or (isset($vanhat_row) and $vanhat_row['kpl'] == 0) or $yhtiorow["splittauskielto"] != "" or $yhtiorow['pakkaamolokerot'] == '') {
        ...
        $kerattyaika_e  = str_replace(mb_substr($lahetepvm_e, 0, mb_strpos($lahetepvm_e, " ")), "", $kerattyaika_e);
        ...
        if ($yhtiorow['pakkaamolokerot'] != '') {
          ...
          while ($kollit_row = mysqli_fetch_assoc($kollit_res)) {
            if (trim(mb_strtolower($kollit_row['pakkaus'])) == 'kolli') {
              $kollit_chk += $kollit_row['kollit'];
            }

            if (trim(mb_strtolower($kollit_row['pakkaus'])) == 'rullakko') {
          ...
          echo "<td valign='top'>";
          if (mysqli_num_rows($pakkaamoresult) > 0) {
            while ($pakkaamo_row = mysqli_fetch_assoc($pakkaamoresult)) {
...
if (($id == 'dummy' and $mista == 'rahtikirja-tulostus.php') or $id != 0) {
  ...
  if ($id == 'dummy' and $mista == 'rahtikirja-tulostus.php') {
    ...
    $otsik = mysqli_fetch_assoc($resul);
    ...
    if ($tila == 'L') {
      ...
      if (mysqli_num_rows($resul) == 0) {
        ...
        $marow = mysqli_fetch_assoc($resul);
      }
    }
  ...
  if ($id == 'dummy' and $mista == 'rahtikirja-tulostus.php') {
    ...
    if (mysqli_num_rows($result) > 1) {
      ...
      if (mysqli_num_rows($rarrr) == 1) {
        $roror          = mysqli_fetch_assoc($rarrr);
    ...
    if (mb_strtoupper($tulostustapa) == 'H' or mb_strtoupper($tulostustapa) == 'K') {
    ...
    if ($otsik['pakkaamo'] > 0 and $yhtiorow['pakkaamolokerot'] != '') {
      ...
      if (mysqli_num_rows($lokero_chk_res) > 0) {
        while ($lokero_chk_row = mysqli_fetch_assoc($lokero_chk_res)) {
    ...
    if ((mb_strtoupper($tulostustapa) == 'H' or mb_strtoupper($tulostustapa) == 'K') and $marow["jv"] != "") {
  ...
  if ($id != 'dummy' and $mista != 'rahtikirja-tulostus.php' and $tunnukset != "") {
    ...
    $painorow = mysqli_fetch_assoc($painoresult);
    ...
    $tilavuusrow = mysqli_fetch_assoc($tilavuusresult);
  ...
  $as_chk_row = mysqli_fetch_assoc($as_chk_res);
  ...
  if ($_tulostustapa_chk and $id != 'dummy' and $mista != 'rahtikirja-tulostus.php' and ($yhtiorow['kerayserat'] == 'P' or ($yhtiorow['kerayserat'] == 'A' and $as_chk_row['kerayserat'] == 'A'))) {
    ...
    if (mysqli_num_rows($rarrr) == 0) {

      $querytunlisa = mb_strpos($tunnukset, ',') !== FALSE ? $tunnukset : $otsik['tunnus'];
    ...
    while ($keraysera_row = mysqli_fetch_assoc($keraysera_res)) {
      ...
      $echotuslisa = mb_strpos($tunnukset, ',') !== FALSE ? $keraysera_row['otunnus'].': ' : "";
      ...
      if ((mb_strtoupper($tulostustapa) == 'E' or mb_strtoupper($tulostustapa) == 'L') and $yhtiorow['oletus_rahtikirja_oslappkpl'] != 0 and !preg_match("/rahtikirja_unifaun_(ps|uo|xp)_siirto\.inc/", $toitarow["rahtikirja"])) {
      ...
      while ($pak_row = mysqli_fetch_assoc($pak_res)) {
  ...
  if ($id != 'dummy' and $mista != 'rahtikirja-tulostus.php') {
    ...
    if ($yhtiorow['rahti_ja_kasittelykulut_kasin'] != '') {
      ...
      if (mysqli_num_rows($rhire) == 1 and $merahti!='') {
        $trow  = mysqli_fetch_assoc($rhire);
