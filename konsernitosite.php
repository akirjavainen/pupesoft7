...
if ($tee == 'I') {
  ...
  if (is_uploaded_file($_FILES['userfile']['tmp_name'])) {
    ...
    $liitostunnus = mysqli_insert_id($GLOBALS["masterlink"]);
    $fnimi = $liitostunnus;
  }
  ...
  for ($i=0; $i<$totmaara; $i++) {
    ...
    if (mb_strlen($itili[$i]) > 0 or mb_strlen($isumma[$i]) > 0) {
      ...
      if (mb_strlen($selite) > 0 and mb_strlen($iselite[$i]) == 0) {
        $iselite[$i] = $selite;
      }

      // Selite puuttuu
      if (mb_strlen($iselite[$i]) == 0) {
