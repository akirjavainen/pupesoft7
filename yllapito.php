...
if ($upd == 1) {
  ...
  for ($i=1; $i < mysqli_num_fields($result)-1; $i++) {
    ...
    $t[mysqli_field_name($result, $i)] = &$t[$i];
  }
  ...
  for ($i=1; $i < mysqli_num_fields($result); $i++) {
    ...
    $pakollisuuden_tarkistus_rivi = mysqli_fetch_assoc($al_res);

    if (mysqli_num_rows($al_res) == 0 and $rajattu_nakyma != '' and isset($t[$i])) {
    ...
    if (mysqli_num_rows($al_res) != 0 and mb_strtoupper($pakollisuuden_tarkistus_rivi['selitetark_3']) == "PAKOLLINEN") {
      if (((mysqli_field_type($result, $i) == 'real' or  mysqli_field_type($result, $i) == 'int') and (float) str_replace(",", ".", $t[$i]) == 0) or
        (mysqli_field_type($result, $i) != 'real' and mysqli_field_type($result, $i) != 'int' and trim($t[$i]) == "")) {
  ...
  if ($errori == "") {
    ...
    if ($onko_tama_insert) {
      ...
      for ($i=1; $i < mysqli_num_fields($result); $i++) {
        ...
        if (isset($t[$i])) {
          ...
          if ($toim == 'tuotteen_toimittajat' and mysqli_field_name($result, $i) == 'tehdas_saldo_paivitetty') continue;

          if (is_array($_FILES["liite_$i"]) and $_FILES["liite_$i"]["size"] > 0) {
            $id = tallenna_liite("liite_$i", "Yllapito", 0, "Yhtio", "$toim.".mysqli_field_name($result, $i), $t[$i]);
          ...
          if (mysqli_field_type($result, $i) == 'real') {
            $t[$i] = $t[$i] != "NULL" ? "'".(float) str_replace(",", ".", $t[$i])."'" : $t[$i];
            $query .= ", ". mysqli_field_name($result, $i)." = {$t[$i]} ";
          }
          elseif (mysqli_field_type($result, $i) == 'int' and $t[$i] == "NULL") {
            $query .= ", ". mysqli_field_name($result, $i)." = NULL ";
          }
          else {
            $query .= ", ". mysqli_field_name($result, $i)." = '".trim($t[$i])."' ";
          }
        }
        else {
          ...
          if (mysqli_num_rows($oletus_tarkistus_result) == 1) {
            $oletuksen_tarkistus_rivi = mysqli_fetch_assoc($oletus_tarkistus_result);
    ...
    // Päivitetään
      ...
      for ($i=1; $i < mysqli_num_fields($result); $i++) {
        if (isset($t[$i]) or (isset($_FILES["liite_$i"]) and is_array($_FILES["liite_$i"]))) {
          ...
          if ($toim == 'tuotteen_toimittajat' and mysqli_field_name($result, $i) == 'tehdas_saldo') $tehdas_saldo_chk = $t[$i];

          if ($toim == 'tuotteen_toimittajat' and mysqli_field_name($result, $i) == 'tehdas_saldo_paivitetty') {
          ...
          if ($toim == 'tuote' and mysqli_field_name($result, $i) == 'vienti' and !empty($maaryhma_vienti)) {
            $t[$i] = $maaryhma_vienti;
          }
    ...
    if ($tunnus > 0 and $toim == "tuotteen_toimittajat_tuotenumerot") {
      ...
      if (mysqli_num_rows($toim_tuoteno_chk_res) == 1) {

        $toim_tuoteno_chk_row = mysqli_fetch_assoc($toim_tuoteno_chk_res);
    ...
    if ($onko_tama_insert and $tunnus > 0 and isset($tee_myos_tuotteen_toimittaja_liitos) and isset($liitostunnus) and $toim == "tuote" and $tee_myos_tuotteen_toimittaja_liitos == 'JOO' and $liitostunnus != '') {
      ...
      if (mysqli_num_rows($tuote_chk_res) == 1 and mysqli_num_rows($toimi_chk_res) == 1) {
        $tuote_chk_row = mysqli_fetch_assoc($tuote_chk_res);
        $toimi_chk_row = mysqli_fetch_assoc($toimi_chk_res);
    ...
    if ($tunnus > 0 and count(array_filter($array_chk, 'strlen')) > 0 and $toim == "asiakas") {
      ...
      if (mysqli_num_rows($otsikres) == 1) {
        $otsikrow = mysqli_fetch_array($otsikres);
        ...
        while ($laskuorow = mysqli_fetch_array($laskuores)) {
          ...
          if ($trow["sisviesti1"] != "" and $otsikrow["sisviesti1"] != $trow["sisviesti1"] and mb_strpos($laskuorow["sisviesti1"], $trow["sisviesti1"]) !== FALSE) {
          ...
          elseif (mb_strpos($laskuorow["sisviesti1"], $otsikrow["sisviesti1"]) === FALSE) {
          ...
          if ($laskuorow["maa"] != "" and $laskuorow["maa"] != $otsikrow["maa"]) {
            ...
            if (mysqli_num_rows($alhire) == 1) {
              ...
              if (mysqli_num_rows($alhire) == 1) {
                ...
                if (mysqli_num_rows($alhire) == 1) {
                  $apualvrow  = mysqli_fetch_assoc($alhire);
    ...
    if ($tunnus > 0 and isset($paivita_myos_avoimet_tilaukset) and $toim == "toimi") {
      ...
      if (mysqli_num_rows($otsikres) == 1) {
        ...
        while ($laskuorow = mysqli_fetch_assoc($laskuores)) {

          if ($yhtiorow['ostolaskujen_paivays'] == "1" and $laskuorow["lapvm"] != '0000-00-00') {
            $ltpp = mb_substr($laskuorow["lapvm"], 8, 2);
            $ltpk = mb_substr($laskuorow["lapvm"], 5, 2);
            $ltpv = mb_substr($laskuorow["lapvm"], 0, 4);
          }
          else {
            $ltpp = mb_substr($laskuorow["tapvm"], 8, 2);
            $ltpk = mb_substr($laskuorow["tapvm"], 5, 2);
            $ltpv = mb_substr($laskuorow["tapvm"], 0, 4);
          }
          ...
          if ($laskuorow["tilaustyyppi"] == "M") {
            ...
            $kukorow = mysqli_fetch_assoc($kukores);
    ...
    if ($lopetus != '' and (isset($yllapitonappi) or isset($paivita_myos_avoimet_tilaukset))) {
      ...
      if (mb_strpos($lopetus, "yllapito.php") === FALSE) {
...
if ($errori == "" and ($del == 1 or $del == 2 or $upd == 1) and isset($js_open_yp) and $js_open_yp != "") {
  ...
  if ($toim == "perusalennus") {
    $query = "SELECT ryhma value, concat_ws(' - ', ryhma, selite) text
              ...
              and yhtio    = '$kukarow[yhtio]'";
    $otsikres = pupe_query($query);
    $otsikrow = mysqli_fetch_assoc($otsikres);
  }
  ...
  if (isset($haku[$i]) and mb_strlen($haku[$i]) > 0) {
    ...
    if ($from == "" and ((($toim == 'rahtisopimukset' or $toim == 'asiakasalennus' or $toim == 'kohde' or $toim == 'asiakashinta') and trim($array[$i]) == 'asiakas') or ($toim == 'yhteyshenkilo' and trim($array[$i]) == 'liitostunnus'))) {
      if (!is_numeric($haku[$i])) {
        ...
        $ashakrow = mysqli_fetch_assoc($ashakres);
    ...
    elseif ($toim == 'asiakas' and $yhtiorow['toimipaikkakasittely'] == 'L' and trim($array[$i]) == "toimipaikka") {
      if (mb_strpos($hakuehto, 'kaikki') === false) $lisa .= " AND asiakas.toimipaikka {$hakuehto} ";
    }
    ...
    elseif ($from == "" and $toim == "tuotteen_toimittajat_tuotenumerot" and trim($array[$i]) == "toim_tuoteno_tunnus") {
      ...
      $tutorow = mysqli_fetch_assoc($tutores);
    ...
    elseif (mb_strpos($array[$i], "/") !== FALSE) {
      $lisa .= " and (";
...
if ((int)$tunnus == 0 and (int)$uusi == 0 and $errori == '') {
  ...
  if (mysqli_num_rows($result) >= 350) {
    ...
    if ($toim == "asiakas" and $yhtiorow['toimipaikkakasittely'] == 'L') {
      for ($i = 1; $i < mysqli_num_fields($result); $i++) {
        if (mysqli_field_name($result, $i) == "toimipaikka" and $haku[$i] != 'kaikki') {
  ...
  if (!in_array($yhtiorow['livetuotehaku_hakutapa'], array('F', 'G')) and $toim == "tuote" and $uusi != 1 and $errori == '' and isset($tmp_tuote_tunnus) and $tmp_tuote_tunnus > 0) {
    ...
    $nykyinentuote = mysqli_fetch_array($nykyinenresult);
    ...
    $noperow = mysqli_fetch_array($noperes);
    ...
    $yesrow = mysqli_fetch_array($yesres);
  ...
  if ($from != "" and mysqli_num_rows($result) > 0) {
    for ($i = 1; $i < mysqli_num_fields($result); $i++) {
      if (mb_strpos(mb_strtoupper(mysqli_field_name($result, $i)), "HIDDEN") === FALSE) {
  ...
  elseif ($from == "") {
    for ($i = 1; $i < mysqli_num_fields($result); $i++) {
      if (mb_strpos(mb_strtoupper(mysqli_field_name($result, $i)), "HIDDEN") === FALSE) {
        ...
        echo "<th><a href='yllapito.php?toim=$aputoim&lopetus=$lopetus&ojarj=".($i+1)."_".$edosuu."$ulisa&limit=$limit&nayta_poistetut=$nayta_poistetut&nayta_eraantyneet=$nayta_eraantyneet&laji=$laji'{$tuote_status_lisa}>" . t(mysqli_field_name($result, $i)) . "</a>";

        if (mysqli_num_fields($result) <= 6 and mysqli_field_len($result, $i) > 10) $size='15';
	  ...
	  if (!is_array($haku)) $haku = array(); // MUOKKAUS: isset()
          ...
          if ($i == 1) {
            ...
            while ($keraysvyohyke_chk_row = mysqli_fetch_assoc($keraysvyohyke_chk_res)) {
        ...
        elseif ($toim == "toimi" and mysqli_field_name($result, $i) == "toimittajaryhma") {
          echo "<br />";
          echo "<select name='haku[{$i}]'>";
          echo "<option value=''></option>";
          ...
          while ($toimittajaryhmarow = mysqli_fetch_assoc($_ryhmares)) {
        ...
        elseif ($toim == "asiakas" and $yhtiorow['toimipaikkakasittely'] == 'L' and mysqli_field_name($result, $i) == "toimipaikka") {
          ...
          $sel = mb_strtolower($haku[$i]) == "kaikki" ? "selected" : "";
          ...
          while ($toimipaikka_chk_row = mysqli_fetch_assoc($toimipaikka_chk_res)) {
        ...
        elseif ($toim == "pakkauskoodit") {
          if (mb_strtolower(mysqli_field_name($result, $i)) == 'koodi') {
            echo "<br><input type='text' name='haku[$i]' value='$haku[$i]' size='$size' maxlength='" . mysqli_field_len($result, $i) ."'>";
          }
          elseif (mb_strtolower(mysqli_field_name($result, $i)) == 'rahdinkuljettaja') {
            ...
            while ($rahkulj_chk_row = mysqli_fetch_assoc($rahdinkuljettaja_chk_res)) {
          ...
          elseif (mb_strtolower(mysqli_field_name($result, $i)) == 'pakkaus') {
  ...
  while ($trow = mysqli_fetch_array($result)) {
    ...
    for ($i=1; $i < mysqli_num_fields($result); $i++) {
      if (mb_strpos(mb_strtoupper(mysqli_field_name($result, $i)), "HIDDEN") === FALSE) {
        ...
        if (mb_stripos(mysqli_field_name($result, $i), "ytunnus") !== FALSE) {
        ...
        if ($i == 1) {
          ...
          if (mysqli_field_name($result, $i) == 'liitedata') {
            ...
            if ($lukitse_laji == "tuote" and $lukitse_avaimeen > 0 and in_array($trow[1], array("image/jpeg", "image/jpg", "image/gif", "image/png", "image/bmp"))) {
              ...
              if (file_exists("pics/tiedostotyyppiikonit/".mb_strtoupper($liitedata2).".ico")) {
                echo "<img src='".$palvelin2."pics/tiedostotyyppiikonit/".mb_strtoupper($liitedata2).".ico' height='80px'><br>".t("Muokkaa liitettä");
              }
              elseif (file_exists("pics/tiedostotyyppiikonit/".mb_strtoupper($ext).".ico")) {
                echo "<img src='".$palvelin2."pics/tiedostotyyppiikonit/".mb_strtoupper($ext).".ico' height='80px'><br>".t("Muokkaa liitettä");
              }
              else {
          ...
          elseif (mysqli_field_name($result, $i) == 'toim_tuoteno_tunnus') {
          ...
          if (mysqli_field_type($result, $i) == 'real' or mysqli_field_type($result, $i) == 'int') {
            echo "<td style='text-align:right'>$fontlisa1 $trow[$i] $fontlisa2</td>";
          }
          elseif (mysqli_field_name($result, $i) == 'koko') {
            echo "<td>$fontlisa1 ".size_readable($trow[$i])." $fontlisa2</td>";
          }
          elseif (mysqli_field_name($result, $i) == 'toim_tuoteno') {
  ...
  if ($from == "" and ($toim == "asiakasalennus" or $toim == "asiakashinta" or $toim == "hinnasto" or $toim == "puun_alkio") and $oikeurow['paivitys'] == 1) {
    $span = mysqli_num_fields($result)-2;
    echo "<tr>";
...
if ((int)$tunnus > 0 or (int)$uusi != 0 or $errori != '') {
  ...
  if ($uusi == '1' and $toim == 'tuote') {
    ...
    if (mysqli_num_rows($toimiresult) > 0) {
      ...
      while ($toimirow = mysqli_fetch_assoc($toimiresult)) {
  ...
  for ($i=0; $i < mysqli_num_fields($result) - 1; $i++) {
    ...
    if ($yhtiorow['maa'] != 'EE' and mysqli_field_name($result, $i) == 'intrastat_kurssi') {
      continue;
    }
    ...
    elseif ($uusi == 1) {
      if (isset($oletus[mysqli_field_name($result, $i)])) {
        $trow[$i] = $oletus[mysqli_field_name($result, $i)];
      }
    }
    ...
    if (mysqli_num_rows($al_res) > 0) {
      ...
      if ($al_row["selitetark"] != '') {
        ...
        $otsikko = t(mysqli_field_name($result, $i));
      }
      ...
      switch (mysqli_field_name($result, $i)) {
      case "printteri0":
      ...
      case "isa_varasto":
        ...
        break;
      default:
        if (isset($mysqlaliasarraysetti) and isset($mysqlaliasarray[$mysqlaliasarraysetti][mysqli_field_name($result, $i)])) {
          $otsikko = t($mysqlaliasarray[$mysqlaliasarraysetti][mysqli_field_name($result, $i)]);
        }
        else {
          $otsikko = t(mysqli_field_name($result, $i));
        }
    ...
    if (mysqli_num_rows($al_res) == 0 and $rajattu_nakyma != '') {
      $ulos = "";
      $tyyppi = 0;
    }
