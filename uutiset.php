...
if (mb_strpos($_SERVER['SCRIPT_NAME'], "uutiset.php")  !== FALSE) {
...
if ($toim == "") {
  if (mb_strpos($_SERVER['SCRIPT_NAME'], "uutiset.php")  !== FALSE) echo "<font class='head'>".t("Intra Uutiset")."</font><hr>";
  $tyyppi = "uutinen";
}
...
if ($tee == 'LISAA') {
  ...
  elseif (mb_strlen($otsikko) > 0 and mb_strlen($uutinen) > 0 and count($lang) > 0) {
    ...
    $uutinen = sanitize_string($uutinen);
    ...
    for ($i=0; $i < count($lang); $i++) {
      ...
      $katunnus = mysqli_insert_id($GLOBALS["masterlink"]);
...
if ($tee == '') {
  ...
  if (mysqli_num_rows($result) > 0) {
    ...
    while ($uutinen = mysqli_fetch_array($result)) {
