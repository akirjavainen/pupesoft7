...
if ($tee == "SYOTA") {
  ...
  $lisays .= "<option value='KOKOPAIVA' $sel>".t("Kokopäivä")."</option>";
  $lisays .= "</select></td></tr>";
...
for ($i=1; $i <= days_in_month($kuu, $year); $i++) {
  ...
  if (mysqli_num_rows($result) != 0) {
    ...
    while ($tyolistarow = mysqli_fetch_assoc($result)) {
...
if (!empty($viikkonakyma)) {
  ...
  // Korjataan tiedot jos kuukausi/vuosi vaihtuu keskellä viikkoa
...
function piirra_kalenteripaiva($year, $kuu, $paiva, $aikasarake = TRUE, $aikavali = 15) {
  ...
  while ($varirow = mysqli_fetch_assoc($varires)) {
  ...
  while ($kello_nyt != $whileloppu) {
    ...
    foreach ($paivantapahtumat["$year-$kuu-$paiva $kello_nyt:00"] as $ind => $row) {
      $kesto = round(($row['kesto']/60)/$aikavali); //kuinka monta solua tämä itemi kestää

      // Suurin päällekkäismäärä tän tapahtuman aikana?
      for ($i = $aikalask; $i < $aikalask+$kesto; $i++) {
  ...
  while ($kello_nyt != $whileloppu) {
    ...
    foreach($paivantapahtumat["$year-$kuu-$paiva $kello_nyt:00"] as $row) {
      ...
      if ($row["liitostunnus"] > 0 and $row['tyyppi'] == 'kalenteri') {
        ...
        $asiak = mysqli_fetch_assoc($asres);
      }
      ...
      elseif ($row['tyyppi'] == 'asennuskalenteri') {
        echo t("Asennustyö"), " $row[liitostunnus] ";
      }
      else {
