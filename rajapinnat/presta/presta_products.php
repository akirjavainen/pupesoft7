...
class PrestaProducts extends PrestaClient {
  ...
  protected function generate_xml($product, ?SimpleXMLElement $existing_product = null) {
    ...
    if (is_numeric($product['ean']) and mb_strlen($product['ean']) < 14) {
      $_ean = $product['ean'];
    }
    ...
    if ($this->visibility_type == 2) {
      ...
      elseif ($product['status'] == 'P' and $stock <= 0) {
        $this->logger->log("Status P ja saldo <= 0, ei näytetä verkkokaupassa.");
        $visibility = 'none';
        $active = 0;
      }
    }
    else {
      $this->logger->log("Tuote aktiivinen ja näytetään verkkokaupassa.");
    }
    ...
    foreach ($product['tuotteen_kaannokset'] as $translation) {
      ...
      $field = mb_strtolower($translation['kentta']);
  ...
  private function add_child_product(SimpleXMLElement &$xml, $product) {
    ...
    $this->logger->log("Lisättiin lapsituote {$sku} ({$product_id})");
