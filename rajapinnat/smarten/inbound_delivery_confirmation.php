...
while (false !== ($file = readdir($handle))) {
  ...
  foreach ($tilausrivit as $rivitunnus => $data) {
    ...
    if (mysqli_num_rows($tilausrivires) != 1) {
    ...
    $tilausrivirow = mysqli_fetch_assoc($tilausrivires);
    ...
    foreach ($varastomaarat as $varastotunnus => $varastomaara) {
      ...
      if (mysqli_affected_rows($GLOBALS["masterlink"]) == 1) {
        ...
        $paikkarow = mysqli_fetch_assoc($paikkares);
