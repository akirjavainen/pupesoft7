...
if (isset($tee) and trim($tee) == 'aja') {
  ...
  */
  ...
  $_normal_positio  = mb_strpos($filu, "normal");
  $_comments        = mb_substr($filu, $_normal_positio + 16, -4);
  ...
  foreach ($rivit as $line) {
    ...
    if (mysqli_num_rows($result) == 1) {
      $varasto = mysqli_fetch_assoc($result);
    }
    else {
    ...
    if (mysqli_num_rows($result) == 1) {
      $tuotteentoimittaja = mysqli_fetch_assoc($result);
    }
    else {
