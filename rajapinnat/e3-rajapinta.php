...
if ($php_cli) {
  ...
  if (mysqli_num_rows($yhtiores) == 1) {
    $yhtiorow = mysqli_fetch_assoc($yhtiores);
  ...
  if ($login_result) {
    ...
    if ($ftpfile != "" and $renameftpfile != "") {
      ...
      if ($upload === TRUE) {
        ...
        if (mb_stripos($rename[0], "renamed as") === FALSE) {
          $rename = FALSE;
        }
      }
    }
...
function xauxi($tanaan) {
  ...
  $rows = mysqli_num_rows($rested);

  $fp = fopen($path_xauxi, 'w+');

  $row = 0;

  while ($tuote = mysqli_fetch_assoc($rested)) {
...
function xlto($tanaan) {
  ...
  while ($xlto = mysqli_fetch_assoc($rest)) {
...
function xswp($tanaan, $korvatut) {
  ...
  while ($korvaavat = mysqli_fetch_assoc($rest)) {
    ...
    $korvaavarows = mysqli_num_rows($korvaavaresult);
    if ($korvaavarows == 0) continue;
    $korvaava = mysqli_fetch_assoc($korvaavaresult);
    ...
    $tuto2 = mysqli_fetch_assoc($tutoq2);

    if ($tuto2['toimittaja'] == '' or $tuto2['tyyppi'] == 'P') continue;

    if ($korvatut != "") {
      // laitetaan xf02 loppuun tieto mikä tuote on poistettu. toimittaja ja tuoteno.
      $xf02loppulause .= "$tuto2[toimittaja] ".mb_str_pad($korvaava['tuoteno'], 17)." 001000000000000000000000000000000000000000000000000000000                                                        D\n";
    }
    else {
      // eka korvatun toimittaja + tuoteno, sitten korvaavan toimittaja ja tuoteno
      $lause = "E3T001$tuto2[toimittaja] ".mb_str_pad($korvaava['tuoteno'], 17)." 001$tuto[toimittaja] ".mb_str_pad($korvaavat['tuoteno'], 17)." U1    000000000000000000000000000AYNY";
...
function xvni($tanaan) {
  ...
  $rows = mysqli_num_rows($resto);

  $fp = fopen($path_xvni, 'w+');

  while ($xvni = mysqli_fetch_assoc($resto)) {
...
function xf04($tanaan) {
  ...
  while ($xf04 = mysqli_fetch_assoc($resto)) {
    ...
    $out   .= sprintf("%-5.5s", mb_substr($xf04['try'], 0, 1));  //XGRP1
    $out   .= sprintf("%-5.5s", mb_substr($xf04['try'], 1, 2));  //XGRP2
    $out   .= sprintf("%-5.5s", mb_substr($xf04['try'], 3, 6));  //XGRP3
...
function xf01($tanaan) {
  ...
  $Q1 = "SELECT tuote.tuoteno,
         ...
         HAVING (korvaavatuoteno = tuote.tuoteno OR korvaavatuoteno is null)";
  $rests = pupe_query($Q1);
  $rows = mysqli_num_rows($rests);

  $fp = fopen($path_xf01, 'w+');

  while ($tuoterow = mysqli_fetch_assoc($rests)) {
    ...
    $q2r =  pupe_query($Q2);
    $myyntirow = mysqli_fetch_assoc($q2r);
    ...
    $Q2 = "SELECT round(SUM(tilausrivi.varattu), 0) myyty2
           ...
           AND tilausrivi.toimitettuaika  <= '$tanaan 23:59:59'";
    $q2r2 =  pupe_query($Q2);
    $myyntirow2 = mysqli_fetch_assoc($q2r2);
    ...
    $q3r =   pupe_query($Q3);
    $ostorow = mysqli_fetch_assoc($q3r);
...
function xf02($tanaan, $xf02loppulause) {
  ...
  while ($valurow = mysqli_fetch_assoc($resaluutta)) {
  ...
  while ($xf02 = mysqli_fetch_assoc($rests)) {
    ...
    $toim_row = mysqli_fetch_assoc($rest_toimittajista);
    ...
    if (trim(mb_strtoupper($valuutta)) != trim(mb_strtoupper($yhtiorow["valkoodi"]))) {
