  ...
  if ($login_result) {
    ...
    if (!file_exists($kokonimi)) {
      ...
      if ($handle === FALSE) {
        ...
        for ($i=0; $i < $fields; $i++) {
          $ulos .= mysqli_field_name($result, $i);
        ...
        while ($row = mysqli_fetch_array($result)) {
          for ($i=0; $i < $fields; $i++) {
            ...
            if (mb_strpos($row[$i], '"') !== FALSE) {
              $row[$i] = str_replace('"', "", $row[$i]);
            }

            if (mb_strpos(($temp = mysqli_field_name($result, $i)), "lyhytkuvaus") !== FALSE) {
              $row[$i] = cut_text($row[$i], 100);
            }

            /*
            if (mb_strpos(($temp = mysqli_field_name($result, $i)),"mainosteksti") !== FALSE or mb_strpos(($temp = mysqli_field_name($result, $i)),"kuvaus") !== FALSE) {
                ...
                $ulcount = mb_substr_count($row[$i], "<ul>");

                $ulppos = 0;
                for ($s=0; $s < $ulcount; $s++) {
                  $ulpos = mb_strpos($row[$i], "<ul>",$ulpos);

                  $ilpos = mb_strpos($row[$i], "[*]", $ulpos);
...
function cut_text($text, $chars) {
  ...
  if (mb_strlen($text) == 0 or mb_strlen($text) < $chars) {
    return $text;
  }
  else {

    if (mb_substr($text, $chars) == " ") {
      return mb_substr($text, 0, $chars)."...";
    }
    elseif (mb_substr($text, $chars) == ".") {
      return mb_substr($text, 0, $chars)."..";
    }
    elseif (mb_substr($text, $chars) == ",") {
      return mb_substr($text, 0, $chars-1)."...";
    }
    else {

      $text = mb_substr($text, 0, $chars);
      $pos[0] = mb_strrpos($text, " ");
      $pos[1] = mb_strrpos($text, ".");
      $pos[2] = mb_strrpos($text, ",");


      sort($pos);

      if (mb_substr($text, $pos[2], 1) == " ") {
        return mb_substr($text, 0, $pos[2])."...";
      }
      elseif (mb_substr($text, $pos[2], 1) == ".") {
        return mb_substr($text, 0, $pos[2])."..";
      }
      elseif (mb_substr($text, $pos[2], 1) == ",") {
        return mb_substr($text, 0, $pos[2]-1)."...";
      }
