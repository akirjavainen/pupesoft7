...
if ($ostotilaus != '' or $tuotenumero != '' or $viivakoodi != '' or $saapumisnro_haku != '') {
  ...
  if (mb_strpos($tuotenumero, "%") !== FALSE) $tuotenumero = urldecode($tuotenumero);
...
$keskeneraiset_query = "SELECT kuka.kesken FROM lasku
                        ...
                        and lasku.tila='K'";
$keskeneraiset = mysqli_fetch_assoc(pupe_query($keskeneraiset_query));
...
$tilausten_lukumaara = mysqli_num_rows($result);
...
echo "</table></div>";
echo "Rivejä: ".mysqli_num_rows($result);
