...
if (isset($submit) and trim($submit) != '') {
  ...
  if ($submit == 'submit') {
    ...
    if ($koodi != '') {
      ...
      if (preg_match('/^([a-zåäö#0-9]{2,4} [a-zåäö#0-9]{2,4})/i', $tuotepaikka) and mb_substr_count($tuotepaikka, " ") == 1) {
      ...
      if (isset($suuntalavan_tuotteet) and count($suuntalavan_tuotteet) > 0) {
        ...
        foreach ($suuntalavan_tuotteet as $_tun => $_maara) {
          ...
          if (trim($_maara) != '') {
            ...
            $chk_varattu_row = mysqli_fetch_assoc($chk_varattu_res);
...
$sscc = mysqli_fetch_assoc($ssccres);
...
if (mysqli_num_rows($tuotteet_res) > 0) {
