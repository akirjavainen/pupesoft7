...
if (isset($alusta_tunnus)) {
  ...
  if (mysqli_num_rows($res) == 0 && $eankoodi != '') {
  ...
  $lava = mysqli_fetch_assoc($result);
}
...
foreach ($tuotteet as $tuote) {
  ...
  if ($row = mysqli_fetch_assoc($onko_suoratoimitus_res)) {
  ...
  if ($oletuspaikat) {
    ...
    $oletus = mysqli_fetch_assoc($oletus_result);
