...
if ($toim != "") {
  ...
  $al_row = mysqli_fetch_assoc($al_res);
...
if ($yhtiorow['konserni'] != "") {
  ...
  $yhtio_array = mysqli_fetch_assoc($yhtio_res);
  ...
  $query = "SELECT nimi, yhtio
            ...
            and konserni   != ''";
  $yhtio_res = pupe_query($query);
  $numrows = mysqli_num_rows($yhtio_res);
  ...
  while ($yhtio_row = mysqli_fetch_assoc($yhtio_res)) {
...
if ($tee == 'laske') {
  ...
  while ($row = mysqli_fetch_assoc($result)) {
    ...
    $kuu = mb_str_pad($kuu, 2, "0", STR_PAD_LEFT);
    $vko = mb_str_pad($vko, 2, "0", STR_PAD_LEFT);
  ...
  if ($tavoitteet) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
      ...
      $vuosi = mb_substr($row['kausi'], 0, 4);
      $kuu   = mb_substr($row['kausi'], 4, 2);
      ...
      for ($i = 0; $i < $paivia; $i++) {
        ...
        $paiva = $i+1;
        $paiva = mb_str_pad($paiva, 2, "0", STR_PAD_LEFT);
  ...
  function aikasorttaus($a, $b) {
    ...
    if ($naytetaan_tulos == 'weekly') {
      $a_vuo = mb_substr($a, 3, 4);
      $a_vko = mb_substr($a, 0, 2);
      $a = strtotime("{$a_vuo}W{$a_vko}");

      $b_vuo = mb_substr($b, 3, 4);
      $b_vko = mb_substr($b, 0, 2);
      $b = strtotime("{$b_vuo}W{$b_vko}");
    }
    elseif ($naytetaan_tulos == 'monthly') {
      $a_vuo = mb_substr($a, 3, 4);
      $a_kuu = mb_substr($a, 0, 2);
      $a = strtotime("{$a_vuo}-{$a_kuu}");

      $b_vuo = mb_substr($b, 3, 4);
      $b_kuu = mb_substr($b, 0, 2);
      $b = strtotime("{$b_vuo}-{$b_kuu}");
    }
    else {
      $a_vuo = mb_substr($a, 6, 4);
      $a_kuu = mb_substr($a, 3, 2);
      $a_pai = mb_substr($a, 0, 2);
      $a = strtotime("{$a_vuo}-{$a_kuu}-{$a_pai}");

      $b_vuo = mb_substr($b, 6, 4);
      $b_kuu = mb_substr($b, 3, 2);
      $b_pai = mb_substr($b, 0, 2);
