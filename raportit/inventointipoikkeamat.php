...
if ($tee == 'KORJAA') {
  ...
  $kpitorow = mysqli_fetch_assoc($kpitores);
...
if ($tee == 'Y') {
  ...
  if ((float) $prosmuutos < 0 and mb_substr($prosmuutos, 0, 1) == '-') {
    $prosmuutos = (float) $prosmuutos;
    $tuotepaikat_lisa .= "and tuotepaikat.inventointipoikkeama <= '$prosmuutos' ";
  }
  elseif ((float) $prosmuutos > 0 and mb_substr($prosmuutos, 0, 1) == '+') {
    $prosmuutos = (float) $prosmuutos;
    $tuotepaikat_lisa .= "and tuotepaikat.inventointipoikkeama >= '$prosmuutos' ";
  }
  ...
  elseif ($tila != 'tulosta' and $tila != 'tulosta_excel') {
    ...
    while ($tuoterow = mysqli_fetch_assoc($saldoresult)) {
      ...
      if ($toim == "SUPER" and $tuoterow["sarjanumeroseuranta"] == "S" and mysqli_num_rows($sarjares) == abs($tuoterow["kpl"])) {
...
if ($tee == "TULOSTA" and mysqli_num_rows($saldoresult) > 0 ) {
  ...
  while ($row = mysqli_fetch_assoc($saldoresult)) {
    if ($rivit >= 19) {
      fwrite($fh, $ots);
      $rivit = 1;
    }
...
if ($tee == "TULOSTAEXCEL" and mysqli_num_rows($saldoresult) > 0 ) {
