...
if (mb_strpos($_SERVER['SCRIPT_NAME'], "tuloslaskelma.php")  !== FALSE) {
...
if (isset($teetiedosto)) {
  ...
  if ($tltee == "aja") {
    ...
    if (mysqli_num_rows($result) > 0 ) {
      ...
      echo "<br><font class='error'>".t("HUOM")."!  ".t("Sinulla on %s tiliä joilta puuttuu tai on virheellinen %s taso", "", mysqli_num_rows($result), $sisulk_txt)."!<br>".t("Jos näillä tileillä on tapahtumia, niin tapahtumat eivät näy laskelmassa").".</font><br><a href=\"javascript:toggleGroup('eee')\">".t("Näytä / Piilota")." ".t("tilit")."</a><br><br>";
  ...
  $result = pupe_query($query);
  $ekakausirow = mysqli_fetch_assoc($result);
  ...
  if (!isset($plvv)) {
    ...
    $result = pupe_query($query);
    $tilikausirow = mysqli_fetch_assoc($result);

    $plvv = mb_substr($tilikausirow['tilikausi_alku'], 0, 4);
    $plvk = mb_substr($tilikausirow['tilikausi_alku'], 5, 2);
    $plvp = mb_substr($tilikausirow['tilikausi_alku'], 8, 2);
  }
  ...
  for ($i = date("Y"); $i >= (float) mb_substr($ekakausirow['alkukausi'], 0, 4); $i--) {
  ...
  for ($i = date("Y")+1; $i >= (float) mb_substr($ekakausirow['loppukausi'], 0, 4); $i--) {
    echo "<option value='$i' $sel[$i]>$i</option>";
  }
  ...
  if ($tltee == "aja") {
    ...
    if ($plvk == '' or $plvv == '') {
      $plvv = mb_substr($yhtiorow['tilikausi_alku'], 0, 4);
      $plvk = mb_substr($yhtiorow['tilikausi_alku'], 5, 2);
    }
    ...
    if ((int) $tkausi > 0) {
      ...
      $result = pupe_query($query);
      $tkrow = mysqli_fetch_assoc($result);

      $plvv = mb_substr($tkrow['tilikausi_alku'], 0, 4);
      $plvk = mb_substr($tkrow['tilikausi_alku'], 5, 2);
      $plvp = mb_substr($tkrow['tilikausi_alku'], 8, 2);

      $alvv = mb_substr($tkrow['tilikausi_loppu'], 0, 4);
      $alvk = mb_substr($tkrow['tilikausi_loppu'], 5, 2);
      $alvp = mb_substr($tkrow['tilikausi_loppu'], 8, 2);
    }
    ...
    if ((isset($lisa) and mb_strpos($lisa, "asiakas.") !== FALSE) or (isset($sarakebox["ASOSASTO"]) and $sarakebox["ASOSASTO"] != "") or (isset($sarakebox["ASRYHMA"]) and $sarakebox["ASRYHMA"] != "")) {
    ...
    if ($groupsarake != "") {
      $groupsarake = "concat(".mb_substr($groupsarake, 0, -7).")";
    }
    else {
  ...
  if ($tltee == "aja") {
    ...
    if (mysqli_num_rows($tulostilires) == 1) {
      $tulostilirow = mysqli_fetch_assoc($tulostilires);
    }
    ...
    if (mysqli_num_rows($oikresult) > 0) {
      $oikrow = mysqli_fetch_assoc($oikresult);
    ...
    for ($i = $startmonth;  $i <= $endmonth;) {
      ...
      else $alku = date("Y-m-d", mktime(0, 0, 0, mb_substr($i, 4, 2), mb_substr($i, 6, 2), mb_substr($i, 0, 4)));

      if ($i == $endmonth) $loppu = $alvv."-".$alvk."-".$alvp;
      else $loppu = date("Y-m-d", mktime(0, 0, 0, mb_substr($i, 4, 2)+1, 0, mb_substr($i, 0, 4)));

      $bukausi = date("Ym",    mktime(0, 0, 0, mb_substr($i, 4, 2), mb_substr($i, 6, 2), mb_substr($i, 0, 4)));
      $headny  = date("Y/m",   mktime(0, 0, 0, mb_substr($i, 4, 2), mb_substr($i, 6, 2),  mb_substr($i, 0, 4)));
      ...
      if ($vertailued != "") {
        ...
        else $alku_ed  = date("Y-m-d", mktime(0, 0, 0, mb_substr($i, 4, 2), mb_substr($i, 6, 2), mb_substr($i, 0, 4)-1));
        ...
        else $loppu_ed = date("Y-m-d", mktime(0, 0, 0, mb_substr($i, 4, 2)+1, 0, mb_substr($i, 0, 4)-1));

        $headed   = date("Y/m",   mktime(0, 0, 0, mb_substr($i, 4, 2), mb_substr($i, 6, 2), mb_substr($i, 0, 4)-1));
    ...
    while ($tilirow = mysqli_fetch_assoc($tilires)) {
    ...
    if (isset($tulostilirow) and ($tyyppi == "T" or $tyyppi == "2")) {
      ...
      $tulosrow = mysqli_fetch_assoc($tulosres);
      ...
      if ($tulosrow['tilit'] != '') {
        ...
        while ($tulosrow = mysqli_fetch_assoc($tulosres)) {
    ...
    while ($tasorow = mysqli_fetch_assoc($tasores)) {

      // millä tasolla ollaan (1,2,3,4,5,6)
      $tasoluku = mb_strlen($tasorow["taso"]);

      // tasonimi talteen (rightpäddätään Z:lla tai Ö:llä, niin saadaan oikeaan järjestykseen)
      if (PUPE_UNICODE) {
        $apusort = mb_str_pad($tasorow["taso"], 20, "Z");
      }
      else {
        $apusort = mb_str_pad($tasorow["taso"], 20, "Ö");
      }
      ...
      for ($i = 0; $i < $tasoluku; $i++) {
        $taso[$i] = mb_substr($tasorow["taso"], 0, $i+1);
      }
        ...
        while ($tilirow = mysqli_fetch_assoc($tilires)) {
          ...
          foreach ($tilirow_summat as $sarake => $tilirow_sum) {
            ...
            foreach ($kaudet as $kausi) {
              ...
              if ($vertailubu2 == "" and mb_substr($kausi, 0, 4) == "budj") {
                $i = $tasoluku - 1;
    ...
    foreach ($tasonimi as $key_c => $value) {
      ...
      if (mb_strlen($key) <= $rtaso or $rtaso == "TILI") {

        $class = "";

        // laitetaan ykkös ja kakkostason rivit tummalla selkeyden vuoksi
        if (mb_strlen($key) < 3 and $rtaso > 2) $class = "tumma";
        ...
        for ($i = $alkukausi; $i < count($kaudet); $i++) {
          foreach ($sarakkeet as $sarake) {
            ...
            if ($vertailubu2 == "") {
              $bu_check = (mb_substr($kaudet[$i], 0, 4) != "budj");
            }
            else {
              $bu_check = true;
            }

            if ($summarow = mysqli_fetch_assoc($summares) and $bu_check) {
        ...
        if (mb_strlen($key) == 2 and ($rtaso > 2 or $rtaso == "TILI")) {
        ...
        if (mb_strlen($key) == 1 and ($rtaso > 1 or $rtaso == "TILI")) {
        ...
        function alku() {
          ...
          for ($i = $alkukausi; $i < count($kaudet); $i++) {
            foreach ($sarakkeet as $sarake) {
              ...
              $oikpos1 = $pdf->mb_strlen($kaudet[$i], $b);
              $oikpos2 = $pdf->mb_strlen($sarakenimi, $b);
    ...
    if ((isset($teepdf) and $teepdf == "OK") or (isset($teexls) and $teexls == "OK")) {
      ...
      foreach ($rivit_px as $sarakkeet_px) {
        ...
        foreach ($sarakkeet_px as $arvo) {
          if ($pi == 0) {
            $sisennys = 10+(mb_strlen($arvo)-1)*3;
