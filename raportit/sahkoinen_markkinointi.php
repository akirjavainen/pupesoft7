...
if (isset($tee) and $tee == "lataa_tiedosto") {
  ...
  while ($kieli_row = mysqli_fetch_assoc($kieli_res)) {
    $sel = '';
  ...
  while ($dynamic_row = mysqli_fetch_assoc($dynamic_res)) {
    ...
    if ($dynamic_row['laskuri'] > 1) {
      ...
      while ($filler_row = mysqli_fetch_assoc($filler_res)) {
        $sel = '';
      ...
      if (mb_strtolower($dynamic_row['selite']) == 'syntymavuosi') {
        ...
        while ($avain_row = mysqli_fetch_assoc($avain_res)) {
          $sel = '';
  ...
  if (isset($etsi)) {
    ...
    if (isset($dyn) and count($dyn) > 0) {
      ...
      foreach ($dyn as $muuttuja) {
        if ($$muuttuja != '') {
          $$muuttuja = sanitize_string($$muuttuja);

          $av_laji .= "'".$muuttuja."',";
          $av_sana .= "'".${$muuttuja}."',";
        }
      }
      $av_laji = mb_substr($av_laji, 0, -1);
      $av_sana = mb_substr($av_sana, 0, -1);
    ...
    if ($nimi != '') {
      $nimi = sanitize_string($nimi);
      $query .= " AND asiakas.nimi like '%$nimi%' ";
    }

    if ($postitp != '') {
      $postitp = sanitize_string($postitp);
    ...
    if ($postino1 != '') {
      ...
      if (mb_strlen($postino1) < 5) {
        $postino1 = mb_str_pad($postino1, 5, "0", STR_PAD_LEFT);
      }
    ...
    if ($postino2 != '') {
      ...
      if (mb_strlen($postino2) < 5) {
        $postino2 = mb_str_pad($postino2, 5, "0", STR_PAD_LEFT);
      }
    ...
    if (isset($workbook)) {
      ...
      while ($otsikot_row = mysqli_fetch_assoc($otsikot_res)) {
    ...
    while ($row = mysqli_fetch_assoc($res)) {
      ...
      $rivit = mysqli_fetch_assoc($summress);


      if ((mysqli_num_rows($summress) > 0 and $myynti != 0) or $myynti == 0) {
        ...
        if (isset($workbook)) {
          ...
          mysqli_data_seek($otsikot_res, 0);
          ...
          while ($otsikot_row = mysqli_fetch_assoc($otsikot_res)) {
            ...
            $attr_row = mysqli_fetch_assoc($attr_res);
