...
function varastosiirrot($yhtiot, $varasto_tunnus) {
  ...
  while ($row = mysqli_fetch_assoc($varastoon_result)) {
  ...
  while ($row = mysqli_fetch_assoc($varastosta_result)) {
...
function saldot($myynti_varasto = '', $myynti_maa = '') {
  ...
  if (mysqli_num_rows($result) > 0) {
    while ($varrow = mysqli_fetch_assoc($result)) {
...
function ostot($myynti_varasto = '', $myynti_maa = '') {
  ...
  if ($toim == "BIO") {
    ...
    $query = "SELECT ifnull(sum(tilausrivi.varattu), 0) tilattu
              ...
              and ((tilausrivi.kerayspvm >= '$vva4-$kka4-$ppa4' and tilausrivi.kerayspvm <= '$vvl4-$kkl4-$ppl4') or tilausrivi.kerayspvm = '0000-00-00')";
    $result = pupe_query($query);
    $ostorow = mysqli_fetch_assoc($result);
  }
  else {
    //tilauksessa
...
if (mysqli_num_rows($res2) > 0) {
  ...
  while ($rivi = mysqli_fetch_assoc($res2)) {
    $mul_check = '';
...
if (mysqli_num_rows($vtresult) > 0) {
  ...
  while ($vrow = mysqli_fetch_assoc($vtresult)) {
    ...
    if ($useampi_yhtio > 1) {
      ...
      $yhtrow = mysqli_fetch_assoc($yhtres);
      echo "$yhtrow[nimi]: ";
    }
