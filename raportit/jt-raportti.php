...
while ($yrow = mysqli_fetch_array($yhtio_result)) {
  $yhtio = $yrow['yhtio'];
  ...
  while ($toimrow = mysqli_fetch_array($toimresult)) {
    ...
    while ($liitostunnus_row = mysqli_fetch_array($liitostunnus_result)) {
      ...
      $asiakasrow = mysqli_fetch_array($asiakasresult);
      $kukarow["kieli"] = mb_strtolower($asiakasrow["kieli"]);
      ...
      if ($asiakasrow["email"] != "") {
        ...
        if (mysqli_num_rows($jtresult) > 0) {
          ...
          while ($jtrow = mysqli_fetch_array($jtresult)) {
