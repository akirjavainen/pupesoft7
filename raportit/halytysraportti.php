...
if ($valitut["OSTOTVARASTOITTAIN"] != "") {
  ...
  while ($vrow = mysqli_fetch_assoc($osvres)) {
...
if ($tee == "RAPORTOI" and isset($RAPORTOI)) {
  ...
  if ($osasto != '') {
    ...
    $trow = mysqli_fetch_assoc($sresult);
  }
  if ($toimittajaid != '') {
    $query = "SELECT nimi
              FROM toimi
    ...
    $trow1 = mysqli_fetch_assoc($sresult);
  }
  if ($asiakasid != '') {
    $query = "SELECT nimi
              FROM asiakas
    ...
    $sresult = pupe_query($query);
    $trow2 = mysqli_fetch_assoc($sresult);
  }
  ...
  if ($onkolaajattoimipaikat) {
    ...
    elseif ($toimipaikka > 0) {
      ...
      $toimipaikka_row = mysqli_fetch_assoc($toimipaikka_res);
  ...
  if ($abcrajaus != "") {
    ...
    $vtresult = pupe_query($query);
    $vrow = mysqli_fetch_assoc($vtresult);
  ...
  if (mysqli_num_rows($res) > 0) {
    ...
    $bar = new ProgressBar();
    $bar->initialize(mysqli_num_rows($res));
    ...
    while ($row = mysqli_fetch_assoc($res)) {
      ...
      if ($paikoittain == '') {
        ...
        while ($saldo = mysqli_fetch_assoc($result)) {
      ...
      $query = "SELECT
                ...
                $lisa";
      $result   = pupe_query($query);
      $puuterow = mysqli_fetch_assoc($result);
      ...
      $query = "SELECT
                ...
                $lisa";
      $result   = pupe_query($query);
      $kulutrow = mysqli_fetch_assoc($result);
      ...
      if (isset($nayta_vain_ykkostoimittaja)) {
        $query = "SELECT sum(if(tilausrivi.tyyppi = 'O', tilausrivi.varattu, 0)) tilattu
                  ...
                  and tilausrivi.laskutettuaika = '0000-00-00'";
        $result = pupe_query($query);
        $ykkostoimittajarajattuna   = mysqli_fetch_assoc($result);
      ...
      $result = pupe_query($query);
      $osaldo = mysqli_fetch_assoc($result);
      ...
      if ($asiakasosasto != '') {
        ...
        $asosrow = mysqli_fetch_assoc($asosresult);
      }
      ...
      if ((!isset($valitut['EHDOTETTAVAT']) or $valitut['EHDOTETTAVAT'] == '') or $ostettavahalytilausmaara > 0 or $ostettavahaly > 0 or $ostettava4kk > 0) {
        ...
        if (mysqli_num_rows($korvaresult1) > 0) {
          $korvarow = mysqli_fetch_assoc($korvaresult1);
          ...
          while ($korvarow = mysqli_fetch_assoc($korvaresult2)) {
        ...
        if ($valitut["SARAKE64"] != '') {
          ...
          $asresult = pupe_query($query);
          $kasrow = mysqli_fetch_assoc($asresult);
        ...
        if (isset($korvaresult2) and is_resource($korvaresult2) and mysqli_num_rows($korvaresult2) > 0) {

          mysqli_data_seek($korvaresult2, 0);
          ...
          while ($korvarow = mysqli_fetch_assoc($korvaresult2)) {
            ...
            while ($korvasaldorow = mysqli_fetch_assoc($korvasaldoresult)) {
...
if ($tee == "JATKA" or $tee == "RAPORTOI") {
  ...
  $chk = "";
  if ((mysqli_num_rows($sresult) > 0 and $srow["selitetark"] == "EHDOTETTAVAT" and $tee == "JATKA") or $valitut["EHDOTETTAVAT"] != '') {
    $chk = "CHECKED";
  }
  ...
  if ($onkolaajattoimipaikat and isset($toimipaikka) and mysqli_num_rows($vtresult) == 0) {
  ...
  while ($vrow = mysqli_fetch_assoc($vtresult)) {
    ...
    if ($vlask == 0) {
      ...
      echo "<tr><th rowspan='".mysqli_num_rows($vtresult)."'>".t("Huomioi saldot varastossa:")."</th>";
    }
    else {
      echo "<tr>";
    }
  ...
  foreach ($sarakkeet as $key => $sarake) {
    ...
    $sel = "";
    if (mysqli_num_rows($sresult) == 1 or $rappari == "") {
      $sel = "CHECKED";
    }
