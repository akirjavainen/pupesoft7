...
if (mysqli_num_rows($avainsana_res) == 0) {
  ...
  echo date("d.m.Y @ G:i:s")." - Aloitetaan ".mysqli_num_rows($res)." tuotteen päivitys. ($kukarow[yhtio])\n";

  while ($row = mysqli_fetch_assoc($res)) {

    mysqli_data_seek($avainsana_res, 0);

    while ($avainsana_row = mysqli_fetch_assoc($avainsana_res)) {
