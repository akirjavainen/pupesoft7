...
if ($ytunnus != '' or $tuoteno != '' or (int) $asiakasid > 0 or (int) $toimittajaid > 0) {
  ...
  if (mysqli_num_rows($result) > 0) {
    ...
    for ($i=1; $i < mysqli_num_fields($result)-$miinus; $i++) {

      echo "<th align='left'><a href='$PHP_SELF?tee=$tee&toim=$toim&ppl=$ppl&vvl=$vvl&kkl=$kkl&ppa=$ppa&vva=$vva&kka=$kka&tuoteno=".urlencode($tuoteno)."&ytunnus=$ytunnus&asiakasid=$asiakasid&jarj=".mysqli_field_name($result, $i)."$pvmtapa_url'>".t(mysqli_field_name($result, $i))."</a></th>";

      $worksheet->write($excelrivi, $j, ucfirst(t(mysqli_field_name($result, $i))), $format_bold);
      $j++;

      if (mysqli_field_name($result, $i) == 'kate') {
        echo "<th align='left'><a href='$PHP_SELF?tee=$tee&toim=$toim&ppl=$ppl&vvl=$vvl&kkl=$kkl&ppa=$ppa&vva=$vva&kka=$kka&tuoteno=".urlencode($tuoteno)."&ytunnus=$ytunnus&asiakasid=$asiakasid&jarj=".mysqli_field_name($result, $i)."$pvmtapa_url'>".t("Katepros")."</a></th>";
    ...
    while ($row = mysqli_fetch_array($result)) {
      ...
      for ($i=1; $i<mysqli_num_fields($result)-$miinus; $i++) {
        ...
        if (mysqli_field_name($result, $i) == 'kerattyaika' or mysqli_field_name($result, $i) == 'toimaika' or mysqli_field_name($result, $i) == 'tuloutettu' or mysqli_field_name($result, $i) == 'Käsittelyyn') {
          echo "<$ero valign='top' $class>".tv1dateconv($row[$i], "pitka")."</$ero>";
        }
        elseif (mb_substr(mysqli_field_name($result, $i), 0, 3) == 'ale' or mysqli_field_name($result, $i) == 'määrä') {
          if ($row[$i] == 0) {
            ...
            elseif ($kukarow['extranet'] == '' and ($row["sarjanumeroseuranta"] == "S")) {
              ...
              elseif ($kpl < 0 and $row["osto_vai_hyvitys"] == "") {
                ...
                while ($sarjarow = mysqli_fetch_array($sarjares)) {
                  ...
                  $sarjarow1 = mysqli_fetch_array($sarjares1);
        ...
        elseif (mysqli_field_name($result, $i) == 'hinta' or mysqli_field_name($result, $i) == 'rivihinta') {
        ...
        if (is_numeric($row[$i]) and mysqli_field_name($result, $i) != 'ytunnus') {
