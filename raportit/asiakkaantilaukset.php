...
if (mb_substr($toim, 0, 8) == "KONSERNI") {
  $logistiikka_yhtio = '';
...
if ($tee != 'NAYTATILAUS' and empty($vaihda) and $ytunnus == '' and $tilausviite == '' and $astilnro == '' and $otunnus == '' and $laskunro == '' and $sopimus == '' and $kukarow['kesken'] != 0 and $til != '') {
  ...
  if (mysqli_num_rows($keskenresult) == 1) {
...
if ($tee == 'NAYTATILAUS') {
  ...
  $_tila = mysqli_fetch_assoc(pupe_query($query));
  ...
  if ($cleantoim == "MYYNTI" or $cleantoim == "TARJOUS" or $cleantoim == 'REKLAMAATIO' or $cleantoim == 'VALMISTUSMYYNTI' or $cleantoim == "TAKUU") {
    ...
    if (mysqli_num_rows($rahtiresult)> 0) {
      ...
      while ($rahtirow = mysqli_fetch_array($rahtiresult)) {
        ...
        $varnimrow = mysqli_fetch_array($varnimresult);
...
if ($ytunnus != '') {
  ...
  if (mb_substr($toim, 0, 8) == "KONSERNI" and $yhtiorow['konsernivarasto'] != '' and $konsernivarasto_yhtiot != '') {
  ...
  if (mysqli_num_rows($result) > 50 and $limittrikkeri == "A") {
  ...
  if (mysqli_num_rows($result) > 0) {
    ...
    for ($i=0; $i < mysqli_num_fields($result)-8; $i++) {
    ...
    for ($i=0; $i < mysqli_num_fields($result)-8; $i++) {
      echo "<td><input type='text' class='search_field' name='search_".t(mysqli_field_name($result, $i))."'></td>";
    }
    ...
    while ($row = mysqli_fetch_array($result)) {
      ...
      if ($row["laskunro"] > 0 and $row["laskunro"] != $edlaskunro) {
        ...
        $pkrow = mysqli_fetch_array($pkres);
      ...
      for ($i=1; $i<mysqli_num_fields($result)-8; $i++) {
        if (mysqli_field_name($result, $i) == 'toimaika') {
          echo "<td valign='top' $class>{$row[$i]}</td>";
        }
        elseif (mysqli_field_name($result, $i) == 'laskunro' and $row['tila'] == "U" and tarkista_oikeus("muutosite.php")) {
        ...
        elseif (is_numeric(trim($row[$i])) and mysqli_field_name($result, $i) != 'tilausviite' and mysqli_field_name($result, $i) != 'astilno') {
