...
function teerivi($tuoteno, $valittu_toimittaja, $abc_rajaustapa) {
  ...
  $alku_kausi  = mb_substr(str_replace("-", "", $nykyinen_alku), 0, 6);
  $loppu_kausi = mb_substr(str_replace("-", "", $nykyinen_loppu), 0, 6);
  ...
  if (mysqli_num_rows($result) == 1) {
    $toimittajarow = mysqli_fetch_assoc($result);
  }
  else {
...
if (isset($tee) and $tee == "TEE_VALMISTUKSET") {
  ...
  $lahde_varasto = sanitize_string($lahde_varasto);
  $kohde_varasto = sanitize_string($kohde_varasto);
  $valmistus_ajankohta = sanitize_string($valmistus_ajankohta);

  foreach ($valmistettavat_tuotteet as $tuoterivi) {

    $maara = (float) $tuoterivi["valmistusmaara"];
    $tuoteno = sanitize_string($tuoterivi["tuoteno"]);
    $valmistuslinja = sanitize_string($tuoterivi["valmistuslinja"]);
...
if (isset($ehdotusnappi) and $ehdotusnappi != "") {
  ...
  if (mysqli_num_rows($res) > 0) {
    ...
    if ($ehdotetut_valmistukset != 'valmistuslinjoittain') {
      ...
      echo t("Valmistuksia")." ".mysqli_num_rows($res)." ".t("kpl").".<br>\n";
  ...
  while ($row = mysqli_fetch_assoc($res)) {
    ...
    if (mysqli_num_rows($samankaltainen_result) > 0) {
    ...
    while ($samankaltainen_row = mysqli_fetch_assoc($samankaltainen_result)) {
      $kasiteltavat_key++;
...
if (!isset($tee) or $tee == "") {
  ...
  if ($toimittajaid == "") {
    ...
    $result = pupe_query($query);
    $toimittaja = mysqli_fetch_assoc($result);
