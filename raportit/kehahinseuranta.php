  ...
  while ($lrow = mysqli_fetch_array($result)) {
    ...
    if (mysqli_num_rows($eresult) > 0) {
      ...
      if ((abs($eropros) <= $pros1 and abs($eropros) >= $pros2) or ($lrow["hinta"] < 0)) {
        $rivit1[] = abs($eropros);
        $rivit2[] = "<tr><td><a href='../tuote.php?tee=Z&tuoteno=".urlencode($lrow["tuoteno"])."'>$lrow[tuoteno]</a></td><td>".t_tuotteen_avainsanat($lrow, 'nimitys')."</td><td>$lrow[hinta]</td><td>$erow[hinta]</td><td>".sprintf('%.1f', $eropros)."</td><td>".mb_substr($lrow["laadittu"], 0, 10)."</td><td>".mb_substr($erow["laadittu"], 0, 10)."</td><td>$lrow[laji] - $erow[laji]</td></tr>";
      }
    }
  }
