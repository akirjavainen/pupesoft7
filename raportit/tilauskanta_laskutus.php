...
if (isset($_POST["tee"])) {
  ...
  if (!isset($_POST["kaunisnimi"])) $_POST["kaunisnimi"] = ""; // MODIFIED, added BUGFIX
  if ($_POST["kaunisnimi"] != '') $_POST["kaunisnimi"] = str_replace("/", "", $_POST["kaunisnimi"]);
}

if (mb_strpos($_SERVER['SCRIPT_NAME'], "tilauskanta_laskutus.php") !== FALSE) {
...
if (isset($tee) and $tee == "lataa_tiedosto") {
  ...
  if ($tee == 'go') {
    ...
    for ($i = $startmonth;  $i <= $endmonth;) {
      ...
      $query .= " round(sum(if(tilausrivi.laadittu <= '$loppu 23:59:59' and (tilausrivi.laskutettuaika = '0000-00-00' or tilausrivi.laskutettuaika > '$loppu'), if(tilausrivi.laskutettuaika = '0000-00-00', tilausrivi.hinta / if('$yhtiorow[alv_kasittely]' = '' and tilausrivi.alv<500, (1+tilausrivi.alv/100), 1) * (tilausrivi.varattu+tilausrivi.jt) * {$query_ale_lisa}, tilausrivi.rivihinta),0))/1000,0) '".(mb_substr($i, 4, 2)*1).".1',\n";
      $query .= " round(sum(if(tilausrivi.laadittu <= '$loppu_ed 23:59:59' and (tilausrivi.laskutettuaika = '0000-00-00' or tilausrivi.laskutettuaika > '$loppu_ed'), if(tilausrivi.laskutettuaika = '0000-00-00', tilausrivi.hinta / if('$yhtiorow[alv_kasittely]' = '' and tilausrivi.alv<500, (1+tilausrivi.alv/100), 1) * (tilausrivi.varattu+tilausrivi.jt) * {$query_ale_lisa}, tilausrivi.rivihinta),0))/1000,0) '".(mb_substr($i, 4, 2)*1).".2',\n";

      $query .= " round(sum(if(tilausrivi.laadittu >= '$alku 00:00:00' and tilausrivi.laadittu <= '$loppu 23:59:59', if(tilausrivi.laskutettuaika = '0000-00-00', tilausrivi.hinta / if('$yhtiorow[alv_kasittely]' = '' and tilausrivi.alv<500, (1+tilausrivi.alv/100), 1) * (tilausrivi.varattu+tilausrivi.jt) * {$query_ale_lisa}, tilausrivi.rivihinta),0))/1000,0) '".(mb_substr($i, 4, 2)*1).".3',\n";
      $query .= " round(sum(if(tilausrivi.laadittu >= '$alku_ed 00:00:00' and tilausrivi.laadittu <= '$loppu_ed 23:59:59', if(tilausrivi.laskutettuaika = '0000-00-00', tilausrivi.hinta / if('$yhtiorow[alv_kasittely]' = '' and tilausrivi.alv<500, (1+tilausrivi.alv/100), 1) * (tilausrivi.varattu+tilausrivi.jt) * {$query_ale_lisa}, tilausrivi.rivihinta),0))/1000,0) '".(mb_substr($i, 4, 2)*1).".4',\n";

      $query .= " round(sum(if(tilausrivi.laadittu >= '$start 00:00:00' and tilausrivi.laadittu <= '$loppu 23:59:59', if(tilausrivi.laskutettuaika = '0000-00-00', tilausrivi.hinta / if('$yhtiorow[alv_kasittely]' = '' and tilausrivi.alv<500, (1+tilausrivi.alv/100), 1) * (tilausrivi.varattu+tilausrivi.jt) * {$query_ale_lisa}, tilausrivi.rivihinta),0))/1000,0) '".(mb_substr($i, 4, 2)*1).".5',\n";
      $query .= " round(sum(if(tilausrivi.laadittu >= '$start_ed 00:00:00' and tilausrivi.laadittu <= '$loppu_ed 23:59:59', if(tilausrivi.laskutettuaika = '0000-00-00', tilausrivi.hinta / if('$yhtiorow[alv_kasittely]' = '' and tilausrivi.alv<500, (1+tilausrivi.alv/100), 1) * (tilausrivi.varattu+tilausrivi.jt) * {$query_ale_lisa}, tilausrivi.rivihinta),0))/1000,0) '".(mb_substr($i, 4, 2)*1).".6',\n";

      $query .= " round(sum(if(tilausrivi.laskutettuaika >= '$alku'  and tilausrivi.laskutettuaika <= '$loppu', tilausrivi.rivihinta, 0))/1000,0) '".(mb_substr($i, 4, 2)*1).".7',\n";
      $query .= " round(sum(if(tilausrivi.laskutettuaika >= '$alku_ed'  and tilausrivi.laskutettuaika <= '$loppu_ed', tilausrivi.rivihinta, 0))/1000,0) '".(mb_substr($i, 4, 2)*1).".8',\n";

      $query .= " round(sum(if(tilausrivi.laskutettuaika >= '$start'  and tilausrivi.laskutettuaika <= '$loppu', tilausrivi.rivihinta, 0))/1000,0) '".(mb_substr($i, 4, 2)*1).".9',\n";
      $query .= " round(sum(if(tilausrivi.laskutettuaika >= '$start_ed'  and tilausrivi.laskutettuaika <= '$loppu_ed', tilausrivi.rivihinta, 0))/1000,0) '".(mb_substr($i, 4, 2)*1).".10',\n";
