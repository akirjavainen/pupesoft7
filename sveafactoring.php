...
if (mysqli_num_rows($factoring_result) == 0) {
  echo t("%s factoring-sopimusta ei ole perustettu!", null, $factoringyhtio);

  $tee = "ohita";
}
elseif (mysqli_num_rows($factoring_result) == 1) {
...
if ($tee == '') {
  ...
  while ($vrow = mysqli_fetch_assoc($factoring_result)) {
...
if ($tee == 'TULOSTA') {
  ...
  if (mysqli_num_rows($laskures) > 0) {
    ...
    while ($laskurow = mysqli_fetch_array($laskures)) {
      ...
      $asirow = mysqli_fetch_array($asires);
      ...
      if ($asirow['laji'] == "H") {
        ...
        $ytunnus = mb_substr($ytunnus, 0, 7)."-".mb_substr($ytunnus, -1);
      }

      $ulos .= sprintf('%-11.11s', $ytunnus);
      $ulos .= sprintf('%-2.2s', mb_strtoupper($asirow['kieli']));  // Language code
      ...
      if (mysqli_num_rows($laskurivires) > 0) {
        while ($laskurivi = mysqli_fetch_array($laskurivires)) {
          ...
          if ($laskurivi['kommentti'] != "") {
            ...
            $laskuri = 0;
            while ((mb_strlen($kommentti) > 0) and ($laskuri < 10)) {
              $ulos .= sprintf('%-2.2s', "11"); // Record type
              $ulos .= sprintf('%07.7s', $frow["sopimusnumero"]);
              $ulos .= sprintf('%010.10s', $laskurow["laskunro"]);
              $ulos .= sprintf('%03.3s', 0);
              ...
              if (mb_strlen($kommentti) > 40)
                $kommentti = mb_substr($kommentti, -40);
              else
                $kommentti = "";
