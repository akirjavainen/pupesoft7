...
if ($tee == 'Z') {
  ...
  if (mysqli_num_rows($result) > 0) {
    ...
    if (mysqli_num_rows($korvaresult) > 0) {
      ...
      $row    = mysqli_fetch_assoc($korvaresult);
      $id    = $row['id'];
      ...
      while ($row = mysqli_fetch_assoc($korva2result)) {
    ...
    if (mysqli_num_rows($vastaresult) > 0) {
      // Varastosaldot ja paikat
      ...
      // tuote löytyi, joten haetaan sen id...
      $row    = mysqli_fetch_assoc($vastaresult);
      $id    = $row['id'];
      ...
      while ($row = mysqli_fetch_assoc($vasta2result)) {
        list($saldo, $hyllyssa, ) = saldo_myytavissa($row["tuoteno"], '', '', '', '', '', '', '', '', $saldoaikalisa);
