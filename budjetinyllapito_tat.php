...
function tuotteenmyynti($tuoteno, $alkupvm) {
  ...
  $rivi = mysqli_fetch_assoc($result);
...
function asiakkaanmyynti($tunnukset, $try, $osasto, $vaintamavuosi=FALSE) {
  ...
  if ((float) $tanvuodenalustamyynti != 0) {
    $tanvuodenennuste = round($tanvuodenalustamyynti / mb_substr($edellinen_kuukausi_loppu, 5, 2) * $myyntiennustekerroin);
    if ($myyntitavoitekerroin > 0 and $tanvuodenennuste > 0) $ensvuodenennuste = round($tanvuodenalustamyynti / mb_substr($edellinen_kuukausi_loppu, 5, 2) * $myyntiennustekerroin * $myyntitavoitekerroin);
  }
  ...
  for ($k = 0; $k < $sarakkeet; $k++) {
    ...
    elseif (isset($budj_taulunrivit[$row[$budj_sarak]][$ik][$ostry_ind])) {
      ...
      if (mysqli_num_rows($xresult) == 1) {
        $brow = mysqli_fetch_assoc($xresult);
        $nro  = $brow['summa'];
...
elseif (isset($_FILES['userfile']) and is_uploaded_file($_FILES['userfile']['tmp_name']) === TRUE) {
  ...
  if (($toim == "ASIAKAS" or $toim == "MYYJA" or $toim == "ASIAKASMYYJA") and mb_stripos($headers[$lukualku], "Myynti")  !== FALSE) {
    $lukualku += 3;
  }
  ...
  $liitostunnukset = mb_substr($liitostunnukset, 0, -1);
}
...
if (($tee == "TALLENNA_BUDJETTI" or $submit_button != "") and ($toim == "ASIAKAS" or $toim == "MYYJA") and isset($osastotryttain) and $osastotryttain != "") {
  if ($osastotryttain == "tuoteryhmittain") {
    ...
    while ($tryrow = mysqli_fetch_assoc($result)) {
...
if ($tee == "TALLENNA_BUDJETTI") {
  ...
  if ($budjetointi_taso == "summa_jaetaan") {
    ...
    $alkaakk = mb_substr($kausi_alku, 0, 4).mb_substr($kausi_alku, 5, 2);
    $loppuukk = mb_substr($kausi_loppu, 0, 4).mb_substr($kausi_loppu, 5, 2);
  ...
  if (isset($poikkeus) and $poikkeus == "totta") {
    ...
    if ($poikkeus_haku == "try" or $poikkeus_haku == "osasto") {
      foreach ($muunto_luvut as $litunnus => $rivit) {
        ...
        while ($tuoterivi = mysqli_fetch_assoc($result)) {
          ...
          foreach ($liitostunnarit_array as $liitostunnari) {
            ...
            elseif ($toim == "TUOTE") {
              ...
              elseif ($budj_kohtelu == "indeksi") {
                ...
                $ed_kausi = (mb_substr($kausi, 0, 4) -1).'-'.mb_substr($kausi, 4, 2).'-01';
...
if ($tee == "AJA_RAPORTTI") {
  ...
  if ($toim == "TUOTE" and $summabudjetti == "on") {
    ...
    if (mb_strpos($lisa, 'tuote.osasto') != 0 and mb_strpos($lisa, 'tuote.try') === FALSE) {
      $grouppaus = " group by tuote.osasto";
      $haen = "osasto";
      $passaan = "yksi";
    }
    elseif (mb_strpos($lisa, 'tuote.osasto') != 0 and mb_strpos($lisa, 'tuote.try') != 0) {
  ...
  if (isset($osastotryttain) and $osastotryttain == "tuoteryhmittain") {
    // Haetaan tuoteryhmät
    $res = t_avainsana("TRY");
    $rivimaara = mysqli_num_rows($res)*mysqli_num_rows($result);
  }
  elseif (isset($osastotryttain) and $osastotryttain == "osastoittain") {
    // Haetaan osastot
  ...
  if ($toim == "ASIAKAS" or $toim == "MYYJA" or $toim == "ASIAKASMYYJA") {
    if ($naytamyyntiennuste != "") {
      if ($rivimaara < $maxrivimaara) {
        ...
        echo "<th>", t("Myynti"), " ", date('Y'), "-01 - ", mb_substr($edellinen_kuukausi_loppu, 0, 7), "</th>";
      ...
      $worksheet->write($excelrivi, $excelsarake, t("Myynti")." ".date('Y')."-01 - ".mb_substr($edellinen_kuukausi_loppu, 0, 7), $format_bold);
      $excelsarake++;
  ...
  $raja     = '0000-00';
  $alkuraja  = mb_substr($tilikaudetrow['tilikausi_alku'], 0, 4)."-".mb_substr($tilikaudetrow['tilikausi_alku'], 5, 2);
  $rajataulu   = array();
  $sarakkeet  = 0;

  while ($raja < mb_substr($tilikaudetrow['tilikausi_loppu'], 0, 7)) {

    $vuosi   = mb_substr($tilikaudetrow['tilikausi_alku'], 0, 4);
    $kk   = mb_substr($tilikaudetrow['tilikausi_alku'], 5, 2);
    $kk   += $sarakkeet;
    ...
    if (($budjetointi_taso == "joka_kk_sama" or $budjetointi_taso == "summa_jaetaan") and $sarakkeet == 1) {
      ...
      $vuosi2  = mb_substr($tilikaudetrow['tilikausi_loppu'], 0, 4);
      $kk2  = mb_substr($tilikaudetrow['tilikausi_loppu'], 5, 2);
  ...
  if ($budjetointi_taso == "joka_kk_sama" or $budjetointi_taso == "summa_jaetaan") {
    if (isset($osastotryttain) and ($osastotryttain == "tuoteryhmittain" or $osastotryttain == "osastoittain")) {
      while ($row = mysqli_fetch_assoc($result)) {
        while ($ostryrow = mysqli_fetch_assoc($res)) {
  ...
  elseif (isset($onko_ilman_budjettia) and $onko_ilman_budjettia != "") {
    ...
    while ($brivi = mysqli_fetch_assoc($bres)) {
