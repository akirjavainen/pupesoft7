...
class Image_Barcode_int25 extends Image_Barcode
{
    ...
    function &draw($text, $imgtype = 'png')
    {
        ...
        $text = mb_strlen($text) % 2 ? '0' . $text : $text;

        // Calculate the barcode width
        $barcodewidth = (mb_strlen($text)) * (3 * $this->_barthinwidth + 2 * $this->_barthickwidth) +
            (mb_strlen($text)) * 2.5 +
        ...
        // Draw $text contents
        for ($idx = 0; $idx < mb_strlen($text); $idx += 2) {       // Draw 2 chars at a time
            $oddchar  = mb_substr($text, $idx, 1);                 // get odd char
            $evenchar = mb_substr($text, $idx + 1, 1);             // get even char

            // interleave
            for ($baridx = 0; $baridx < 5; $baridx++) {

                // Draws odd char corresponding bar (black)
                $elementwidth = (mb_substr($this->_coding_map[$oddchar], $baridx, 1)) ?  $this->_barthickwidth : $this->_barthinwidth;
                imagefilledrectangle($img, $xpos, 0, $xpos + $elementwidth - 1, $this->_barcodeheight, $black);
                $xpos += $elementwidth;

                // Left enought space to draw even char (white)
                $elementwidth = (mb_substr($this->_coding_map[$evenchar], $baridx, 1)) ?  $this->_barthickwidth : $this->_barthinwidth;
                $xpos += $elementwidth; 
                $xpos ++;
            }
        }
