...
class Image_Barcode_Code39 extends Image_Barcode
{
    ...
    public function Image_Barcode_Code39( $text = '', $wThin = 0, $wThick = 0 ) {
         self::__construct($text, $wThin, $wThick);
    }
    ...
    */
    function plot($noText = false, $bHeight = 0)
    {
       ...
       $barcode = '';
       foreach ( mb_str_split( $final_text ) as $character ) {
       ...
       // draw barcode bars to image
        if ( $noText ) {
            foreach (mb_str_split($barcode) as $character_code ) {
                if ($character_code == 0 ) {
            ...
            imagestring(
                $img,
                $this->_font_size,
                ( $barcode_len - $font_width * mb_strlen( $this->text ) )/2,
    ...
    function _dumpCode($code)
    {
        ...
        foreach ( mb_str_split( $code ) as $bit ) {
