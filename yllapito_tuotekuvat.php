...
if ($tee == '' and count($mul_del) > 0) {
  ...
  foreach ($mul_del as $key => $val) {
    ...
    $ltiedtunnus = sanitize_string($ltiedtunnus);
    $kayttotarkoitus = sanitize_string($kayttotarkoitus);
    $filetype = sanitize_string($filetype);
...
if ($tee == 'LISTAA') {
  ...
  if ($liitetiedosto != '' and ($lisa != '' or ($kem != '' or count($mul_kem) > 0))) {
    $liitetiedosto = sanitize_string(trim($liitetiedosto));
  ...
  if (count($mul_ext) > 0) {
    ...
    $lisa = mb_substr($lisa, 0, -2); // vika "or" pois
    $lisa .= ")";
  }
  //      else {
  ...
  if ($tuoteno != '') {
    $tuoteno = sanitize_string(trim($tuoteno));
  ...
  $tuotekuvia_count = mysqli_num_rows($result);
  ...
  if ($tuotekuvia_count == 0) {
    ...
    while ($row = mysqli_fetch_array($result)) {
      ...
      $row['muutospvm'] = mb_substr($row['muutospvm'], 0, 10);
      ...
      if ($row['filename'] != '') {
        if ((mb_strtolower($row['kayttotarkoitus']) == 'tk' and $nayta_tk != 'naytetaan') or (mb_strtolower($row['kayttotarkoitus']) == 'hr' and $nayta_hr != 'naytetaan') or (mb_strtolower($row['kayttotarkoitus']) == 'th' and $nayta_th != 'naytetaan') or (mb_strtolower($row['kayttotarkoitus']) == 'mu')) {
