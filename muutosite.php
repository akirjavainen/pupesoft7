...
if (isset($tee_Y) or isset($tee_Z) or isset($tee_X) or isset($tee_XKAIKKI) or isset($tee_W) or isset($tee_T) or isset($tee_S) or isset($tee_Å) or isset($tee_Ä) or isset($tee_automaattikirjauksia_muutettu) or isset($tee_kasintehtyja_alvkirjauksia) or isset($tee_alvkirjauksia_ilmanalvtasoa) or isset($tee_automaattikirjauksia_alv_muutettu) or isset($tee_KP)) {
  ...
  if  (isset($tee_Z) or isset($tee_X) or isset($tee_XKAIKKI) or isset($tee_W) or isset($tee_T) or isset($tee_S) or isset($tee_Å) or isset($tee_Ä) or isset($tee_automaattikirjauksia_muutettu) or isset($tee_kasintehtyja_alvkirjauksia) or isset($tee_alvkirjauksia_ilmanalvtasoa) or isset($tee_automaattikirjauksia_alv_muutettu) or isset($tee_KP)) {
    ...
    if (isset($tee_automaattikirjauksia_alv_muutettu)) {
      ...
      $vrow = mysqli_fetch_assoc($alhire);
  ...
  elseif ((int) $tav > 0 or mb_strlen($selite) > 0 or mb_strlen($tilino) > 0 or mb_strlen($summa) > 0 or mb_strlen($laatija) > 0 or ($kpexport == 1 and mb_strlen($tositenro) > 0)) {
  ...
  if (mysqli_num_rows($result) == 0) {
    ...
    for ($i = 1; $i < mysqli_num_fields($result)+$miinusta; $i++) {
    ...
    while ($trow = mysqli_fetch_assoc($result)) {
      ...
      if ($edtunnus > 0 and ($tee != 'KP' or $ero != 0)) {
        ...
        if ($trow["ltunnus"] != $edtunnus) {
          echo "</tr><tr><th style='padding:3px;' colspan='".(mysqli_num_fields($result)+$miinusta)."'></th></tr><tr class='aktiivi'>";
        }
        else {
      ...
      for ($i=1; $i < mysqli_num_fields($result)+$miinusta; $i++) {
        ...
        if ($i == 1) {
          if (mysqli_field_name($result, $i) == 'tapvm' or (mysqli_field_name($result, $i) == 'alku' and isset($tee_KP))) {
            $orgpvm     = $trow[$kennimi];
        ...
        elseif (is_numeric($trow[$kennimi]) and (mysqli_field_type($result, $i) == 'real' or mysqli_field_type($result, $i) == 'int')) {
          if ($kennimi == "vero") $trow[$kennimi] = $trow[$kennimi] * 1;

          echo "<td align='right'>$trow[$kennimi]</td>";
        }
        elseif (mysqli_field_name($result, $i) == "tapvm") {
          echo "<td>".tv1dateconv($trow[$kennimi])."</td>";
        }
        elseif (mysqli_field_name($result, $i) == "ytunnus") {
          echo "<td>".tarkistahetu($trow["ytunnus"])."</td>";
        }
        elseif (mysqli_field_name($result, $i) == "tiliote" and isset($tee_KP)) {
          $pano = sprintf("%.2f", mb_substr($trow[$kennimi], 21, 19) / 100);
          $otto = sprintf("%.2f", mb_substr($trow[$kennimi], 48, 19) / 100);
        ...
        elseif (mysqli_field_name($result, $i) == "kirjanpito" and isset($tee_KP)) {

          $huom   = "";
          $linkki = "";

          $orgpvm_pankki = mb_substr(str_replace("-", "", $orgpvm), 2);
          ...
          $asumma = mysqli_fetch_assoc($subr);
          ...
          if ($trow[$kennimi] != "") {
            ...
            $subr = pupe_query($subq);
            $ltunnukset = mysqli_fetch_assoc($subr);
            ...
            $subq = "SELECT oletus_rahatili
                     ...
                     and tilino  = '$trow[tilino]'";
            $subr = pupe_query($subq);
            $oratil = mysqli_fetch_assoc($subr);
            ...
            $tsumma = mysqli_fetch_assoc($subres);
        ...
        elseif (mysqli_field_name($result, $i) == "ero" and isset($tee_KP)) {
          $ero = sprintf("%.2f", $saldo-$kpito);

          echo "<td align='right'><br><br>$ero<br></td>";
        }
        elseif (mysqli_field_name($result, $i) == "muutetut_tositteet" and isset($tee_KP)) {
          if ($ero != 0) {
            ...
            while ($tpoistetut = mysqli_fetch_assoc($subres)) {
...
if ($tee == 'P') {
  ...
  if (mysqli_num_rows($alv_tili_res) == 1) {
    $alv_tili_row = mysqli_fetch_assoc($alv_tili_res);
    ...
    if ($tositenro != 0) {
      ...
      case "X" : // Tämä on muistiotosite, sillä voi olla vain yksi tositenro
        ...
        if (mysqli_num_rows($result) != 1) {
          ...
          $tositerow=mysqli_fetch_assoc($result);
...
if ($tee == 'E' or $tee == 'F') {
  ...
  if (mysqli_num_rows($result) != 1) {
    echo t("Tapahtumaa ei löydy (lasku-taulun tunnus $tunnus)."); // MUOKKAUS: tarkennus lisatty
    exit;
  }
  ...
  if ($tee2 == "1") {
    ...
    if ($trow["tila"] != "U" and $trow["tila"] != "L") {
      ...
      $keekrow = mysqli_fetch_assoc($keikres);
  ...
  if ($trow['tila'] == 'U' or $trow['tila'] == 'L') {
    ...
    $karhu_liitos = mysqli_fetch_assoc($karhu_result);
    ...
    $karhu_query = "SELECT ifnull(group_concat(ktunnus), '') ktunnus
                    ...
                    ORDER BY ktunnus";
    $karhu_result = pupe_query($karhu_query);
    $karhu_row = mysqli_fetch_assoc($karhu_result);
    ...
    if ($karhu_row["ktunnus"] != "") {
      ...
      if (mysqli_num_rows($karhu_result) > 0) {
        echo "<tr><th>", t('Maksukehotus / Tratta'), ":</th><td>";

        $laskuri = 0;

        while ($karhu_row = mysqli_fetch_assoc($karhu_result)) {
    ...
    if (mysqli_num_rows($korko_result) > 0) {

      $korkolaskurow = mysqli_fetch_assoc($korko_result);
      ...
      if (mysqli_num_rows($korko2_result) > 0) {

        echo "<tr><th>", t('Korkolaskut'), ":</th><td>";

        while ($korkolaskurow2 = mysqli_fetch_assoc($korko2_result)) {
    ...
    $faktarow = mysqli_fetch_assoc($faktares);
    ...
    if ($_kirjanpidollinen_varastosiirto and $trow['tila'] == "U") {
      ...
      if (mysqli_num_rows($varastosiirto_result) > 0) {
        ...
        while ($varastosiirto = mysqli_fetch_assoc($varastosiirto_result)) {
  ...
  // Jotain muuta kuin Myytilasku tai Tosite
    ...
    if ($tee2 != 1) {
      ...
      if ($trow["vanhatunnus"] != 0) {
        ...
        echo "<tr><td colspan='2'><br><font class='message'>".sprintf(t("Lasku on jaettu %s osaan!"), mysqli_num_rows($jaetutres))."</font><br></td></tr>";
      ...
      while ($muutkeikrow = mysqli_fetch_assoc($muutkeikres)) {
  ...
  if ($trow['tila'] == 'U') {
    ...
    $masrow = mysqli_fetch_assoc($masres);
  ...
  if ($liitetiedosto == 1) {
    ...
    if (mysqli_num_rows($tulokset) >0) {
      $ktarkoitus = mysqli_fetch_assoc($tulokset);
