...
if (isset($_POST['piirtele_laiteluettelo'])) {
  ...
  while ($avainsanarivi = mysqli_fetch_assoc($res)) {
...
elseif (isset($valitut_sarakkeet) and count($valitut_sarakkeet) > 0) {
  ...
  $sopimuskohtaisetrivi = mysqli_fetch_assoc($sopimuskohtaisetresult);
  ...
  mysqli_data_seek($sopimuskohtaisetresult, 0);
  $totalvalue = 0;
  while ($sopimusrivi = mysqli_fetch_assoc($sopimuskohtaisetresult)) {
  ...
  if (in_array("NAYTATYOT", $valitut_sarakkeet) and !empty($hintahopinat)) {
    ...
    $lisatyosarakerivi = mysqli_fetch_assoc($lisatyosarakeres);
  ...
  foreach ($valitut_sarakkeet as $key => $value) {
    ...
    $sarakeotsikkorivi = mysqli_fetch_assoc($sarakeotsikkoresult);
  ...
  while ($row = mysqli_fetch_assoc($laiteresult)) {
    ...
    $laiterow = mysqli_fetch_assoc($laiteres);
    ...
    while ($rivi = mysqli_fetch_assoc($palveluresult)) {
