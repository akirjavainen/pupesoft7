...
if (isset($_REQUEST["ajax"]) and $_REQUEST["ajax"] == "OK") {
  ...
  elseif ($tee == 'hae_avainsana_lista') {
    ...
    while ($dp_row = mysqli_fetch_assoc($dpavainsanat_res)) {
  ...
  elseif ($tee == 'hae_liite_lista') {
    ...
    while ($dp_row = mysqli_fetch_assoc($dpliitteet_res)) {
      if ($saamuokata) {
  ...
  $uusi_nimi_en  = (isset($uusi_nimi_en)) ? $uusi_nimi_en: "";
...
?>
    ...
    if (mysqli_num_rows($dpavainsanat_res) > 0) {
    ...
    if (mysqli_num_rows($dpliitteet_res) > 0) {
...
if (mb_strtoupper($toim) == "TUOTE") {
  $otsikko = t("Tuotepuu");
}
elseif (mb_strtoupper($toim) == "ASIAKAS") {
