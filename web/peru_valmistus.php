...
if ($id != 0) {
  ...
  $laskurow = mysqli_fetch_array($res);
  ...
  while ($rivirow = mysqli_fetch_array($res)) {
    ...
    if ($rivirow["perheid"] == $rivirow["tunnus"]) {
      ...
      while ($valm_row = mysqli_fetch_array($valm_res)) {
    ...
    $tuoterow = mysqli_fetch_array($tuoteres);
    ...
    while ($taparow = mysqli_fetch_array($tapares)) {
      ...
      $taparow2 = mysqli_fetch_array($tapares2);
      ...
      if (mysqli_num_rows($tapares3) > 0) {
        $taparow3 = mysqli_fetch_array($tapares3);
      }
      else {
        $query = "SELECT *
                  from tapahtuma
        ...
        if ($tuoterow["ei_saldoa"] == "") {
          ...
          if (mysqli_affected_rows($link) == 0) {
            ...
            if (mysqli_affected_rows($link) == 0) {
              ...
              $korjres = pupe_query($query);
              $hyllyrow =  mysqli_fetch_array($korjres);
