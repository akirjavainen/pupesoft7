...
$konsyhtiot = " in (".mb_substr($konsyhtiot, 0, -1).") ";
...
if (isset($messenger) and $message != "") {
  ...
  // jos kyseessä ryhmä
  if (mb_substr($vastaanottaja, 0, 5) == '!###!') {
    $vastaanottajat_res = t_avainsana("MESSENGER_RYHMA", "", "and selite = '".mb_substr($vastaanottaja, 5)."'");
    while ($vastaanottaja_row = mysqli_fetch_assoc($vastaanottajat_res)) {
      $vastaanottajat[] = array('vastaanottaja' => $vastaanottaja_row['selitetark'],
                                'ryhma' => mb_substr($vastaanottaja, 5),);
    }
  }
  else {
...
$viimeisin_row = mysqli_fetch_array($viimeisin_result);
...
if (mysqli_num_rows($ryhmaresult) > 0) {
  echo "<optgroup label='".t("Messenger ryhmät")."'>";
  while ($ryhmarow = mysqli_fetch_assoc($ryhmaresult)) {
