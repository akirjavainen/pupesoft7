...
if (trim($konserni) != '') {
  ...
  $konsernit = "yhtio in (".mb_substr($konsernit, 0, -1).") ";
}
else {
...
if ($tee == "LISAA") {
  ...
  if ($tee == "LISAA" and $tyotunnus > 0) {
    ...
    if ($yhtiorow['tyomaarays_asennuskalenteri_muistutus'] == 'K' and isset($lisaa_muistutus) and trim($lisaa_muistutus) == 'kylla') {
      ...
      if (mysqli_num_rows($muistutus_chk_res) != 0) {
        $muistutus_chk_row = mysqli_fetch_assoc($muistutus_chk_res);
        ...
        $viestit_chk_row = mysqli_fetch_assoc($viestit_chk_res);
...
if ($tee == "MUOKKAA") {
  ...
  if (mysqli_num_rows($result) == 1) {
    $kalerow = mysqli_fetch_array($result);

    $year      = mb_substr($kalerow["pvmalku"], 0, 4);
    $month      = mb_substr($kalerow["pvmalku"], 5, 2);
    $day      = mb_substr($kalerow["pvmalku"], 8, 2);
    $aika      = mb_substr($kalerow["pvmalku"], 11, 5);
    $lyear      = mb_substr($kalerow["pvmloppu"], 0, 4);
    $lmonth      = mb_substr($kalerow["pvmloppu"], 5, 2);
    $lday      = mb_substr($kalerow["pvmloppu"], 8, 2);
    $aikaloppu    = mb_substr($kalerow["pvmloppu"], 11, 5);
...
if ($tee == "POISTA") {
  ...
  if ($yhtiorow['tyomaarays_asennuskalenteri_muistutus'] == 'K') {
    ...
    if (mysqli_num_rows($muistutus_chk_res) != 0) {
      ...
      if (mysqli_num_rows($kappale_chk_res) == 0) {
        ...
        while ($kappale_chk_row = mysqli_fetch_assoc($kappale_chk_res)) {
...
if ($tee == "VARAA") {
  ...
  if (!isset($aikaloppu)) $aikaloppu = date("H:i", mktime(mb_substr($aika, 0, 2), mb_substr($aika, 3, 2)+60, 0));
  ...
  while ($whileaika != $whileloppu) {
    ...
    $whileaika = date("H:i", mktime(mb_substr($whileaika, 0, 2), mb_substr($whileaika, 3, 2)+60, 0));
  }
        ...
        while ($vrow = mysqli_fetch_array($vres)) {
          foreach ($ASENTAJA_ARRAY as $b) {
            ...
            foreach ($AIKA_ARRAY as $a) {
              ...
              $alku = str_replace(array(":", "-", " "), "", mb_substr($vrow["pvmalku"], 0, 14)."00");

              $lopp = str_replace(array(":", "-", " "), "", mb_substr($vrow["pvmloppu"], 0, 16));
          ...
          if (isset($varaukset[$b][$a])) {
            ...
            foreach ($varaukset[$b][$a] as $varaus) {
              ...
              if ($tyyppi == 'asennuskalenteri') {
                ...
                $tdekotus .= "<a class='tooltip' id='$tilausnumero' href='".$palvelin2."crm/kalenteri.php?valitut=$kuka&kenelle=$kuka&tee=SYOTA&kello=".mb_substr($pvmalku, 11, 5)."&year=$year&kuu=$month&paiva=$i&tunnus=$tilausnumero&tyomaarays=$tunnus&konserni=$konserni&lopetus=$lopetus'>$zul</a> ";
              }
            }
