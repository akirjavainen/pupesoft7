...
if (is_uploaded_file($_FILES['userfile']['tmp_name']) === TRUE) {
  ...
  foreach ($filerivit as $rivi) {
    ...
    if ($tiedostomuoto == "M2MATKALASKU") {
      if (!isset($tpv)) {
        $tpv=mb_substr($rivi, 639, 4);
        $tpk=mb_substr($rivi, 643, 2);
        $tpp=mb_substr($rivi, 645, 2);
      }

      if ($flip == 1) { // Seuraavalla rivillä tulee veronmäärä. Lisätään se!
        $maara--;
        $alv = (float) mb_substr($rivi, 24, 12);
        if (mb_substr($rivi, 23, 1) == 'K') $alv *= -1;
        $isumma[$maara] += $alv;
        $flip = 0;
      }
      else {
        $isumma[$maara] = (float) mb_substr($rivi, 24, 12);
        if (mb_substr($rivi, 23, 1) == 'K') $isumma[$maara] *= -1;
        $itili[$maara]  = (int) mb_substr($rivi, 13, 4);
        $ikustp[$maara] = (int) mb_substr($rivi, 228, 5);
        ...
        $iselite[$maara] = "Matkalasku ". $tpp . "." . $tpk . "." . $tpv . " " . trim(mb_substr($rivi, 240, 50)) . " " . trim(mb_substr($rivi, 431, 60));
        $ivero[$maara] = (float) mb_substr($rivi, 332, 5);
    ...
    elseif ($tiedostomuoto == "PRETAX") {
      if (!isset($tpv)) {
        $tpv=mb_substr($rivi, 39, 4);
        $tpk=mb_substr($rivi, 43, 2);
        $tpp=mb_substr($rivi, 45, 2);
      }
      $isumma[$maara]  = (float) mb_substr($rivi, 117, 16) / 100;
      $itili[$maara]   = (int) mb_substr($rivi, 190, 7);

      // Kustannuspaikka
      $ikustp_tsk     = trim(mb_substr($rivi, 198, 3));
      $ikustp[$maara]  = 0;
    ...
    elseif ($tiedostomuoto == "AMMATTILAINEN") {
      ...
      */
      ...
      //Tositepvm
      if (!isset($tpv)) {
        $tpv=mb_substr($kentat[5], 6, 4);
        $tpk=mb_substr($kentat[5], 3, 2);
        $tpp=mb_substr($kentat[5], 0, 2);
      }
