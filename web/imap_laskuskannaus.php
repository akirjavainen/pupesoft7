...
if ($php_cli) {
  ...
  $imaplaskut_skannauskansio = mb_substr($yhtiorow['skannatut_laskut_polku'], -1) != '/' ? $pupe_root_polku."/".$yhtiorow['skannatut_laskut_polku'].'/' : $pupe_root_polku."/".$yhtiorow['skannatut_laskut_polku'];
...
function sqimap_read_data_list($imap_stream, $tag_uid, $handle_errors, &$response, &$message, $query = '', $filter = false, $outputstream = false, $no_return = false) {
  ...
  while ($read) {
    ...
    case mb_substr($tag, 0, 1):
      {
        /* get the command */
        $arg = '';
        $i = mb_strlen($tag)+1;
        $s = mb_substr($read, $i);
        if (($j = mb_strpos($s, ' ')) || ($j = mb_strpos($s, "\n"))) {
          $arg = mb_substr($s, 0, $j);
        }
        $found_tag = mb_substr($read, 0, $i-1);
        if ($arg && $found_tag==$tag) {
          ...
          case 'PREAUTH':
            $response = $arg;
            $message = trim(mb_substr($read, $i+mb_strlen($arg)));
        ...
        if (preg_match('/^\*\s\d+\sFETCH/', $read)) {
          ...
          do { /* outer loop, continue until next untagged fetch
            ...
            do { /* innerloop for fetching literals. with this loop
              ...
              if (mb_substr($read, 0, 1) == '*') break 2;
              $s = mb_substr($read, -3);
            } while ($s === "}\r\n");
            $s = mb_substr($read, -3);
          } while (mb_substr($read, 0, 1) !== '*' &&
            mb_substr($read, 0, mb_strlen($tag)) !== $tag);
  ...
  case 'NO':
    ...
    if (mb_strstr($message, 'command resulted in') === false) {
    ...
    if ($okdomain) {
      ...
      for ($bodyind = 1; $bodyind <= count($bodyt); $bodyind++) {
        ...
        if (preg_match("/\(\"(FILE)?NAME\" \"(.*?)\"\)/", $bodyt[($bodyind-1)], $matches)) {
          ...
          // Hyväksytyt filet
          if (mb_strtoupper($ext) != "JPG" and mb_strtoupper($ext) != "JPEG" and mb_strtoupper($ext) != "PNG" and mb_strtoupper($ext) != "GIF" and mb_strtoupper($ext) != "PDF") {
...
if ($imap_stream) {
  ...
  if (sq_is8bit($imaplaskut_username)) {
    $query .= ' {' . mb_strlen($imaplaskut_username) . "}\r\n$imaplaskut_username";
  }
  else {
    $query .= ' "' . quoteimap($imaplaskut_username) . '"';
  }
  if (sq_is8bit($imaplaskut_password)) {
    $query .= ' {' . mb_strlen($imaplaskut_password) . "}\r\n$imaplaskut_password";
  }
  else {
  ...
  foreach ($read_list[0] as $r) {
    if (preg_match("/ \"([^\"]*?Matkalaskut\/[a-z]*)\"/i", $r, $matches)) {
      ...
      $skannauskansio = $imaplaskut_skannauskansio."/".mb_strtolower($matches[1])."/";
      $in_box = $matches[1];
      $ok_box = $matches[1]."/Ok";
