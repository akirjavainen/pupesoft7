...
elseif ($toim == "RESEPTI") {
  ...
  while ($resepti_kentta = mysqli_fetch_assoc($resepti_kentat_result)) {
...
if ($tee == 'LISAA' and $oikeurow['paivitys'] == '1') {
  ...
  if (trim($isatuoteno) != trim($tuoteno)) {
    ...
    if (mysqli_num_rows($result) > 0) {
      ...
      if (mysqli_num_rows($result) > 0 and $tunnus == "") {
        ...
        if ($yhtiorow["tuoteperhe_kasittely"] == "") {
          ...
          $_count = mysqli_num_rows($result) + 1;
    ...
    if ($ok == 1) {
      ...
      $isatrow = mysqli_fetch_assoc($res);
      ...
      $query = "SELECT *
                ...
                AND yhtio     = '$kukarow[yhtio]'";
      $res = pupe_query($query);
      $laptrow = mysqli_fetch_assoc($res);
...
if ($tee == 'POISTA' and $oikeurow['paivitys'] == '1') {
  ...
  $result = pupe_query($query);
  $fakrow = mysqli_fetch_array($result);
...
if (($hakutuoteno != '' or $isatuoteno != '') and $tee == "") {
  ...
  if (mysqli_num_rows($result) == 1) {
    ...
    if (mysqli_num_rows($result) == 0 and ($hakutuoteno != '' or $isatuoteno != '')) {
    ...
    elseif (mysqli_num_rows($result) == 1) {
      ...
      $query = "SELECT tuoteperhe.muuttaja,
                ...
                LIMIT 1";
      $ressu = pupe_query($query);
      $muuttajarow = mysqli_fetch_assoc($ressu);
	...
	// MUOKKAUS: BUGIKORJAUS (width = 140):
      ...
      while ($prow = mysqli_fetch_array($res)) {
        ...
        if (mysqli_num_rows($res1)==0) {
          ...
          $error = "";
          $tuoterow = mysqli_fetch_array($res1);
        ...
        if ($tunnus != $prow["tunnus"]) {
          ...
          if ($toim == "PERHE") {
            if ($hintatyyppi == '') {
              ...
              $lapsituoterow = mysqli_fetch_assoc($lapsituoteres);
          ...
          if ($toim == "RESEPTI") {
            ...
            $ttrow = mysqli_fetch_assoc($ttrow);
            ...
            $valuurow = mysqli_fetch_assoc($valuurow);
