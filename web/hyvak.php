...
if ($keikalla == "on") {
  ...
  $laskurow = mysqli_fetch_assoc($abures);
      ...
      if (mysqli_num_rows($result) > 0) {
        ...
        if (in_array($laskurow["vienti"], array("B", "E", "H"))) {
          ...
          while ($row = mysqli_fetch_assoc($result)) {
            ...
            $alvirow = mysqli_fetch_assoc($alvires);
...
if ($tee == 'D' and $oikeurow['paivitys'] == '1') {
  ...
  if ($trow["tapvm"] >= $yhtiorow["ostoreskontrakausi_alku"] or ($trow["tapvm"]=="0000-00-00" and $trow["laskutyyppi"] == "M")) {
    ...
    if ($trow['ebid'] == 'TECCOM-INVOICE') {
      ...
      $toiminro_row = mysqli_fetch_assoc($toiminro_res);
...
if ($tee == 'Z') {
  ...
  for ($i=1; $i<mysqli_num_fields($result); $i++) {
    if ($i == mysqli_num_fields($result)-1) {
      if ($trow[mysqli_field_name($result, $i)] != "") {
        echo "<tr><td colspan='".mysqli_num_fields($result)."'><font class='error'>".$trow[mysqli_field_name($result, $i)]."</font></td>";
      }
    }
    else {
...
if ($tee == "H") {
  ...
  if ($yhtiorow["tarkenteiden_tarkistus_hyvaksynnassa"] == "K") {
    ...
    if ($ollaan_viimeinen_hyvaksyja) {
      ...
      while ($tilioinnit_row = mysqli_fetch_assoc($tilioinnit_tsek)) {
...
if ($tee == 'H') {
  ...
  if ($laskurow['h5time'] == '0000-00-00 00:00:00' and mb_strlen(trim($laskurow['hyvak5'])) != 0) {
    $hyvaksyja_nyt = $laskurow['hyvak5'];
  }
  if ($laskurow['h4time'] == '0000-00-00 00:00:00' and mb_strlen(trim($laskurow['hyvak4'])) != 0) {
    $hyvaksyja_nyt = $laskurow['hyvak4'];
  }
  if ($laskurow['h3time'] == '0000-00-00 00:00:00' and mb_strlen(trim($laskurow['hyvak3'])) != 0) {
    $hyvaksyja_nyt = $laskurow['hyvak3'];
  }
  if ($laskurow['h2time'] == '0000-00-00 00:00:00' and mb_strlen(trim($laskurow['hyvak2'])) != 0) {
    $hyvaksyja_nyt = $laskurow['hyvak2'];
  }
  if ($laskurow['h1time'] == '0000-00-00 00:00:00' and mb_strlen(trim($laskurow['hyvak1'])) != 0) {
...
if ($tee == 'P') {
  ...
  if (mysqli_num_rows($result) != 0) {
    $summarow = mysqli_fetch_assoc($result);
...
if ($tee == 'U') {
  ...
  if (isset($kpexport) and $kpexport == 1 or mb_strtoupper($yhtiorow['maa']) != 'FI') {
...
if (mb_strlen($tunnus) != 0) {
  ...
  if ($laskurow["laskunro"] != "") {
    ...
    if (mysqli_num_rows($tarkres) != 0) {
      ...
      while ($tarkrow = mysqli_fetch_assoc($tarkres)) {
  ...
  if (($kukarow['taso'] == '2' or $kukarow["taso"] == '3') and $onko_eka_hyvaksyja) {
    ...
    elseif ($_POST['iframe_id'] != '') {
      echo "&iframe_id=".sanitize_string($_POST['iframe_id']);
    }
  ...
  if (($kukarow['taso'] == '1' or $kukarow['taso'] == '2' or $kukarow["taso"] == '3') and $oikeurow['paivitys'] == '1' and mysqli_num_rows($keikres) == 0 and $laskurow["tapvm"] >= $yhtiorow["ostoreskontrakausi_alku"]) {
  ...
  $toimrow = mysqli_fetch_assoc($toimres);
      ...
      while ($vrow = mysqli_fetch_assoc($vresult)) {
        ...
        if ($hyvak[$i] == $vrow['kuka']) {
          ...
          if (!$saako_hyvaksya and $kukarow['hierarkia'] != '') {
            ...
            if (mysqli_num_rows($esimies_result) > 0) $saako_hyvaksya = TRUE;
          }
        }
  ...
  if (in_array($laskurow["vienti"], array("B", "E", "H", "C", "J", "F", "K", "I", "L"))) {
    ...
    if (mysqli_num_rows($apure) > 0) {
      while ($apurow = mysqli_fetch_assoc($apure)) {
    ...
    $apurow = mysqli_fetch_assoc($apure);
...
elseif ($kutsuja == "") {
  ...
  while ($trow = mysqli_fetch_assoc($result)) {
      ...
      if ($invoice != FALSE and $invoice != 'ei_loydy_edia') {
        if (count($purchaseorder_ei_loydy) == 0 and count($invoice_ei_loydy) == 0 and count($loytyy_kummastakin) > 0) {
          ...
          foreach ($loytyy_kummastakin as $tuoteno => $null) {
            if (mb_substr($tuoteno, 0, 15) != "Ei_tuotekoodia_" and ($invoice[$tuoteno]['tilattumaara'] != $purchaseorder[$tuoteno]['tilattumaara'] or abs($invoice[$tuoteno]['nettohinta'] - $purchaseorder[$tuoteno]['nettohinta']) > 1)) {
    ...
    if ($kukarow['taso'] == '1' or $kukarow['taso'] == '2' or $kukarow["taso"] == '3') {
      ...
      if ($oikeurow['paivitys'] == '1' and mysqli_num_rows($keikres) == 0 and $trow["tapvm"] >= $yhtiorow["ostoreskontrakausi_alku"]) {
        echo "<td class='back' valign='top'>
