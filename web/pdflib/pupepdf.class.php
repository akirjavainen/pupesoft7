...
class PDF extends pdffile {
  ...
  function countParagraphHeight($txt, $font, $sW=0) {
    ...
    foreach (preg_split("/([\s\n])+/U", $txt, -1, PREG_SPLIT_DELIM_CAPTURE) as $osa) {
      if ($i==0) {
        $stringw+=$this->mb_strlen($osa, $fonts[$font]);
        if ($stringw>$sW) {
          $riveja++;
          $stringw=0;
        }
  ...
  function setLocales($maa, $valuutta) {
    $maa = mb_strtoupper($maa);
    $valuutta = mb_strtoupper($valuutta);
  ...
  function write($top, $ln, $left, $right, $txt, $font, $align="", $style="", $page="") {
    ...
    if (mb_strtoupper($style) == "B") {
      $p["font"] = $p["font"]."-Bold";
    }
    if (mb_strtoupper($style) == "I") {
      $p["font"] = $p["font"]."-Oblique";
    }
    if (mb_strtoupper($style) == "BI" or mb_strtoupper($style) == "IB") {
      $p["font"] = $p["font"]."-BoldOblique";
    }

    if (mb_strtoupper($align) == "C") {
      $p["align"] = "center";
    }
    if (mb_strtoupper($align) == "R") {
      $p["align"] = "right";
    }
    if (mb_strtoupper($align) == "L") {
      $p["align"] = "left";
    }
  ...
  function placeImage($top, $width, $imageFile, $page="") {
    ...
    if ($filedata != "") {
      if (mb_strpos($imageFile, ".png")) {
        $image = $this->png_embed($filedata);
      }
      elseif (mb_strpos($imageFile, ".jpg")) {
  ...
  function placeImageToTemplate($top, $width, $imageFile, $template, $maxSize="") {
    ...
    if ($filedata != "") {
      if (mb_strpos($imageFile, ".png") or $imageFile["filetype"] == "image/png") {
        $image = $this->png_embed($filedata);
      }
      elseif (mb_strpos($imageFile, ".jpg") or $imageFile["filetype"] == "image/jpeg") {
        $image = $this->jfif_embed($filedata);
      }
