...
class Spreadsheet_Excel_Reader
{
    ...
    /*
        ...
        foreach ($ole->_list as $i => $pps) {
            ...
            //var_dump(mb_strlen($ole->getData($i, 0, $ole->getDataLength($i))), $pps->Name, md5($this->data), $ole->getDataLength($i));
        }
//exit;
        $this->_parse();
        ...
        // Read in the number of characters
                                        ...
                                        // Read in the crun
                                                ...
                                                if ($spos + $len < $limitpos) {
                                                                $retstr = mb_substr($this->data, $spos, $len);
                                                                $spos += $len;
                                                }else{
                                                        // found countinue
                                                        $retstr = mb_substr($this->data, $spos, $limitpos - $spos);
                ...
                case SPREADSHEET_EXCEL_READER_TYPE_FORMAT:
                        ...
                        if ($version == SPREADSHEET_EXCEL_READER_BIFF8) {
                            ...
                            if (ord($this->data[$pos+8]) == 0){
                                $formatString = mb_substr($this->data, $pos+9, $numchars);
                            } else {
                                $formatString = mb_substr($this->data, $pos+9, $numchars*2);
                            }
                        } else {
                            $numchars = ord($this->data[$pos+6]);
                            $formatString = mb_substr($this->data, $pos+7, $numchars*2);
                        }
                ...
                case SPREADSHEET_EXCEL_READER_TYPE_BOUNDSHEET:
                    //echo "Type.BOUNDSHEET\n";
                        ...
                        if ($version == SPREADSHEET_EXCEL_READER_BIFF8){
                            ...
                            if ($chartype == 0){
                                $rec_name    = mb_substr($this->data, $pos+12, $rec_length);
                            } else {
                                $rec_name    = $this->_encodeUTF16(mb_substr($this->data, $pos+12, $rec_length*2));
                            }
                        }elseif ($version == SPREADSHEET_EXCEL_READER_BIFF7){
                                $rec_name    = mb_substr($this->data, $pos+11, $rec_length);
                        }
...
//            $r = &$this->file->nextRecord();
            ...
            //var_dump($this->sst);
                ...
                case SPREADSHEET_EXCEL_READER_TYPE_NUMBER:
                    $row    = ord($this->data[$spos]) | ord($this->data[$spos+1])<<8;
                    $column = ord($this->data[$spos+2]) | ord($this->data[$spos+3])<<8;
                    $tmp = unpack("ddouble", mb_substr($this->data, $spos + 6, 8)); // It machine machine dependent
                    if ($this->isDate($spos)) {
                ...
                case SPREADSHEET_EXCEL_READER_TYPE_LABEL:
                    $row    = ord($this->data[$spos]) | ord($this->data[$spos+1])<<8;
                    $column = ord($this->data[$spos+2]) | ord($this->data[$spos+3])<<8;
                    $this->addcell($row, $column, mb_substr($this->data, $spos + 8, ord($this->data[$spos + 6]) | ord($this->data[$spos + 7])<<8));
