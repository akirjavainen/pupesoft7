...
if ($tee == 'GO' and $error == 0) {
  ...
  if ($error == 0) {
    ...
    if (!in_array(mb_strtolower($path_parts['extension']), array('xls','txt','csv'))) {
      if ($cli === true) {
        pupesoft_log('tehdas_saldot', "Virheellinen tiedostopääte: ".mb_strtolower($path_parts['extension']));
      }
      else {
  ...
  if ($error == 0) {
    ...
    if (mb_strtolower($path_parts['extension']) == 'xls') {
      ...
      for ($excei = 0; $excei < $data->sheets[0]['numRows']; $excei++) {
        ...
        $tuo = sanitize_string(trim($cell[$tuo_sarake]));
      ...
      if ($file = fopen($_FILES['userfile']['tmp_name'], "r")) {
        while ($rivi = fgets($file)) {
          ...
          elseif (mb_strtolower($path_parts['extension']) == 'txt') {
          ...
          $tuo = sanitize_string(trim($rivi[$tuo_sarake]));
    ...
    if ($error == 0 and $tee == 'OK') {
      ...
      $tuotenumeron_sijainti_pupessa = sanitize_string($tuotenumeron_sijainti_pupessa);
...
if ($cli === false and $tee == '') {
  ...
  while ($tuotteen_toimittajat_row = mysqli_fetch_assoc($tuotteen_toimittajat_result)) {
