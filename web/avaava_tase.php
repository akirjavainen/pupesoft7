...
if (isset($tee) and $tee == 'I') {
  ...
  if (mysqli_num_rows($tilikausi_alku_loppu_res) == 1) {
    $tilikausi_alku_loppu_row = mysqli_fetch_assoc($tilikausi_alku_loppu_res);
...
while ($tilikausi_row = mysqli_fetch_assoc($tilikausi_result)) {
...
if (trim($tee) == 'go') {
  ...
  $query = "SELECT tunnus
            ...
            and alatila = 'T'";
  $result = pupe_query($query);
  $tulostositerow = mysqli_fetch_assoc($result);
  ...
  $query = "SELECT
            ...
            WHERE yhtio = '$kukarow[yhtio]'";
  $tilires = pupe_query($query);
  $tilitrow = mysqli_fetch_assoc($tilires);
  ...
  while ($trow = mysqli_fetch_assoc($result)) {
    ...
    $tilires = pupe_query($query);
    $tilinimirow = mysqli_fetch_assoc($tilires);
