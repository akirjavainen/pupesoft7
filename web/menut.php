...
if ($kukarow['kuka'] == 'admin' and (isset($synkronoireferenssi) or isset($synkronoireferenssialapaivita)) and count($syncyhtiot) > 0) {
  ...
  foreach ($referenssit as $rivi) {
    ...
    if ($sovellus == '' or mb_strtoupper($sovellus) == mb_strtoupper($rivi[0])) {
  ...
  foreach ($rows as $row) {
    ...
    foreach ($syncyhtiot as $yhtio) {
      ...
      if (mysqli_num_rows($result) == 0 and $row["sovellus"] != "") {
        $query = "INSERT into oikeu SET
                  kuka          = '',
                  profiili      = '',
                  sovellus      = '$row[sovellus]',
...
if ($tee == "PAIVITA") {
  ...
  if (mysqli_num_rows($tarkistus) > 0) {
...
if ($tee == "") {
  ...
  $sovyhtiot = mb_substr($sovyhtiot, 0, -1);
