...
if (php_sapi_name() == 'cli') {
  ...
  // Adminin oletus
  $kukarow = mysqli_fetch_assoc($result);
  $yhtiorow = hae_yhtion_parametrit($yhtio);
}
else {
...
function kasittele_xml_tiedosto(SimpleXMLElement $xml, $tiedosto_polku) {
  ...
  if ($xml !== FALSE) {
    ...
    if (!empty($toim_asiakkaantiedot["maa"])) {
      ...
      if (mysqli_num_rows($maaresult)) {
        $maarow = mysqli_fetch_assoc($maaresult);
    ...
    $laskun_tapvm = mb_substr($laskun_tapvm, 0, 4)."-".mb_substr($laskun_tapvm, 4, 2)."-".mb_substr($laskun_tapvm, 6, 2);
    $laskun_lapvm = mb_substr($laskun_lapvm, 0, 4)."-".mb_substr($laskun_lapvm, 4, 2)."-".mb_substr($laskun_lapvm, 6, 2);
    $laskun_erapaiva = mb_substr($laskun_erapaiva, 0, 4)."-".mb_substr($laskun_erapaiva, 4, 2)."-".mb_substr($laskun_erapaiva, 6, 2);
    $laskun_kapvm = mb_substr($laskun_kapvm, 0, 4)."-".mb_substr($laskun_kapvm, 4, 2)."-".mb_substr($laskun_kapvm, 6, 2);
    ...
    if (empty($maksuehto)) {
      ...
      if (mysqli_num_rows($maksuehtores) == 1) {
        ...
        if (mysqli_num_rows($maksuehtores) == 1) {
          $maksuehto = mysqli_fetch_assoc($maksuehtores);
        }
      }
    }
    ...
    if (mysqli_num_rows($tseklasku_res)) {
      siirra_tiedosto_kansioon($tiedosto_polku, $finvoice_myyntilasku_kansio_error);
