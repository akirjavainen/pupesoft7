...
if (!function_exists("ennakkolaskuta")) {
  function ennakkolaskuta($tunnus) {
    ...
    $laskurow = mysqli_fetch_assoc($stresult);
    ...
    for ($i=0; $i<mysqli_num_fields($stresult); $i++) {
      ...
      $fieldname = mysqli_field_name($stresult, $i);
...
if (strpos($_SERVER['SCRIPT_NAME'], "maksusopimus_laskutukseen.php") !== FALSE) {
  ...
  if (mysqli_num_rows($tresult) == 0) die(t("VIRHE: Yhtiöllä EI OLE ennakkolaskutustuotetta, sopimuslaskutusta ei voida toteuttaa!"));
  ...
  if ($tee == "ennakkolaskuta_kaikki") {
    ...
    $posrow = mysqli_fetch_assoc($rahres);
  ...
  if ($tee == "vapauta_tilaus_keraykseen") {
    ...
    while ($laskurow = mysqli_fetch_assoc($laskures)) {
      ...
      if (mysqli_affected_rows($link) != 0) $laskurow['alatila'] = '';
  ...
  if ($tee == "") {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
      ...
      $query = "SELECT *
                ...
                LIMIT 1";
      $rahres = pupe_query($query);
      $laskurow = mysqli_fetch_assoc($rahres);
      ...
      while ($laskurow2 = mysqli_fetch_assoc($rahres)) {
      ...
      if ($row["yhteensa_kpl"] - $row["laskutettu_kpl"] <= 1) {
        ...
        if (mysqli_num_rows($tarkres) == 0 or $tarkrow["tilaok"] <> $tarkrow["toimituksia"]
          ...
          if (mysqli_num_rows($tila_chk_res) > 0) {
            echo "<br />";
