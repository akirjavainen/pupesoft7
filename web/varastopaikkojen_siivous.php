...
if ($tee == 'CLEAN') {
  ...
  if (count($valittu) != 0) {
    foreach ($valittu as $rastit) {
      ...
      $tuoterow = mysqli_fetch_array($presult);
...
if ($tee == 'CLEANTUNTEMATTOMAT') {
  ...
  foreach ($paikkatunnus as $tunnus) {
    ...
    $query = "SELECT *
              ...
              and tunnus  = '$tunnus'";
    $presult = pupe_query($query);
    $paikkarow = mysqli_fetch_array($presult);
    ...
    if (mysqli_num_rows($presult) == 1) {
      $vanharow = mysqli_fetch_array($presult);
...
if ($tee == 'CLEANRIVIT') {
  ...
  foreach ($rivitunnus as $tunnus) {
    ...
    $rivirow = mysqli_fetch_array($presult);
...
if ($tee == 'LISTAA' and !$_cli) {
  ...
  if ($ahyllyalue != '') {
    $apaikka  = mb_strtoupper(sprintf("%-05s", $ahyllyalue));
    $apaikka .= mb_strtoupper(sprintf("%05s", $ahyllynro));
    $apaikka .= mb_strtoupper(sprintf("%05s", $ahyllyvali));
    $apaikka .= mb_strtoupper(sprintf("%05s", $ahyllytaso));
  ...
  if ($lhyllyalue != '') {
    $lpaikka  = mb_strtoupper(sprintf("%-05s", $lhyllyalue));
    $lpaikka .= mb_strtoupper(sprintf("%05s", $lhyllynro));
    $lpaikka .= mb_strtoupper(sprintf("%05s", $lhyllyvali));
    $lpaikka .= mb_strtoupper(sprintf("%05s", $lhyllytaso));
  ...
  if (mysqli_num_rows($result) > 0) {
    ...
    while ($lrow = mysqli_fetch_array($result)) {
      ...
      echo "<td>$lrow[saldo]</td>";
      echo "<td>".mb_substr($lrow["saldoaika"], 0, 10)."</td>";
      echo "<td>$lrow[paikka]</td>";
...
if ($tee == "LISTAAVIRHEELLISETRIVIT" and !$_cli) {
  ...
  while ($tuoterow = mysqli_fetch_array($tilrivires)) {
...
if ($tee == "LISTAATAPAHTUMATILMANPAIKKAA" and !$_cli) {
  ...
  while ($tapahtumarow = mysqli_fetch_array($tapahtumares)) {
