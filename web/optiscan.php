...
if ($sanoma == "SignOn") {
  ...
  $kukarow  = hae_kukarow(sanitize_string(trim($sisalto[2])), $yhtiorow["yhtio"]);
...
elseif ($sanoma == "GetPicks") {
  ...
  $query = "SELECT nro, min(keraysvyohyke) keraysvyohyke, GROUP_CONCAT(tilausrivi) AS tilausrivit,
            ...
            LIMIT 1";
  $result = pupe_query($query);
  $kerattavat_rivit_row = mysqli_fetch_assoc($result);
  ...
  if (trim($kerattavat_rivit_row['tilausrivit']) != '') {
    ...
    $keraysjarjestys_row = mysqli_fetch_assoc($keraysjarjestys_res);
    ...
    while ($rivi_row = mysqli_fetch_assoc($rivi_result)) {

      $rivi_row['kerayskommentti'] = str_replace(array("'", ","), "", $rivi_row['kerayskommentti']);

      $pakkauskirjain = mb_strtoupper(chr((64+$rivi_row['pakkausnro'])));
      ...
      $hyllypaikka = implode(" ", mb_str_split(mb_strtoupper(trim($hyllypaikka))));

      // W7 12 80 => W71280 => w71280 => w 7 128 0
      $rivi_row['tuoteno'] = mb_strtoupper(str_replace(" ", "", $rivi_row['tuoteno']));

      $_tmp = mb_str_split($rivi_row['tuoteno']);
      $_cnt = count($_tmp);
      $_arr = array();
      ...
      $rivi_row['tuoteno'] = implode(" ,,, ", mb_str_split($rivi_row['tuoteno'], 3));
      $rivi_row['yksikko'] = t_avainsana("Y", "", "and avainsana.selite='{$rivi_row['yksikko']}'", "", "", "selite");

      $response .= "N,";
      $response .= mb_substr($rivi_row['ker_nimitys'], 0, 255).",";
...
elseif ($sanoma == "PrintContainers") {
  ...
  if (trim($response) == '') {
    ...
    $duplikaatti = 0;
    $riveja_tot  = mysqli_num_rows($result);
...
elseif ($sanoma == "Picked") {
  ...
  $package     = sanitize_string(trim($sisalto[6]));
  ...
  elseif ($splitlineflag == 2 or $splitlineflag == 3) {
    ...
    for ($i = 0; $i < mysqli_num_fields($result); $i++) {
      ...
      case 'sscc':
        ...
        $selite_row = mysqli_fetch_assoc($selite_result);
    ...
    $printteri_row = mysqli_fetch_assoc($printteri_res);
...
elseif ($sanoma == "StopAssignment") {
  ...
  if ($row['kpl'] == 0) {
    ...
    if (mysqli_num_rows($valmis_era_chk_res) == 0) {
      $response = "1,Virhe ei keräyserää\r\n\r\n";
    }
    else {

      while ($valmis_era_chk_row = mysqli_fetch_assoc($valmis_era_chk_res)) {
      ...
      while ($valmis_era_chk_row = mysqli_fetch_assoc($valmis_era_chk_res)) {
        ...
        $varattu_row = mysqli_fetch_assoc($varattu_res);
      ...
      if (count($print_array) == 0) {
        ...
        if (mysqli_num_rows($avainsanares) > 0) {
          $avainsanarow = mysqli_fetch_assoc($avainsanares);
...
elseif ($sanoma == "NewContainer") {
  ...
  $printer_chk_row = mysqli_fetch_assoc($printer_chk_res);
...
elseif ($sanoma == "ChangeContainer") {
  ...
  $result = pupe_query($query);
  $orig_row = mysqli_fetch_assoc($result);
