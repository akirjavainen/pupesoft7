...
if (mysqli_num_rows($result) > 1) {
  ...
  if (isset($goso) and $goso != "") {
    ...
    $gorow = mysqli_fetch_array($gores);
  ...
  $orow = mysqli_fetch_array($result);
...
while ($orow = mysqli_fetch_array($result)) {
  ...
  $mrow = mysqli_fetch_array($xresult);

  $nimitys_lukumaara = "";

  if ($mrow['nimi'] == 'extranet_tarjoukset_ja_ennakot.php' and mb_stristr($mrow['alanimi'], "EXTENNAKKO")) {
  ...
  if ($mrow['nimi'] == 'extranet_tarjoukset_ja_ennakot.php' and mb_stristr($mrow['alanimi'], "EXTTARJOUS")) {
  ...
  if (mysqli_num_rows($xresult) > 1) {
    ...
    if (isset($mrow['nimitys']) and isset($menu[$mrow['nimitys']]) and $menu[$mrow['nimitys']] == 1) {
      ...
      while ($mrow = mysqli_fetch_array($xresult)) {
        echo "<tr><td><a class='menu' href='$mrow[nimi]";

        if (mb_strpos($mrow['nimi'], '?') === FALSE) {
          echo "?";
        }
        else {
          echo "&";
    ...
    if (mb_substr($mrow["nimi"], 0, 4) == "http" and $mrow["alanimi"] != "") {
