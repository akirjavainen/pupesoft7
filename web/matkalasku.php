...
if ($tee == "UUSI") {
  ...
  */
  ...
  $query = "SELECT tuote.tuoteno, tuote.nimitys, tuote.myyntihinta, tuote.malli, tuote.myymalahinta
            ...
            LIMIT 1";
  $result1 = pupe_query($query);
  $vtrow = mysqli_fetch_assoc($result1);
...
if ($tee == 'TARKISTA_ILMAISET_LOUNAAT' and !$muokkauslukko) {
  ...
  if ($rivihinta != $row['rivihinta']) {
    ...
    $result = pupe_query($query);
    $tiliointi_tilausrivi_row = mysqli_fetch_assoc($result);
...
if ($tee == "MUOKKAA") {
  ...
  if (!$muokkauslukko) {
    ...
    if (mysqli_num_rows($keikres) == 0 and ($laskurow["tilaustyyppi"] == "M" or $lisaatapahtumaan == "OK" or $toim == "SUPER")) {
  ...
  if ($tyyppi != "" and $tuoteno != "" and !$muokkauslukko) {
    ...
    if ($trow['myymalahinta'] > 0 and mb_strpos($trow['nimitys'], 'Kilometrikorvaus') !== false) {
    ...
    elseif ($tyyppi == "B") {
      ...
      if ($maa != "" and $maa != $yhtiorow["maa"]) {
        ...
        if ($alvulk == "") {
          ...
          $alvrow = mysqli_fetch_assoc($alhire);
    ...
    if ($laskurow["tilaustyyppi"] != "M") {
      ...
      $result = pupe_query($query);
      $rivisumma = mysqli_fetch_assoc($result);
  ...
  if (mysqli_num_rows($result) > 0) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
      ...
      if ($row['tunnus'] == $row['perheid2']) {
        $tilausrivien_lkm1 = mysqli_fetch_row($result2);
...
if ($tee == "") {
  ...
  if ($toim == "SUPER") {
    ...
    if (mysqli_num_rows($result) > 0) {
      ...
      while ($krow = mysqli_fetch_assoc($result)) {
        $valittu = "";
...
function hae_ensimmainen_matkalaskurivi($kukarow, $tilausnumero, $perheid2) {
  ...
  return mysqli_fetch_assoc($abures);
}
...
function tarkista_loytyyko_paivalle_matkalasku($alkuaika, $loppuaika, $tilausnumero) {
  ...
  if ($tilausrivi_row = mysqli_fetch_assoc($result)) {
...
function korjaa_ostovelka($tilausnumero) {
  ...
  $summarow = mysqli_fetch_assoc($summares);
  ...
  if (mysqli_num_rows($velkares) == 1) {
    $velkarow = mysqli_fetch_assoc($velkares);
  ...
  $result = pupe_query($query);
  $tiliointi_summa = mysqli_fetch_assoc($result);
      ...
      for ($i=0; $i < count($tuotetiedot); $i++) {
        ...
        if ($rtuoteno[$i]["kpl"] > 0) {
          ...
          $query = "INSERT into tilausrivi set
                    ...
                    tyyppi         = 'M',
                    toimaika       = '',
                    kommentti      = '".sanitize_string($kommentti)."',
                    var            = '',
                    try            = '$trow[try]',
                    osasto         = '$trow[osasto]',
                    perheid        = '0',
...
function tee_matkalaskurivin_kirjaukset($variables) {
  ...
  if ($kpexport != 1 and mb_strtoupper($yhtiorow['maa']) == 'FI') $tositenro = 0; // Jos tätä ei tarvita
  ...
  $query = "INSERT into tiliointi set
            ...
            summa    = '$rivihinta',
            vero     = '$vero',
            selite   = '".sanitize_string($selite_array[$indeksi])."',
            lukko    = '',
            tosite   = '$tositenro',
            laatija  = '$kukarow[kuka]',
            laadittu = now()";
  ...
  if (mb_strpos($lisaa_tuoteno, "PR") === false) {
    $perheid = 0;
    $perheid2 = 0;
  }
