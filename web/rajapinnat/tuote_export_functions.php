...
function tuote_export_hae_tuotetiedot($params) {
  ...
  while ($row = mysqli_fetch_array($res)) {
    ...
    if ($tuotteiden_asiakashinnat_magentoon !== false) {
      ...
      while ($asiakashintarow = mysqli_fetch_assoc($asiakashintares)) {
        $asiakashinnat[] = array(
    ...
    while ($parametrirow = mysqli_fetch_assoc($parametritres)) {
    ...
    $hinnastoresult = mysqli_fetch_assoc($hinnastoq);
    ...
    $dnstuote[] = array(
      ...
      'nakyvyys'             => mb_strtolower($row["nakyvyys"]),
      'nimi'                 => $row["nimitys"],
      'nimitys'              => $row["nimitys"],
      'onsale'               => $row["onsale"],
      'osasto'               => $row["osasto"],
...
function magento_tuote_export_tehdas_saldot($tuoteno) {
  ...
  if (mysqli_num_rows($ttres) == 0) return 0;
...
function tuote_export_hae_lajitelmatuotteet($params) {
  ...
  while ($rowselite = mysqli_fetch_assoc($resselite)) {
    ...
    while ($alirow = mysqli_fetch_assoc($alires)) {
      // Haetaan kaikki tuotteen atribuutit
      ...
      while ($syvinrow = mysqli_fetch_assoc($alinres)) {
      ...
      $dnslajitelma[$rowselite["selite"]][] = array(
        ...
        'myyntihinta'           => $myyntihinta,
        'myyntihinta_veroton'   => $myyntihinta_veroton,
        'nakyvyys'              => mb_strtolower($alirow["nakyvyys"]),
        'nimi'                  => $alirow["nimitys"],
...
function tuote_export_hae_tuotteen_avainsanat($tuoteno) {
  ...
  while ($avainsana_row = mysqli_fetch_assoc($avainsana_result)) {
    $tuotteen_avainsanat[] = array(
