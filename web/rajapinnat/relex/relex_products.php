...
if (!function_exists("relex_product_ostoehdotus_update")) {
  function relex_product_ostoehdotus_update($hakukentta, $tuoterajaus, $paiva_ajo) {
    ...
    $_hakukentta_loytyi = mb_strpos($tuoterajaus, "tuote.$hakukentta");

    if ($_hakukentta_loytyi !== FALSE and $paiva_ajo) {

      $_rajaus_alkaa_hakukentalla = mb_substr($tuoterajaus, $_hakukentta_loytyi);
...
foreach ($update_kentat as $_kentta) {
  ...
  $_hakukentta_loytyi = mb_strpos($_tuoterajaus_ilman_hakukenttia, "tuote.$_kentta");

  if ($_hakukentta_loytyi !== FALSE and $_tee_ostoehdotus_update) {

    $_rajaus_alkaa_hakukentalla = mb_substr($_tuoterajaus_ilman_hakukenttia, $_hakukentta_loytyi);
    $_rajaukset = explode(" AND", $_rajaus_alkaa_hakukentalla);
...
if ($_tuoterajaus) {
  ...
  $_tuoterajaus = $_tuoterajaus_ilman_hakukenttia ." AND (". mb_substr($_tuoterajaus, 0, -4).")";
...
while ($row = mysqli_fetch_assoc($res)) {
  ...
  if (mysqli_num_rows($ttres) > 0) {
    ...
    while ($ttrow = mysqli_fetch_assoc($ttres)) {
      ...
      if (mysqli_num_rows($emares)) {
        while ($emarow = mysqli_fetch_assoc($emares)) {
      ...
      if ($ttrow['valuutta'] != $yhtiorow['valkoodi']) {
        ...
        if (mysqli_num_rows($vresult) == 1) {
          $valtrow = mysqli_fetch_assoc($vresult);
        }
      }
  ...
  if ($vastaavat->onkovastaavia()) {
    ...
    // Vika : pois
    $vastaavat_t = mb_substr($vastaavat_t, 0, -1);
  }
