...
function rest_tilaa($params) {
  ...
  $tilausnumero  = isset($params["tilausnumero"])  ? sanitize_string(trim($params["tilausnumero"])) : 0;
  $tuoteno    = isset($params["tuoteno"])      ? sanitize_string(trim($params["tuoteno"])) : "";
  $tunnus      = isset($params["asiakastunnus"])      ? (int) trim($params["asiakastunnus"]) : "";
  $kommentti    = isset($params["tilauskommentti"])    ? sanitize_string(trim($params["tilauskommentti"])) : "";
  $toim        = "RIVISYOTTO";
  ...
  if (mysqli_num_rows($kesken) == 0) {
    rest_virhe_header("Tilausta ei löytynyt järjestelmästä");
  }

  $laskurow = mysqli_fetch_assoc($kesken);
...
function rest_login($params) {
  ...
  // Hyväksytään seuraavat parametrit
  $user = isset($params["user"]) ? sanitize_string(trim($params["user"])) : "";
  $pass = isset($params["pass"]) ? md5($params["pass"]) : "";
  $yhtio = isset($params["yhtio"]) ? sanitize_string(trim($params["yhtio"])) : "";
...
elseif ($tyyppi == "customer") {
  $toiminto = isset($data["toiminto"]) ? mb_strtoupper(pupesoft_cleanstring($data["toiminto"])) : "";
