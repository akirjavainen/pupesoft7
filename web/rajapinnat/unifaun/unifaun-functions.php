...
function unifaun_tilauksen_vak_koodit($tilausnumero) {
  ...
  // ei vak tuoteita
  if (mysqli_num_rows($vak_chk_res) == 0) {
    return $vak_koodit;
  }
...
function unifaun_limited_quantity($vak_maara, $limited_qty) {
  ...
  // putstaan arvot
  $ltq_yks = preg_replace("/[^a-z]/", "", mb_strtolower($vak_maara));
...
// palauttaa lasku-taulun kentän nimen, jota tulee käyttää unifaun sanomassa lähettäjän viitteenä
