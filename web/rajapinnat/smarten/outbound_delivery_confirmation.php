  ...
  */
  ...
  if (mysqli_num_rows($laskures) > 0) {
      ...
      foreach ($line->ItemReserve as $stock_item) {
        ...
        if (!isset($tilausrivit[$tilausrivin_tunnus])) {
          $tilausrivit[$tilausrivin_tunnus] = array(
            'item_number' => sanitize_string($line->SellerItemCode),
            'keratty'     => $kpl,
            'kollit'      => array()
          );
        }
    ...
    if (!empty($keratyt_tunnukset)) {
      ...
      while ($ei_keratyt_row = mysqli_fetch_assoc($ei_keratyt_res)) {
    ...
    if (mysqli_num_rows($edi_chk_res) and $packageinfo_sanomalla) {
    ...
    if ($keratty_yhteensa == 0 and $laskurow['tila'] == 'L') {
      ...
      $rahtikirjantunnus = mysqli_insert_id();
      ...
      if ($paivitettiin_tilausrivi_onnistuneesti) {
        ...
        elseif (!empty($tracking_code)) {
          ...
          $toimitusvahvistus_row = mysqli_fetch_assoc($toimitusvahvistus_res);
