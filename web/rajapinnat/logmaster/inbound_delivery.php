...
if (php_sapi_name() != 'cli') {
  ...
  if (mb_strpos($_SERVER['SCRIPT_NAME'], "inbound_delivery.php") !== false) {
...
while ($rivit_row = mysqli_fetch_array($rivit_res)) {
...
elseif ($varasto_check['L'] == 0 and $varasto_check['P'] > 0) {
  ...
  pupesoft_log('logmaster_inbound_delivery', "Saapuminen {$row['laskunro']} sisältää useamman ulkoisen varaston tuotteita. Ei saa sisältää kuin yhtä.");
  exit("Saapuminen {$row['laskunro']} sisältää useamman ulkoisen varaston tuotteita. Ei saa sisältää kuin yhtä.");
}
...
if ($xml_chk) {
  ...
  $_name = mb_substr("in_{$row['laskunro']}_".implode('_', $ostotilaukset), 0, 25);
