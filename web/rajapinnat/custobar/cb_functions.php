...
function cb_hae_asiakkaat() {
  ...
  while ($row = mysqli_fetch_array($res)) {
    ...
    $yht = mysqli_fetch_assoc($yhtresult);
    ...
    $magyht = mysqli_fetch_assoc($magyhtresult);
    ...
    $row['kieli'] = mb_strtoupper($row['kieli']);
    if ($row['kieli'] == 'SE') {
      $row['kieli'] = "SV";
    }
    elseif ($row['kieli'] == 'FI') {
...
function cb_hae_myynnit() {
  ...
  while ($row = mysqli_fetch_array($maksuehtores)) {
  ...
  while ($row = mysqli_fetch_array($res)) {
    ...
    if (isset($cb_picture_url) and !empty($cb_picture_url)) {
      ...
      if (mysqli_num_rows($result) == 1) {
        $liite_row = mysqli_fetch_assoc($result);
    ...
    $try_row = mysqli_fetch_assoc($try_res);
    ...
    $tuotteet[$i] = array(
      ...
      "language"                  => mb_strtoupper($yhtiorow['kieli']),
