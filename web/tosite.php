...
if ($tee == 'I') {
    ...
    if ($retval === true) {
      ...
      foreach ($excelrivi as $erivi) {
        foreach ($erivi as $e => $eriv) {
          ...
          if (mb_strtolower($otsikot[$e]) == "kustp" or mb_strtolower($otsikot[$e]) == "kohde" or mb_strtolower($otsikot[$e]) == "projekti") {
            // Kustannuspaikka, kohde, projekti
            $kukopr = mb_strtolower($otsikot[$e]);
          ...
          elseif (mb_strtolower($otsikot[$e]) == "summa") {
            ${"i".mb_strtolower($otsikot[$e])}[$maara] = sprintf("%.2f", round($eriv, 2));
          }
          else {
            ${"i".mb_strtolower($otsikot[$e])}[$maara] = $eriv;
          }
  ...
  for ($i=1; $i<$maara; $i++) {
    ...
    if (mb_strlen($itili[$i]) > 0 or mb_strlen($isumma[$i]) > 0) {
      ...
      if (mb_strlen($iselite[$i]) == 0 and mb_strlen($comments) == 0) { // Selite ja kommentti puuttuu
      ...
      $liitos      = sanitize_string($iliitos[$i]);
      $liitostunnus  = sanitize_string($iliitostunnus[$i]);
      ...
      if (!isset($ivirhe[$i]) and mb_strlen($virhe) > 0) {
        $ivirhe[$i] = $virhe;
      }
...
if ($tee == '') {
  ...
  if (isset($tunnus) and $tunnus > 0 and count($tiliointirivit) > 0) {
    ...
    $lasku_chk_row = mysqli_fetch_assoc($lasku_chk_res);
