...
if ($_REQUEST["tee"] == "login") {
  ...
  if (trim($user) != '') {
    ...
    $session = "";
    $user = sanitize_string(trim($user));
...
if (!function_exists("menu")) {
  function menu($osasto = "", $try = "") {
    ...
    if ($kukarow["kesken"] > 0) {
      ...
      $vierow = mysqli_fetch_array($vieres);
    }
    elseif ($verkkokauppa != "") {
      $vierow = array();
    ...
    elseif ($try == "" and ($verkkokauppa_anon or $kukarow["kuka"] != "www")) {
      ...
      while ($tryrow = mysqli_fetch_array($tryres)) {
        ...
        while ($trow = mysqli_fetch_array($res) and $ok == 0) {
          ...
          if ($trow["hinnastoon"] == "V" or $kukarow["naytetaan_tuotteet"] == "A") {
            ...
            if (!is_array($asiakasrow)) {
              ...
              $asiakasrow = mysqli_fetch_array($asres);
            }
    ...
    elseif ($verkkokauppa_tuotemerkit and ($verkkokauppa_anon or $kukarow["kuka"] != "www")) {
      ...
      while ($merow = mysqli_fetch_array($meres)) {
...
if ($tee == "poistakori") {
  ...
  if (mysqli_num_rows($result) == 1) {
    ...
    //$ostoskori = mysqli_fetch_array($result);
    $kalakori = mysqli_fetch_array($result);
...
if ($tee == "asiakastiedot") {
  ...
  elseif ($nayta == "tilaushistoria") {
    ...
    if ($tila == "haku") {
      ...
      if (mb_strlen($tilaushaku) > 2 or $tilaustila != "") {
        $where = " and tila in ('N','L') ";

        if ($hakutapa == "viitteet") {
          $where .= " and concat_ws(' ', viesti, comments, sisviesti2, sisviesti1, asiakkaan_tilausnumero) like ('%".sanitize_string($tilaushaku)."%') ";
        }
        elseif ($hakutapa == "toimitusosoite") {
          $where .= " and concat_ws(' ', toim_nimi, toim_nimitark, toim_osoite, toim_postino, toim_postitp) like ('%".sanitize_string($tilaushaku)."%') ";
        }
...
if ($tee == "") {
  ...
  $tuotenumero = sanitize_string(trim($_GET["tuotenumero"]));
  $tuotenimitys = sanitize_string(trim($_GET["tuotenimitys"]));

  if (mb_stripos($_SERVER["HTTP_USER_AGENT"], "MSIE") === FALSE and ($verkkokauppa_anon or $kukarow["kuka"] != "www")) {
