...
if ($toimittajaid > 0) {
  ...
  $toimittaja = mysqli_fetch_assoc($toimittaja_result);
}
...
if ($tee == "raportoi" and $alkupvm != "" and $loppupvm != "") {
  ...
  while ($tama_rivi = mysqli_fetch_assoc($saapumiset_result)) {
    ...
    $query = "SELECT group_concat(vanhatunnus) as vanhatunnus
              ...
              AND yhtio        = '{$kukarow['yhtio']}'";
    $vanhatunnus = mysqli_fetch_assoc(pupe_query($query));
    ...
    if ($vanhatunnus['vanhatunnus'] != "") {
      ...
      $vols_query = "SELECT round(sum(summa),2) as summa
                     ...
                     AND yhtio     = '{$kukarow['yhtio']}'";
      $vols = mysqli_fetch_assoc(pupe_query($vols_query));
    }
    ...
    if ($tama_rivi['tunnus'] != "") {
      ...
      $sks_query = "SELECT round(sum(tilausrivi.rivihinta),2) as saapumisen_summa
                    ...
                    AND yhtio         = '{$kukarow['yhtio']}'";
      $sks = mysqli_fetch_assoc(pupe_query($sks_query));
  ...
  if (mysqli_num_rows($saapumiset_result) > 0) {
  ...
  // mysqli_num_rows == 0, ei löytynyt yhtään saapumista
  else {
