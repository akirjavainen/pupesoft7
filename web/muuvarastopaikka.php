...
if (mb_strpos($_SERVER['SCRIPT_NAME'], "muuvarastopaikka.php")  !== FALSE) {
...
if (mb_strtolower($toim) == 'oletusvarasto' and $kukarow['oletus_varasto'] != '' and $kukarow['oletus_varasto'] != 0) {
...
if ($tee == 'MUUTA' and $toim != "VAINSIIRTO") {
  ...
  $oletusrow = mysqli_fetch_array($oletus_result);
  ...
  if (mysqli_num_rows($paikatresult1) > 0) {
    while ($saldorow = mysqli_fetch_array($paikatresult1)) {
  ...
  if (count($poista) > 0) {
    foreach ($poista as $poistettava) {
      ...
      elseif ($saldot[$poistetaan] != 0) {
        ...
        while ($jtrivi = mysqli_fetch_assoc($rivires)) {
          ...
          $nykyvarasto_tunnus = mysqli_fetch_assoc(pupe_query($query));
          ...
          if (mysqli_num_rows($uusivarasto_res) == 0) {
            $query = "SELECT varasto,
                      hyllyalue,
                      hyllynro,
                      hyllyvali,
          ...
          if ($uusivarasto = mysqli_fetch_assoc($uusivarasto_res)) {
  ...
  if (isset($oletus) and $oletus != $oletusrow["tunnus"]) {
    ...
    if (mysqli_num_rows($oletus_result) == 1) {

      $uusi_oletusrow = mysqli_fetch_assoc($oletus_result);
...
if ($tee == 'N') {
  ...
  $result = pupe_query($query);
  $mistarow = mysqli_fetch_array($result);
  ...
  $result = pupe_query($query);
  $minnerow = mysqli_fetch_array($result);
  ...
  $sarjacheck_row = mysqli_fetch_array($sarjaresult);
  ...
  elseif ($sarjacheck_row["erat"] > 0) {
    ...
    $siirrettava_era_row = mysqli_fetch_assoc($siirrettava_era_res);
  ...
  elseif ($sarjacheck_row["sarjat"] > 0) {
    ...
    foreach ($sarjano_array as $sarjatun) {
      ...
      if (mysqli_num_rows($sarjares) == 1) {
        ...
        while ($lisavarrow = mysqli_fetch_array($sarjares)) {
  ...
  for ($iii=0; $iii< count($tuotteet); $iii++) {
    ...
    if (mysqli_num_rows($result) == 1) {
      ...
      $lisavartprow = mysqli_fetch_array($result);
      ...
      # Kaikki saldot tilauksilla ja kannassa ovat kahden desimaalin tarkkuudella, joten tehdään vertailut näillä tarkkuuksilla myös
      if (round($kappaleet[$iii], 2) == round($hyllyssa, 2) and round($myytavissa, 2) < round($kappaleet[$iii], 2) and mb_strpos($_SERVER['SCRIPT_NAME'], "muuvarastopaikka.php") !== false) {
  ...
  if ($saldook > 0 and (round($kappaleet[$iii], 2) < round($myytavissa, 2) or round($kappaleet[$iii], 2) < round($hyllyssa, 2))) {
    echo "<font class='error'>".t("Voit siirtää vain myytävissä olevaa määrää tai koko hyllyssä olevan määrän")."</font><br><br>";
    $tee = $uusitee;
  }
  ...
  elseif ($sarjacheck_row["erat"] > 0 and count($sarjano_array) > 0) {
    foreach ($sarjano_array as $sarjano) {
      if ($sarjano > 0) {
        ...
        $sarrr_row = mysqli_fetch_assoc($sarrr_res);
        ...
        if (mysqli_num_rows($sarrr_res2) == 1) {
          $sarrr_row2 = mysqli_fetch_assoc($sarrr_res2);
...
if ($tee == 'M') {
  ...
  if (mysqli_num_rows($paikatresult2) > 0) {
    while ($saldorow = mysqli_fetch_array($paikatresult2)) {
  ...
  if ($trow["sarjanumeroseuranta"] != '') {
    ...
    if (mysqli_num_rows($sarjares) > 0) {
      ...
      if (mysqli_num_rows($sarjares) > 10) {
  ...
  if ($toim != "VAINSIIRTO") {
    ...
    if (mysqli_num_rows($paikatresult1) > 0) {
      $query = "SELECT *
                ...
                and oletus    != ''";
      $result = pupe_query($query);
      $oletusrow = mysqli_fetch_array($result);

      mysqli_data_seek($paikatresult1, 0);
      ...
      while ($saldorow = mysqli_fetch_array($paikatresult1)) {
        ...
        if ($saldorow["saldo"] == 0 and $hyllyssa == 0 and $myytavissa == 0) {
          ...
          $reklacheck = (mysqli_num_rows($reklares) > 0);
        }
        else {
          $reklacheck = false;
        }
