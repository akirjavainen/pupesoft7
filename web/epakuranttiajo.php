...
if ($php_cli or (isset($ajo_tee) and ($ajo_tee == "NAYTA" or $ajo_tee == "NAYTAPV" or $ajo_tee == "EPAKURANTOI"))) {
  ...
  while ($epakurantti_row = mysqli_fetch_assoc($epakurantti_result)) {
    ...
    if ($php_cli and $kaikkiepakur != "") {
      ...
      if (!$tulorow = mysqli_fetch_assoc($tapres)) {

        $_luontiaika            = mb_substr($epakurantti_row["luontiaika"], 0, 10);
      ...
      if (!$laskutusrow = mysqli_fetch_assoc($tapres)) {
        // Jos ei löydy laskua, laitetaan jotain vanhaa
        $laskutusrow = array("laadittu" => "1970-01-01");
      }

      list($vv1, $kk1, $pp1) = explode("-", mb_substr($tulorow["laadittu"], 0, 10));
      list($vv2, $kk2, $pp2) = explode("-", mb_substr($laskutusrow["laadittu"], 0, 10));
