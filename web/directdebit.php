...
if (mysqli_num_rows($directdebit_result) == 0) {
  echo t("%s Direct Debit-sopimusta ei ole perustettu!", null, $directdebityhtio);
  $tee = "ohita";
}
elseif (mysqli_num_rows($directdebit_result) == 1) {
  // meillä on vaan yksi, ei tarvitse valita
  $vrow = mysqli_fetch_assoc($directdebit_result);
...
if ($tee == '') {
  ...
  while ($vrow = mysqli_fetch_assoc($directdebit_result)) {
...
if ($tee == 'TULOSTA') {
  ...
  if (mysqli_num_rows($laskures) > 0) {
    ...
    while ($laskurow = mysqli_fetch_assoc($laskures)) {
      ...
      $dd_asirow = mysqli_fetch_assoc($dd_asires);
