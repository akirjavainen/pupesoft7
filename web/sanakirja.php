...
foreach ($GLOBALS["sanakirja_kielet"] as $sanakirja_kieli => $sanakirja_kieli_nimi) {
  ...
  $sels = '';
  if (mb_strtoupper($kieli[$r]) == mb_strtoupper($sanakirja_kieli)) {
    $sels = "CHECKED";
  }
...
if (count($kieli) > 0) {
  ...
  if (mysqli_num_rows($result) > 0) {
    echo "<table>";
    echo "<tr>";
    for ($i=1; $i<mysqli_num_fields($result); $i++) echo "<th>".mysqli_field_name($result, $i)."</th>";
    echo "</tr>";
    ...
    while ($row = mysqli_fetch_array($result)) {
      ...
      for ($i=2; $i<mysqli_num_fields($result); $i++) {
        echo "<td><input type='text' size='30' name='".mysqli_field_name($result, $i)."[$row[tunnus]]' value='".trim($row[$i])."'></td>";
      }
    ...
    echo "<tr><th colspan='".mysqli_num_rows($result)."'>".t("Yhteensä")." $laskkaannos ".t("riviä")."</th></tr>";
    echo "</table>";
