...
if (isset($siirra_lahtoon) and $onko_paivitysoikeuksia_ohjelmaan) {
  ...
  if (isset($checkbox_child) and (is_array($checkbox_child) or is_string(trim($checkbox_child)))) {
    ...
    if (isset($uusi_lahto) and is_numeric(trim($uusi_lahto)) and trim($uusi_lahto) > 0) {
      ...
      if (mb_strpos($valittu_lahto, "__") !== false) {
      ...
      foreach ($checkbox_child as $tilausnumero) {
        ...
        if ($tila_chk_row['tila'] == 'K' or $tila_chk_row['tila'] == 'T') {
          ...
          mysqli_data_seek($tila_chk_res, 0);

          $erat['tilaukset'][$tilausnumero] = $tilausnumero;

          while ($tila_chk_row = mysqli_fetch_assoc($tila_chk_res)) {
      ...
      foreach ($checkbox_child as $tilnro) {
        ...
        while ($sscc_chk_row = mysqli_fetch_assoc($sscc_chk_res)) {
          ...
          if ($old_row['liitostunnus'] != $new_row['liitostunnus']
            ...
            $toitarow_old = mysqli_fetch_assoc($toitares);
            ...
            $toitares = pupe_query($query);
            $toitarow_new = mysqli_fetch_assoc($toitares);
            ...
            $tmpviesti = "";
            while ($viesti_row = mysqli_fetch_assoc($viesti_res)) {
      ...
      $asluok_row = mysqli_fetch_assoc($asluok_res);
      ...
      while ($lahdot_row = mysqli_fetch_assoc($lahdot_res)) {
  ...
  if (isset($checkbox_parent) and ((is_array($checkbox_parent) and count($checkbox_parent) > 0) or is_string($checkbox_parent))) {
    ...
    if (isset($lahto_muokkaus_kellonaika) and trim($lahto_muokkaus_kellonaika) != ''
      ...
      foreach ($array_chk as $klo) {
        ...
        if (mb_strpos($klo, " ") !== FALSE or !is_numeric($klo_arr[0]) or !is_numeric($klo_arr[1]) or isset($klo_arr[2])) {
    ...
    if ($virhe == "" and isset($lahto_muokkaus_aktiivi)) {
      ...
      $lahto_row = mysqli_fetch_assoc($lahto_res);
  ...
  if (isset($checkbox_parent) and ((is_array($checkbox_parent) and count($checkbox_parent) > 0) or is_string($checkbox_parent))) {
    ...
    if (isset($jv) and (isset($rakirsyotto_laskutulostin) and isset($komento) and isset($valittu_rakiroslapp_tulostin))) {
      ...
      foreach ($checkbox_parent as $lahto) {
        ...
        if (mysqli_num_rows($chk_conflict_res) == 0) {
...
return return_data;
      ...
      $default_printer_row = mysqli_fetch_assoc($default_printer_res);
      ...
      while ($lahto_row = mysqli_fetch_assoc($lahdot_result)) {
        $lahdot_joissa_tilauksien_toimitustapa_rahtikirja_eritelty[] = array(
...
if ($select_varasto > 0) {
  ...
  if ($lasku_where != "") {
    $lasku_where = mb_substr($lasku_where, 0, -1).")";
  }
  ...
  while ($row = mysqli_fetch_assoc($result)) {
    ...
    $isatuote_chk_row = mysqli_fetch_assoc($isatuote_chk_res);
    ...
    $ohita_kerays_chk_row = mysqli_fetch_assoc($ohita_kerays_chk_res);
  ...
  if ($tee == '') {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
      ...
      if (mb_strpos($row['vakisin_kerays'], "X") !== false) {
      ...
      $rivit_row = mysqli_fetch_assoc($rivit_res);
      ...
      $kg_row = mysqli_fetch_assoc($kg_res);
  ...
  if ($tee == 'lahto' and trim($tilaukset) != '') {
    ...
    if (mysqli_num_rows($lahto_res) > 0) mysqli_data_seek($lahto_res, 0);

    while ($lahto_row = mysqli_fetch_assoc($lahto_res)) {
    ...
    while ($lahto_row = mysqli_fetch_assoc($lahto_res)) {
      ...
      $til_row = mysqli_fetch_assoc($til_res);
      ...
      if ($lahto_row['pakkausnro'] != '') {
        ...
        $pakkauskuvaus_row = mysqli_fetch_assoc($pakkauskuvaus_res);
        ...
        while ($pakkauskuvaus_row = mysqli_fetch_assoc($pakkauskuvaus_res)) {
      ...
      while ($rivi_row = mysqli_fetch_assoc($rivi_res)) {
...
function hae_suljetut_lahdot($toimitustavan_tunnukset, $ajax_request) {
  ...
  while ($suljetut_lahdot_row = mysqli_fetch_assoc($suljetut_lahdot_result)) {
...
function lahdot_joissa_tilauksia_keraamatta($lahdot) {
  ...
  if ($lahdot != "") {
    ...
    while ($lahto = mysqli_fetch_assoc($result)) {
      $keraamatta[] = $lahto;
    }
  }
