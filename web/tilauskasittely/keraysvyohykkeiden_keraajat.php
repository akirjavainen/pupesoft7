...
while ($row = mysqli_fetch_assoc($result)) {
  ...
  $kap_row = mysqli_fetch_assoc($kap_res);
  ...
  if (mysqli_num_rows($era_res)> 0) {
    ...
    while ($era_row = mysqli_fetch_assoc($era_res)) {
      ...
      $y = 1;
      $max_y = mysqli_num_rows($asiakas_res);

      while ($asiakas_row = mysqli_fetch_assoc($asiakas_res)) {
        echo "<tr class='asiakas_{$i}_{$x}' style='display:none;'>";

        echo "<td class='asiakas' id='asiakas_{$i}_{$x}_{$y}'>";

        if (mb_strpos($asiakas_row['tila'], 'K') !== false) {
          echo t("Aloitettu");
        }
        else {
          echo t("Kerätty");
...
while ($varastorow = mysqli_fetch_assoc($res)) {
...
while ($keraysvyohykerow = mysqli_fetch_assoc($res)) {
...
while ($toimitustaparow = mysqli_fetch_assoc($res)) {
...
while ($priorow = mysqli_fetch_assoc($res)) {
