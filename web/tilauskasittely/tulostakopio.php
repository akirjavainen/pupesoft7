...
if ((int)$laskunro <> 0 and (int)$laskunroloppu <> 0 and (int)$laskunro < (int)$laskunroloppu) {
  ...
  for ($las = (int)$laskunro; $las<=(int)$laskunroloppu; $las++) {
    ...
    if ($laskurow = mysqli_fetch_assoc($rrrresult)) {
...
if ($kukarow["extranet"] != "") {
  if ($kukarow["oletus_asiakas"] > 0 and ($toim == "LAHETE" or $toim == "KOONTILAHETE" or $toim == "LASKU" or $toim == "VIENTILASKU" or $toim == "TILAUSVAHVISTUS")) {
    ...
    if (mysqli_num_rows($vieres) == 1) {
      $asiakasrow = mysqli_fetch_assoc($vieres);
...
if ($tee == "ETSILASKU") {
  ...
  if ($toim == "VALMISTUS") {
    ...
    if (mb_strlen($ytunnus) > 0 and mb_substr($ytunnus, 0, 1) == '£') {
      $where2 .= $wherenimi;
    }
    elseif ($asiakasid > 0) {
...
if ($tee == "TULOSTA" or $tee == 'NAYTATILAUS') {
  ...
  if ($toim != "OSOITELAPPU" and $kappaleet > 0) {
    foreach ($tulostimet as $tulostin) {
      if ($komento[$tulostin] != 'email' and mb_substr($komento[$tulostin], 0, 12) != 'asiakasemail') {
  ...
  while ($laskurow = mysqli_fetch_assoc($rrrresult)) {
    ...
    $pos  = mb_strpos($komento_tilausvahvistus, "excel_lahete_geodis_wilson");
    $pos2 = mb_strpos($komento_tilausvahvistus, "excel_lahete_hinnoilla");
    $pos3 = mb_strpos($komento_tilausvahvistus, "excel_lahete_hinta_muillatiedoilla");
    ...
    $pos  = mb_strpos($komento_siirtolista, "excel_siirtolista");
    ...
    if ($toim == "TILAUSVAHVISTUS" or $toim == "YLLAPITOSOPIMUS") {
      ...
      if (isset($seltvtyyppi) and $seltvtyyppi != "") {
        ...
        if (mb_strpos($laskurow['tilausvahvistus'], 'JT') !== FALSE) $seltvtyyppi .= "JT";

        // Jos alkuperäisessä TV-ssä oli tuotperheet yhdessä optzione täpätty niin laitetaan se tännekin.
        if (mb_strpos($laskurow['tilausvahvistus'], 'Y') !== FALSE) $seltvtyyppi .= "Y";
    ...
    if ($toim == "VALMISTUS") {
      ...
      if (mysqli_num_rows($asiakas_apu_res) == 1) {
        $asiakas_apu_row = mysqli_fetch_array($asiakas_apu_res);
    ...
    if ($toim == "LAHETE" or $toim == "KOONTILAHETE" or $toim == "PAKKALISTA") {
      if ($toim == "KOONTILAHETE") {
        ...
        $laskurow_chk = mysqli_fetch_assoc($alk_til_res);
    ...
    if ($toim == "DGD") {
      ...
      $tunnukset = mysqli_fetch_assoc($rahti_result);
    ...
    if ($toim == "LAVAKERAYSTARRA" or $toim == "LAVATARRA" or $toim == "LAVAKERAYSLISTA") {
      ...
      $pakvel_row = mysqli_fetch_assoc($pakvel_result);
      ...
      if ($oslapp != '' or $tee == 'NAYTATILAUS') {
        ...
        elseif ($oslarow['osoitelappu'] == 'oslap_mg' and $yhtiorow['kerayserat'] == 'K') {
          ...
          $query  = "SELECT mediatyyppi
                     FROM kirjoittimet
                     WHERE yhtio = '$kukarow[yhtio]'
                     AND komento = '".sanitize_string($oslapp)."'
                     LIMIT 1";
