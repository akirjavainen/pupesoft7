...
if (isset($ajax_toiminto) and trim($ajax_toiminto) == 'tarkista_tehtaan_saldot') {
  ...
  if (!isset($data)) $data = array('id' => 0, 'error' => true, 'error_msg' => t("Haku ei onnistunut! Ole yhteydessa IT-tukeen"));
...
if ($ajax_request) {
  if ($hae_toimittajien_saldot) {
    ...
    $tuote_saldot = array();
    while ($tuote_saldo = mysqli_fetch_assoc($result)) {
...
if ($tee != "" and $tee != "MUUOTAOSTIKKOA") {
  ...
  $toimittajarow = mysqli_fetch_assoc($toimittajaresult);
  ...
  if ($tee == 'poista_kohdistamattomat') {
    ...
    while ($ostotilausrivi = mysqli_fetch_assoc($result)) {
  ...
  if ($tee == 'valmis' or $tee == 'valmis_ja_saavuta') {
    ...
    if (count($komento) == 0) {
      ...
      if ($laskurow["varasto"] != 0) {
        ...
        while ($ostotilausrivit = mysqli_fetch_assoc($result)) {
          ...
          if (mysqli_num_rows($tuopaires) != 0) {
            $tuopairow = mysqli_fetch_assoc($tuopaires);
          ...
          if ($kuuluu == 0) {
            ...
            $hyllyrow =  mysqli_fetch_assoc($hyllyres);
      ...
      if ($tee == 'valmis_ja_saavuta') {
        ...
        if (mysqli_num_rows($tarkistus_r) == 0) {
          ...
          $saaprow = mysqli_fetch_assoc($saapres);
  ...
  if ($tee == "lisaa_uudetostorivit_myyntiin") {
    ...
    while ($tilausrivi = mysqli_fetch_assoc($rivi_result)) {
      ...
      if (!$myyntirivi = mysqli_fetch_assoc($result)) {
        ...
        $laskurow = mysqli_fetch_assoc(pupe_query($query));
  ...
  if ($tee == 'PV') {
    ...
    if ($tapa == 'VAIHDARIVI') {
      ...
      if (!empty($tilausrivirow['tilausrivilinkki'])) {
        ...
        $result = pupe_query($query);
        $myyntitilausrow = mysqli_fetch_assoc($result);
      ...
      if ($toimi_tunnus == $laskurow['liitostunnus']) {
        ...
        if (mysqli_num_rows($myynnista_osto_avainsanat) > 0) {
          ...
          $param_fetch_row = mysqli_fetch_assoc($param_fetch_res);
          ...
          if ($kukarow['toimipaikka'] != 0) {
            ...
            if (mysqli_num_rows($toimpaikka_chk_res) > 0) {
              $toimpaikka_chk_row = mysqli_fetch_assoc($toimpaikka_chk_res);
        ...
        $oletus_tuotepaikka_row = mysqli_fetch_assoc($oletus_tuotepaikka_result);
    ...
    foreach ($tuoteno_array as $tuoteno) {
      ...
      if (!empty($lisatyt_rivit2)) {
        foreach ($lisatyt_rivit2 as $rivitunnus_temp) {
          ...
          $result = pupe_query($query);
          $tilausrivin_lisatiedot_row = mysqli_fetch_assoc($result);
  ...
  if ($tee == 'Y') {
    ...
    if ($toim != "HAAMU") {
      ...
      if (mysqli_num_rows($saapumisriveja_res) > 0) {
    ...
    $rivienmaara = mysqli_num_rows($presult);
        ...
        if ($prow["uusiotunnus"] == 0 or $naytetaankolukitut != "EI") {
          ...
          if ($tuoteperheita and ($prow["perheid"] == $prow["tunnus"] or ($prow["perheid2"] == $prow["tunnus"] and $prow["perheid"] == 0) or ($prow["perheid2"] == -1))) {
            ...
            if ($prow["perheid2"] == 0 or $prow["perheid2"] == -1) {
              ...
              if (mysqli_num_rows($lisaresult) > 0) {
                $lisays = mysqli_num_rows($lisaresult)+1;
              }
            }
          ...
          if ($prow["uusiotunnus"] == 0) {
            ...
            if ($varaosavirhe == "") {
              ...
              if (mysqli_num_rows($lisaresult) > 0 and $prow["perheid2"] == -1) {
                ...
                while ($xprow = mysqli_fetch_assoc($lisaresult)) {
                  ...
                  if ($lask == mysqli_num_rows($lisaresult)) {
              ...
              elseif (mysqli_num_rows($lisaresult) > 0 and $prow["perheid2"] != -1) {
                echo "  <td class='back'>
          ...
          $tlrow = mysqli_fetch_assoc($tlres);
          ...
          if (!empty($tlrow['tilausrivilinkki'])) {
            ...
            $linkattu_myyntitilaus_row = mysqli_fetch_assoc($linkattu_myyntitilaus_result);
    ...
    if ($rivienmaara > 0 and $laskurow["liitostunnus"] != '' and $tilausok == 0) {
      ...
      if ($sahkoinen_tilausliitanta and ($yhtiorow['vastaavat_tuotteet_esitysmuoto'] == 'S' or $yhtiorow['vastaavat_tuotteet_esitysmuoto'] == 'A')) {
        ...
        if (mysqli_num_rows(t_avainsana('SAHKTILTUN', '', " AND selite = '{$laskurow['vanhatunnus']}' AND selitetark = '{$laskurow['liitostunnus']}' ")) > 0) {
          $hae = 'nappi_kaikki';
          require "inc/sahkoinen_tilausliitanta.inc";
        }
      }
      ...
      if ($toim != "HAAMU") {
        ...
        if (mysqli_num_rows($saapumisriveja_res) == 0 and tarkista_oikeus("keikka.php")) {
         echo "  <td class='back'>
              <form method='post' action='{$palvelin2}tilauskasittely/tilaus_osto.php'>
              <input type='hidden' name='toim'          value = '$toim'>
              <input type='hidden' name='lopetus'        value = '$lopetus'>
