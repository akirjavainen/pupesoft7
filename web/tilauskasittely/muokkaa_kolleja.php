...
if ($tee == 'paivita') {
  ...
  foreach ($uusi_pakkaus as $sscc => $pak) {
    if ($pak != "") {
      if (mb_strpos($pak, '####') !== FALSE) $siirretaan_pakkausta = true;
...
if ($tee == 'paivita') {
  ...
  foreach ($uusi_pakkaus as $sscc => $pak) {
    ...
    if ($pak != "") {
      ...
      if (mb_strpos($pak, '####') !== FALSE) list($pak, $toisen_sscc) = explode('####', $pak);
      ...
      if ($pak != 999) {
        ...
        $pak_row = mysqli_fetch_assoc($pak_res);
      }
      else {
      ...
      if ($toisen_sscc != "") {
        ...
        $toisen_row = mysqli_fetch_assoc($toisen_res);
      }
      ...
      while ($check_row = mysqli_fetch_assoc($check_res)) {
        ...
        while ($rahtikirjat_row = mysqli_fetch_assoc($rahtikirjat_res)) {
      ...
      foreach ($otunnukset as $otun) {
        ...
        if ($toisen_sscc != "") {
          ...
          if (mysqli_num_rows($row_count_chk_res) > 1) {

            $row_count_chk_row = mysqli_fetch_assoc($row_count_chk_res);
...
if ($tee == '' and isset($checkbox_parent) and count($checkbox_parent) == 1) {
  ...
  if (mysqli_num_rows($rakir_res) == 0) {
    ...
    while ($rakir_row = mysqli_fetch_assoc($rakir_res)) {
      ...
      while ($rivi = mysqli_fetch_assoc($res)) {
        ...
        $summaukset_row = mysqli_fetch_assoc($summaukset_res);
