...
if ($ytunnus != '') {
  ...
  if (mysqli_num_rows($result) > 0) {
    $tunnusrow = mysqli_fetch_assoc($result);
...
if (isset($laskurow)) {
  ...
  if (mysqli_num_rows($result) > 0) {
    while ($keikkaselrow = mysqli_fetch_assoc($result)) {
      $selkeikka = '';
  ...
  for ($i = 0; $i < mysqli_num_fields($presult)-$miinus; $i++) {
    $apu = $i + 1;
    echo "<th align='left'><a href = '?ytunnus={$ytunnus}&otunnus={$otunnus}&toimittajaid={$toimittajaid}&ojarj={$apu}&toimipaikka=$toimipaikka'>", t(mysqli_field_name($presult, $i)), "</a></th>";
  }
  ...
  while ($prow = mysqli_fetch_array($presult)) {
    ...
    for ($i=0; $i < mysqli_num_fields($presult)-$miinus; $i++) {
      if (mysqli_field_name($presult, $i) == 'tuoteno') {
      ...
      elseif (mysqli_field_name($presult, $i) == 'toimaika') {
        echo "<td align='right'><input type='text' name='toimaikarivi[{$prow['tunnus']}]' value='{$prow[$i]}' size='10'></td>";
        $_toimaika = $prow[$i];
      }
      elseif (mysqli_field_name($presult, $i) == 'vahvistettu') {
        $chekkis = "";
