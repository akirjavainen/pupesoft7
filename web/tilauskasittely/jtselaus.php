...
if (mb_strpos($_SERVER['SCRIPT_NAME'], "jtselaus.php") !== FALSE) {
...
if ($yhtiorow["saldo_kasittely"] == 'U' and $toim != 'ENNAKKO' and mb_strpos($_SERVER['SCRIPT_NAME'], "jtselaus.php") === FALSE) {
  $jt_huomioi_pvm = "on";
}
...
if (
  ...
  or (
    (
      mb_strpos($_SERVER['SCRIPT_NAME'], "keikka.php") !== FALSE
      or mb_strpos($_SERVER['SCRIPT_NAME'], "verkkolasku-in.php") !== FALSE
      or mb_strpos($_SERVER['SCRIPT_NAME'], "vastaanota.php") !== FALSE
    )
    and (
...
if ($kukarow["extranet"] != "") {
  ...
  if (mysqli_num_rows($result) == 1) {
    $asiakas = mysqli_fetch_assoc($result);
...
if ($tilaus_on_jo != '') {
  ...
  if (mysqli_num_rows($result) == 1) {
    $tilaus_on_jo_row = mysqli_fetch_assoc($result);
...
if ($jtselaus_paivitys_oikeus and $kukarow["extranet"] == "" and $tilaus_on_jo == "" and $tee == 'TOIMITA') {
  if ($toim == "ENNAKKO") {
    ...
    while ($osatoimrow = mysqli_fetch_assoc($stresult)) {
      $ostok .= $osatoimrow["tilaus"].",";
    }

    $ostok = mb_substr($ostok, 0, -1);
    ...
    if (mysqli_num_rows($apure) == 0) {
      ...
      if ($laskurow['ohjelma_moduli'] == 'MAGENTOJT') {
        ...
        if ($_magento_kaytossa) {
          ...
          $toimitustapa_row = mysqli_fetch_assoc($toimitustapa_re);
    ...
    if ($sms_jt == "kaikki_toimitettu") {
      ...
      while ($row = mysqli_fetch_assoc($result)) {
        ...
        if ($smsnumero == "") {
          ...
          $result = pupe_query($query);
          $vanhalaskurow = mysqli_fetch_assoc($result);
...
if ($jtselaus_paivitys_oikeus and $kukarow["extranet"] == "" and $tilaus_on_jo == "" and $tee == 'MUOKKAARIVI') {
  ...
  if (is_array($varastosta)) {
    ...
    $tilausnumero = mb_substr($tilausnumero, 0, -2);
  }
...
if ($jtselaus_paivitys_oikeus and $kukarow["extranet"] == "" and $tilaus_on_jo == "" and $tee == 'LISAARIVI') {
  ...
  if ($kpl > 0) {
    ...
    if (mysqli_num_rows($tuoteresult) == 1) {
...
if ($tee == "JATKA") {
  ...
  if (isset($tuotenumero) and $tuotenumero != '') {
    ...
    if ($tuoterajaus != "") {
      $tilausrivilisa .= "and tilausrivi.tuoteno in (".mb_substr($tuoterajaus, 0, -1).") ";
    }
  }
  ...
  if (in_array($jarj, array("ytunnus", "tuoteno", "luontiaika", "toimaika"))) {
    if (isset($summarajaus) and $summarajaus != '') {
      ...
      if (mysqli_num_rows($summarajausres) > 0) {
        $summarajauslisa = " and lasku.liitostunnus in (";

        while ($summarajausrow = mysqli_fetch_assoc($summarajausres)) {
          $summarajauslisa .= "$summarajausrow[liitostunnus],";
        }

        $summarajauslisa = mb_substr($summarajauslisa, 0, -1);
        $summarajauslisa .= ")";
      }
      else {
  ...
  if (in_array($jarj, array("ytunnus", "tuoteno", "luontiaika", "toimaika")) and $summarajausfail == '') {
    ...
    if (mysqli_num_rows($isaresult) > 0) {
      ...
      while ($jtrow = mysqli_fetch_assoc($isaresult)) {
        ...
        if ($yhtiorow['tee_siirtolista_myyntitilaukselta'] == 'K' and $onko_suoratoimi == '') {
          ...
          if (mysqli_num_rows($siirtolista_result) > 0) {
            $onko_suoratoimi = "ON";
          }
        }
        ...
        if (($onko_suoratoimi == "" and $suoratoimit == "") or ($onko_suoratoimi == "ON" and $suoratoimit != "") or count($suoratoimitus_rivit) > 0) {
          ...
          if (isset($lapsires) and mysqli_num_rows($lapsires) > 0) {
            while ($perherow = mysqli_fetch_assoc($lapsires)) {
          ...
          if ($jtrow["ei_saldoa"] == "") {
            ...
            if (!empty($yhtiorow['jt_automatiikka']) and $yhtiorow['automaattinen_jt_toimitus'] == 'A') {
              ...
              while ($jt_muiden_mukana_row = mysqli_fetch_assoc($jt_muiden_mukana_result)) {
	      ...
	      echo "<br><br>vko " . date("W", strtotime($jtrow["ttoimaika"])); // MUOKKAUS: lisatty toimitusviikko
              ...
              if ($jtrow["valkoodi"] != '' and trim(mb_strtoupper($jtrow["valkoodi"])) != trim(mb_strtoupper($yhtiorow["valkoodi"]))) {
            ...
            if ($jtselaus_paivitys_oikeus) {
              ...
              $jurow    = mysqli_fetch_assoc($juresult);
	      ...
	      if (isset($kpl)) $kpl = array();
            ...
            if (isset($lapsires) and mysqli_num_rows($lapsires) > 0 and $automaaginen == '') {

              mysqli_data_seek($lapsires, 0);
              ...
              while ($perherow = mysqli_fetch_assoc($lapsires)) {
                ...
                if ($perherow["valkoodi"] != '' and trim(mb_strtoupper($perherow["valkoodi"])) != trim(mb_strtoupper($yhtiorow["valkoodi"]))) {
            ...
            if ($kukarow['extranet'] == '' and (!isset($php_cli) or !$php_cli) and $automaaginen == '') {
              ...
              if (mysqli_num_rows($korvaresult) > 0) {
                // tuote löytyi, joten haetaan sen id...
                $korvarow = mysqli_fetch_assoc($korvaresult);
                ...
                if (mysqli_num_rows($korva2result) > 0) {
                  while ($krow2row = mysqli_fetch_assoc($korva2result)) {
...
if ($tilaus_on_jo == "" and $from_varastoon_inc == "" and $tee == '') {
  ...
  if (!empty($kukarow['varasto'])) {
    $_kukarow_varasto = sanitize_string($kukarow['varasto']);
  ...
  while ($row = mysqli_fetch_assoc($result)) {
    ...
    echo "<option value = '".mb_strtoupper($row['koodi'])."' $sel>".t($row['nimi'])."</option>";
  }
