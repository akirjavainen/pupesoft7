...
if ($aja=='run') {
  ...
  $echoulos .= "<font class='message'>Käsitellään tuotteita (".mysqli_num_rows($res)." kpl)...<br>";

  // arvioidaan kestoa
  $arvio     = array();
  $timeparts = explode(" ", microtime());
  $alkuaika  = $timeparts[1].mb_substr($timeparts[0], 1);
  ...
  while ($row = mysqli_fetch_array($res)) {
    ...
    $korow  = mysqli_fetch_array($kores);
...
*/
    ...
    if (mysqli_num_rows($kores)>0) {
      ...
      while ($korow = mysqli_fetch_array($kores)) {
        if ($nexti == 1) {
          // vanha
