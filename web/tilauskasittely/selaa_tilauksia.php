...
echo "<font class='head'>".ucfirst(mb_strtolower($toim)).t("selaus").":<hr></font>";
...
if (mysqli_num_rows($result) > 0) {
  ...
  for ($i = 0; $i < mysqli_num_fields($result); $i++) {
    if (mysqli_field_name($result, $i) == "hankintakulut") {
  ...
  while ($row = mysqli_fetch_array($result)) {
    ...
    for ($i = 0; $i < mysqli_num_fields($result); $i++) {
      ...
      elseif (is_numeric($row[$i]) and (mysqli_field_type($result, $i) == 'real' or mysqli_field_type($result, $i) == 'int')) {
        echo "<td align='right'>$row[$i]</td>";
      }
      elseif (mysqli_field_name($result, $i) == "hankintakulut") {
    ...
    if (($toim == "MYYNTI" or $toim == "MYYNTI_KATE") and $tee == "paiva" and mysqli_num_rows($apuoikeures) > 0) {
      ...
      $tila_row = mysqli_fetch_array($aputilares);
  ...
  if ($arvo != 0 or $summa != 0) {
    ...
    echo "<tr>";
    echo "<th colspan='".(mysqli_num_fields($result)-$i)."'>".t("Yhteensä").": </th>";
