...
if ($tee == "VALMIS") {
  ...
  if (isset($row) and $row['kuka'] != $kukarow['kuka']) {
    ...
    if (mysqli_num_rows($result) == 1) {
      ...
      while ($srow1 = mysqli_fetch_assoc($jres)) {
      ...
      while ($srow = mysqli_fetch_assoc($jres)) {
        ...
        $ostorow = mysqli_fetch_assoc($monista);
        ...
        if ($ostorow["kpl"] > 1) {
          ...
          while ($ostokokorow = mysqli_fetch_assoc($monistares2)) {
            ...
            if ($maara > 0) {
              ...
              for ($i=0; $i < mysqli_num_fields($monistares2)-1; $i++) { // Ei monisteta tunnusta
                ...
                $kennimi = mysqli_field_name($monistares2, $i);
	      ...
	      // MUOKKAUS: mysqli_insert_id():
              $insres2 = pupe_query($kysely, $GLOBALS["link"]);
              ...
              if (mysqli_num_rows($tapares) == 1) {
                ...
                for ($i=0; $i < mysqli_num_fields($tapares)-1; $i++) { // Ei monisteta tunnusta

                  $kennimi = mysqli_field_name($tapares, $i);
        ...
        while ($lisarow = mysqli_fetch_assoc($kres)) {
          $ostohinta         = 0;
          ...
          if ($lisarow["sarjanumeroseuranta"] == "S") {
            ...
            $sarjares = pupe_query($query);
            $sarjarow2 = mysqli_fetch_assoc($sarjares);
            ...
            if ($lisarow["hyllyalue"] != $srow["hyllyalue"] or $lisarow["hyllynro"] != $srow["hyllynro"] or $lisarow["hyllyvali"] != $srow["hyllyvali"] or $lisarow["hyllytaso"] != $srow["hyllytaso"]) {
              ...
              if (mysqli_num_rows($sarjares) == 0) {
                ...
                $query = "SELECT *
                          ...
                          and tunnus    = '$minne'";
                $sarjares = pupe_query($query);
                $minnerow  = mysqli_fetch_assoc($sarjares);
              }
              else {
                $minnerow  = mysqli_fetch_assoc($sarjares);
              }
              ...
              $kehahin_row = mysqli_fetch_assoc($kehahin_result);
...
if ($tee == "") {
  ...
  if (mysqli_num_rows($result)!=0) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
      ...
      for ($i = 0; $i < mysqli_num_fields($result)-2; $i++) {
        echo "<td>".$row[mysqli_field_name($result, $i)]."</td>";
      }
