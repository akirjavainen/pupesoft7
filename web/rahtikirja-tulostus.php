...
if (mb_strpos($_SERVER['SCRIPT_NAME'], "rahtikirja-tulostus.php") !== FALSE) {
...
if ($tee == 'tulosta') {
  ...
  $toimitustapa = sanitize_string(trim($toimitustapa));
  $varasto = (int) $varasto;
  ...
  if (mysqli_num_rows($toitares) == 1) {
    ...
    if (mb_strpos($_SERVER['SCRIPT_NAME'], "rahtikirja-kopio.php") === FALSE and isset($tulosta_rahtikirjat_nappulatsukka) and preg_match("/rahtikirja_unifaun_(ps|uo)_siirto\.inc/", $toitarow["rahtikirja"])) {
...
if ($tee == 'tulosta' or $tee == 'close_with_printer') {
  ...
  if ($toitarow['tulostustapa'] == 'L' and $toitarow['uudet_pakkaustiedot'] == 'K' and $tultiin != 'koonti_eratulostus_pakkaustiedot' and mb_strpos($_SERVER['SCRIPT_NAME'], "rahtikirja-kopio.php") === FALSE and isset($tulosta_rahtikirjat_nappulatsukka)) {
  ...
  $avainrow = mysqli_fetch_assoc($avainres);
  ...
  elseif (mb_strpos($toitarow['rahtikirja'], 'pdf') === false) {
    $kirjoitin_tunnus = $print["printteri2"]; // Rahtikirja matriisi
  }
  else {
    $kirjoitin_tunnus = $print["printteri6"]; // Rahtikirja A4
  ...
  if ($valittu_rakiroslapp_tulostin != '') {
    ...
    if (mb_strpos($_SERVER['SCRIPT_NAME'], "rahtikirja-tulostus.php") !== FALSE or mb_strpos($_SERVER['SCRIPT_NAME'], "rahtikirja-kopio.php") !== FALSE) {
      $query  = "SELECT *
                 from kirjoittimet
...
if ($tee == 'close_with_printer') {
  ...
  while ($row = mysqli_fetch_assoc($rakir_res)) {
...
if ($tee == 'tulosta') {
  ...
  elseif (mb_strpos($_SERVER['SCRIPT_NAME'], "rahtikirja-kopio.php") === FALSE) {
    $query .= " and rahtikirjat.tulostettu  = '0000-00-00 00:00:00' ";
  }
  ...
  if (mysqli_num_rows($rakir_res) == 0) {
    if (mb_strpos($_SERVER['SCRIPT_NAME'], "rahtikirja-kopio.php") === FALSE) {
      // poistetaan lukko
      $query = "UNLOCK TABLES";
      $res   = pupe_query($query);
    ...
    while ($mysqli_rakir_row = mysqli_fetch_assoc($rakir_res)) {

      if ($kieli != "") {
        $kieli = trim(mb_strtoupper($kieli));
      }
      elseif (trim($mysqli_rakir_row["kieli"]) != "") {
        $kieli = trim(mb_strtoupper($mysqli_rakir_row["kieli"]));
      }
      elseif (trim($kukarow["kieli"]) != "") {
        $kieli = trim(mb_strtoupper($kukarow["kieli"]));
      }
      else {
        $kieli = trim(mb_strtoupper($yhtiorow["kieli"]));
      }
      ...
      if ($toitarow["tulostustapa"] == "L" or $toitarow["tulostustapa"] == "K") {
        ...
        $asiakaslisa = "and lasku.ytunnus      = '$mysqli_rakir_row[ytunnus]'
                and lasku.toim_maa      = '$mysqli_rakir_row[toim_maa]'
                and lasku.toim_nimi      = '$mysqli_rakir_row[toim_nimi]'
                and lasku.toim_nimitark    = '$mysqli_rakir_row[toim_nimitark]'
                and lasku.toim_osoite    = '$mysqli_rakir_row[toim_osoite]'
                and lasku.toim_ovttunnus  = '$mysqli_rakir_row[toim_ovttunnus]'
                and lasku.toim_postino    = '$mysqli_rakir_row[toim_postino]'
                and lasku.toim_postitp    = '$mysqli_rakir_row[toim_postitp]' ";
      }

      if ($mysqli_rakir_row['jv'] != '') {
        $jvehto = " having jv!='' ";
      }
      else {
        $jvehto = " having jv='' ";
      ...
      $query .= "  and rahtikirjat.toimitustapa  = '$toimitustapa'
            and rahtikirjat.tulostuspaikka  = '$varasto'
            $asiakaslisa
            and rahtikirjat.merahti      = '$mysqli_rakir_row[merahti]'
            and rahtikirjat.rahtisopimus  = '$mysqli_rakir_row[rahtisopimus]'
            $jvehto
            ORDER BY lasku.toim_nimi, lasku.toim_nimitark, lasku.toim_osoite, lasku.toim_postino, lasku.toim_postitp, lasku.toim_maa, rahtikirjat.merahti, rahtikirjat.rahtisopimus, lasku.tunnus";
      $res = pupe_query($query);
      ...
      if (mysqli_num_rows($res) > 0) {
        ...
        $mysqli_rakir_row["tunnukset_res"] = $res;

        while ($rivi = mysqli_fetch_assoc($res)) {
          // Kopiotulostuksen tulostetut otsikot
          $kopiotulostuksen_otsikot[$rivi["otunnus"]] = $rivi["otunnus"];

          if (mb_strpos($_SERVER['SCRIPT_NAME'], "rahtikirja-kopio.php") === FALSE and !$unifaun_era_vainkollitarra) {
    ...
    foreach ($rakir_rows as $rakir_row) {
      ...
      $rivi = mysqli_fetch_assoc($res);

      //vikat pilkut pois
      $otunnukset = mb_substr($otunnukset, 0, -1);
      ...
      if (mb_strpos($toitarow["rahtikirja"], "unifaun") !== FALSE) {
        $query = "SELECT min(rahtikirjanro) rahtikirjanro
                  FROM rahtikirjat
                  WHERE yhtio = '{$kukarow['yhtio']}'
                  AND tunnus  IN ({$tunnukset})";
        $rahtikirjanrores = pupe_query($query);
        $rahtikirjanrorow = mysqli_fetch_assoc($rahtikirjanrores);
      ...
      if ($_tulostustapa and $_paktiedot) {
        ...
        $koonti_era_pakkaustieto_tunnus_row = mysqli_fetch_assoc($koonti_era_pakkaustieto_tunnus_res);
      ...
      while ($pak = mysqli_fetch_assoc($pakka)) {
      ...
      if (mysqli_num_rows($vres) > 0) {
        while ($vak = mysqli_fetch_assoc($vres)) {
      ...
      if (mb_strpos($_SERVER['SCRIPT_NAME'], "rahtikirja-kopio.php") === FALSE and $rakir_row['jv'] != '') {
        $tee       = "TARKISTA";
        $laskutakaikki   = "KYLLA";
        $silent       = "KYLLA";
      ...
      elseif (mb_strpos($_SERVER['SCRIPT_NAME'], "rahtikirja-kopio.php") !== FALSE and $rakir_row['jv'] != '') {
        ...
        $laskurow = mysqli_fetch_array($lasres);
      ...
      $kaikki_lotsikot = mb_substr($kaikki_lotsikot, 0, -2);

      if (!isset($nayta_pdf)) echo "$rahinta $jvtext<br>";

      // Kopsutulostus toistaiseksi vain A4-paperille unifaun keississä
      if (mb_strpos($_SERVER['SCRIPT_NAME'], "rahtikirja-kopio.php") !== FALSE and preg_match("/rahtikirja_unifaun_(ps|uo|xp)_siirto\.inc/", $toitarow["rahtikirja"])) {
      ...
      if (!isset($tee_varsinainen_tulostus) or (isset($tee_varsinainen_tulostus) and $tee_varsinainen_tulostus)) {
        ...
        if (mb_strpos($_SERVER['SCRIPT_NAME'], "rahtikirja-kopio.php") === FALSE and (!isset($muutos) or $muutos != 'yes')) {
          $query = "UPDATE rahtikirjat
      ...
      $_desadv = (mb_strpos($rakir_row['toimitusvahvistus'], 'desadv') !== false);
      $_ra_kopio_check = (mb_strpos($_SERVER['SCRIPT_NAME'], "rahtikirja-kopio.php") !== false);
      ...
      if (mb_strpos($_SERVER['SCRIPT_NAME'], "rahtikirja-kopio.php") === FALSE and $rakir_row['jv'] == '' and !$unifaun_era_vainkollitarra) {
      ...
      // Tulostetaan DGD
      if ((mb_strpos($_SERVER['SCRIPT_NAME'], "rahtikirja-tulostus.php") !== FALSE or $tultiin == 'koonti_eratulostus_pakkaustiedot') and $rakirsyotto_dgd_tulostin != "" and $dgdkomento != '' and $dgdkpl > 0) {
      ...
      if (mb_strpos($_SERVER['SCRIPT_NAME'], "rahtikirja-tulostus.php") !== FALSE or mb_strpos($_SERVER['SCRIPT_NAME'], "rahtikirja-kopio.php") !== FALSE) {
        if ($valittu_rakiroslapp_tulostin != "" and $oslapp != '' and $kollityht > 0) {
          ...
          $viestirarrow = mysqli_fetch_assoc($viestirar);
      ...
      if (!isset($nayta_pdf) and mb_strpos($_SERVER['SCRIPT_NAME'], "rahtikirja-kopio.php") === FALSE) echo "<br>";
    }
    ...
    if ($toitarow['erittely'] == 't' and $kaikki_lotsikot_per_toimitus != "" and $toitarow['rahtikirja'] != 'rahtikirja_hrx_siirto.inc') {
      $kaikki_lotsikot_per_toimitus = mb_substr($kaikki_lotsikot_per_toimitus , 0 , -2); //poistetaan pilkku ja välilyönti viimosen perästä
...
if ($tee == '') {
  ...
  if (mysqli_num_rows($result) > 0) {
    ...
    $jvrow = mysqli_fetch_assoc($jvres);
    ...
    while ($kirrow = mysqli_fetch_assoc($kires)) {
    ...
    while ($asdf_row = mysqli_fetch_assoc($result)) {
