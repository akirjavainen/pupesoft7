...
if ($tee == "TEE" or $tee == "UPDATE") {
  ...
  // Eka rivi
  $otsikot = explode("\t", mb_strtoupper(trim(array_shift($sanakirja))));
    ...
    if (isset($sync_otsikot["fi"])) {
      ...
      foreach ($sanakirja as $rivi) {
        ...
        if ($rivi[$sync_otsikot["fi"]] != "") {
          ...
          if (mysqli_num_rows($sanakirjaresult) > 0) {
            $sanakirjarow = mysqli_fetch_assoc($sanakirjaresult);
            ...
            foreach ($kieliarray as $kieli) {
              ...
              if ($sanakirjarow[$kieli] != $rivi[$sync_otsikot[$kieli]]) {
                ...
                if (ctype_upper(mb_substr($sanakirjarow["fi"], 0, 1)) === TRUE) {
                  // Eka merkki iso kirjain
                  $sanakirjarow[$kieli] = ucfirst($sanakirjarow[$kieli]);
                }
                else {
                  // Muuten koko stringi pienillä
                  $sanakirjarow[$kieli] = mb_strtolower($sanakirjarow[$kieli]);
                }
      ...
      while ($sanakirjarow = mysqli_fetch_assoc($sanakirjaresult)) {
