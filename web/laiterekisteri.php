...
if (mb_strpos($_SERVER['SCRIPT_NAME'], "laiterekisteri.php") !== FALSE) {
...
if ($toiminto == "LINKKAA" or $vanhatoiminto == "LINKKAA") {
  ...
  while ($vanhalaiterivi = mysqli_fetch_assoc($res)) {
    echo "<tr>";
...
elseif ($toiminto == 'UUSILAITE') {
  ...
  if (isset($valitse_sarjanumero) and !empty($valitse_sarjanumero)) {
    $kveri = "SELECT
              ...
              LIMIT 1";
    $resu = pupe_query($kveri);
    $rivikka = mysqli_fetch_assoc($resu);
...
elseif ($toiminto == 'NAYTALAITTEET') {
  while ($rowi = mysqli_fetch_assoc($res)) {
    ...
    if (isset($rowi['sopimusrivin_tunnukset'])) {
      ...
      $ed_sop_tun = '';
      $lassurivi = '';
      while ($lelo = mysqli_fetch_assoc($ressi)) {
        ...
        if ($ed_sop_tun == '' or $ed_sop_tun != $lelo['sopimusnumero']) {
          ...
          $lassurivi = mysqli_fetch_assoc($ressukka);
      ...
      $sarjanumerorow = mysqli_fetch_assoc($sarjanumerores);
