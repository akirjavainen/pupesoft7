...
if ($korjataan != '') {
  ...
  for ($id = 0; $id < $countti; $id++) {
    ...
    if ($uusipaikka[$id] == 'uusi' and $hyllyalue[$id] != '') {
      ...
      $hyllyalue[$id] = mb_strtoupper($hyllyalue[$id]);
      ...
      if ($kuuluuko > 0) {
        ...
        if (mysqli_num_rows($oleresult) == 0) {
          $oletus = 'X';
        }
        else {
          $oletus = '';
        ...
        if (mysqli_num_rows($loytyykoresult) == 0) {
    ...
    if (mysqli_num_rows($result2) == 0) {
      ...
      if (mysqli_num_rows($nimresult) == 1) {
        $nimrow = mysqli_fetch_assoc($nimresult);
    ...
    elseif (mysqli_num_rows($result2) > 1) {
      if ($rivipaikka[$id]== '') {
        ...
        if (mysqli_num_rows($nimresult) == 1) {
          ...
          while ($varow = mysqli_fetch_assoc($result2)) {
            $sel='';
    ...
    elseif (mysqli_num_rows($result2) == 1) {
      $varow = mysqli_fetch_assoc($result2);
  ...
  while ($varselrow = mysqli_fetch_assoc($result)) {
    $sel = '';
