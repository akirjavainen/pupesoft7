...
if ($tee == 'POISTA' and $profiili != "") {
  ...
  $query = "DELETE
            ...
            AND lukittu  = ''";
  $result = pupe_query($query);
  $maara = mysqli_affected_rows($link);
  ...
  while ($krow = mysqli_fetch_assoc($kres)) {
    ...
    if (count($profiilit) > 0) {
      ...
      foreach ($profiilit as $prof) {
        ...
        if (mb_strtoupper($prof) == mb_strtoupper($profiili)) {
          $triggeri = "HAPPY";
        }
      }
        ...
        foreach ($profiilit as $prof) {
          ...
          while ($trow = mysqli_fetch_assoc($pres)) {
            ...
            if (mysqli_num_rows($tarkesult) == 0) {
              ...
              $tarkrow = mysqli_fetch_assoc($tarkesult);
...
if ($profiili != '') {
  ...
  if (mb_stripos($profiili, "EXTRANET") !== FALSE) {
  ...
  while ($orow=mysqli_fetch_assoc($result)) {
    ...
    if (mysqli_num_rows($or) != 0) {
      ...
      $oikeurow = mysqli_fetch_assoc($or);
    ...
    if ($orow["nimi"] == 'yllapito.php' and mb_strpos($orow["alanimi"], "!!!") !== FALSE) {
      list( , $aliassetti, ) = explode("!!!", $orow["alanimi"]);
      echo "  ($aliassetti)";
    }

    echo "</td>
        <td align='center'><input type='checkbox' class='A".mb_str_pad($lask, 6, 0, STR_PAD_LEFT)." shift' $checked   value='$orow[nimi]#$orow[alanimi]#$orow[sovellus]' name='valittu[$lask]'></td>
        <td align='center'><input type='checkbox' class='B".mb_str_pad($lask, 6, 0, STR_PAD_LEFT)." shift' $paivit    value='$orow[nimi]#$orow[alanimi]#$orow[sovellus]' name='paivitys[$lask]'></td>
        </tr>";
