...
if (mb_strpos($_SERVER['SCRIPT_NAME'], "extranet_tyomaaraykset.php") !== FALSE) {
...
if (isset($valmnro) and !empty($valmnro)) {
  ...
  if (mysqli_num_rows($result) == 1) {
    $laiterow = mysqli_fetch_assoc($result);
...
function hae_kayttajan_tyomaaraykset() {
  ...
  while ($row = mysqli_fetch_assoc($result)) {
    ...
    $historiarow = mysqli_fetch_assoc($historiaresult);
...
function hae_asiakasdata() {
  ...
  $result = pupe_query($query);
  $asiakasdata = mysqli_fetch_assoc($result);
  ...
  if (mysqli_num_rows($result2) == 1) {
    ...
    $jokurow = mysqli_fetch_assoc($result2);
