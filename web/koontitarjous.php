...
if ($submit and (trim($asiakasnimi) != '' or trim($asiakasytunnus) != '' or trim($asiakasnro) != '')) {
  ...
  if (isset($asiakasnimi) and trim($asiakasnimi) != '') {
    $wherelisa = " and asiakas.nimi like '%".sanitize_string(trim($asiakasnimi))."%' ";
  }

  if (isset($asiakasytunnus) and trim($asiakasytunnus) != '') {
    $wherelisa .= " and asiakas.ytunnus like '%".sanitize_string(trim($asiakasytunnus))."%' ";
  }
  ...
  if (mysqli_num_rows($asiakasres) > 1) {
    while ($asiakasrow = mysqli_fetch_assoc($asiakasres)) {
      echo "<form method='post'>";
  ...
  if (mysqli_num_rows($asiakasres) == 1) {
    ...
    if (mysqli_num_rows($tarjousres) > 0) {

      echo "<font class='message'>", t("Tarjouksia löytyi"), " ", mysqli_num_rows($tarjousres), " ", t("kappaletta"), "! ", t("Vain ensimmäiset 100 tarjousta näytetään"), ".</font><br /><br />";

      echo "<form method='post'>";
      echo "<table>";
      echo "<tr><th>", t("Tunnus"), "</th><th>", t("Summa"), "</th><th>", t("Luontiaika"), "</th><th>", t("Valitse"), "</th></tr>";

      while ($tarjousrow = mysqli_fetch_assoc($tarjousres)) {
