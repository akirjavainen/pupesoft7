...
if ($tee == '') {
  ...
  if ($kukarow['saatavat'] <= 1) {
    ...
    if (mysqli_num_rows($result) == 1) {
      $sytunnusrow = mysqli_fetch_assoc($result);
  ...
  if (mysqli_num_rows($result)) {
    ...
    while ($uutinen = mysqli_fetch_array($result)) {
      ...
      if (!empty($segtunnarit)) {
        ...
        while ($puun_tunnukset = mysqli_fetch_assoc($preres)) {
      ...
      if (count($matches) > 0) {
        ...
        foreach ($matches as $m) {
          ...
          if (mysqli_num_rows($tres) <> 1) {
            ...
            $asiakastemprow = mysqli_fetch_array($asiakastempres);
            ...
            if ($temp_laskurowwi['valkoodi'] != "" and $temp_laskurowwi['valkoodi'] != $yhtiorow["valkoodi"]) {
              ...
              if (mysqli_num_rows($hintaresult) > 0) {
                $hintarow = mysqli_fetch_array($hintaresult);
              }
              else {
