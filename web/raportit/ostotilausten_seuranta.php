...
if ($_GET['tee'] == 'hae_kommentti') {
  ...
  $comm_row = mysqli_fetch_assoc($comm_res);
...
if ($_GET['tee'] == 'kommentti') {
  if ($_GET['tilaus'] != '') {
    ...
    $kommentti = sanitize_string($_GET['kommentti']);
...
if ($tee == 'aja') {
  ...
  if ($toimittajaid != '') {
    $toimittajaid = sanitize_string($toimittajaid);
  ...
  while ($toimittajarow = mysqli_fetch_assoc($res)) {
    ...
    if (mysqli_num_rows($tilrivi_res) > 0) {
      ...
      while ($tilrivi_row = mysqli_fetch_assoc($tilrivi_res)) {
        ...
        $i = 1;
        $x = mysqli_num_rows($result) != 0 ? mysqli_num_rows($result) : 1;

        $rows = array();

        while ($keikkarow = mysqli_fetch_assoc($result)) {
          $rows[] = $keikkarow;
        }

        if ($tilrivi_laskuri == mysqli_num_rows($tilrivi_res)) {
          ...
          $xx += mysqli_num_rows($ei_tilriveja_res);

          while ($ei_tilriveja_row = mysqli_fetch_assoc($ei_tilriveja_res)) {
        ...
        echo "<td style='vertical-align: top;' rowspan='$x'>".date("W", strtotime($tilrivi_row['h1time']))."/".mb_substr($tilrivi_row['h1time'], 2, 2)."</td>";
        ...
        $yht_varastossa_kpl += mb_substr($varastossa_kpl, 0, -1);
        $yht_kpl += $tilrivi_row['kpl'];
        $yht_varastossa_rivit += mb_substr($varastossa_riveja, 0, -1);
        ...
        foreach ($rows as $keikkarow) {
          ...
          if (mysqli_num_rows($ostolaskures) > 0) {
            echo "<td style='vertical-align: top;' nowrap>";
            while ($ostolaskurow = mysqli_fetch_assoc($ostolaskures)) {
              echo tv1dateconv($ostolaskurow['luontiaika'])."<br>";
            }
            echo "</td>";

            mysqli_data_seek($ostolaskures, 0);
          ...
          $saldoille_row = mysqli_fetch_assoc($saldoille_res);
          ...
          // and (mysqli_num_rows($result) > 1 or count($rows) == end($rows))
          if ($i < ($x + $xx)) {
      ...
      while ($ei_liitetyt_row = mysqli_fetch_assoc($ei_liitetyt_res)) {
        ...
        if (mb_strtoupper($ei_liitetyt_row["valkoodi"]) != mb_strtoupper($yhtiorow["valkoodi"])) {
