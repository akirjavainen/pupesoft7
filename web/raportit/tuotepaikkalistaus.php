...
if ($tee == "TULOSTA") {
  ...
  if (mysqli_num_rows($tuotepaikka_result) == 0) {
  ...
  if ($tee == "TULOSTA") {
    if (mysqli_num_rows($tuotepaikka_result) > 0 ) {
      ...
      while ($tuotepaikka_row = mysqli_fetch_array($tuotepaikka_result)) {
        ...
        $prn .= "\t";
        $prn .= sprintf('%-21.21s',   mb_strtoupper($tuotepaikka_row["hyllyosoite"]));
        $prn .= "\n";
        fwrite($fh, $prn);
        $rivit++;
      }
