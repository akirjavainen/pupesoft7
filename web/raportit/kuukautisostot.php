...
if (isset($tee) and $tee == "lataa_tiedosto") {
  ...
  if ($tee == "RAPORTOI" and isset($RAPORTOI)) {
    ...
    if (count($tuoryh) > 0) {
      ...
      $srow = array();
      while ($sro = mysqli_fetch_assoc($sresult)) {
        $srow[] = $sro;
      }
    }
    if ($osasto != '') {
    ...
    while ($row = mysqli_fetch_assoc($res)) {
      ...
      for ($i = 1; $i < 13; $i++) {
        ...
        if ($i == 12) $selectlisa = mb_substr($selectlisa, 0, -1);
      }
      ...
      if ($valitut['EHDOTETTAVAT'] == '' or $ostettavahalytilausmaara > 0 or $ostettavahaly > 0 or $ostettava4kk > 0) {
        ...
        if (is_resource($korvaresult2) and mysqli_num_rows($korvaresult2) > 0) {
          ...
          foreach (explode(",", $vastaavat->getIDt()) as $ketju) {
            ...
            foreach ($_tuotteet as $_tuoteno_arr) {
              if ($_tuoteno_arr['tuoteno'] != $row['tuoteno']) {
                ...
                $ostohinta = mb_substr($ostohinta, 0, -3);
                $tuotteen_toimittajat_string = mb_substr($tuotteen_toimittajat_string, 0, -3);
...
function hae_tuotteen_toimittajat($tuoteno) {
  ...
  while ($tuotteen_toimittaja = mysqli_fetch_assoc($result)) {
