...
if (!$php_cli) {
  ...
  $varastorow = mysqli_fetch_array($varastores);
  ...
  if (!empty($argv[3])) {
    $pp = mb_substr($argv[3], 8, 2);
    $kk = mb_substr($argv[3], 5, 2);
    $vv = mb_substr($argv[3], 0, 4);
  }
...
if (isset($supertee) and $supertee == "RAPORTOI") {
  ...
  if (isset($tuotteet_lista) and $tuotteet_lista != '') {
    ...
    if ($tuoterajaus != "") $tuote_lisa .= "and tuote.tuoteno in (".mb_substr($tuoterajaus, 0, -1).") ";
  }
  ...
  if (isset($varatturajaus) and $varatturajaus != "") {
    ...
    while ($vararivi = mysqli_fetch_assoc($varares)) {
  ...
  if ($summaustaso == 'TRY' or $summaustaso == 'VTR') {
    ...
    while ($row = mysqli_fetch_assoc($try_result)) {
    ...
    while ($row = mysqli_fetch_assoc($osasto_result)) {
  ...
  if ($variaatiosummaus != "") {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
      $kaikkikoot[mb_strtoupper($row['kokonimi'])] = $row['koko'];
    }
  ...
  do {
    ...
    if (!$row = mysqli_fetch_assoc($result)) $do = FALSE;
    ...
    if ($row["sarjanumeroseuranta"] == "S" or $row["sarjanumeroseuranta"] == "G") {
      ...
      while ($vararvorow = mysqli_fetch_assoc($vararvores)) {
      ...
      $query = "SELECT sum(tuotepaikat.saldo) saldo,
                ...
                $summaus_lisa";
      $vararvores = pupe_query($query);
      $vararvorow = mysqli_fetch_assoc($vararvores);
    ...
    if (date("Y-m-d") != $vv."-".$kk."-".$pp) {
      ...
      if (mysqli_num_rows($muutosres) == 0) {
        $uusintapahtuma = "$vv-$kk-$pp 23:59:59";
      }
      else {
        $muutosrow = mysqli_fetch_assoc($muutosres);

        $uusintapahtuma = $muutosrow["laadittu"];

        mysqli_data_seek($muutosres, 0);
      }
      ...
      if (mysqli_num_rows($epares) > 0) {
        $eparow = mysqli_fetch_assoc($epares);
      ...
      if (mysqli_num_rows($muutosres) > 0) {
        while ($muutosrow = mysqli_fetch_assoc($muutosres)) {
    ...
    if ($variaatiosummaus != "") {
      if ($row['variaatio'] == $ed_variaatio and $row['vari'] == $ed_vari) {
        ...
        $variaatiosum_koot[mb_strtoupper($row['kokonimi'])] += $muutoskpl;
      ...
      elseif ($row['variaatio'] != $ed_variaatio or $row['vari'] != $ed_vari) {
        ...
        $variaatiosum_2_koot[mb_strtoupper($row['kokonimi'])]  = $muutoskpl;
    ...
    if ($ok == TRUE) {
      ...
      if ($variaatiosummaus != "") {
        ...
        if ($kiertoviilasku != "") {
          ...
          $xmyyres = pupe_query($query);
          $xmyypvmrow = mysqli_fetch_assoc($xmyyres);
          ...
          if (!empty($huomioi_varastosiirrot)) {
            ...
            $xsiirtopvmrow = mysqli_fetch_assoc($xsiirtores);
          }

          $vikamykupaiva = max($xmyypvmrow['laskutettuaika'], $xsiirtopvmrow['vihapvm']);

          if ($vikamykupaiva > 0) {
            $vikamykupaiva = mb_substr($vikamykupaiva, 0, 4)."-".mb_substr($vikamykupaiva, 4, 2)."-".mb_substr($vikamykupaiva, 6, 2);
          }
          else {
            $vikamykupaiva = "";
          }
      ...
      if (isset($valitut_varastot_rajaus) and $valitut_varastot_rajaus != "") {
        ...
        $resultti_a = mysqli_fetch_assoc($result_a);
