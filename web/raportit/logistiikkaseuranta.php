...
if ($tee == "rahtisopparitilanne") {
  ...
  while ($asiakasrow = mysqli_fetch_assoc($asiakas_res)) {
    ...
    $toimtaparow = mysqli_fetch_assoc($toimtapa_res);
    ...
    if ($toimtaparow["nouto"] == "" and $toimtaparow["rahtikirja"] != "rahtikirja_tyhja.inc") {
      ...
      if (mb_strlen($rahsoprow['rahtisopimus']) > 0) {
        if (($toimtaparow['virallinen_selite'] == "KKSTD" or
            $toimtaparow['virallinen_selite'] == "IT09" or
            $toimtaparow['virallinen_selite'] == "IT14") and mb_strlen($rahsoprow['rahtisopimus']) < 6) {
          $pitvirh = TRUE;
        }
        elseif ($toimtaparow['virallinen_selite'] == "KLGRP" and mb_strlen($rahsoprow['rahtisopimus']) < 4) {
          $pitvirh = TRUE;
        }
        elseif ($toimtaparow['virallinen_selite'] == "MH10" and mb_strlen($rahsoprow['rahtisopimus']) < 8) {
          $pitvirh = TRUE;
        }
      }
...
if ($tee == "nayta") {
  ...
  $tilaus = sanitize_string($tilaus);
  $lasku = sanitize_string($lasku);
  $paiva = sanitize_string($paiva);
  ...
  if ($tilaus > 0) {
    ...
    if (mysqli_num_rows($lasku_res) > 0) {
      $laskurow = mysqli_fetch_assoc($lasku_res);
  ...
  while ($laskurow = mysqli_fetch_assoc($lasku_res)) {
    ...
    while ($tilausrow = mysqli_fetch_assoc($tilaus_res)) {
    ...
    $tilaukset = mb_substr($tilaukset, 0, -1);
    ...
    while ($tilausrivirow = mysqli_fetch_assoc($tilausrivi_res)) {
      ...
      $kerayserarow = mysqli_fetch_assoc($keraysera_res);
      ...
      if ($tilausrivirow['perheid'] > 0 and $tilausrivirow['tunnus'] != $tilausrivirow['perheid']) {
        ...
        $ohita_chk_row = mysqli_fetch_assoc($ohita_chk_res);
      ...
      $rivi .= "<td>".mb_substr($tilausrivirow["nimitys"], 0, 20)."</td>";
    ...
    while ($rakirrow = mysqli_fetch_assoc($rakir_res)) {
      $rivi .= "<tr>";
    ...
    */
    ...
    while ($rakurow = mysqli_fetch_assoc($raku_res)) {
      $rivi .= "<tr>";
    ...
    foreach ($yhdista as $otsikot) {
      ...
      $ramarow = mysqli_fetch_assoc($otsre);
      ...
      if ($rahtihinta != 0 and $virhe == 0 and mysqli_num_rows($rhire) == 1) {
