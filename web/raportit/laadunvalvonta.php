...
if ($submit) {
  ...
  if (isset($mul_try) and count($mul_try) > 0) {
    ...
    if (!in_array("PUPEKAIKKIMUUT", $mul_try)) {
      ...
      if (count($mul_try) == 1 and $mul_try[0] == '') {
        ...
        $tuotelisa = " and tuote.try in (".mb_substr($tuotelisa, 0, -1).") ";
      }
    }
  }
  ...
  if (isset($tuoteryhmittain) and trim($tuoteryhmittain) != '') {
    while ($toimaikarow = mysqli_fetch_array($toimaikares)) {
    ...
    mysqli_data_seek($toimaikares, 0);
...
if (isset($tuoteryhmittain) and trim($tuoteryhmittain) != '') {
  ...
  foreach ($tuoteryh as $key) {
    ...
    $seliterow = mysqli_fetch_array($seliteres);
