...
if ($toiminto == 'TULOSTA' and $kukarow["kuka"] == "admin") {
  ...
  foreach ($paikat as $tuote => $kpl) {
    ...
    if ($tunnus == 0) {
      ...
      $ekavarrow = mysqli_fetch_array($ekavarres);
  ...
  foreach ($saldot as $tuote => $kpl) {
    ...
    if ($tunnus != 0) {
      ...
      if (mysqli_num_rows($alkuresult) == 0) {
      ...
      elseif (mysqli_num_rows($alkuresult) == 1) {
...
function hae_tuotteet() {
  ...
  while ($sarjarow = mysqli_fetch_array($sarjares)) {
    ...
    if ($sarjarow["osto_perheid2"] > 0) {
      ...
      while ($tilrivirow2 = mysqli_fetch_array($tilrivires)) {
    ...
    // pitää uppercaseta!
    $sarjarow["tuoteno"]   = mb_strtoupper($sarjarow["tuoteno"]);
    $sarjarow["hyllyalue"] = mb_strtoupper($sarjarow["hyllyalue"]);
    $sarjarow["hyllynro"]  = mb_strtoupper($sarjarow["hyllynro"]);
    $sarjarow["hyllyvali"] = mb_strtoupper($sarjarow["hyllyvali"]);
    $sarjarow["hyllytaso"] = mb_strtoupper($sarjarow["hyllytaso"]);
