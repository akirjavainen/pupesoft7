...
if (isset($tee) and $tee == "lataa_tiedosto") {
  ...
  // errorcheckejä
  if (!checkdate(mb_substr($alkupvm, 5, 2), mb_substr($alkupvm, 8, 2), mb_substr($alkupvm, 0, 4))) {
    echo "<font class='error'>".t("Virheellinen päivämäärä")." $alkupvm!</font><br><br>";
    unset($subnappi);
  }

  if (!checkdate(mb_substr($loppupvm, 5, 2), mb_substr($loppupvm, 8, 2), mb_substr($loppupvm, 0, 4))) {
    echo "<font class='error'>".t("Virheellinen päivämäärä")." $loppupvm!</font><br><br>";
    unset($subnappi);
  }

  if (!checkdate(mb_substr($taysraja, 5, 2), mb_substr($taysraja, 8, 2), mb_substr($taysraja, 0, 4))) {
  ...
  if (isset($subnappi) and $subnappi != '') {
    ...
    echo "<br><font class='message'>".t("Löytyi")." ".mysqli_num_rows($result)." ".t("sopivaa tuotetta. Lasketaan ehdotus.")."</font><br>";
    ...
    while ($myyjarow = mysqli_fetch_assoc($myyjares)) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
      ...
      $taprow = mysqli_fetch_assoc($tapres);
      ...
      if (($saapunut < $alaraja) and ($epaku1pv < $epa2raja or $tyyppi == '25')) {
        ...
        $toimittajarow = mysqli_fetch_assoc($toimittajares);
        ...
        $myyrow = mysqli_fetch_assoc($myyres);
        ...
        $kulrow = mysqli_fetch_assoc($kulres);
        ...
        $ennrow = mysqli_fetch_assoc($ennres);
        ...
        if ($yhteensopivuus_table_check) {
          ...
          $yhteensopivuus_row = mysqli_fetch_assoc($yhteensopivuus_res);
        }
