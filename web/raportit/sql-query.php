...
if ($tee == "") {
  ...
  if (mysqli_num_rows($muisti_result) > 0) {
    $valittu_query = isset($valittu_query) ? $valittu_query : "";

    if ($toim == "READ") {
      echo "<table>";

      while ($muisti_row = mysqli_fetch_assoc($muisti_result)) {
  ...
  $sqlhaku = stripslashes(mb_strtolower(trim($sqlhaku)));

  // laitetaan aina kuudes merkki spaceks.. safetymeasure ni ei voi olla ku select
  if (!empty($sqlhaku) and mb_substr($sqlhaku, 6, 1) != " ") {
    $sqlhaku = mb_substr($sqlhaku, 0, 6)." ".mb_substr($sqlhaku, 6);
  }
  ...
  if (!empty($sqlhaku) and mb_substr($sqlhaku, 0, mb_strpos($sqlhaku, " ")) != 'select') {
  ...
  if ($sqlhaku != '' and isset($suoritanappi)) {
    ...
    if (mysqli_num_rows($result) > 0) {
      ...
      $excelrivi   = 0;
      $sarakemaara = mysqli_num_fields($result);

      for ($i=0; $i < $sarakemaara; $i++) $worksheet->write($excelrivi, $i, ucfirst(t(mysqli_field_name($result, $i))), $format_bold);
      $excelrivi++;

      $bar = new ProgressBar();
      $bar->initialize(mysqli_num_rows($result));
      ...
      echo "<font class='message'>".t("Haun tulos")." ".mysqli_num_rows($result)." ".t("riviä").".</font><br>";
