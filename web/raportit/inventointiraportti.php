  ...
  if ($tee == 'go') {
    ...
    if ($query != "") {
      ...
      for ($i=0; $i < $numfields; $i++) {
        if ($elements <= $rivilimitti) echo "<th>".t(mysqli_field_name($result, $i))."</th>";
      }

      if (isset($worksheet)) {
        for ($i=0; $i < $numfields; $i++) $worksheet->write($excelrivi, $i, ucfirst(t(mysqli_field_name($result, $i))), $format_bold);
        $excelrivi++;
      }
      ...
      while ($row = mysqli_fetch_assoc($result)) {
        ...
        for ($i=0; $i < $numfields; $i++) {
          ...
          if ($gluku > 1 and $edluku != $row[mysqli_field_name($result, 0)] and $edluku != 'x' and mb_strpos($group, ',') !== FALSE ) {
          ...
          $edluku = $row[mysqli_field_name($result, 0)];

          if (is_numeric($row[$fieldname]) and (mysqli_field_type($result, $i) == 'real' or mysqli_field_type($result, $i) == 'int' or mb_substr($fieldname, 0 , 4) == 'kate')) {
...
if ($lopetus == "") {
  ...
  while ($sxrow = mysqli_fetch_array($sresult)) {
    $mul_check = '';
