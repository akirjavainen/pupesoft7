...
if ($tee == 'aja') {
  ...
  while ($row = mysqli_fetch_assoc($result)) {
    ...
    echo "<td>";
    echo "<a href='' onclick=\"window.open('{$palvelin2}raportit/toimitetut_rivit.php?tee=NAYTATILAUS&tunnus=$row[tunnus]', '_blank' ,'toolbar=0,scrollbars=1,location=0,statusbar=0,menubar=0,resizable=1,left=200,top=100,width=1000,height=600'); return false;\">".t("Näytä tilaus")."</a>";
    echo "</td>";
    echo "</tr>";

    $worksheet->writeString($excelrivi, 0, $row['tunnus']);
    $worksheet->writeString($excelrivi, 1, $row['laskunro']);
    $worksheet->writeDate($excelrivi, 2, mb_substr($row['luontiaika'], 0, 10));
    $worksheet->writeDate($excelrivi, 3, mb_substr($row['toimitettuaika'], 0, 10));
    $worksheet->writeDate($excelrivi, 4, mb_substr($row['tapvm'], 0, 10));
  ...
  echo "<tr class='spec'>";
  echo "<td colspan='5'>".t("Yhteensä").":</td>";
