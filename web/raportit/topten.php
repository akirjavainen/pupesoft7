...
if (mb_strpos($_SERVER['SCRIPT_NAME'], "topten.php") !== false) {
...
if (empty($aja_raportti)) {
  ...
  // Haetaan alku ja loppupäivämäärät valinnan mukaan
...
echo "<option value='tvv' $tvv_sel>".t("Tällä viikolla")."</option>";
echo "<option value='tkk' $tkk_sel>".t("Tässä kuussa")."</option>";
echo "<option value='tvvv' $tvvv_sel>".t("Tänä vuonna")."</option>";
echo "</select>";
echo "</td></tr>";
echo "<tr>
...
function hae_data($tyyppi, $limitit, $rajaukset, $asumvalinta) {
  ...
  if (!empty($rajaukset['tuotteet']) and count($rajaukset['tuotteet']) > 0) {
    ...
    $tuoterajaus = mb_substr($tuoterajaus, 0, -1);
    $tuoterajaus .= ") ";
  }
  ...
  if (!empty($rajaukset['asiakasryhmat']) and count($rajaukset['asiakasryhmat']) > 0) {
    ...
    $ryhmarajaus = mb_substr($ryhmarajaus, 0, -1);
    $ryhmarajaus .= ") ";
  }
  ...
  if (!empty($rajaukset['asiakasmyyjat']) and count($rajaukset['asiakasmyyjat']) > 0) {
    ...
    $myyjarajaus = mb_substr($myyjarajaus, 0, -1);
    $myyjarajaus .= ") ";
  }
  ...
  if (!empty($rajaukset['tuotemerkit']) and count($rajaukset['tuotemerkit']) > 0) {
    ...
    $tuotemerkkirajaus = mb_substr($tuotemerkkirajaus, 0, -1);
    $tuotemerkkirajaus .= ") ";
  }
  ...
  case "asiakasryhmat":
    ...
    break;
  case "asiakasmyyjat":
    $haettu_data['otsikko'] = t('Asiakasmyyjät');
