...
function alvlaskelma($kk, $vv) {
  ...
  function laskeveroja($taso, $tulos) {
    ...
    if ($tulos == '22' or $tulos == 'veronmaara' or $tulos == 'summa') {
      ...
      $tilirow = mysqli_fetch_array($tilires);
      ...
      if ($tilirow['tilit300'] != '' or $tilirow['tilitMUU'] != '') {
        ...
        while ($verorow = mysqli_fetch_array($verores)) {
  ...
  if (isset($kk)) {
    ...
    ******************************************/
    ...
    echo "<tr><th>Ilmoitettava kausi</th><th>".mb_substr($startmonth, 0, 4)."/".mb_substr($startmonth, 5, 2)."</th></tr>";
    ...
    $query = "SELECT sum(tiliointi.summa) vero
              ...
              AND tiliointi.tapvm    <= '$endmonth'";
    $verores = pupe_query($query);
    $verorow = mysqli_fetch_array($verores);
    ...
    if (mb_strpos($_SERVER['SCRIPT_NAME'], "viranomaisilmoitukset.php") !== FALSE) {
      ...
      $ilmoituskausi = mb_substr($startmonth, 0, 4).mb_substr($startmonth, 5, 2);
      $file  = "000:VSRALVKK\n";
      $file .= "100:".date("dmY")."\n";
      $file .= "105:E03\n";
      $file .= "010:$uytunnus\n";
