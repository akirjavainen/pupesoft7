...
if ($tee == 'NAYTA' or $eiliittymaa == 'ON') {
  ...
  if ($eiliittymaa != 'ON' and isset($lisa) and mb_strpos($lisa, "tiliointi.kustp") !== FALSE) {
    $tiliointilisa = $lisa;
  }
  else {
    $tiliointilisa = '';
  ...
  if (mysqli_num_rows($result) > 0) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
      ...
      if ($ylilimiitin == '' or ($ylilimiitin == 'ON' and $luottoraja > 0 and $luottotilanne_nyt < 0)) {
        ...
        if ($grouppaus != "kustannuspaikka") {
          ...
          if (mb_substr_count($row['nimi'], '<br>') > 5) {
        ...
        if ($grouppaus == "kustannuspaikka" or $tiliointilisa != "") {
          ...
          if (mysqli_num_rows($nimiresult) == 1) {
            $nimirow = mysqli_fetch_assoc($nimiresult);

            $kustpmuuttuja = (mb_strpos($nimirow["nimi"], $nimirow["koodi"]) === FALSE) ? $nimirow["nimi"]." ".$nimirow["koodi"] : $nimirow["nimi"];
          }
          else {
	...
	// MUOKKAUS: BUGIKORJAUS (string + string):
        if (!isset($saatavat_yhteensa["yli_{$saatavat_array[count($saatavat_array)-1]}"])) $saatavat_yhteensa["yli_{$saatavat_array[count($saatavat_array)-1]}"] = (float)$row["yli_{$saatavat_array[count($saatavat_array)-1]}"];
        else $saatavat_yhteensa["yli_{$saatavat_array[count($saatavat_array)-1]}"] += (float)$row["yli_{$saatavat_array[count($saatavat_array)-1]}"];
    ...
    if ($sytunnus != '') {
      ...
      $tratat = mysqli_fetch_assoc($trattares);
