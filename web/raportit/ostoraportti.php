...
if (isset($RAPORTOI) and $tee == "RAPORTOI") {
  ...
  if ($mul_tme != '' and count($mul_tme) > 0) {
    ...
    $tme = mb_substr($tme, 0, -1);

    $sresult = t_avainsana("TUOTEMERKKI", "", "and avainsana.selite  in ($tme)");

    while ($tmerow = mysqli_fetch_array($sresult)) {
      $tme2 .= "{$tmerow['selite']}<br>";
    }

    $tme2 = mb_substr($tme2, 0, -4);
  }
  ...
  if (mysqli_num_rows($presult) > 0) {
    ...
    $konsyhtiot = mb_substr($konsyhtiot, 0, -1);
  ...
  $varastot2       = mb_substr($varastot2, 0, -1);
  $varastot_yhtiot = mb_substr($varastot_yhtiot, 0, -1);
  ...
  if (table_exists("yhteensopivuus_rekisteri")) {
    ...
    if (table_exists("yhteensopivuus_mp")) {
      ...
      if (mysqli_num_rows($res_rekyht) > 0) {
        $tyyppi = 'mp';

        while ($tyyppi_row = mysqli_fetch_assoc($res_rekyht)) {
    ...
    if ($tyyppi != '') {
      ...
      $tyyppilisa = mb_substr($tyyppilisa, 0, -1);
  ...
  echo t("Tuotteita")." ".mysqli_num_rows($res)." ".t("kpl").".<br>";
  ...
  $elements = mysqli_num_rows($res); // total number of elements to process
  ...
  while ($row = mysqli_fetch_array($res)) {
    ...
    $query = "SELECT
              ...
              $paikkalisa";
    $result   = pupe_query($query);
    $puuterow = mysqli_fetch_array($result);
    ...
    $query = "SELECT
              ...
              $paikkalisa";
    $result   = pupe_query($query);
    $kulutrow = mysqli_fetch_array($result);
    ...
    $query = "SELECT
              ...
              and (varattu+jt > 0)";
    $result = pupe_query($query);
    $ennp   = mysqli_fetch_assoc($result);
    ...
    $query = "SELECT sum(if(tyyppi = 'G', jt $lisavarattu, 0)) siirtojt
              ...
              and varattu + jt > 0";
    $result = pupe_query($query);
    $siirtojtrow = mysqli_fetch_array($result);
    ...
    if ($paikoittain == '' and $varastot_yhtiot != '') {
      ...
      while ($saldo = mysqli_fetch_array($result)) {
      ...
      if ($varastot2 != '') {
        ...
        while ($saldo2 = mysqli_fetch_array($result2)) {
    ...
    $result = pupe_query($query);
    $osaldo = mysqli_fetch_array($result);
    ...
    $ennarow = mysqli_fetch_array($ennaresult);
    ...
    if ($asiakasosasto != '') {
      ...
      $asosrow = mysqli_fetch_array($asosresult);
    }
    ...
    if ($asiakasid != '') {
      ...
      $asrow = mysqli_fetch_array($asresult);
    }
      ...
      foreach ($valitut as $key => $sarake) {
        ...
        if (isset($sarake_keyt[$sarake]) and $sarake_keyt[$sarake] != '') {
          ...
          elseif ($sarake == 'Määrä') {
            ...
            if (mysqli_num_rows($korvaresult1) > 0) {
              $korvarow = mysqli_fetch_array($korvaresult1);
              ...
              if (mysqli_num_rows($korvaresult2) > 0) {

                $i = 0;

                //tulostetaan korvaavat
                while ($korvarow = mysqli_fetch_array($korvaresult2)) {
                  $korvaavat_tunrot .= "'$korvarow[tuoteno]', ";
                }
              }
            }

            $korvaavat_tunrot = mb_substr($korvaavat_tunrot, 0, -2);
            ...
            if ($tyyppilisa != "") {
              ...
              $asresult = pupe_query($query);
              $kasrow = mysqli_fetch_array($asresult);
            ...
            if (mysqli_num_rows($korvaresult1) > 0) {
              ...
              if (mysqli_num_rows($korvaresult2) > 0) {
                ...
                while ($korvarow = mysqli_fetch_array($korvaresult2)) {
                  ...
                  $korvasaldorow = mysqli_fetch_array($korvasaldoresult);
...
if ($tee == "JATKA" or $tee == "RAPORTOI") {
  ...
  if (is_array($mul_try) and count($mul_try) > 0) {
    ...
    foreach ($mul_try as $tr) {
      ...
      $row_tr = mysqli_fetch_assoc($res);
      $try .= "$tr {$row_tr['selitetark']}<br>";
    }

    $try = mb_substr($try, 0, -4);
  }
  ...
  if (is_array($mul_osasto) and count($mul_osasto) > 0) {
    ...
    foreach ($mul_osasto as $os) {
      ...
      $row_os = mysqli_fetch_assoc($res);
      $osasto .= "$os {$row_os['selitetark']}<br>";
    }

    $osasto = mb_substr($osasto, 0, -4);
  }
  if (is_array($mul_tme) and count($mul_tme) > 0) {
    ...
    $tuotemerkki = mb_substr($tuotemerkki, 0, -4);
  }
  if ($toimittajaid != '') {
    $query = "SELECT nimi
              FROM toimi
  ...
  if (mysqli_num_rows($presult) > 0) {
    while ($prow = mysqli_fetch_array($presult)) {
      ...
      if ($vlask == 0) {
        echo "<tr><th rowspan='".mysqli_num_rows($presult)."'>Huomioi yhtiön myynnit:</th>";
      }
      else {
        echo "<tr>";
      }
  ...
  echo "<tr>";
  echo "<th rowspan='".(mysqli_num_rows($vtresult)+1)."'>".t("Varastot").":</th>";
  ...
  while ($vrow = mysqli_fetch_array($vtresult)) {
    ...
    echo "<tr>";
    echo "<td><input type='checkbox' class='A".mb_str_pad($cboxlask, 6, 0, STR_PAD_LEFT)." shift' name='valitut[VARASTO##$vrow[tunnus]]' value='VARASTO##$vrow[tunnus]'$chk> $vrow[nimitys] ($vrow[yhtio])</td>";
    echo "<td><input type='checkbox' class='B".mb_str_pad($cboxlask, 6, 0, STR_PAD_LEFT)." shift' name='valitut[VARASTO2##$vrow[tunnus]]' value='VARASTO2##$vrow[tunnus]'$chk2> $vrow[nimitys] ($vrow[yhtio])</td>";
    echo "</tr>";
  ...
  foreach ($sarakkeet as $key => $sarake) {
    ...
    echo "<td><input type='checkbox' class='C".mb_str_pad($lask, 6, 0, STR_PAD_LEFT)." shift' name='valitut[$key]' value='".trim($sarake)."' $sel> ".ucfirst($sarake)."</td>";
    $lask++;
  }
