...
for ($i = 1; $i <= 12; $i++) {
  $sel = $i == $kk ? "selected" : "";
  echo "<option value='{$i}' {$sel}>", mb_str_pad($i, 2, floor($i / 10), STR_PAD_LEFT), "</option>";
}
echo "</select>";
...
if ($tee == 'laskelma') {
  ...
  $_csv = array(
    'header' => array(
      ...
      'year' => $vv,
      'month' => mb_str_pad($kk, 2, floor($kk / 10), STR_PAD_LEFT),
      'declarationType' => 1
    ),
  );
  ...
  while ($row = mysqli_fetch_assoc($result)) {
    ...
    if (!empty($laskurow['tunnus']) and $laskelma == 'a') {
      ...
      $_tilsum_row = mysqli_fetch_assoc($_tilsum_res);
    ...
    if ($laskelma == 'a') {
      ...
      if (mysqli_num_rows($asiakasres) != 0) $aineistoon = $_red;
    }

    if (mysqli_num_rows($laskures) == 0 or $laskurow['laskunro'] == 0) $aineistoon = $_red;
