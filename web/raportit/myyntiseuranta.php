...
if (mb_strpos($_SERVER['SCRIPT_NAME'], "myyntiseuranta.php") !== FALSE) {
...
if ($lopetus == "") {
  ...
  if (mysqli_num_rows($result) > 0 and $oikeurow['paivitys'] != "") {
    echo "<table>";
    echo "<tr>";
  ...
  if (mysqli_num_rows($cresult) > 0) {
    ...
    while ($krow = mysqli_fetch_assoc($cresult)) {
      $sel = '';
      if (mb_strtoupper($campaign_id) == mb_strtoupper($krow["id"])) {
        $sel = "selected";
      }
...
if ((isset($aja_raportti) or isset($valitse_asiakas)) and count($_REQUEST) > 0) {
  if (!function_exists("vararvo")) {
    function vararvo($tuoteno, $vv, $kk, $pp) {
      ...
      $xkulrow = mysqli_fetch_assoc($xkulres);
  ...
  if ($tee == 'go') {
    ...
    foreach ($apu as $i => $mukaan) {
      ...
      if ($mukaan == "asiakasosasto") {
        ...
        if (mb_strpos($select, "'asiakaslista',") === FALSE) $select .= "group_concat(DISTINCT asiakas.tunnus) 'asiakaslista', ";
      ...
      if ($mukaan == "ytunnus") {
        ...
        if (mb_strpos($select, "'asiakaslista',") === FALSE) $select .= "asiakas.tunnus 'asiakaslista', ";
      ...
      if (mb_strtolower(mb_substr($mukaan, 0, 18)) == "dynaaminen_asiakas") {
        // HUOMHUOM: Myynnit summautuu kun asiakas kuuluu useampaan segmenttiin
        $dyna_ms_luku = mb_substr($mukaan, -1);
        $mukaan_join  = mb_substr($mukaan, 0, -1).$dynaaminen_syvintaso["asiakas"];
      ...
      if ($mukaan == "osasto") {
        ...
        if (mb_strpos($select, "'tuotelista',") === FALSE) $select .= "group_concat(DISTINCT concat('\'',tuote.tuoteno,'\'')) 'tuotelista', ";
        $order  .= "tuote.osasto,";
        $gluku++;
        $tuosagroups++;
      }
      ...
      if ($mukaan == "tuote") {
        ...
        if (mb_strpos($select, "'tuotelista',") === FALSE) $select .= "concat('\'',tuote.tuoteno,'\'') 'tuotelista', ";
        $order  .= "tuote.tuoteno,";
        $gluku++;
      ...
      if (mb_strtolower(mb_substr($mukaan, 0, 16)) == "dynaaminen_tuote") {
        // HUOMHUOM: Myynnit summautuu kun tuote kuuluu useampaan segmenttiin
        $dyna_ms_luku = mb_substr($mukaan, -1);
        $mukaan_join  = mb_substr($mukaan, 0, -1).$dynaaminen_syvintaso["tuote"];
      ...
      if ($mukaan == "tilauksittain") {
        ...
        if ($laskutuspaiva != "" and mb_strpos($select, "lasku.tapvm laskutuspvm, ") === FALSE) {
    ...
    if ($tee == 'go') {
      ...
      //(HUOM: ", " (pilkku-space) stringiä käytetään vain sarakkeiden välillä, eli ole tarkkana concatissa ja muissa funkkareissa $select-muuttujassa)
      $data_start_index = mb_substr_count($select, ", ");
      ...
      if ($kuukausittain != "") {
        ...
        for ($i = $startmonth;  $i <= $endmonth;) {
          ...
          if (($ajotapa == 'tilausjaauki' or $ajotapa == 'tilausauki') and $piilota_myynti == "") {
            $query .= " sum(if(lasku.luontiaika >= '{$alku} 00:00:00' and lasku.luontiaika <= '{$loppu} 23:59:59' and tilausrivi.uusiotunnus=0, tilausrivi.hinta / if('{$yhtiorow['alv_kasittely']}' = '' and tilausrivi.alv<500, (1+tilausrivi.alv/100), 1) * (tilausrivi.varattu+tilausrivi.jt) * {$query_ale_lisa}, 0)) '".mb_substr($i, 0, 4).mb_substr($i, 4, 2)."_laskuttamatta', ";
          }

          if (($ajotapa == 'tilausjaaukiluonti' or $ajotapa == 'ennakot') and $piilota_myynti == "") {
            $query .= " sum(if(lasku.luontiaika >= '{$alku} 00:00:00' and lasku.luontiaika <= '{$loppu} 23:59:59', if(tilausrivi.uusiotunnus=0, tilausrivi.hinta / if('{$yhtiorow['alv_kasittely']}' = '' and tilausrivi.alv<500, (1+tilausrivi.alv/100), 1) * (tilausrivi.varattu+tilausrivi.jt) * {$query_ale_lisa}, tilausrivi.rivihinta), 0)) '".mb_substr($i, 0, 4).mb_substr($i, 4, 2)."_myynti', ";
          }
          elseif (($ajotapa != 'tilausauki') and $piilota_myynti == "") {
            $query .= " sum(if(tilausrivi.laskutettuaika >= '{$alku}' and tilausrivi.laskutettuaika <= '{$loppu}', tilausrivi.rivihinta, 0)) '".mb_substr($i, 0, 4).mb_substr($i, 4, 2)."_myynti', ";
          }

          if ($vertailubu != "") {
            $query .= " sum(0) '".mb_substr($i, 0, 4).mb_substr($i, 4, 2)."_tavoitenyt', ";
            $query .= " sum(0) '".mb_substr($i, 0, 4).mb_substr($i, 4, 2)."_tavoiteindnyt', ";
          }

          if ($piilota_kappaleet == "") {
            $query .= " sum(if(tilausrivi.laskutettuaika >= '{$alku}' and tilausrivi.laskutettuaika <= '{$loppu}', tilausrivi.kpl,0)) '".mb_substr($i, 0, 4).mb_substr($i, 4, 2)."_kpl', ";
          }
          ...
          if (($ajotapa == 'tilausjaauki') and $piilota_myynti == "") {
            $query .= " sum(if(lasku.luontiaika >= '{$alku} 00:00:00' and lasku.luontiaika <= '{$loppu} 23:59:59' and tilausrivi.uusiotunnus=0, tilausrivi.hinta / if('{$yhtiorow['alv_kasittely']}' = '' and tilausrivi.alv<500, (1+tilausrivi.alv/100), 1) * (tilausrivi.varattu+tilausrivi.jt) * {$query_ale_lisa}, 0)) +
                  sum(if(tilausrivi.laskutettuaika >= '{$alku}' and tilausrivi.laskutettuaika <= '{$loppu}', tilausrivi.rivihinta, 0)) '".mb_substr($i, 0, 4).mb_substr($i, 4, 2)."_myyntiyht', ";
          }

          if ($piiloed == "") {
            // MYYNTIED
            if ($ajotapa == 'tilausjaaukiluonti' or $ajotapa == 'ennakot') {
              $query .= " sum(if(lasku.luontiaika >= '{$alku_ed} 00:00:00' and lasku.luontiaika <= '{$loppu_ed} 23:59:59', if(tilausrivi.uusiotunnus=0, tilausrivi.hinta / if('{$yhtiorow['alv_kasittely']}' = '' and tilausrivi.alv<500, (1+tilausrivi.alv/100), 1) * (tilausrivi.varattu+tilausrivi.jt) * {$query_ale_lisa}, tilausrivi.rivihinta), 0)) '".(mb_substr($i, 0, 4)-1).mb_substr($i, 4, 2)."_myynti', ";
            }
            elseif ($ajotapa != 'tilausauki') {
              $query .= " sum(if(tilausrivi.laskutettuaika >= '{$alku_ed}' and tilausrivi.laskutettuaika <= '{$loppu_ed}', tilausrivi.rivihinta, 0)) '".(mb_substr($i, 0, 4)-1).mb_substr($i, 4, 2)."_myynti', ";
            }

            if ($vertailubu != "") {
              $query .= " sum(0) '".(mb_substr($i, 0, 4)-1).mb_substr($i, 4, 2)."_tavoiteed', ";
              $query .= " sum(0) '".(mb_substr($i, 0, 4)-1).mb_substr($i, 4, 2)."_tavoiteinded', ";
            }

            if ($piilota_kappaleet == "") {
              $query .= " sum(if(tilausrivi.laskutettuaika >= '{$alku_ed}' and tilausrivi.laskutettuaika <= '{$loppu_ed}', tilausrivi.kpl,0)) '".(mb_substr($i, 0, 4)-1).mb_substr($i, 4, 2)."_kpl', ";
            }
          }
      ...
      if ($query != "") {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
          ...
          for ($i = $data_start_index; $i >= 0; $i--) {
            if (mb_substr(mysqli_field_name($result, $i), 0, 10) == "dynaaminen") {

              list($null, $dynlaji, $null) = explode("_", mysqli_field_name($result, $i));
              ...
              if (!${"dyn_".$dynlaji}) {
                $dynpuu_q = "SELECT subparent.nimi
                             ...
                             WHERE subnode.tunnus = ".$row[mysqli_field_name($result, $i)]."
                ...
                $dylask = 0;
                while ($dyprow = mysqli_fetch_assoc($dynpuu_r)) {
                  if (isset($row["dynaaminen_".$dynlaji."_".$dylask])) $row["dynaaminen_".$dynlaji."_".$dylask] = $row[mysqli_field_name($result, $i)]." / ".$dyprow["nimi"];
                  $dylask++;
                }
              }
            }
          ...
          if (isset($vertailubu) and
            ...
            if (!empty($kumulatiivinen_valittu)) {
              ...
              while ($dyprow = mysqli_fetch_assoc($budj_r)) {
          ...
          if ($ruk140chk != '' and $liitetiedostot != '') {
            ...
            while ($liitetiedosto_row = mysqli_fetch_assoc($liitetiedosto_result)) {
        ...
        if ($mukaan == "ytunnus" and isset($ytun_yhteyshenk) and $ytun_yhteyshenk != '' and count($rows) > 0) {
          ...
          while ($yhteyshenkilo_row = mysqli_fetch_assoc($yhteyshenkilo_result)) {
        ...
        if (isset($worksheet)) {
          ...
          if (isset($ytun_yhteyshenk) and $ytun_yhteyshenk != '' and isset($asiakas_tunnukset_sarja)) {
            ...
            $maksimi_maara_row = mysqli_fetch_assoc($maksimi_maara_result);
        ...
        foreach ($rows as $row) {
          ...
          if ($piilotanollarivit == "" or (float) $piilosumma != 0) {
            ...
            if ($piiyhteensa == '' and $gluku > 1 and $edluku != 'x' and $edluku != $row[$row_keys[0]] and mb_strpos($group, ',') !== FALSE and mb_substr($group, 0, 13) != "tuote.tuoteno") {
            ...
            foreach ($row as $ken_nimi => $kentta) {
              ...
              if ($ken_nimi == "yhtio_toimipaikka") {
                ...
                if (mysqli_num_rows($toimipaikka_result) == 1) {
                  $toimipaikka_row = mysqli_fetch_assoc($toimipaikka_result);
              ...
              if ($ken_nimi == "sarjanumero") {
                ...
                foreach ($sarjat as $sarja) {
                  ...
                  $rilirow = mysqli_fetch_assoc($rilires);
                  ...
                  if (mysqli_num_rows($osre) > 0) {
                    while ($osrow = mysqli_fetch_assoc($osre)) {
                ...
                $row[$ken_nimi] = mb_substr($row[$ken_nimi], 0, -4);
              }
        ...
        $apu = mysqli_num_fields($result)-11;
