...
if (!isset($alku))  $alku  = mb_substr($yhtiorow['tilikausi_alku'], 0, 4)  - 1 . mb_substr($yhtiorow['tilikausi_alku'], 4);
if (!isset($loppu)) $loppu = mb_substr($yhtiorow['tilikausi_loppu'], 0, 4) - 1 . mb_substr($yhtiorow['tilikausi_loppu'], 4);
...
echo "<br><br>".t("Pakettiin lisättävät aineistot").":<br>";
echo "<table>";
echo "<tr>";
...
if ($tee == "raportti") {
  ...
  if (!empty($tilioinnit_mukaan)) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
      ...
      if (empty($haetut_muikkarit[$row['ltunnus']]) and mb_strtoupper($row['tila']) == "X") {
  ...
  if (!empty($ostolaskut_mukaan)) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
      ...
      if (in_array($row["vienti"], array('C', 'F', 'I', 'J', 'K', 'L'))) {
        ...
        while ($keikrow = mysqli_fetch_assoc($keikres)) {
