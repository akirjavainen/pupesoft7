...
function teerivi($tuoteno, $valittu_toimittaja) {
  ...
  $alku_kausi = mb_substr(str_replace("-", "", $nykyinen_alku), 0, 8);
  $loppu_kausi = mb_substr(str_replace("-", "", $nykyinen_loppu), 0, 8);
  ...
  while ($isatuote_row = mysqli_fetch_assoc($isatuote_result)) {
...
if (isset($tee) and $tee == "TEE_OSTOTILAUKSET") {
  ...
  foreach ($ostettava_maara as $index => $maara) {
    ...
    $toimittaja = sanitize_string($ostettava_toimittaja[$index]);
    $tuoteno = sanitize_string($ostettava_tuoteno[$index]);
