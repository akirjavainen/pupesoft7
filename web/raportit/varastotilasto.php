...
echo "<option value = 'kappaleet3'{$sel['kappaleet3']}>".t("Listauksessa näytetään myynti kappaleina, rajattu")."</option>";
...
echo "<option value = 'eimyyntia'{$sel['eimyyntia']}>".t("Listauksessa ei näytetä myyntejä eikä kulutuksia")."</option>";

if ($toim == "") {
  echo "<option value = 'osto'{$sel['osto']}>".t("Ostoraportti")."</option>";
  echo "<option value = 'ostoryhma'{$sel['ostoryhma']}>".t("Ostoraportti ryhmittäin")."</option>";
}
...
if ($listaustyyppi == "osto" or $listaustyyppi == "ostoryhma") {
  ...
  echo "<tr>";
  echo "<th>", t("Alkupäivämäärä"), " (", t("pp-kk-vvvv"), ")</th>";
  ...
  echo "<tr>";
  echo "<th>", t("Loppupäivämäärä"), " (", t("pp-kk-vvvv"), ")</th>";
...
if (empty($oletusvarasto_chk)) {
  ...
  while ($varasto = mysqli_fetch_assoc($result)) {
...
echo "<option value = ''>".t("Näytetään kaikki rivit")."</option>";
echo "<option value = 'einollia'{$sel['einollia']}>".t("Piilotetaan nollarivit")."</option>";
echo "<option value = 'vainsaldo'{$sel['vainsaldo']}>".t("Näytetään vain saldolliset tuotteet")."</option>";
echo "</select>";
echo "</td>";
echo "</tr>";
...
if ($tee != "" and isset($painoinnappia)) {
  ...
  $total_rows = mysqli_num_rows($eresult);
  ...
  if ($total_rows > 0) {
    ...
    if ($toim == "") {
      ...
      if ($listaustyyppi != "osto" and $listaustyyppi != "ostoryhma") {
        ...
        $worksheet->writeString($excelrivi, $excelsarake++, t("Tilattu määrä"), $format_bold);
    ...
    $osastot = array();
    while ($osastorow = mysqli_fetch_assoc($osastores)) {
    ...
    $mallit = array();
    while ($mallirow = mysqli_fetch_assoc($mallires)) {
    ...
    while ($row = mysqli_fetch_assoc($eresult)) {
      ...
      if ($listaustyyppi == "osto" or $listaustyyppi == "ostoryhma") {
        ...
        $tapahtumarivi = mysqli_fetch_assoc($tuloresult);
      }
      elseif ($listaustyyppi != "eimyyntia") {
        // ostopuoli
        ...
        $ostorivi = mysqli_fetch_assoc($ostoresult);
        ...
        if($kuukausittainen_lista) {
          ...
          $eurmyynti = mysqli_fetch_assoc($eurmyyntiresult);
          ...
          $kplmyynti = mysqli_fetch_assoc($kplresult);
        }
        ...
        if ($listaustyyppi == "kappaleet2") {
          ...
          $kulutusrivi = mysqli_fetch_assoc($kulutusresult);
      ...
      if ($nayta_vapaa_saldo == "on") {
        ...
        $jt_rivi = mysqli_fetch_assoc($jt_result);
