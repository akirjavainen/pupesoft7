...
if (!function_exists("piirra_tuntiraportti")) {
  function piirra_tuntiraportti($asentaja, $kukarow, $yhtiorow, $vva, $kka, $ppa, $vvl, $kkl, $ppl, $tyom_nro = '', $asiakasid = '', $asiakasosasto = '', $asiakasryhma = '', $tyojono = '', $tyostatus = '', $ytunnus = '') {
    ...
    if (trim($asiakasosasto) != '') {
      $asiakaslisa .= " and asiakas.osasto = '".sanitize_string($asiakasosasto)."' ";
    }

    if (trim($asiakasryhma) != '') {
      $asiakaslisa .= " and asiakas.ryhma = '".sanitize_string($asiakasryhma)."' ";
    }

    if (trim($ytunnus) != '') {
      $asiakaslisa .= " and asiakas.ytunnus = '".sanitize_string($ytunnus)."' ";
    }

    $tyomaarayslisa = '';

    if (trim($tyojono) != '') {
      $tyomaarayslisa .= " and tyomaarays.tyojono = '".sanitize_string($tyojono)."' ";
    }

    if (trim($tyostatus) != '') {
      $tyomaarayslisa .= " and tyomaarays.tyostatus = '".sanitize_string($tyostatus)."' ";
    }
    ...
    if (mysqli_num_rows($sresult) > 0) {
      ...
      while ($row = mysqli_fetch_array($sresult)) {
        ...
        if ($asentaja == "" or mysqli_num_rows($keikkares) > 0 or $row["asennuskalenteri"] != "") {
          ...
          if ($row["asennuskalenteri"] != "") {
            ...
            foreach (explode(",", $row["asennuskalenteri"]) as $asekale) {
              ...
              $atstamp = mktime(mb_substr($alku, 11, 2), mb_substr($alku, 14, 2), 0, mb_substr($alku, 5, 2), mb_substr($alku, 8, 2), mb_substr($alku, 0, 4));
              $ltstamp = mktime(mb_substr($loppu, 11, 2), mb_substr($loppu, 14, 2), 0, mb_substr($loppu, 5, 2), mb_substr($loppu, 8, 2), mb_substr($loppu, 0, 4));
          ...
          $rivihinta_row = mysqli_fetch_assoc($rivihinta_res);
          ...
          if ($row["tunnus"] != "") {
            ...
            while ($kalenteri_row = mysqli_fetch_assoc($kalenteri_res)) {
              echo "<table width='100%'>";
          ...
          if (mysqli_num_rows($keikkares) > 0) {
            ...
            while ($keikkarow = mysqli_fetch_array($keikkares)) {
