...
if (!isset($vv) or !isset($lvv)) {
  ...
  if (!isset($vv)) {
    $vv = mb_substr($tilikausirow['tilikausi_alku'], 0, 4);
    $kk = mb_substr($tilikausirow['tilikausi_alku'], 5, 2);
    $pp = mb_substr($tilikausirow['tilikausi_alku'], 8, 2);
  }

  if (!isset($lvv)) {
    $lvv = mb_substr($tilikausirow['tilikausi_loppu'], 0, 4);
    $lkk = mb_substr($tilikausirow['tilikausi_loppu'], 5, 2);
    $lpp = mb_substr($tilikausirow['tilikausi_loppu'], 8, 2);
  }
}
...
if ($tee == "aja" and $alisa != "" and $llisa != "") {
  ...
  if (mysqli_num_rows($result) > 0) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
      ...
      if (mysqli_num_rows($liotsres) == 1) {
        $liotsrow = mysqli_fetch_assoc($liotsres);
        ...
        $varivirow = mysqli_fetch_assoc($varivires);
      ...
      elseif (mysqli_num_rows($liotsres) > 1) {
