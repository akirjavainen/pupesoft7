...
function lockdown($vv, $kk, $pp, $tasmayskassa) {
  ...
  $tasmaysrow = mysqli_fetch_assoc($tasmays_result);
...
function tosite_print($vv, $kk, $pp, $ltunnukset, $tulosta = null) {
  ...
  if (!is_array($kassakone) and mb_strlen($kassakone) > 0) {
  ...
  while ($tasmaysrow = mysqli_fetch_assoc($tasmays_result)) {

    if ($tasmaysrow["tilino"] != $row["kateistilitys"] and $tasmaysrow["tilino"] != $row["kassaerotus"] and $tasmaysrow["tilino"] != $row["kateisotto"] and !mb_stristr($tasmaysrow["selite"], t("erotus"))) {
      ...
      if ($edselitelen != mb_strlen($tasmaysrow["selite"]) and $edselitelen != 0) {
        $prn = "\n";
        fwrite($fh, $prn);
        $rivit++;
      }
      ...
      $edselitelen = mb_strlen($tasmaysrow["selite"]);
    }
  }
...
if (isset($tasmays) and $tasmays != "") {
  ...
  if (count($kassakone) > 1) {
    ...
    $kassat_temp = mb_substr($kassat_temp, 0, -1);
  ...
  if (count($kassakone) > 0) {
    ...
    if (mysqli_num_rows($result) != count($kassakone)) {
...
if ($tee == "tiliointi") {
  ...
  if (count($kassalipas_tunnus) > 0) {
    ...
    $ktunnukset = mb_substr($ktunnukset, 0, -1);
  }
  ...
  foreach ($_POST as $kentta => $arvo) {
    ...
    if (mb_stristr($kentta, "pohjakassa")) {
      if (mb_stristr($kentta, "tyyppi")) {
        $tyyppi = $arvo;
    ...
    if (mb_stristr($kentta, "loppukassa")) {
      $loppukassa = $arvo;
    }

    if (mb_stristr($arvo, "pankkikortti")) {
      ...
      if (mysqli_num_rows($result) == 1) {
        $kassalipasrow = mysqli_fetch_assoc($result);
    ...
    elseif (mb_stristr($arvo, "luottokortti")) {
    ...
    elseif (mb_stristr($arvo, "kateinen")) {
    ...
    if (abs(str_replace(",", ".", $arvo)) > 0 and (mb_stristr($kentta, "solu") or mb_stristr($kentta, "erotus"))) {
      ...
      if (mb_stristr($kentta, "soluerotus")) {
      ...
      if (mb_stristr($kentta, "soluerotus") or mb_stristr($kentta, "erotus")) {
        $arvo = $arvo * -1;
      }
      ...
      elseif (mb_stristr($kentta, "erotus")) {
        $selitelisa .= " ".t("erotus");
      }
    ...
    if (abs(str_replace(",", ".", $arvo)) > 0 and mb_stristr($kentta, "kateistilitys")) {
    ...
    if (abs(str_replace(",", ".", $arvo)) > 0 and mb_stristr($kentta, "kateisotto")) {
      $arvo = (float) str_replace(",", ".", $arvo);
...
elseif ($tee != '') {
  ...
  // MUOKKAUS: isset():
  if (!is_array($kassakone)) $kassakone = array();
  ...
  $myyntisaamiset_tilit = mb_substr($myyntisaamiset_tilit, 0, -1);
  ...
  if ($i > 1) {
    ...
    if ($toim == "" and isset($tasmays) and $tasmays != '') {
      ...
      echo "<td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td align='center' style='width:100px' nowrap>".mb_strtoupper(t("Tilitys"))." 1</td>";

      if ($tilityskpl > 1) {
        for ($yyy = 1; $yyy < $tilityskpl; $yyy++) {
          $yyyy = $yyy + 1;
          echo "<td align='center' style='width:100px' nowrap>".mb_strtoupper(t("Tilitys"))." $yyyy</td>";
        }
      }

      echo "<td align='center' style='width:100px' nowrap>".mb_strtoupper(t("Myynti"))."</td><td align='center' style='width:100px' nowrap>".mb_strtoupper(t("Erotus"))."</td></tr>";
      echo "</tr>";
      ...
      $tasmaytys_row = mysqli_fetch_assoc($tasmaytys_result);
    ...
    if ($toim == "" and isset($tasmays) and $tasmays != '') {
      ...
      while ($row = mysqli_fetch_assoc($result)) {
        ...
        if (mb_stristr($row["tyyppi"], 'kateinen')) {

          $solu = "kateinen";

          if ($edkassa != $row["kassa"] or ($kateinen != $row["tilino"] and $kateinen != '')) {

            if (mb_stristr($kateismaksu, 'kateinen')) {
                ...
                foreach ($kateisotot as $kassalipas_tunnus_temp => $kateisotot_kassalipas) {
                  if ($edktunnus == $kassalipas_tunnus_temp) {
                    foreach ($kateisotot_kassalipas as $kateisotto) {
                      ...
                      echo "<td>".mb_substr($kateisotto[kuka_nimi], 0, 23)."</td>";
          ...
          echo "<td>".mb_substr($row["nimi"], 0, 23)."</td>";
      ...
      if ($solu != 'kateinen' and ($pankkikortti === true or $luottokortti === true)) {
        ...
        $kassalipasrow = mysqli_fetch_assoc($kassalipasresult);
      ...
      while ($row = mysqli_fetch_assoc($result)) {
        ...
        if ($vaiht == 1 and empty($vainyhteensa)) {
          ...
          $prn .= sprintf('%-25.25s',   mb_substr($row["nimi"], 0, 23));
...
function get_pohjakassa($row) {
  ...
  if (mysqli_num_rows($pk_result) == 1) {
...
function populoi_kassalipas_muuttujat_kassakohtaisesti($_post) {
  ...
  foreach ($_post as $kentan_nimi => $kentan_arvo) {
    if (mb_stristr($kentan_nimi, 'tyyppi_pohjakassa')) {
      ...
      foreach ($_post as $etsi_kassalipas_nimi => $etsi_kassalipas_arvo) {
        ...
        if (mb_strstr($etsi_kassalipas_nimi , $kassalippaan_indeksi[0][0])) {

          if (!mb_stristr($etsi_kassalipas_nimi, 'solu') and !mb_stristr($etsi_kassalipas_nimi, 'kateisotto') and !mb_stristr($etsi_kassalipas_nimi, 'kateistilitys')) {
            ...
            $monisoluisen_indeksi = mb_strlen($monisoluisen_indeksi_array[0][0]);
    ...
    elseif (mb_stristr($kentan_nimi , 'maksutapa')) {
      //tämä hoitaa pankki ja luottokortit, jotka eivät ole kassa kohtaisia
      if (mb_stristr($kentan_arvo, 'pankkikortti') or mb_stristr($kentan_arvo, 'luottokortti')) {
        ...
        foreach ($_post as $etsi_kortti_nimi => $etsi_kortti_arvo) {
          ...
          //etsitään kassalippaalle kuuluvat tilitys arvot
          if (mb_strstr($etsi_kortti_nimi , $kortin_indeksi[0][0])) {
...
function hae_kassalippaiden_kateisotot($tapvm_where) {
  ...
  while ($kateisotto = mysqli_fetch_assoc($kateisotot_result)) {
