    ...
    if ($tee == 'go') {
      ...
      if ($query != "") {
        ...
        if ($vain_excel != "") {
          ...
          if (mysqli_num_rows($result) > $rivilimitti) {
      ...
      if ($query != "") {
        ...
        if (mysqli_num_rows($result) <= $rivilimitti) echo "<table><tr>";

        // echotaan kenttien nimet
        for ($i=0; $i < mysqli_num_fields($result); $i++) {
          if (mysqli_num_rows($result) <= $rivilimitti) echo "<th>".t(mysqli_field_name($result, $i))."</th>";
        }

        if (isset($worksheet)) {
          for ($i=0; $i < mysqli_num_fields($result); $i++) $worksheet->write($excelrivi, $i, ucfirst(t(mysqli_field_name($result, $i))), $format_bold);
          $excelrivi++;
        }

        if (mysqli_num_rows($result) <= $rivilimitti) echo "</tr>\n";
        ...
        if (mysqli_num_rows($result) > $rivilimitti) {
          ...
          $bar = new ProgressBar();
          $elements = mysqli_num_rows($result); // total number of elements to process
          $bar->initialize($elements); // print the empty bar
        }

        while ($row = mysqli_fetch_array($result)) {

          if (mysqli_num_rows($result) > $rivilimitti) $bar->increase();
          ...
          for ($i=0; $i < mysqli_num_fields($result); $i++) {
            ...
            if (mysqli_field_name($result, $i) == "tuos") {
              $osre = t_avainsana("OSASTO", "", "and avainsana.selite  = '$row[$i]'", $yhtio);
              $osrow = mysqli_fetch_array($osre);
            ...
            // jos kyseessa on tuoteosasto, haetaan sen nimi
            if (mysqli_field_name($result, $i) == "tuoteryhmä") {
            ...
            if (mysqli_field_name($result, $i) == "tuoteostaja") {
              $query = "SELECT nimi
                        FROM kuka
            ...
            if ($gluku > 1 and $edluku != $row[0] and $edluku != 'x' and $piiyhteensa == '' and strpos($group, ',') !== FALSE and substr($group, 0, 13) != "tuote.tuoteno") {
              ...
              foreach ($valisummat as $vnim => $vsum) {
                ...
                if (mysqli_num_rows($result) <= $rivilimitti) echo "<td class='tumma' align='right'>$vsum</td>";
              ...
              $excelrivi++;
              if (mysqli_num_rows($result) <= $rivilimitti) echo "</tr><tr>";
            ...
            if (is_numeric($row[$i]) and (mysqli_field_type($result, $i) == 'real' or mysqli_field_type($result, $i) == 'int')) {
              if (mysqli_num_rows($result) <= $rivilimitti) echo "<td valign='top' align='right'>".sprintf("%.02f", $row[$i])."</td>";

              if (isset($worksheet)) {
                $worksheet->writeNumber($excelrivi, $i, sprintf("%.02f", $row[$i]));
              }
            }
            elseif (mysqli_field_name($result, $i) == 'sarjanumero') {
              if (mysqli_num_rows($result) <= $rivilimitti) echo "<td valign='top'>$row[$i]</td>";
          ...
          for ($i=0; $i < mysqli_num_fields($result); $i++) {
            ...
            if ($i < substr_count($select, ", ")) {
              ...
              $valisummat[mysqli_field_name($result, $i)] += $row[mysqli_field_name($result, $i)];
              $totsummat[mysqli_field_name($result, $i)]  += $row[mysqli_field_name($result, $i)];
            }
          }
        }
        ...
        if (mysqli_num_rows($result) <= $rivilimitti) echo "</tr></table>";
