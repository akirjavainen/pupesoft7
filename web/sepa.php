...
function sepa_credittransfer($laskurow, $popvm_nyt, $netotetut_rivit = '') {
  ...
  if ($yhtiorow['maa'] == 'EE' and mb_strlen(trim($laskurow["laskunro"])) > 0 and $laskurow['viesti'] != "") {
    ...
    if (mb_strlen(trim($laskurow["viite"])) > 0 and $laskurow["tilaustyyppi"] == 'M') {             // Matkalaskuilla viite viestiksi sanomalle
      $Ustrd = $RmtInf->addChild('Ustrd', sprintf("%-1.140s", $laskurow['viite']));          // Unstructured (max 140 char)
    }
    elseif (mb_strlen(trim($laskurow["viite"])) > 0) {
  ...
  if ($netotetut_rivit != "") {
    ...
    while ($nettorow = mysqli_fetch_assoc($result)) {
      ...
      if (mb_strlen(trim($nettorow["viite"])) > 0) {
        $CdtrRefInf = $Strd->addChild('CdtrRefInf', '');                         // CreditorReferenceInformation
        $CdtrRefTp = $CdtrRefInf->addChild('CdtrRefTp', '');                     // CreditorReferenceType
        $Cd = $CdtrRefTp->addChild('Cd', 'SCOR');                         // Code (SCOR = Structured Communication Reference)
...
if ($tee == "") {
  ...
  if (SEPA_PANKKIYHTEYS and $_num > 0) {
    $_temp = mysqli_fetch_assoc($result);
  ...
  if (mysqli_num_rows($result) > 0 and $virheita == 0) {
...
if ($tee == "KIRJOITA" or $tee == "KIRJOITAKOPIO") {
  ...
  if (mysqli_num_rows($result) > 0) {
    ...
    $popvm_row = mysqli_fetch_assoc($result);
    ...
    if (mb_strtoupper($yhtiorow['maa']) == 'EE' and mb_substr($popvm_row['yriti_iban'], 0, 2) == "EE") {
  ...
  while ($laskurow = mysqli_fetch_assoc($result)) {
    ...
    while ($nettolaskurow = mysqli_fetch_assoc($nettolaskures)) {
      if (!$summa_plussalla) {
  ...
  foreach ($netotettava_laskut as $i => $tunnukset) {
    $query = "SELECT lasku.*, if(lasku.ultilno_maa != '', lasku.ultilno_maa, lasku.maa) iban_maa,
              ...
              LIMIT 1";
    $result = pupe_query($query);
    $nettorow = mysqli_fetch_assoc($result);
