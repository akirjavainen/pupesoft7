...
if ($kasitellaan_tiedosto) {
  ...
  foreach ($headers as $column) {
    ...
    $column = mb_strtoupper(trim($column));

    if ($column != '') {
      //laitetaan kaikki paitsi valintasarake talteen.
      if ($column != "TOIMINTO") {
        if (!in_array($column, $trows)) {
          echo "<br><font class='message'>", t("Saraketta"), " \"<b>", mb_strtoupper($column), "</b>\" ", t("ei löydy"), " $table-taulusta!</font>";
          $vikaa++;
        }
  ...
  foreach ($taulunrivit as $rivinumero => $rivi) {
    ...
    if ($table == "korvaavat" or $table == "vastaavat") {
      ...
      if (mysqli_num_rows($hresult) == 0) {
        ...
        if ($table == "vastaavat" and mb_strtoupper(trim($rivi[$postoiminto])) != 'LISAA') {
      ...
      elseif (mysqli_num_rows($hresult) == 1) {
        $frow =  mysqli_fetch_array($hresult);
        $id = $frow[0];
      }
      ...
      if ($id > 0) {
        if (mb_strtoupper(trim($rivi[$postoiminto])) == 'LISAA') {
          $alku     = "INSERT into $table SET yhtio = '{$kukarow['yhtio']}'";
          $loppu     = ", id='$id'";
          $toiminto   = "LISAA";
        }
        elseif (mb_strtoupper(trim($rivi[$postoiminto])) == 'MUUTA') {
          $toiminto   = "MUUTA";
        }
        elseif (mb_strtoupper(trim($rivi[$postoiminto])) == 'POISTA') {
          ...
          if ($headers[$j] == "TUOTENO" and trim($rivi[$j]) != '') {
            ...
            if (mysqli_num_rows($tresult) > 0) {
              ...
              elseif ($toiminto == 'POISTA') {
                if (mysqli_num_rows($kresult) == 0 and !($table == "vastaavat" and $vastaava_paatuote == $rivi[$j])) {
    ...
    if ($table == "tuoteperhe") {
      ...
      for ($r = 0; $r < count($headers); $r++) {
        ...
        if (mb_strtoupper(trim($headers[$r])) == "ISATUOTENO") {
          ...
          if (mysqli_num_rows($result) == 0 and mb_strtoupper(trim($rivi[$postoiminto])) != 'POISTA') {
          ...
          if (mysqli_num_rows($result) == 0 and mb_strtoupper(trim($rivi[$postoiminto])) == 'MUUTA') {
            echo t("tuoteperhettä ei löydy! ei voida muuttaa"), "<br>";
            $virhe++;
          }
          elseif (mysqli_num_rows($result) != 0 and mb_strtoupper(trim($rivi[$postoiminto])) == 'LISAA') {
            echo t("tuoteperhe on jo olemassa! ei voida lisätä"), "<br>";
            $virhe++;
          }
          elseif (mysqli_num_rows($result) == 0 and mb_strtoupper(trim($rivi[$postoiminto])) == 'POISTA') {
            echo t("tuoteperhettä ei löydy! ei voida poistaa"), "<br>";
            $virhe++;
          }
        }

        if (mb_strtoupper(trim($headers[$r])) == "TUOTENO" and mb_strtoupper(trim($rivi[$postoiminto])) != 'POISTA') {
          $query = "SELECT tunnus
                    FROM tuote
                    WHERE yhtio = '{$kukarow['yhtio']}'
                    AND tuoteno = '{$rivi[$r]}'";
      ...
      if ($virhe == 0 and $isatuote != "") {
        ...
        if (mb_strtoupper(trim($rivi[$postoiminto])) != 'POISTA') {
        ...
        for ($r = 0; $r < count($headers); $r++) {
          if (mb_strtoupper(trim($rivi[$postoiminto])) != 'POISTA' and mb_strtoupper(trim($headers[$r])) == "TUOTENO") {
          ...
          elseif (mb_strtoupper(trim($headers[$r])) == "TUOTENO") {
            $query  = "DELETE FROM tuoteperhe
                       WHERE yhtio    = '{$kukarow['yhtio']}'
                       AND isatuoteno = '$isatuote'
                       AND tuoteno    = '{$rivi[$r]}'
