...
function kasittele_tuote_tiedosto($file_name, $real_name = '') {
  ...
  while ($rivi = fgets($file)) {
    ...
    $uustuoteno = mb_strtoupper(trim($rivi[1]));
    ...
    if ($vantuoteno != '' and $uustuoteno != '') {
      ...
      if (mysqli_num_rows($tuoteuresult) == 1) {

        // uusi tuoteno löytyy jo. tarkistetaan onko sama kuin vanha...
        if (mb_strtoupper($vantuoteno) == $uustuoteno) {
          ...
          if (ctype_upper($vantuoteno)) {
            ...
            $suuraakkosiin[] = mb_strtoupper($vantuoteno."!¡!".$uustuoteno);
            echo "<font class='message'>";
...
elseif (isset($_FILES['userfile']) and is_uploaded_file($_FILES['userfile']['tmp_name']) !== TRUE and $tee == "file") {
  ...
  $vantuoteno = mb_strtoupper(trim($vantuoteno));
  $uustuoteno = mb_strtoupper(trim($uustuoteno));
  ...
  if (mysqli_num_rows($tuoteresult) != 0) {
    $vantuoterow = mysqli_fetch_array($tuoteresult);
    ...
    if (mysqli_num_rows($tuoteuresult) != 0) {
      $uustuoterow = mysqli_fetch_array($tuoteuresult);

      echo "<font class='message'>".t("UUSI TUOTENUMERO LÖYTYY JO").": $uustuoteno</font><br>";

      if (mb_strtoupper(trim($vantuoterow["tuoteno"])) == mb_strtoupper(trim($uustuoterow["tuoteno"]))) {
...
if ($error == 0 and $tee == "file") {
  ...
  while ($tables = mysqli_fetch_array($tabresult)) {
    ...
    while ($fields = mysqli_fetch_array($fieldresult)) {
      if ((mb_strpos($fields[0], "tuotenumero") !== false or mb_strpos($fields[0], "tuoteno") !== false) and $tables[0] != 'tuote_muutokset' and $fields[0] != 'toim_tuoteno') {
  ...
  $tyypitrow = mysqli_fetch_assoc($tyypitresult);
  ...
  while ($rivi = fgets($file)) {
    ...
    if (trim($rivi[0]) != '' and trim($rivi[1]) != '') {
      ...
      $vantuoteno = mb_strtoupper(trim($rivi[0]));
      $uustuoteno = mb_strtoupper(trim($rivi[1]));
      ...
      if (mysqli_num_rows($tuoteresult) == 1) {
        ...
        $trivi = mysqli_fetch_assoc($tuoteresult);
        ...
        if (in_array(mb_strtoupper($vantuoteno."!¡!".$uustuoteno), $suuraakkosiin)) {
          $uusi_on_jo = "SAMA";
        }

        if (mysqli_num_rows($tuoteuresult) == 0 or $uusi_on_jo == "OK" or $uusi_on_jo == "SAMA") {
          ...
          foreach ($tulos as $saraketaulu) {
            ...
            elseif ($taulu == 'tuotepaikat' and $uusi_on_jo != "SAMA") {
              ...
              while ($pairow = mysqli_fetch_array($paires)) {
              ...
              elseif (mysqli_num_rows($paires) > 1) {
                $query = "UPDATE tuotepaikat
                          SET oletus  = ''
                          WHERE yhtio  = '$kukarow[yhtio]'
                          and tuoteno  = '$uustuoteno'
                          and oletus  != ''
                          ORDER BY tunnus
                          LIMIT ".(mysqli_num_rows($paires)-1);
                pupe_query($query);
              }
            }
          ...
          if ($jatavanha != '' and $uusi_on_jo != "SAMA") {
            if ($uusi_on_jo == "") {
              ...
              if (mysqli_num_rows($alvresult) != '0') {
                $alvrow = mysqli_fetch_array($alvresult);
                $alv = $alvrow['selite'];
              }
            ...
            $korvid = mysqli_fetch_array($korvresult);
            ...
            if (mysqli_num_rows($korvvresult) != '0') {
              $korvid = mysqli_fetch_array($korvvresult);
  ...
  if (count($postit) > 0 and $muistutus == "KYLLA") {
    ...
    foreach ($postit as $key => $values) {
      ...
      while ($posti = mysqli_fetch_assoc($resuposti)) {
