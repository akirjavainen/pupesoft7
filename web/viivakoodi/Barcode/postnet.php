...
class Image_Barcode_postnet extends Image_Barcode
{
    ...
    function draw($text, $imgtype = 'png')
    {
        ...
        $barcodewidth = (mb_strlen($text)) * 2 * 5 * $this->_barwidth + $this->_barwidth*3;
        ...
        for ($idx = 0; $idx < mb_strlen($text); $idx++) {
            $char  = mb_substr($text, $idx, 1);

            for ($baridx = 0; $baridx < 5; $baridx++) {
                $elementheight = (mb_substr($this->_coding_map[$char], $baridx, 1)) ?  0 : $this->_barshortheight;
