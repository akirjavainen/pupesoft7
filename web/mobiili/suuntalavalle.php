...
$sscchaku = !empty($sscc) ? "and suuntalavat.sscc = '".mysqli_escape_string($sscc)."'" : "";
...
if (isset($submit) and $tullaan != 'pre_vahvista_kerayspaikka') {
  ...
  if (!empty($sscc) and $submit == "hae") {
    if (mysqli_num_rows($suuntalavat_res) == 1) {
      $_chk_row = mysqli_fetch_assoc($suuntalavat_res);
...
$loytyiko = false;
while ($row = mysqli_fetch_assoc($suuntalavat_res)) {
