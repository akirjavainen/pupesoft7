...
if (isset($submit)) {
  ...
  case 'ok':
    ...
    if (count($errors) == 0) {
      ...
      elseif (mb_strstr($minne_hyllypaikka, '-') or mb_strstr($minne_hyllypaikka, ' ')) {
      ...
      if (count($errors) == 0) {
        ...
        if (mysqli_num_rows($chk_res) == 0) $errors[] = t("Tuotepaikkaa (%s-%s-%s-%s) ei ole perustettu tuotteelle", "", $hyllyalue, $hyllynro, $hyllyvali, $hyllytaso).'.';
      }
    ...
    if (count($errors) == 0) {
      ...
      $res = pupe_query($query);
      $minnerow = mysqli_fetch_assoc($res);
