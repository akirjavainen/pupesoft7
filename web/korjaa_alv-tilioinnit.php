...
if (isset($argv[5]) and $argv[5] != "") {
  ...
  if (mb_strlen($kpitokausi_auki) != 8) {
    die("Päivämäärä muodossa vvvvkkpp");
  }

  $kpitokausi_auki_pvm = mb_substr($kpitokausi_auki, 0, 4)."-".mb_substr($kpitokausi_auki, 4, 2)."-".mb_substr($kpitokausi_auki, 6, 2);
}
...
if (mysqli_num_rows($result) > 0) {
  ...
  while ($laskurow = mysqli_fetch_assoc($result)) {
    ...
    $old_alv_chk_row = mysqli_fetch_assoc($old_alv_chk_res);
    ...
    $new_alv_chk_row = mysqli_fetch_assoc($new_alv_chk_res);

    if (mysqli_num_rows($old_alv_chk_res) > 0 and mysqli_num_rows($new_alv_chk_res) > 0) {
      ...
      if ($old_alv != $new_alv) {
        ...
        $kulu_chk_row = mysqli_fetch_assoc($kulu_chk_res);

        $kulu_chk_row['summa'] -= $alvin_ero;
        list($_tunnus, ) = explode(',', $new_alv_chk_row['tunnukset']);
        echo "$laskurow[laskunro], $laskurow[tapvm], $laskurow[tunnus], $laskurow[nimi], $laskurow[yhtio_toimipaikka], $old_alv_chk_row[summa], $new_alv_chk_row[summa], $_tunnus, ero: $alvin_ero"."\n";
        $kpl += $kpl;
  ...
  echo "Korjattuja laskuja yhteensä: $kpl"."\n";
  echo "\n";
  flush();
}
