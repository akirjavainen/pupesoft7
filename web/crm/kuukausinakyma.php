...
$konsernit1 = " and yhtio in (".mb_substr($konsernit, 0, -1).") ";
$konsernit2 = " and kalenteri.yhtio in (".mb_substr($konsernit, 0, -1).") ";
$konsernit3 = " and kuka.yhtio in (".mb_substr($konsernit, 0, -1).") ";
...
while ($row = mysqli_fetch_array($result)) {
  ...
  for ($i = 1; $i <= days_in_month($month, $year); $i++) {
    ...
    if (mysqli_num_rows($kresult) > 0) {
      ...
      list($rvv, $rkk, $rpp) = explode("-", mb_substr($krow["pvmloppu"], 0, 10));
      ...
      if ($krow['tunnus'] != '' and $krow["kuittaus"] == "" and $kaleloppu >= $aikanyt and ($krow['kuka'] == $kukarow["kuka"] or $krow['laatija'] == $kukarow["kuka"])) {
        ...
        if (mysqli_num_rows($kresult) > 0) {
          mysqli_data_seek($kresult, 0);

          while ($krow2 = mysqli_fetch_array($kresult)) {
