...
if ($tee == '') {
  ...
  while ($asosrow = mysqli_fetch_assoc($asosresult)) {
  ...
  if (isset($kaletapa)) {
    ...
    $vali_kale     = mb_substr($vali_kale, 0, -1); // Viimeinen pilkku poistetaan
  ...
  foreach ($yhtiot as $yhtio) {
    ...
    if (mysqli_num_rows($result_group) > 0) {
      ...
      if ($piirra_yhteenveto) {
        ...
        while ($rivi = mysqli_fetch_assoc($result_group)) {
          ...
          while ($divirivi = mysqli_fetch_assoc($ressu)) {
            echo "<tr>";
