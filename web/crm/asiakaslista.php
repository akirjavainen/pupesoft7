...
if (isset($tee) and $tee == "lataa_tiedosto") {
  ...
  while ($count_row = mysqli_fetch_assoc($dynpuu_count_result)) {
  ...
  if ($dynaaminen_check) {
    mysqli_data_seek($dynpuu_count_result, 0);
    ...
    while ($count_row = mysqli_fetch_assoc($dynpuu_count_result)) {
      ...
      if (count(${"mul_".$param.$count_row['syvyys']}) > 0) {
        ...
        foreach (${"mul_".$param.$count_row['syvyys']} as $dynaaminenx) {
          if (trim($dynaaminenx) != '') {
            $dynaaminenx = trim(sanitize_string($dynaaminenx));
            $dynaamiset .= "'$dynaaminenx',";
          }
        }

        $dynaamiset = mb_substr($dynaamiset, 0, -1);
        ...
        if (trim($dynaamiset) != '') {
          ...
          $kaikki_puun_tunnukset_row = mysqli_fetch_assoc($kaikki_puun_tunnukset_res);
  ...
  while ($row = mysqli_fetch_assoc($res)) {
    fwrite($toot, mb_substr($row['myyja'], 0, 10).";");
    ...
    fwrite($toot, mb_substr($row['asiakastunnus'], 0, 10).";");

    fwrite($toot, "{$row['ytunnus']};");

    $nimi = trim($row['nimi'].' '.$row['nimitark']);
    $nimi = mb_substr($nimi, 0, 140);
    fwrite($toot, "{$nimi};");

    fwrite($toot, mb_substr($row['yhteyshenkilo'], 0, 140).";");
    fwrite($toot, mb_substr($row['postitp'], 0, 25).";");
    fwrite($toot, mb_substr($address, 0, 60).";");
    fwrite($toot, mb_substr($row['postino'], 0, 10).";");
    fwrite($toot, ";"); // region
    fwrite($toot, "{$row['maa']};"); // country
    fwrite($toot, mb_substr($row['puhelin'], 0, 30).";");
    fwrite($toot, mb_substr($row['email'], 0, 241).";");
    fwrite($toot, mb_substr($row['luontiaika'], 0, 10));
    fwrite($toot, ";".mb_substr($row['kentta02'], 0, 100));
    fwrite($toot, ";".mb_substr($row['kentta03'], 0, 8));
    fwrite($toot, ";".(int) $row['kentta04']);
    fwrite($toot, ";{$row['kentta05']}");
    fwrite($toot, ";".mb_substr($row['kentta06'], 0, 100));
...
if (trim($konserni) != '') {
  ...
  $konsernit = " asiakas.yhtio in (".mb_substr($konsernit, 0, -1).") ";
}
else {
...
if ($tee == 'laheta' or $tee == 'lahetalista') {
  ...
  if ($tee == "lahetalista") {
    for ($i=1; $i<mysqli_num_fields($result)-1; $i++) {
      $worksheet->write($excelrivi, $excelsarake, t(mysqli_field_name($result, $i)) , $format_bold);
      $excelsarake++;
    }
...
for ($i = 1; $i < mysqli_num_fields($result)-1; $i++) {
  echo "<th><a href='$PHP_SELF?konserni=$konserni&ojarj=".mysqli_field_name($result, $i).$ulisa."'>" . t(mysqli_field_name($result, $i)) . "</a>";

  if   (mysqli_field_len($result, $i)>10) $size='20';
...
if (mysqli_num_rows($asosresult) > 0) {
  echo "<br/>";
