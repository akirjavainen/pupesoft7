...
if (mb_strpos($_SERVER['SCRIPT_NAME'], "asiakasmemo.php") !== FALSE) {
...
$crm_haas_row = mysqli_fetch_assoc($crm_haas_res);
$crm_haas_check = (mysqli_num_rows($crm_haas_res) > 0 and $crm_haas_row['selite'] == 'K');
...
///* Asiakas on valittu *///
if (mb_strpos($_SERVER['SCRIPT_NAME'], "asiakasmemo.php") !== FALSE and $ytunnus != '') {
  ...
  if ($tee == "SAHKOPOSTI") {
    ...
    if ($email != "" and $ekuka != "") {
      ...
      $tulos = mail($email, mb_encode_mimeheader(t("Asiakasmemo")." $yhtiorow[nimi]", "UTF-8", "Q"), $meili, "From: ".mb_encode_mimeheader($yhtiorow["nimi"], "UTF-8", "Q")." <$yhtiorow[postittaja_email]>\n", "-f $yhtiorow[postittaja_email]");
    ...
    if ($korjaus == '') {
      if ($viesti != '') {
        ...
        if ($kukarow["kieli"] != 'fi') {
          ...
          $tapa_row = mysqli_fetch_assoc($tapa_res);
        ...
        if ($tyyppi == "Muistutus") {
          ...
          if ($row["puhno"] != '' and mb_strlen($viesti) > 0 and $sms_palvelin != "" and $sms_user != "" and $sms_pass != "") {
            $ok = 1;

            $teksti = mb_substr("Muistutus $yhtiorow[nimi]. $tapa. ".$viesti, 0, 160);
  ...
  if ($tee == "KORJAAMEMO") {
    ...
    $query = "SELECT *
              ...
              LIMIT 1";
    $res = pupe_query($query);
    $korjrow = mysqli_fetch_array($res);
    ...
    $mvva = mb_substr($korjrow["pvmalku"], 0, 4);
    $mkka = mb_substr($korjrow["pvmalku"], 5, 2);
    $mppa = mb_substr($korjrow["pvmalku"], 8, 2);

    $mhh = mb_substr($korjrow["pvmalku"], 11, 2);
    $mmm = mb_substr($korjrow["pvmalku"], 14, 2);
    ...
    if ($crm_haas_check) {
      ...
      $haas_opp_proj_date_dd = mb_substr($korjrow['kentta05'], 8);
      $haas_opp_proj_date_mm = mb_substr($korjrow['kentta05'], 5, 2);
      $haas_opp_proj_date_yy = mb_substr($korjrow['kentta05'], 0, 4);
...
if ($ytunnus != '' and $tee == '') {
  ...
  if (mb_strpos($_SERVER['SCRIPT_NAME'], "asiakasmemo.php") !== FALSE) {
    ...
    $gurl = $asiakasrow["osoite"].", ".$asiakasrow["postitp"];
    echo js_openUrlNewWindow("http://maps.google.com/?q=$gurl", $asiakasrow["osoite"], "", 1200);
    echo "</td><td>";

    $gurl = $asiakasrow["toim_osoite"].", ".$asiakasrow["toim_postitp"];
    ...
    while ($asosrow = mysqli_fetch_array($asosresult)) {
      $sel2 = '';
  ...
  while ($memorow = mysqli_fetch_assoc($res)) {
    ...
    if ($memorow["perheid"] == 0) {
      ...
      if (mb_strpos($_SERVER['SCRIPT_NAME'], "asiakasmemo.php") !== FALSE and mb_substr($memorow['tyyppi'], 0, 7) != 'DELETED' and empty($memorow["LASKURIVI"])) {
    ...
    if (mb_strpos($_SERVER['SCRIPT_NAME'], "asiakasmemo.php") !== FALSE and $memorow["perheid"] == 0 and ($memorow["tyyppi"] == "Memo" or $memorow["tyyppi"] == "Lead")) {
