...
while ($chk_row = mysqli_fetch_assoc($chk_res)) {
  ...
  if (mysqli_num_rows($chk_res2) == 0 and mysqli_num_rows($chk_res3) == 0) {
    $query = "DELETE FROM lahdot
              WHERE yhtio = '{$kukarow['yhtio']}'
              AND tunnus  = '{$chk_row['tunnus']}'";
    $del_res = pupe_query($query);
  }
  elseif (mysqli_num_rows($chk_res2) == 0 and mysqli_num_rows($chk_res3) != 0) {
    $query = "UPDATE lahdot
              SET aktiivi = 'T'
              WHERE yhtio = '{$kukarow['yhtio']}'
              AND tunnus  = '{$chk_row['tunnus']}'";
...
for ($i = 0; $i <= $paivia_eteenpain; $i++) {
  ...
  while ($t_row = mysqli_fetch_assoc($toimitustavan_lahdot_res)) {
