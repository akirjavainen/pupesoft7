...
if ($tee == 'MONISTA' and mb_strlen($kommentti) < 20 and $kommenttikentta == "P") {
  ...
  foreach ($monistettavat as $lasku_x => $kumpi_x) {
    ...
    if ($kumpi_x == 'HYVITA') {
      ...
      if ($chk_row['clearing'] == 'HYVITYS') {
        ...
        if (mysqli_num_rows($clearing_chk_res) > 0) {

          while ($clearing_chk_row = mysqli_fetch_assoc($clearing_chk_res)) {
        ...
        if ($asiakas_panttitili_chk_row['panttitili'] == "K") {
          ...
          if (mysqli_num_rows($chk_til_res) > 0) {
            while ($chk_til_row = mysqli_fetch_assoc($chk_til_res)) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
      ...
      if ($toim == '') {
        ...
        echo "<input type='checkbox'
                     ...
                     value='on'>" . t('Säilytä laskun verkkolaskutustunnus') . "<br>";
      ...
      if ($toim == '') {
        ...
        $otsikot_row = mysqli_fetch_assoc($otsikot_res);
        ...
        if ($otsikot_row['otsikot'] != '') {
          ...
          if (mysqli_num_rows($nayta_rivit_res) > 0) {
            ...
            $rivilaskuri = mysqli_num_rows($nayta_rivit_res);
            ...
            while ($nayta_rivit_row = mysqli_fetch_assoc($nayta_rivit_res)) {
...
if ($tee == 'MONISTA') {
  ...
  foreach ($monistettavat as $lasku => $kumpi) {
    ...
    for ($monta = 1; $monta <= $kklkm; $monta++) {
      ...
      $monistarow = mysqli_fetch_assoc($monistares);
      ...
      $asiakrow = mysqli_fetch_assoc($asiakres);
      ...
      for ($i = 1; $i < mysqli_num_fields($monistares) - 1; $i++) {
        $fieldname = mysqli_field_name($monistares, $i);
        $fields .= ", ".$fieldname;
        switch ($fieldname) {
        ...
        case 'verkkotunnus':
          if (empty($verkkotunnus_laskulta[$lasku]) and !empty($asiakrow["verkkotunnus"])) {
            // halutaan käyttää asiakkaan nykyistä verkkolaskutunnusta
            $values .= ", '{$asiakrow['verkkotunnus']}'";
          }
          else {
            // halutaan käyttää verkkolaskutunnusta vanhalta laskulta
        ...
        case 'alv':
          ...
          if ($alvik == "on") {
            ...
            $wtrow  = mysqli_fetch_assoc($wtres);
        ...
        case 'vienti_kurssi';
          ...
          if ($kumpi == 'HYVITA' or $kumpi == 'REKLAMA' or ($toim == '' and $kumpi == 'MONISTA' and $korjrahdit == 'on')) {
            ...
            $valrow = mysqli_fetch_assoc($vresult);
      ...
      if ($toim == "TARJOUS" and $monistarow["jaksotettu"] > 0) {
        ...
        if (mysqli_num_rows($sompmonres) > 0) {

          while ($sopmonrow = mysqli_fetch_assoc($sompmonres)) {

            $fields = "yhtio";
            $values = "'{$kukarow['yhtio']}'";

            // Ei monisteta tunnusta
            for ($i = 1; $i < mysqli_num_fields($sompmonres) - 1; $i++) {
              $fieldname = mysqli_field_name($sompmonres, $i);
              $fields .= ", ".$fieldname;
      ...
      if (mysqli_num_rows($monistalisres) > 0) {
        $monistalisrow = mysqli_fetch_assoc($monistalisres);
      ...
      if ($toim == 'TYOMAARAYS' or $koptyom == 'on' or $kumpi == 'REKLAMA') {
        ...
        for ($i = 1; $i < mysqli_num_fields($monistalisres); $i++) {
          $fieldname = mysqli_field_name($monistalisres, $i);
          $fields .= ", ".$fieldname;
	...
	// MUOKKAUS: BUGIKORJAUS:
      ...
      while ($rivirow = mysqli_fetch_assoc($rivires)) {
      ...
      foreach ($_rivit as $rivirow) {
        ...
        if (mysqli_num_rows($presult) == 0) {
          ...
          if (mysqli_num_rows($presult) == 1) {
            $uusipaikka = mysqli_fetch_assoc($presult);
          }
          else {
            ...
            $oletus_tuotepaikka_row = mysqli_fetch_assoc($oletus_tuotepaikka_res);
        ...
        for ($i = 1; $i < mysqli_num_fields($rivires) - 1; $i++) {
          $fieldname = mysqli_field_name($rivires, $i);
          $rfields .= ", ".$fieldname;
          switch ($fieldname) {
	...
	// MUOKKAUS: mysqli_insert_id():
        ...
        if (mysqli_num_rows($monistares2) > 0) {
          ...
          for ($i = 0; $i < mysqli_num_fields($monistares2) - 1; $i++) {
        ...
        if (($kumpi == 'HYVITA' or $kumpi == 'REKLAMA') and $kukarow["yhtio"] == $monistarow["yhtio"]) {
          ...
          while ($sarjarow = mysqli_fetch_assoc($sarjares)) {
            ...
            $sarjatuoterow = mysqli_fetch_assoc($sarjatuoteres);
            ...
            if ($sarjatuoterow["sarjanumeroseuranta"] == "E" or $sarjatuoterow["sarjanumeroseuranta"] == "F" or $sarjatuoterow["sarjanumeroseuranta"] == "G") {
              ...
              if (mysqli_num_rows($sarjares1) == 1) {
                $sarjarow1 = mysqli_fetch_assoc($sarjares1);
      ...
      while ($coprivirow = mysqli_fetch_assoc($copresult)) {
        $query = "UPDATE tilausrivi
      ...
      if ($toim == '' and $kumpi == 'MONISTA' and $korjrahdit == 'on' and $monistarow['laskunro'] > 0 and $yhtiorow['rahti_hinnoittelu'] == '') {
        ...
        while ($rahtirow = mysqli_fetch_assoc($raresult)) {
