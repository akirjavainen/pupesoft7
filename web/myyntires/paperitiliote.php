...
function alku() {
  ...
  $oikpos = $pdf->mb_strlen(t("Summa", $kieli), $pieni);
...
function loppu($firstpage, $summat, $astunnus) {
  ...
  $fcheck_row = mysqli_fetch_assoc($result);
  ...
  if (isset($fcheck_row["factoring_id"])) {
    ...
    $result = pupe_query($query);
    $factoring_row = mysqli_fetch_assoc($result);
...
if ($alatila == "T") {
  ...
  $query = "SELECT group_concat(tunnus) tunnukset
            ...
            and ytunnus = '$ytunnus'";
  $result = pupe_query($query);
  $asiakasrow2 = mysqli_fetch_assoc($result);
...
$result = pupe_query($query);
$laskutiedot = mysqli_fetch_assoc($result);
...
while ($row = mysqli_fetch_assoc($suoritusresult)) {
