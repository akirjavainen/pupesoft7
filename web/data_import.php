...
if (isset($tee) and ($tee == "lataa_tiedosto" or $tee == "poista_file")) {
  ...
  elseif ($tee == "poista_file") {
    ...
    unlink($pupe_root_polku."/datain/".mb_substr($datain_filenimi, 0, -3)."ERR");
  }
}
...
if (isset($tee) and $tee == "poistakaikki_filetsut") {
  if ($files = scandir($pupe_root_polku."/datain")) {
    foreach ($files as $file) {
      ...
      if (mb_substr($file, 0, 11+mb_strlen($kukarow["kuka"])+mb_strlen($kukarow["yhtio"])) == "lue-data#{$kukarow["kuka"]}#{$kukarow["yhtio"]}#" and mb_substr($file, -4) == ".LOG") {
        unlink($pupe_root_polku."/datain/".$file);
        unlink($pupe_root_polku."/datain/".mb_substr($file, 0, -3)."ERR");
      }
    }
  }
}
...
if ($tee == "file" and $laheta != "") {
  ...
  if (isset($_FILES['userfile']) and is_uploaded_file($_FILES['userfile']['tmp_name']) === TRUE) {
    ...
    $kasitellaan_tiedosto_tyyppi = mb_strtoupper($path_parts['extension']);
    ...
    if ($kasitellaan_tiedosto === TRUE and $kasitellaan_tiedosto_tyyppi == "XLS") {
      ...
      if ($return != 0 or mb_strpos($return_string, "CRITICAL") !== FALSE) {
  ...
  if ($kasitellaan_tiedosto === TRUE and $kasitellaan_tiedosto_tyyppi == "DATAIMPORT") {
    ...
    if ($handle = opendir($kasiteltava_filepath)) {
      while (false !== ($file = readdir($handle))) {
        ...
        if (mb_substr($file, 0, mb_strlen($kasiteltava_filenimi)) == $kasiteltava_filenimi) {

          // Jos kyseessä on eka file (loppuu "aa"), ei laiteta headeriä
          if (mb_substr($file, -2) == "aa") {
...
if ($handle = opendir($pupe_root_polku."/datain")) {
  while (false !== ($file = readdir($handle))) {
    ...
    if (mb_substr($file, 0, 9) == "lue-data#" and mb_substr($file, -11) == ".DATAIMPORT") {
      $tiedostoja_jonossa++;

      // Tämä on tämän käyttäjän file
      if (mb_substr($file, 0, 11+mb_strlen($kukarow["kuka"])+mb_strlen($kukarow["yhtio"])) == "lue-data#{$kukarow["kuka"]}#{$kukarow["yhtio"]}#") {
...
echo "<tr><th>".t("Tietokantataulun pakolliset tiedot").":<br>".t("(Näitä tietoja ei voi muuttaa)")."</th>";
echo "<td><ul><li>".mb_strtolower(implode("</li><li>", $pakolliset))."</li></ul></td></tr>";

if (!empty($wherelliset)) {
  echo "<tr><th>".t("Sarakkeet jotka pitää aineistossa kertoa").":</td>";
...
if ($files = scandir($pupe_root_polku."/datain")) {
  foreach ($files as $file) {
    ...
    if (mb_substr($file, 0, 11+mb_strlen($kukarow["kuka"])+mb_strlen($kukarow["yhtio"])) == "lue-data#{$kukarow["kuka"]}#{$kukarow["yhtio"]}#" and mb_substr($file, -4) == ".LOG") {
      ...
      if (mb_strpos($log, "## LUE-DATA-EOF ##") !== FALSE) {
        ...
        if (mb_strpos($taulu, ".") !== FALSE) {
          $cleantaulu = mb_substr($taulu, 0, mb_strpos($taulu, "."));
        }
        else {
          $cleantaulu = $taulu;
        }
        ...
        $kasitelty[$kasitelty_i]["errfilename"] = mb_substr($file, 0, -3)."ERR";
