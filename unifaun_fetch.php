  ...
  while (($file = readdir($handle)) !== FALSE) {
    if (is_file($ftpget_dest[$operaattori]."/".$file)) {
      ...
      while ($rivi = fgets($fh)) {
        ...
        if (is_numeric($sscc_ulkoinen) and mb_stripos($laskurow["toimitustapa"], "DPD") === FALSE) {
        ...
        if ($yhtiorow['kerayserat'] == 'K') {
          ...
          if (mysqli_num_rows($sscc_ulkoinen_chk_res) == 1) {

            $sscc_ulkoinen_chk_row = mysqli_fetch_assoc($sscc_ulkoinen_chk_res);
          ...
          if (!empty($eranumero_sscc)) {
            ...
            if (!empty($rakir_row['tunnus'])) {
              ...
              $lasku_row = mysqli_fetch_assoc($lasku_res);
        ...
        $tunnukset_row = mysqli_fetch_assoc($tunnukset_res);
      ...
      foreach ($otunnukset_arr as $key => $otunnukset) {
        ...
        $_desadv = (mb_strpos($laskurow['toimitusvahvistus'], 'desadv') !== false);
