...
// MUOKKAUS: isset():
foreach (array("perusta_tilaustyyppi", "projekti", "jarjlisa", "yt", "postitp", "kantaasiakastunnus", "ytunnus", "kateinen", "projektilask", "rivitunnus", "kplmaara", "myyjanro") as $v) {
  if (!isset(${$v})) ${$v} = null;
}
...
if ($tila == "YHENKPUHELIN") {
  ...
  if ($yrow = mysqli_fetch_assoc($yres)) {
...
if (mb_strpos(dirname(__FILE__), "/tilauskasittely") !== FALSE) {
...
elseif (!isset($tilausrivi_alvillisuus) && !isset($_POST["tilausrivi_alvillisuus"])) { // MUOKKAUS: isset()
...
if ((int) $valitsetoimitus > 0 and $valitsetoimitus != $tilausnumero) {
  ...
  $result = pupe_query($query);
  $toimrow = mysqli_fetch_assoc($result);
  ...
  if ($toimrow["tila"] == "A" or (($toimrow["tila"] == "L" or $toimrow["tila"] == "N") and $toimrow["tilaustyyppi"] == "A")) {
    $toim = (mb_strtolower($asentaja) == 'tyomaarays_asentaja' or $toim == 'TYOMAARAYS_ASENTAJA') ? "TYOMAARAYS_ASENTAJA" : "TYOMAARAYS";
  }
...
if (!aktivoi_tilaus($tilausnumero, $session, $orig_tila, $orig_alatila)) {
  ...
  elseif ($toim == "VALMISTAASIAKKAALLE" and $tilausnumero != "") {
    ...
    if (mysqli_num_rows($tyyppiresult) != 0) {
      $tyyppirow = mysqli_fetch_assoc($tyyppiresult);

      if (mb_strtoupper($tyyppirow['tilaustyyppi']) == 'W') {
        $toim = "VALMISTAVARASTOON";
      }
    }
    else {
...
if ($kukarow["extranet"] != '') {
  ...
  if (mysqli_num_rows($result) == 1) {
    $extra_asiakas = mysqli_fetch_assoc($result);
    ...
    if ($kukarow["kesken"] > 0) {
      ...
      if (mysqli_num_rows($result) > 0) {
        $kesken_row = mysqli_fetch_assoc($result);
...
if ((int) $valitsetoimitus_vaihdarivi > 0 and $tilausnumero == $kukarow["kesken"] and $kukarow["kesken"] > 0 and ($toim != "TARJOUS" and $toim != "EXTTARJOUS")) {
  ...
  if (mysqli_num_rows($result) > 0) {
    ...
    if ($yhtiorow["splittauskielto"] != 'K') {
      ...
      $ajat = mysqli_fetch_assoc(pupe_query($ajat_query));

      $aikalisa = ", kerayspvm = '{$ajat["kerayspvm"]}', toimaika = '{$ajat["toimaika"]}'";
    }

    while ($aburow = mysqli_fetch_assoc($result)) {
...
if ((int) $kukarow["kesken"] > 0) {
  ...
  $asiakasrow = mysqli_fetch_assoc($a_res);
  ...
  if ($laskurow["valkoodi"] != '' and trim(mb_strtoupper($laskurow["valkoodi"])) != trim(mb_strtoupper($yhtiorow["valkoodi"])) and $laskurow["vienti_kurssi"] != 0 and $yhtiorow["suoratoim_ulkomaan_alarajasumma"] > 0) {
...
$check  = true;
if (isset($laskurow)) { // MUOKKAUS: isset()
...
if (isset($laskurow["tunnusnippu"]) and $laskurow["tunnusnippu"] > 0 and $toim == "PROJEKTI") {
  ...
  $projektilask = (int) mysqli_num_rows($abures);
}
...
if ($kukarow["extranet"] == "" and $tee == "HYLKAATARJOUS" and $muokkauslukko == "") {
  ...
  if (mysqli_num_rows($abures) > 0) {
    while ($row = mysqli_fetch_assoc($abures)) {
...
if (in_array($jarjesta, array("moveUp", "moveDown")) and $rivitunnus > 0) {
  ...
  // MUOKKAUS: lisatty tilausrivitunnus kyselyyn:
...
if ($sahkoinen_lahete and $kukarow["extranet"] == "" and (int) $kukarow['kesken'] > 0 and !empty($laskurow)) {
  ...
  if (mysqli_num_rows($as_avain_chk_res) > 0) {
...
if ($tee == 'POISTA' and $muokkauslukko == "" and $kukarow["mitatoi_tilauksia"] == "" and (int) $kukarow['kesken'] > 0) {
  ...
  if (mysqli_num_rows($jt_rivien_muisti_res) > 0) {
    $jt_saldo_lisa = $yhtiorow["varaako_jt_saldoa"] == "" ? ", jt = varattu, varattu = 0 " : '';

    while ($jt_rivien_muisti_row = mysqli_fetch_assoc($jt_rivien_muisti_res)) {
      $query = "UPDATE tilausrivi SET
  ...
  if ($sahkoinen_lahete) {
    ...
    $chk_toimipaikka_row = mysqli_fetch_assoc($chk_toimipaikka_res);
    ...
    if ($chk_toimipaikka_row['yhtio_toimipaikka'] != 0) {
      ...
      if (mysqli_num_rows($toimipaikat_res) != 0) {
        ...
        $toimipaikat_row = mysqli_fetch_assoc($toimipaikat_res);
...
if ($tee == "VALMIS"
  ...
  elseif (!isset($kassamyyja_kesken) and !isset($seka) and $laskurow['tilaustyyppi'] != 'W') {
    ...
    if (mysqli_num_rows($maksuehtores) > 1) {
      echo "<font class='head'>$otsikko</font><hr><br>";
      echo "<table><tr><th>" . t("Maksutapa") . ":</th>";

      while ($maksuehtorow = mysqli_fetch_assoc($maksuehtores)) {
        echo "<form method='post' action='{$palvelin2}{$tilauskaslisa}tilaus_myynti.php'>";
...
if ($tee == 'PALAUTA_SIIVOTUT' and $kukarow['extranet'] != '') {
  ...
  while ($palauta_siivotut_row = mysqli_fetch_assoc($palauta_siivotut_res)) {
...
if ($tee == 'VALMIS' and $kukarow['extranet'] != '') {
  ...
  while ($varattu_check_row = mysqli_fetch_assoc($varattu_check_res)) {
...
if ($tee == "VALMIS" and ($muokkauslukko == "" or $toim == "PROJEKTI")) {
  ...
  if ($kukarow["extranet"] == "" and $toim == "TARJOUS") {
    ...
    if ($laskurow["tunnusnippu"] > 0) {
      ...
      $tarjous = $laskurow["tunnusnippu"]."/".mysqli_num_rows($result);
    }
    else {
  ...
  // Myyntitilaus valmis
    ...
    if ($kukarow['extranet'] != '' and ($kukarow['saatavat'] == 0 or $kukarow['saatavat'] == 2)) {
      ...
      $saarow = mysqli_fetch_assoc($saaresult);
    ...
    if ($kukarow["tilaus_valmis"] == "2") {
      ...
      if ($tm_laskurow_talteen["tilaustyyppi"] == "W") {
        ...
        if ($laskurow = mysqli_fetch_assoc($result)) {
...
if (!isset($laskurow) or !is_array($laskurow)) $laskurow = array(); // MUOKKAUS: isset()
if (!isset($laskurow['alatila'])) $laskurow['alatila'] = null; // MUOKKAUS: isset()
...
if ($kukarow["extranet"] == "" and $toim == 'REKLAMAATIO'
  ...
  if ($_tilaustyyppi and trim($laskurow['tilausvahvistus']) != ""
    and (mb_strpos($laskurow['tilausvahvistus'], 'S') !== FALSE or mb_strpos($laskurow['tilausvahvistus'], 'O') !== FALSE)) {
...
if ($kukarow["extranet"] == "" and $toim == 'REKLAMAATIO'
  ...
  if (mysqli_num_rows($varasto_chk_res) == 0) {
    $query = "UPDATE lasku set
              tila        = 'L',
              alatila     = 'D'
              WHERE yhtio = '$kukarow[yhtio]'
...
if ($kukarow["extranet"] == "" and ($tee == "OTSIK" or ($toim != "PIKATILAUS" and !isset($laskurow["liitostunnus"])))) {
  ...
  $result = pupe_query($query);
  $laskurow = mysqli_fetch_assoc($result);
  $laskurow_liitostunnus = isset($laskurow) ? $laskurow['liitostunnus'] : '';

  $a_qry = "SELECT *
            FROM asiakas
            WHERE yhtio = '{$kukarow['yhtio']}'
            AND tunnus  = '$laskurow_liitostunnus'";
  $a_res = pupe_query($a_qry);
  $asiakasrow = mysqli_fetch_assoc($a_res);
...
if ($tee == '') {
  ...
  if (isset($laskurow['mapvm']) and isset($laskurow['chn'])) { // MUOKKAUS: isset()
  ...
  if ($kukarow["kesken"] > 0 and (
    ...
    if (mysqli_num_rows($meapu) == 1 and $toimitustapa != '') {
      ...
      if ($meapurow["kateinen"] != "") {
        ...
        $meapu2row = mysqli_fetch_assoc($meapu2);
  ...
  if (isset($laskurow["liitostunnus"]) and $laskurow["liitostunnus"] <> 0) { // MUOKKAUS: isset()
    ...
    if (mysqli_num_rows($result)==1) {
      $maksuehtorow = mysqli_fetch_assoc($result);
    ...
    if ($kukarow["extranet"] == "" and $saako_liitaa_laskuja_tilaukseen == "") {
      ...
      if (mysqli_num_rows($keikkares) == 1) {
    ...
    if (tarkista_oikeus('jtselaus.php')) {
      ...
      $jtapurow = mysqli_fetch_assoc($jtapuresult);
    ...
    if ($toim == "TYOMAARAYS" or $toim == "TYOMAARAYS_ASENTAJA" or $toim == "REKLAMAATIO") {
      ...
      if (tarkista_oikeus("tyom_tuntiraportti.php")) {
        echo "<form method='POST' action='{$palvelin2}raportit/tyom_tuntiraportti.php'>
            ...
            <input type='hidden' name='vva' value='".mb_substr($laskurow['luontiaika'], 0, 4)."'>
            <input type='hidden' name='kka' value='".mb_substr($laskurow['luontiaika'], 5, 2)."'>
            <input type='hidden' name='ppa' value='".mb_substr($laskurow['luontiaika'], 8, 2)."'>
            <input type='hidden' name='vvl' value='".mb_substr($laskurow['luontiaika'], 0, 4)."'>
            <input type='hidden' name='kkl' value='".mb_substr($laskurow['luontiaika'], 5, 2)."'>
            <input type='hidden' name='ppl' value='".mb_substr($laskurow['luontiaika'], 8, 2)."'>
  ...
  $tilausok = 0;
  $sarjapuuttuu = 0;
  $laskurow_maksuehto = isset($laskurow["maksuehto"]) ? $laskurow["maksuehto"] : 0; // MUOKKAUS: isset()

  $apuqu = "SELECT * from maksuehto where yhtio='$kukarow[yhtio]' and tunnus='$laskurow_maksuehto'";
  $meapu = pupe_query($apuqu);
  $meapurow = mysqli_fetch_assoc($meapu);
  ...
  // MUOKKAUS: isset():
  if (isset($laskurow["liitostunnus"]) and $kukarow['extranet'] == '' and ($laskurow["liitostunnus"] != 0 or ($laskurow["liitostunnus"] == 0 and $kukarow["kesken"] > 0 and $toim != "PIKATILAUS"))) {
  ...
  // jos asiakasnumero on annettu
  // MUOKKAUS: isset():
  if (isset($laskurow["liitostunnus"]) and ($laskurow["liitostunnus"] != 0 or ($laskurow["liitostunnus"] == 0 and $kukarow["kesken"] > 0 and $toim != "PIKATILAUS"))) {
    ...
    if ($toim == "VALMISTAVARASTOON") {
      ...
      $tm_toimitustaparow = mysqli_fetch_assoc($tresult);
    }
    else {
	...
	// MUOKKAUS: isset():
      ...
      $onvastuumyyja   = mysqli_num_rows($vastuumyyja_result) !== 0;
      ...
      if (mysqli_num_rows($rakre_chkres) > 0) {
        $state_chk = 'disabled';
      }
      ...
      $params = array(
        ...
        'varasto_toimipaikka' => is_array($_varasto) ? $_varasto['toimipaikka'] : '', // MUOKKAUS: isset()
      ...
      foreach ($toimitustavat as $toimitustapa) {
        ...
        if (!empty($toimitustapa['sallitut_maat']) and !mb_stristr($toimitustapa['sallitut_maat'], $laskurow['toim_maa'])) {
          continue;
        }
    ...
    if ($laskurow["tunnusnippu"] > 0) {
      ...
      if (mysqli_num_rows($toimres) > 0) {
        ...
        while ($row = mysqli_fetch_assoc($toimres)) {
    ...
    if ($kukarow["extranet"] == "") {
      ...
      if (mysqli_num_rows($liiteres) > 0) {
        ...
        while ($liiterow = mysqli_fetch_array($liiteres)) {
      ...
      if ($toim == 'TYOMAARAYS' or $toim == "TYOMAARAYS_ASENTAJA") {
        ...
        if (mysqli_num_rows($liiteres) > 0) {
          ...
          while ($liiterow = mysqli_fetch_array($liiteres)) {
            ...
            $asekal_atstamp = mktime(mb_substr($asekal_alku, 11, 2), mb_substr($asekal_alku, 14, 2), 0, mb_substr($asekal_alku, 5, 2), mb_substr($asekal_alku, 8, 2), mb_substr($asekal_alku, 0, 4));
            $asekal_ltstamp = mktime(mb_substr($asekal_loppu, 11, 2), mb_substr($asekal_loppu, 14, 2), 0, mb_substr($asekal_loppu, 5, 2), mb_substr($asekal_loppu, 8, 2), mb_substr($asekal_loppu, 0, 4));

            $kaletunnit[$nimi] += ($ltstamp - $atstamp)/60;

            if ($toim == 'TYOMAARAYS' or $toim == "TYOMAARAYS_ASENTAJA") {

              if ($asekal_distinct_chk[$asekal_kuka][$laskurow['tunnus']] == $liiterow['pvmalku_sort'] and mb_substr($asekal_alku, 5, 2).mb_substr($asekal_alku, 8, 2).mb_substr($asekal_alku, 0, 4) == mb_substr($asekal_loppu, 5, 2).mb_substr($asekal_loppu, 8, 2).mb_substr($asekal_loppu, 0, 4)) {
                continue;
              }
              ...
              if ($kukarow['kuka'] == $asekal_kuka) {
                ...
                $query = "SELECT right(pvmalku, 8) pvmalku, right(pvmloppu, 8) pvmloppu
                          ...
                          AND pvmalku  like '".mb_substr($asekal_alku, 0, 4)."-".mb_substr($asekal_alku, 5, 2)."-".mb_substr($asekal_alku, 8, 2)."%'
                ...
                while ($tunti_chk_row = mysqli_fetch_assoc($tunti_chk_res)) {
              ...
              if (mb_substr($asekal_alku, 5, 2).mb_substr($asekal_alku, 8, 2).mb_substr($asekal_alku, 0, 4) != mb_substr($asekal_loppu, 5, 2).mb_substr($asekal_loppu, 8, 2).mb_substr($asekal_loppu, 0, 4)) {
                ...
                $query = "SELECT right(pvmalku, 8) pvmalku, right(pvmloppu, 8) pvmloppu
                          ...
                          AND kuka     = '$kukarow[kuka]'
                          AND kentta02 = '$laskurow[tunnus]'
                          AND pvmloppu like '".mb_substr($asekal_loppu, 0, 4)."-".mb_substr($asekal_loppu, 5, 2)."-".mb_substr($asekal_loppu, 8, 2)."%'
                          AND tyyppi   = 'kalenteri'";
                $tunti_chk_res = pupe_query($query);
      ...
      if (($toim == "TARJOUS" or $toim == "EXTTARJOUS")) {
        ...
        if (mysqli_num_rows($kaleresult) > 0) {
          ...
          while ($kalerow = mysqli_fetch_assoc($kaleresult)) {
            ...
            echo "<option value='$kalerow[tunnus]' $sel>".mb_substr($kalerow["kentta01"], 0, 60)."</option>";
          }
  ...
  // MUOKKAUS: isset():
  if (isset($laskurow['tila']) and $laskurow['tila'] == 'N' and $laskurow['alatila'] == 'F' and $laskurow['sisviesti3'] != '') {

    echo "<br>";

    echo "<table>";

    if (mb_strpos($laskurow['sisviesti3'], '|||') !== false) {
  ...
  if ($smsnumero != "" and mb_strlen("smsviesti") > 0) {

    if (mb_strlen($smsviesti) > 160) {
    ...
    if ($smsnumero != '' and mb_strlen($smsviesti) > 0 and mb_strlen($smsviesti) < 160 and $sms_palvelin != "" and $sms_user != "" and $sms_pass != "") {
  ...
  if ($kukarow["extranet"] == "" and ($tila == "LISATIETOJA_RIVILLE" or $tila == "ASPOSITIO_RIVILLE" or $tila == "PALAUTUSVARASTO") and (int) $kukarow["kesken"] > 0) {
    ...
    while ($lapsi = mysqli_fetch_assoc($lapsires)) {
      //  Päivitetään positio
  ...
  if ($tila == 'MUUTA' and (int) $kukarow["kesken"] > 0) {
    ...
    foreach ($rivitunnukset_array as $rivitunnus) {
      ...
      if (mysqli_num_rows($result) == 1) {
        ...
        $tilausrivi = mysqli_fetch_assoc($result);
        ...
        if ($tilausrivi["sarjanumeroseuranta"] != '') {
          ...
          if (isset($myy_sarjatunnus) and $myy_sarjatunnus != "") {
            ...
            $m_eranro = mysqli_fetch_assoc(pupe_query($query));
          }
          ...
          if ($yhtiorow['laiterekisteri_kaytossa'] != '') {
            ...
            $spessurivi = mysqli_fetch_assoc($spessures);
        ...
        elseif ($tapa != "POISJTSTA" and $tapa != "PUUTE" and $tapa != "JT") {
          if ($tapa == "POISTA") {
            ...
            $query = "SELECT tilausrivilinkki
                      ...
                      AND tilausrivitunnus = '{$rivitunnus}'";
            $result = pupe_query($query);
            $_myyntirivi = mysqli_fetch_assoc($result);

            if (mysqli_num_rows($result) == 1 and $_myyntirivi["tilausrivilinkki"] != 0) {
              ...
              $query = "DELETE tilausrivi.*
                        ...
                        AND tilausrivi.kpl = 0";
              $ostotilaus_tarkistus = mysqli_fetch_assoc(pupe_query($query));
        ...
        if (mb_substr($tapa, 0, 6) != "VAIHDA" and $tilausrivi["var"] == "T" and mb_substr($paikka, 0, 3) != "¡¡¡") {
          $paikka = "¡¡¡".$tilausrivi["toimittajan_tunnus"];
        }

        if (mb_substr($tapa, 0, 6) != "VAIHDA" and $tilausrivi["var"] == "U" and mb_substr($paikka, 0, 3) != "!!!") {
          $paikka = "!!!".$tilausrivi["toimittajan_tunnus"];
        }
        ...
        if ($yhtiorow['myyntitilausrivi_rekisterinumero'] == 'K' and mb_stristr($kommentti, $tilausrivi['rekisterinumero'])) {
        ...
        elseif ($tapa == "VAIHDAJAPOISTA") {
          $perheid = "";
          $tila    = "";
          if (mb_substr($paikka, 0, 3) != "!!!" and mb_substr($paikka, 0, 3) != "¡¡¡") $paikka = "";
        }
        ...
        if ($tila == "VARMUUTOS" and in_array($tapa, array("POISJTSTA","PUUTE","JT"))) {
          ...
          // niin tehdään samat jutu myös perheen lapsille
            ...
            while ($muutettavarivi = mysqli_fetch_assoc($mriviresult)) {
  ...
  if ($_ei_extranet) {
    ...
    if ($_varastoon and $_tila_check and empty($perheid)) {
      ...
      if ($valmiste_vai_raakaaine == 'valmiste' and !empty($tuoteno)) {
        ...
        if (mysqli_num_rows($tuoteperhe_chk_res) == 0) {
          $perheid2 = -100;
        }
      }
    }
  ...
  if ((trim($tuoteno) != '' or is_array($tuoteno_array)) and ($kpl != '' or is_array($kpl_array)) and ($tila != "MUUTA" and $tila != "VARMUUTOS") and $ulos == '' and ($variaatio_tuoteno == "" or (is_array($kpl_array) and array_sum($kpl_array) != 0))) {
    ...
    $kayttajan_netto   = mb_strtoupper(trim($netto));
    $kayttajan_var    = $var;
    $kayttajan_kpl    = $kpl;
    $kayttajan_alv    = $alv;
    ...
    if ($yhtiorow["reklamaation_hinnoittelu"] == "K" and ($toim == "REKLAMAATIO" or $toim == "EXTRANET_REKLAMAATIO") and $kayttajan_var != "H") {
      ...
      foreach ($palautus as $index => $arvot) {
        ...
        if (mb_stripos($kommentti, $palautus[$index]["kommentti"]) === FALSE) $hyvityssaanto_kommentti_array[$index][$tuoteno] = $palautus[$index]["kommentti"];
    ...
    foreach ($tuoteno_array as $tuoteno) {
      ...
      if ($toim != "YLLAPITO") {
        ...
        if ($kerayspvm == "" or $kerayspvm == "0000-00-00") {
          $kerayspvm = mb_substr($laskurow["kerayspvm"], 0, 10);
        }
      }
      ...
      if (is_array($netto_array)) {
        $netto = mb_strtoupper(trim($netto_array[$tuoteno]));
      }
      else {
      ...
      if (mysqli_num_rows($result) > 0) {
        ...
        if ($kukarow["extranet"] != '' and trim($trow["vienti"]) != '') {
          ...
          if (mb_strpos(mb_strtoupper($trow["vienti"]), mb_strtoupper("+$laskurow[toim_maa]")) !== FALSE and mb_strpos($trow["vienti"], "+") !== FALSE) {
      ...
      if ($tuoteno != '') { // MUOKKAUS: poistettu "and $kpl != 0" jotta sallitaan < 0 kpl (hyvitys)
    ...
    if (    $lisatty_tun > 0
      ...
      if ($suoratoimrow = mysqli_fetch_assoc($suoratoimresult)) {
    ...
    if ($tapa == "VAIHDA" and $perheid2 > 0 and $kayttajan_paikka != "" and mb_substr($kayttajan_paikka, 0, 3) != "¡¡¡" and mb_substr($kayttajan_paikka, 0, 3) != "!!!") {
  ...
  // MUOKKAUS: isset():
  $laskurow_tunnus = isset($laskurow['tunnus']) ? $laskurow['tunnus'] : 0;
  $logmaster_errors = logmaster_verify_order($laskurow_tunnus, $toim);
  ...
  $smarten_errors = smarten_verify_order($laskurow_tunnus, $toim);
  ...
  if ((int) $kukarow["kesken"] > 0) {
    ...
    $numres_saatavt = mysqli_num_rows($numres);
  }
  ...
  $_asiakas = (isset($laskurow['liitostunnus']) and $laskurow['liitostunnus'] > 0); // MUOKKAUS: isset()
  $_mika_toim = in_array($toim, array("RIVISYOTTO", "PIKATILAUS", "ENNAKKO", "EXTENNAKKO", "VALMISTAASIAKKAALLE"));
  $_mika_toim = (isset($laskurow['clearing']) and $_mika_toim and $laskurow['clearing'] != 'HYVITYS'); // MUOKKAUS: isset()

  $_luottoraja_ylivito = false;

  $_keratty_toimitettu  = (isset($laskurow["tila"]) and $laskurow["tila"] == "L" and in_array($laskurow["alatila"], array("B", "C", "D", "E"))); // MUOKKAUS: isset()
  ...
  if ($_kukaextranet and $_kat_jv and $_asiakas and $_saako and $_mika_toim) {
    ...
    $_tilauksen_rivihinnat_row = mysqli_fetch_assoc($_tilauksen_rivihinnat_res);
  ...
  elseif ($_mika_toim and $_kat_jv) {
    ...
    if ((float) $laskurow['luottoraja'] != 0) {
      if ((float) $asiakasrow['luottoraja'] != 0) {
        ...
        $tilauksen_rivihinnat_row = mysqli_fetch_assoc($tilauksen_rivihinnat_res);
  ...
  if ($toim == "REKLAMAATIO") {
    ...
    if (mysqli_num_rows($tpares) > 0) {
      ...
      while ($trrow = mysqli_fetch_assoc($tpares)) {
        $toimpalautusasiakkat .= $trrow['tunnus'].",";
      }

      $toimpalautusasiakkat = mb_substr($toimpalautusasiakkat, 0, -1);
    }
      ...
      while ($tuote = mysqli_fetch_assoc($result)) {
        ...
        if ($yhtiorow['naytetaanko_ale_peruste_tilausrivilla'] != '') {
          ...
          if (isset($ale_peruste) and !empty($ale_peruste) and $haettu_alehinta > 1 and $yhtiorow['naytetaanko_ale_peruste_tilausrivilla'] == 'o') {
            echo "<tr><th>{$ap_font}".mb_substr($ale_peruste, 0, mb_strpos($ale_peruste, "Hinta: "))."</font></th><td align='right'>{$ap_font}".hintapyoristys($haettu_alehinta[0])." $yhtiorow[valkoodi]</font></td></tr>";
          }
        ...
        while ($cur_date >= $date_2yo) {
          ...
          if (mysqli_num_rows($viimhintares) != 0) {
            $viimhinta = mysqli_fetch_assoc($viimhintares);
        ...
        if ($trow["ei_saldoa"] == "") {
          ...
          // MUOKKAUS: isset():
          if (isset($row) and ($row["sarjanumeroseuranta"] == "E" or $row["sarjanumeroseuranta"] == "F" or $row["sarjanumeroseuranta"] == "G")) {
          ...
          if (mysqli_num_rows($varresult) > 0 && isset($row)) { // MUOKKAUS: isset()
            ...
            while ($saldorow = mysqli_fetch_assoc($varresult)) {
              ...
              if ($sallittu === TRUE) {
                ...
                if ($myytavissa != 0) {
                  ...
                  // MUOKKAUS: isset():
                  if (isset($row)) echo sprintf("%.2f", $myytavissa)." ".t_avainsana("Y", "", " and avainsana.selite='$row[yksikko]'", "", "", "selite");
        ...
        if ($toim == "REKLAMAATIO" and $toimpalautusasiakkat != "") {
          ...
          if (mysqli_num_rows($abures) > 0) {
            while ($trrow = mysqli_fetch_assoc($abures)) {
        ...
        if (in_array($toim, array('RIVISYOTTO', 'PIKATILAUS', 'REKLAMAATIO'))) {
          ...
          while ($cur_date >= $date_2yo) {
            ...
            if (mysqli_num_rows($tapahtuma_chk_res) > 0) {

              while ($tapahtuma_chk_row = mysqli_fetch_assoc($tapahtuma_chk_res)) {
  ...
  if ((int) $kukarow["kesken"] > 0) {
    ...
    if ($kukarow['extranet'] != '' and $laskurow['rahtivapaa'] == '' and $asiakasrow['rahtivapaa'] == '' and ($asiakasrow['rahtivapaa_alarajasumma'] != 0 or $yhtiorow['rahtivapaa_alarajasumma'] != 0)) {
      ...
      $tilriv_chk_row = mysqli_fetch_assoc($tilriv_chk_res);
    ...
    elseif ($toim == "YLLAPITO") {
      ...
      if (mysqli_num_rows($dynamic_result) > 0) {
        ...
        while ($drow = mysqli_fetch_assoc($dynamic_result)) {
        ...
        $laskentalisa_riveille = mb_substr($laskentalisa_riveille, 0, -3).") ";
      }
      else {
    ...
    $ruuturow = mysqli_fetch_assoc($ruuturesult);
    ...
    while ($vakrow = mysqli_fetch_assoc($result)) {
    ...
    if (count($vak_chk_array) > 0) {
      ...
      elseif ($kukarow['extranet'] == '') {
        ...
        if ($tm_toimitustaparow['vak_kielto'] != '' and $tm_toimitustaparow['vak_kielto'] != 'K') {
          ...
          if (mysqli_num_rows($vak_check_res) == 1) {
            $query = "UPDATE lasku SET
    ...
    if ($kukarow["extranet"] == "" and $toim == "REKLAMAATIO" and
      ...
      while ($varasto_chk_row = mysqli_fetch_assoc($result)) {
    ...
    if ($toim == "TARJOUS" or $toim == "EXTTARJOUS" or $toim == "TYOMAARAYS" or $toim == "TYOMAARAYS_ASENTAJA" or $laskurow["tilaustyyppi"] == "T" or $kukarow["yhtio"] == "savt") {
      ...
      if (mysqli_num_rows($trivityyppi_result) > 0) {
    ...
    if ($yhtiorow["extranet_nayta_saldo"] == 'X' and $kukarow['extranet'] != '') {
      ...
      $extranet_saldo_varjays = mysqli_fetch_assoc($avainsana_result);
    }
    ...
    if ($toim != "VALMISTAVARASTOON" and $toim != "SIIRTOLISTA") {
      ...
      if ($kukarow['hinnat'] == 0) {
        ...
        $headerit = mb_substr($headerit, 0, -3);
        $headerit .= "</th>";
        $sarakkeet++;
    ...
    if ($rivilaskuri > 0) {
          ...
          if (in_array($toim, array('RIVISYOTTO', 'PIKATILAUS')) and !empty($v_toimipaikka) and $varasto['tyyppi'] != 'E') {
            ...
            if ($_toimipaikan_kerayserat_mittatiedot and $tarvii_lahdon and $_toimitustapa) {
              ...
              if (!empty($toimitustapa['tunnus'])) {
                ...
                if (mysqli_num_rows($lahdot_result) == 0) {
      ...
      if ($kukarow["extranet"] == "") {
        ...
        if ($tarkrow = mysqli_fetch_assoc($tarkres)) {
	...
	if (isset($yhtiorow['active_css']) and isset($varitmatch)) { // MUOKKAUS: isset()
      ...
      foreach ($rows as $row) {
        ...
        if ($kukarow['extranet'] != '' and $row['tunnus'] != $row['perheid'] and mb_strtoupper($row['var']) == 'P' and $row['perheid'] != 0) {
        ...
        if (mb_strpos($row['sorttauskentta'], 'ZZZZZZZZZZZZZZZZZZZZZZ') !== FALSE) {
	...
	if (isset($asiakasrow)) {
        ...
        if ($muokkauslukko_rivi == "" and empty($_etukateen_maksettu) and $yhtiorow["tilauksen_jarjestys"] == "M" and in_array($toim, array("TARJOUS", "EXTTARJOUS", "PIKATILAUS", "RIVISYOTTO", "VALMISTAASIAKKAALLE", "SIIRTOLISTA", "TYOMAARAYS", "TYOMAARAYS_ASENTAJA", "REKLAMAATIO", "PROJEKTI"))) {
          ...
          if (($rivino > 1 and $yhtiorow["tilauksen_jarjestys_suunta"] == "ASC") or ($rivino < $rivilaskuri and $yhtiorow["tilauksen_jarjestys_suunta"] == "DESC")) {
            $buttonit .= "  <a href='#' onClick=\"getElementById('rivi_$rivino').value='moveUp'; document.forms['siirra_$rivino'].submit(); fixRowNumbering('$yhtiorow[yhtio]', '$laskurow[tunnus]', '$toim');\"><img src='".$palvelin2."pics/lullacons/arrow-single-up-green.png' height = '20' width = '20'></a><br>";
          }
          ...
          if (($rivilaskuri > $rivino and $yhtiorow["tilauksen_jarjestys_suunta"] == "ASC") or ($rivino > 1 and $yhtiorow["tilauksen_jarjestys_suunta"] == "DESC")) {
            $buttonit .= "  <a href='#' onClick=\"getElementById('rivi_$rivino').value='moveDown'; document.forms['siirra_$rivino'].submit(); fixRowNumbering('$yhtiorow[yhtio]', '$laskurow[tunnus]', '$toim');\"><img src='".$palvelin2."pics/lullacons/arrow-single-down-red.png' height = '20' width = '20'></a>";
          }
        ...
        if ($tilauksen_jarjestys != 'M' and $tuoteperhe_kayty != $row['perheid'] and (($row['perheid'] != 0 and ($tilauksen_jarjestys == '1' or $tilauksen_jarjestys == '5' or ($tilauksen_jarjestys == '4' or $tilauksen_jarjestys == '0' and $erikoistuote_tuoteperhe[$row['perheid']] == $row['sorttauskentta']))) or $row["perheid"] == $row["tunnus"] or ($row["perheid2"] == $row["tunnus"] and $row["perheid"] == 0) or ($row["perheid2"] == -1 or ($row["perheid"] == 0 and $row["perheid2"] == 0 and ($row["var"] == "T" or $row["var"] == "U"))))) {
          ...
          if (($toim != "TARJOUS" and $toim != "EXTTARJOUS")) {
            if ($muokkauslukko_rivi == "" and $row["toimitettuaika"] == '0000-00-00 00:00:00' and $row["uusiotunnus"] == 0 and $laskurow["tunnusnippu"] > 0 and $yhtiorow["splittauskielto"] != "K") {
              ...
              if (mysqli_num_rows($toimres) > 1) {
                ...
                while ($toimrow = mysqli_fetch_assoc($toimres)) {
                  $sel = "";
          ...
          if ($tilauksen_jarjestys != '0' and $tilauksen_jarjestys != '1' and $tilauksen_jarjestys != '4' and $tilauksen_jarjestys != '5' and $tilauksen_jarjestys != '8') {
            ...
            if (($yhtiorow["salli_jyvitys_myynnissa"] == "V" and $kukarow['jyvitys'] == 'S') or $yhtiorow["salli_jyvitys_myynnissa"] == "S") {
              ...
              $perhe_row = mysqli_fetch_assoc($perhe_result);
        ...
        elseif ($row["perheid"] == 0 and $row["perheid2"] == 0 or ($tilauksen_jarjestys != '0' and $tilauksen_jarjestys != '1' and $tilauksen_jarjestys != '4' and $tilauksen_jarjestys != '5' and $tilauksen_jarjestys != '8') or (($tilauksen_jarjestys == '0' or $tilauksen_jarjestys == '4') and $erikoistuote_tuoteperhe[$row['perheid']] == $row['sorttauskentta'] and $tuoteperhe_kayty != $row['perheid'])) {
          ...
          if ($kommenttirivi_nakyviin or $row["kommentti"] != "" or ($row["ale_peruste"] != '' and $yhtiorow['naytetaanko_ale_peruste_tilausrivilla'] != '') or (isset($GLOBALS['eta_yhtio']) and $GLOBALS['eta_yhtio'] != '' and $koti_yhtio == $kukarow['yhtio'])) {
            ...
            echo "<td $class rowspan = '2'>$echorivino_id" . $echorivino; // MUOKKAUS: lisatty #id
          ...
          elseif ($tilauksen_jarjestys == '1' and $row['perheid'] != 0) {
            ...
            echo "<td $class>$echorivino_id" . $echorivino; // MUOKKAUS: lisatty #id
        ...
        if ($toim == "TARJOUS" or $toim == "EXTTARJOUS" or $toim == "TYOMAARAYS" or $toim == "TYOMAARAYS_ASENTAJA" or $laskurow["tilaustyyppi"] == "T" or $kukarow["yhtio"] == "savt") {
          if ($muokkauslukko_rivi == "" and $row["ei_nayteta"] == "") {
            if (mysqli_num_rows($trivityyppi_result) > 0) {
              ...
              mysqli_data_seek($trivityyppi_result, 0);

              $trivityyulos .= "<option value=''>".t("Valitse")."</option>";

              while ($trrow = mysqli_fetch_assoc($trivityyppi_result)) {
                $sel = "";
          ...
          elseif (mysqli_num_rows($trivityyppi_result) > 0) {
            $trivityyulos = "&nbsp;";
          }
        }
        ...
        if ($toim == "REKLAMAATIO") {
          ...
          elseif (mysqli_num_rows($tpares) > 0) {
            $paltoimiulos = "&nbsp;";
          }
        }
        ...
        elseif ((($toim != "TARJOUS" and $toim != "EXTTARJOUS") or $yhtiorow['tarjouksen_tuotepaikat'] == "") and $muokkauslukko_rivi == "" and ($kukarow['extranet'] == '' or ($kukarow['extranet'] != '' and $yhtiorow['tuoteperhe_suoratoimitus'] == 'E')) and $trow["ei_saldoa"] == "") {
          ...
          if ($paikat != '') {
            ...
            if ($selpaikkamaa != '' and $yhtiorow['varastopaikan_lippu'] != '') {
              echo "<img src='{$palvelin2}pics/flag_icons/gif/".mb_strtolower($selpaikkamaa).".gif'>";
            }
            ...
            if (!isset($varow['maa'])) $varow['maa'] = null; // MUOKKAUS: isset()
        ...
        elseif (isset($varow) and (($toim != "TARJOUS" and $toim != "EXTTARJOUS") or $yhtiorow['tarjouksen_tuotepaikat'] == "") and $kukarow['extranet'] == '') { // MUOKKAUS: isset()
        ...
        elseif ((($toim != "TARJOUS" and $toim != "EXTTARJOUS") or $yhtiorow['tarjouksen_tuotepaikat'] == "") and $kukarow['extranet'] != '' and $yhtiorow['varastopaikan_lippu'] != '') {
          ...
          if ($varow['maa'] != '' ) {
            echo "<td $class align='left'><img src='{$palvelin2}pics/flag_icons/gif/".mb_strtolower($varow['maa']).".gif'></td>";
          }
          else {
            echo "<td $class align='left'></td>";
          }
	...
	if ($kukarow['extranet'] == '' and $tuotekyslinkki != "") {
	  ...
	  if (file_exists("../../raportit")) {
        ...
        if ($yhtiorow['laiterekisteri_kaytossa'] != '' and $toim == "YLLAPITO") {
          ...
          $sarjanumerotres = mysqli_fetch_assoc($res);
          echo "<br>";
        ...
        elseif ($toim == "MYYNTITILI" and $laskurow["alatila"] == "V" and ($row["perheid"] == 0 or $row["perheid"] == $row["tunnus"])) {
          ...
          elseif ($kpl_ruudulle > 0 and $row["sarjanumeroseuranta"] == "S") {
            ...
            $muutarow = mysqli_fetch_assoc($muutares);
          ...
          elseif (in_array($toim, array('VALMISTAVARASTOON', 'VALMISTAASIAKKAALLE', 'RIVISYOTTO', 'PIKATILAUS'))) {
            ...
            // MUOKKAUS: lisatty onclick():
            echo "<a href='#' id='link_kpl_$row[tunnus]' onclick='askForValue(\"$row[yhtio]\", \"$row[otunnus]\", \"$row[tunnus]\", \"Muuta lukumäärää:\", \"$kpl_ruudulle\", \"kpl\", \"$toim\"); return false;'>{$kpl_ruudulle} " . mb_strtolower($row["tilausrivin_yksikko"]) . "</a>";
			...
			// MUOKKAUS: lisatty onclick():
            echo "<td $class align='right' nowrap><a href='#' id='link_kpl_$row[tunnus]' onclick='askForValue(\"$row[yhtio]\", \"$row[otunnus]\", \"$row[tunnus]\", \"Muuta lukumäärää:\", \"$kpl_ruudulle\", \"kpl\", \"$toim\"); return false;'>$kpl_ruudulle</a></td>";
          }
        }
        ...
        if ($toim != "VALMISTAVARASTOON" and $toim != "SIIRTOLISTA") {
          ...
          if ($yhtiorow["alv_kasittely"] == "") {
            ...
            // MUOKKAUS: BUGIKORJAUS (string * float):
            $hinta        = hintapyoristys((float)$hinta * $alvillisuus_kerto);
            $summa        = hintapyoristys((float)$summa * $alvillisuus_kerto);
            $myyntihinta  = hintapyoristys(tuotteen_myyntihinta($laskurow, isset($trow) ? $trow : null, 1) * $alvillisuus_kerto);
          }
          ...
          elseif ($kukarow['hinnat'] == 0) {
            ...
            // MUOKKAUS: onclick() ja myyntihinta -> hinta:
            echo "<td $class align='right'><a href='#' id='link_summa_$row[tunnus]' onclick='askForValue(\"$row[yhtio]\", \"$row[otunnus]\", \"$row[tunnus]\", \"Muuta kappalehintaa:\", \"$hinta\", \"summa\", \"$toim\"); return false;'>$hinta</a></td>";
            ...
            if (!empty($row["netto"])) {
              ...
              for ($alepostfix = 1; $alepostfix <= $yhtiorow['myynnin_alekentat']; $alepostfix++) {
				// MUOKKAUS: onclick():
                $ale_echo .= "<a href='#' id='link_ale1_$row[tunnus]' onclick='askForValue(\"$row[yhtio]\", \"$row[otunnus]\", \"$row[tunnus]\", \"Muuta rivin alennusprosenttia:\", \"" . $row["ale{$alepostfix}"] . "\", \"ale1\", \"$toim\"); return false;'>" . $row["ale{$alepostfix}"] . "</a> + ";
              }
        ...
        if ($muokkauslukko_rivi == "" and $kukarow["extranet"] == "" and ($toim == "TARJOUS" or $toim == "EXTTARJOUS" or $laskurow["tilaustyyppi"] == "T" or $toim == "SIIRTOTYOMAARAYS") and $riviok == 0) {
          ...
          if (mysqli_num_rows($lisaresult) > 0 and ($row["perheid2"] == 0 and ($row["var"] == "T" or $row["var"] == "U")) or $row["perheid2"] == -1) {
            echo "</tr>";
            ...
            while ($prow = mysqli_fetch_assoc($lisaresult)) {
              ...
              if ($lislask == mysqli_num_rows($lisaresult)) {
          ...
          elseif ($kukarow["extranet"] == "" and mysqli_num_rows($lisaresult) > 0) {
        ...
        if (isset($GLOBALS['eta_yhtio']) and $GLOBALS['eta_yhtio'] != '' and $koti_yhtio == $kukarow['yhtio']) {
          ...
          $trow_eta = mysqli_fetch_assoc($tres_eta);
        ...
        if ($kommenttirivi_nakyviin or $row['kommentti'] != '' or $yhtiorow['tilausrivin_korvamerkinta'] == 'A' or ($yhtiorow['naytetaanko_ale_peruste_tilausrivilla'] != '' and $row['ale_peruste'] != '')) {
          ...
          if ($yhtiorow['naytetaanko_ale_peruste_tilausrivilla'] != '' and $row['ale_peruste'] != '') {
            ...
            if ($yhtiorow['naytetaanko_ale_peruste_tilausrivilla'] == 'o') {
              echo $ap_font.mb_substr($row["ale_peruste"], 0, mb_strpos($row["ale_peruste"], "XXX"))."</font><br>";
            }
            elseif ($ap_text != "" and $yhtiorow['naytetaanko_ale_peruste_tilausrivilla'] == 't') {
	  ...
	  // MUOKKAUS: isset():
        ...
        while ($alvrow = mysqli_fetch_assoc($alvresult)) {
          ...
          while ($arow = mysqli_fetch_assoc($aresult)) {
            ...
            if ($arow['varattu'] > 0) {
              ...
              if (trim(mb_strtoupper($alvrow["maa"])) == trim(mb_strtoupper($laskurow["toim_maa"]))) {
        ...
        if (mb_strtoupper($yhtiorow['maa']) == 'EE') {
	...
	if (isset($asiakasrow)) {
        ...
        if ($yhtiorow['kerayserat'] == 'K' and $toimitustavan_tunnus > 0 and $kukarow['extranet'] == "" and $_lahdot_toim_check) {
          ...
          if ($laskurow['toimitustavan_lahto'] > 0 and $laskurow['tila'] == 'L' and $laskurow['alatila'] == 'D') {
            ...
            $lahdot_row = mysqli_fetch_assoc($lahdot_res);
            ...
            $varasto_chk_res = pupe_query($query);
            $varasto_chk_row = mysqli_fetch_assoc($varasto_chk_res);
            ...
            foreach ($chk_arr as $vrst) {
              ...
              foreach ($lahdot as $lahdot_row) {
                ...
                if (!$selectoitunut and $sel == "" and $laskurow['toimitustavan_lahto'] == 0 and mb_strtolower($state) != 'disabled' and (count($toimitustavan_lahto_chk) == 0 or !in_array($lahto, $toimitustavan_lahto_chk))) {
                  $sel = " selected";
                  $selectoitunut = TRUE;
                }
	    ...
	    if (file_exists("../../toimitusvahvistus") and $toim != 'TARJOUS') echo "<a href='/toimitusvahvistus/?nonavi=1&summary=1&order=$laskurow[tunnus]'><input type='submit' value='Kuljetustilaus'></a>";
          ...
          if (mb_strlen(sprintf("%.2f", $summa)) > 7) {
            $koko = mb_strlen(sprintf("%.2f", $summa));
          }
          else {
            $koko = '7';
          }
          ...
          if ($toim != "PROJEKTI" and $sallijyvitys) {
            if ($toim == 'TARJOUS' and !empty($yhtiorow['salli_jyvitys_tarjouksella'])) {
              // MUOKKAUS: lisatty postinumero ja yhteensa kpl:
	    ...
	    $jysum = (float)$jysum; // MUOKKAUS: BUGIKORJAUS (string - float)
            ...
            if ($toim == 'TARJOUS' and !empty($yhtiorow['salli_jyvitys_tarjouksella'])) {
              ...
              if (($kotiarvo_eieri - $ostot_eieri) > 0) echo "<input type='text' size='$koko' name='jysum' value='".sprintf("%.2f", 100*$kate_eieri/($kotiarvo_eieri-$ostot_eieri))."' Style='text-align:right' $state></td>";
            ...
            if (isset($loytyy_maksutapahtumia) and !$loytyy_maksutapahtumia) { // MUOKKAUS: isset()
              if ($toim == 'TARJOUS') {
    ...
    if ($jt_kayttoliittyma == "kylla" and $laskurow["liitostunnus"] != 0 and $toim != "TYOMAARAYS" and $toim != "TYOMAARAYS_ASENTAJA" and $toim != "REKLAMAATIO" and $toim != "VALMISTAVARASTOON" and $toim != "MYYNTITILI" and ($toim != "TARJOUS" and $toim != "EXTTARJOUS")) {
      ...
      if (isset($asiakasrow) and $asiakasrow["jtrivit"] == 0) {
        ...
        while ($vrow = mysqli_fetch_assoc($vtresult)) {
          if ($vrow["tyyppi"] != 'E' or $laskurow["varasto"] == $vrow["tunnus"]) {
            $varastosta[$vrow["tunnus"]] = $vrow["tunnus"];
          }
        }

        if (mysqli_num_rows($vtresult) != 0 and count($varastosta) != 0) {
          if ($kukarow['extranet'] != '') {
  ...
  if (!isset($puutetta_on)) $puutetta_on = false; // MUOKKAUS: isset()
  ...
  if ((int)$kukarow["kesken"] > 0 and (!isset($ruutulimit) or (int)$ruutulimit == 0)) {
    if ($maksupaate_kassamyynti and
      $_kassamyyntiok and
      $maksuehtorow["kateinen"] != "" and
      $muokkauslukko == "" and
    ...
    // MUOKKAUS: BUGIKORJAUS (string == int):
    if ($kukarow["extranet"] == "" and $muokkauslukko == "" and ($toim == "TARJOUS" or $toim == "EXTTARJOUS") and (int)$laskurow["liitostunnus"] != 0 and (int)$tilausok == 0 and (int)$rivilaskuri > 0) {
      ...
      if (mysqli_num_rows($optiotarkres) == 0) {
      ...
      elseif (mysqli_num_rows($optiotarkres) > 0) {
    ...
    // MUOKKAUS: BUGIKORJAUS (string == int):
    if (($muokkauslukko == "" or $toim == "PROJEKTI" or $toim == "YLLAPITO") and (int)$laskurow["liitostunnus"] != 0 and (int)$tilausok == 0 and (int)$rivilaskuri > 0 and $asiakasOnProspekti != "JOO") {
      ...
      elseif ($kukarow["extranet"] == "" and $toim == 'REKLAMAATIO' and
        ...
        if ($mista == 'keraa') {
          ...
          if (!empty($laskurow['kerayslista'])) {
            ...
            $takaisin_keraa_row = mysqli_fetch_assoc($takaisin_keraa_res);
      ...
      elseif ($kukarow['tilaus_valmis'] != "4" and ($toim != 'REKLAMAATIO' or
        ...
        if ($kateinen == '' and in_array($toim, array("RIVISYOTTO", "PIKATILAUS")) and !empty($yhtiorow["ennakkolasku_myyntitilaukselta"]) and !empty($yhtiorow["ennakkomaksu_tuotenumero"])) {
          ...
          if (mysqli_num_rows($jaksoresult) == 0 and $laskelisa == '') {
            echo "<br><br>";
    ...
    if ($laskurow["tunnusnippu"] > 0 and $toim == "PROJEKTI") {
      ...
      $projektilask = mysqli_num_rows($abures);
    }
    else {
      $projektilask = 0;
    }
    ...
    // MUOKKAUS: isset():
    if (isset($row) and isset($row["laskutettuaika"])) {
...
function loytyyko_myyja_tunnuksella($tunnus) {
  ...
  $maara = mysqli_fetch_assoc($result);
...
function piirra_toimitusosoite($laskurow) {
  ...
  while ($maa = mysqli_fetch_assoc($maa_result)) {
    $sel = "";

    if (mb_strtoupper($laskurow["toim_maa"]) == mb_strtoupper($maa["koodi"])) {
      $sel = "selected";
    }
    elseif ($laskurow["toim_maa"] == "" and
      mb_strtoupper($maa["koodi"]) == mb_strtoupper($yhtiorow["maa"])
    ) {
      $sel = "selected";
    }

    echo
    "<option value='" . mb_strtoupper($maa["koodi"]) . "' {$sel}>" . t($maa["nimi"]) . "</option>";
  }
