...
if ($tee == "NAYTATILAUS") {
  ...
  while ($sarjarow = mysqli_fetch_assoc($sarja_result)) {
...
if ($from != '' and $rivitunnus != "") {
  ...
  $rivirow  = mysqli_fetch_assoc($sarjares);
  ...
  $sarjares = pupe_query($query);
  $laskurow  = mysqli_fetch_assoc($sarjares);
...
if ($toiminto == 'MUOKKAA') {
  if (isset($PAIVITA)) {
    ...
    $sarrow = mysqli_fetch_assoc($sarres);
    ...
    if ($sarjanumeronLisatiedot) {
      ...
      $lisarow = mysqli_fetch_assoc($lisares);
    ...
    if (mysqli_num_rows($muutares) == 1) {
      ...
      if ((mb_strpos($_SERVER['SCRIPT_NAME'], "sarjanumeroseuranta.php") !== false or
          $PHP_SELF == "sarjanumeroseuranta.php" or
          mb_strpos($_SERVER['SCRIPT_NAME'], "tervetuloa.php") !== false or
          $PHP_SELF == "tervetuloa.php") and
        ($muutarow["myynti_laskaika"] == "" or $muutarow["myynti_laskaika"] == "0000-00-00" or
          (mb_substr($muutarow['sarjanumero'], 0, $viiva) == "PUUTTUU" or
            mb_substr($muutarow['sarjanumero'], 0, $viiva) == t("PUUTTUU") or
            mb_substr($muutarow['sarjanumero'], 0, $viiva) == t("PUUTTUU", $yhtiorow["kieli"])))
      ) {
        $disabled = "";
        $style = "";
      }
      ...
      if ($muutarow["sarjanumeroseuranta"] == "F") {
        ...
        $pevva = mb_substr($muutarow["parasta_ennen"], 0, 4);
        $pekka = mb_substr($muutarow["parasta_ennen"], 5, 2);
        $peppa = mb_substr($muutarow["parasta_ennen"], 8, 2);
      ...
      if ($muutarow["sarjanumeroseuranta"] == "S") {
        ...
        $tvva = mb_substr($muutarow["takuu_alku"], 0, 4);
        $tkka = mb_substr($muutarow["takuu_alku"], 5, 2);
        $tppa = mb_substr($muutarow["takuu_alku"], 8, 2);

        $tvvl = mb_substr($muutarow["takuu_loppu"], 0, 4);
        $tkkl = mb_substr($muutarow["takuu_loppu"], 5, 2);
        $tppl = mb_substr($muutarow["takuu_loppu"], 8, 2);
...
if ($toiminto == 'LISAA' and (trim($sarjanumero) != '' or $is_uploaded_file === true)) {
  ...
  foreach ($sarjanumero_array as $sarjanumero) {
    ...
    if ($insok == "OK" and mysqli_num_rows($sarjares) == 0) {

      if ($rivirow["sarjanumeroseuranta"] == "E") {
        $sarjanumero = mb_strtoupper($sarjanumero);
      }
    ...
    elseif ($insok == "OK" and mysqli_num_rows($sarjares) == 1) {
...
if ($from != '' and $rivitunnus != "" and $formista == "kylla") {
  ...
  elseif (count($sarjataan) > 0) {
    ...
    $result = pupe_query($query);
    $_vap = mysqli_fetch_assoc($result);

    if ($_vap["tunnusKpl"] > 0) {

      // Jos rivin kappalemäärä on vähennetty sarjanumeroiden liittämisen jälkeen
      // voi olla tarvetta ottaa kohdistuksia pois ylimääräisiltä riveiltä
      $query = "UPDATE sarjanumeroseuranta
                SET $tunnuskentta = ''
                WHERE yhtio = '{$kukarow['yhtio']}'
  ...
  if ((($rivirow["varattu"] >= count($sarjataan) and count($sarjataan) > 0) or(($rivirow["sarjanumeroseuranta"] == "E" or $rivirow["sarjanumeroseuranta"] == "F" or $rivirow["sarjanumeroseuranta"] == "G") and $rivirow["varattu"] > 0 and count($sarjataan) > 0)) and $lisaysok == "OK") {
    foreach ($sarjataan as $sarjatun) {
      ...
      $query = "SELECT *, $tunnuskentta rivitunnus
                ...
                WHERE tunnus = '$sarjatun'";
      $sarres = pupe_query($query);
      $sarjarow = mysqli_fetch_assoc($sarres);

      // Päivitetään laitetaulu kun ruksataan sarjanumero myyntiriville
      $kveri = "UPDATE laite
      ...
      if ($tunnuskentta == 'ostorivitunnus' and $from == "kohdista") {
        ...
        while ($varastoon_row = mysqli_fetch_assoc($varastoon_result)) {
...
if (mb_strpos($_SERVER['SCRIPT_NAME'], "sarjanumeroseuranta.php") !== FALSE or $PHP_SELF == "sarjanumeroseuranta.php") {
...
if (is_resource($sarjaresiso) and mysqli_num_rows($sarjaresiso) > 0) {
  while ($sarjarow = mysqli_fetch_assoc($sarjaresiso)) {
    ...
    echo "<td class='ptop'>
      ...
      mb_strtoupper($sarjarow["sarjanumero"]).
      "</a>
    ...
    if ($rivirow["sarjanumeroseuranta"] == "E" or $rivirow["sarjanumeroseuranta"] == "F" or $rivirow["sarjanumeroseuranta"] == "G") {
      ...
      if ($tunnuskentta == 'ostorivitunnus' and $from == "kohdista") {
        ...
        $lisat_row = mysqli_fetch_assoc($lisat_res);
    ...
    if ($sarjarow["osto_uusiotunnus"] > 0) {
      ...
      if (mysqli_num_rows($keikkares) > 0) {
        $keikkarow2 = mysqli_fetch_assoc($keikkares);
    ...
    if (($sarjarow["siirtorivitunnus"] > 0 and $tunnuskentta != 'siirtorivitunnus') or ($sarjarow["osto_perheid2"] > 0 and $sarjarow["osto_perheid2"] != $sarjarow["osto_rivitunnus"])) {
      ...
      if ($siirow["tyyppi"] == "O") {
        ...
        $fnlina1 .= "";
        $fnlina2 .= "<br><br>".t("Varattu lisävarusteena").":<br>".$siirow["tuoteno"]." <a href='{$palvelin2}tilauskasittely/sarjanumeroseuranta.php?tuoteno_haku=".urlencode($siirow["tuoteno"])."&sarjanumero_haku=".urlencode($siirow["sarjanumero"])."&lopetus=$sarjalopetus'>$siirow[sarjanumero]</a>";
      }
    ...
    $viiva = mb_strpos($sarjarow['sarjanumero'], "-");
    if ($viiva === FALSE) {
      $viiva = 0;
    }
    ...
    while ($lisarow = mysqli_fetch_assoc($lisares)) {
...
if (file_exists("../../common")) HTML::sql_table($sarjaresiso, true); // MUOKKAUS: lisatty sarjataulukko
