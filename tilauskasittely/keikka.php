...
if ($toiminto == "tulosta") {
  ...
  if (mysqli_num_rows($llres) > 0) {
    $llrow = mysqli_fetch_assoc($llres);
...
if ($toiminto == 'kalkyyli' and $yhtiorow['suuntalavat'] == 'S' and $tee == '' and trim($suuntalavan_tunnus) != '' and trim($koko_suuntalava) == 'X') {
  if ((isset($suuntalavanhyllyalue) and trim($suuntalavanhyllyalue) == '') or (isset($suuntalavanhyllypaikka) and trim($suuntalavanhyllypaikka) == '')) {
    ...
    $suuntalavanhyllyalue = sanitize_string($suuntalavanhyllyalue);
    $suuntalavanhyllynro  = sanitize_string($suuntalavanhyllynro);
    $suuntalavanhyllyvali = sanitize_string($suuntalavanhyllyvali);
    $suuntalavanhyllytaso = sanitize_string($suuntalavanhyllytaso);
...
if ($ytunnus == "" and $keikka != "") {
  ...
  if (mysqli_num_rows($keikkahaku_res) > 0) {
    $keikkahaku_row = mysqli_fetch_assoc($keikkahaku_res);
...
if ($toiminto == "" and $ytunnus == "" and $keikka == "") {
  ...
  if ($yhtiorow['varastopaikkojen_maarittely'] == 'M') {
    ...
    if (mysqli_num_rows($keraysvyohyke_result) > 0) {
      echo "</tr><tr>";
    ...
    while ($keikan_laatija_row = mysqli_fetch_assoc($keikan_laatija_res)) {
  ...
  if (mysqli_num_rows($result) > 0) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
      ...
      $laskuja_row = mysqli_fetch_assoc($laskuja_result);
      ...
      if ($keikkarajaus == '' and $row['keikat'] != '' and mb_strpos($row['keikat'], ',') === FALSE) {
...
if ($toiminto == "" and (($ytunnus != "" or $keikkarajaus != '') and $toimittajarow["ytunnus"] != '')) {
  ...
  if (mysqli_num_rows($result) > 0) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
      ...
      $tilausrivirow = mysqli_fetch_assoc($result2);
      ...
      if ($llrow["volasku"] != $llrow["volasku_ok"] or $llrow["kulasku"] != $llrow["kulasku_ok"]) {
        ...
        while ($volasrow = mysqli_fetch_assoc($volasresult)) {
...
if ($toiminto == "kohdista" or $toiminto == "yhdista" or $toiminto == "poista" or $toiminto == "tulosta" or $toiminto == "lisatiedot" or
  ...
  $tsekkirow = mysqli_fetch_assoc($tsekkiresult);
...
function hae_yhteenveto_tiedot($toimittajaid = null, $toimipaikka = 0, $pp = null, $kk = null, $vv = null) {
  ...
  while ($row_vaihto = mysqli_fetch_assoc($result_vaihto_omaisuus)) {
  ...
  while ($row_huorah = mysqli_fetch_assoc($result_huolintarahdit)) {
  ...
  while ($row = mysqli_fetch_assoc($result)) {
    ...
    if ($row['liitetty'] == 0) {
      ...
      while ($laskut = mysqli_fetch_assoc($result_laskut)) {
