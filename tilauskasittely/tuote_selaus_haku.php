...
if (isset($livesearch_tee)) { // MUOKKAUS: isset()
...
if (empty($verkkokauppa)) {
  ...
  $hae_ja_selaa_row = mysqli_fetch_assoc($hae_ja_selaa_result);
...
// Katsotaan tässä, että viedään käyttäjä oikeaan paikaan sen mukaan millainen tilaus hänellä on kesken
if (isset($vierow)) {
...
if (!empty($variaatio)) {
  ...
  echo "<button onclick='window.close();'>" . t("Palaa takaisin perustuotenäkymään") . "</button>";
...
if ($verkkokauppa == "") {
  ...
  elseif ($kukarow["kuka"] != "" and isset($laskurow) and $laskurow["tila"] == "O") { // MUOKKAUS: isset()
  ...
  elseif ($kukarow["kuka"] != "" and isset($laskurow) and $laskurow["tila"] != "" and $laskurow["tila"] != "K" and $toim_kutsu != "" and $toim_kutsu != "EXTENNAKKO") { // MUOKKAUS: isset()
    ...
    if (!isset($tyojono)) $tyojono = null; // MUOKKAUS: isset()
...
if (mb_strlen($ojarj) > 0) {
  $ojarj = trim(sanitize_string($ojarj));
...
if (trim($toim_tuoteno) != '') {
  ...
  $toimtuotteet = mb_substr($toimtuotteet, 0, -1);
...
if (trim($alkuperaisnumero) != '') {
  $alkuperaisnumero = sanitize_string(trim($alkuperaisnumero));
  ...
  $origtuotteet = mb_substr($origtuotteet, 0, -1);
...
if (table_exists("tuotteen_orginaalit")) {
  ...
  if (mysqli_num_rows($orginaaleja_res) > 0) {
    $orginaaalit = TRUE;
  }
}
...
if ($verkkokauppa == "") {
  ...
  if (!isset($valittu_tarjous_tunnus)) $valittu_tarjous_tunnus = ""; // MUOKKAUS: isset()
  ...
  if ($kukarow['extranet'] == "" and $verkkokauppa == "") {
    ...
    // MUOKKAUS: isset():
    if (isset($laskurow) and in_array($laskurow['tila'], array('N', 'T')) and $kukarow['kesken'] != 0 and $kukarow['hinnat'] == 0) {
...
if (isset($sort) and $sort != '') {
  $sort = trim(sanitize_string($sort));
}
  ...
  if (mysqli_num_rows($result) > 0) {
      ...
      if ($mrow["vastaavat"] != $mrow["tuoteno"]) {
        ...
        foreach ($vastaavat as $mrow['vastaavat']) {
          ...
          if (mysqli_num_rows($kores) > 0) {
            ...
            $vastaavamaara = mysqli_num_rows($kores);
          ...
          if (mysqli_num_rows($sarjares) > 0) {
            ...
            while ($sarjarow = mysqli_fetch_assoc($sarjares)) {
              ...
              if (($sarjarow["siirtorivitunnus"] > 0) or ($sarjarow["osto_perheid2"] > 0 and $sarjarow["osto_perheid2"] != $sarjarow["osto_rivitunnus"])) {
                ...
                $siirow = mysqli_fetch_array($siires);
    ...
    foreach ($rows as $row_key => &$row) {
      ...
      if ($kukarow['extranet'] != '' or $verkkokauppa != "") {
        ...
        // MUOKKAUS: isset():
        $hae_ja_selaa_asiakas = isset($laskurow) ? (int) $laskurow['liitostunnus'] : 0;
      }
      ...
      if ($hae_ja_selaa_row['selite'] != 'B' and $verkkokauppa == "" and mb_strtoupper($row["status"]) == "P") {
        $vari = "tumma";
      ...
      if (in_array($yhtiorow['livetuotehaku_hakutapa'], array('O', 'P'))) {
        ...
        if (mb_strlen($row['lyhytkuvaus']) > 100) {
          $row['lyhytkuvaus'] = mb_substr($row['lyhytkuvaus'],0, 100)."..";
        }
      ...
      if (!isset($row["osaluettelo"])) $row["osaluettelo"] = null; // MUOKKAUS: isset()
      ...
      if ($orginaaalit === TRUE) {
        ...
        if (mysqli_num_rows($orgres)>0) {
          ...
          while ($orgrow = mysqli_fetch_assoc($orgres)) {
            $linkkilisa .= "<tr>
      ...
      elseif ($kukarow["extranet"] != "" or $tuotekyslinkki == "") {
        ...
        if ($kukarow["extranet"] != "" and $row["kuvaus"] != "" and $yhtiorow["extranet_nayta_kuvaus"] == "Y") {
          ...
          if ($kuvaus != $row['kuvaus'] and mb_strpos($kuvaus, "*") !== FALSE) {
      ...
      if ($verkkokauppa != "") {
        if (mb_stripos($_SERVER["HTTP_USER_AGENT"], "MSIE") !== FALSE) {
...
function piirra_extranet_saldo($row, $oleasrow) {
  ...
  if ($verkkokauppa == "") {
    ...
    if (!isset($noutovarres)) {
      ...
      mysqli_data_seek($noutovarres, 0);
    }

    if (mysqli_num_rows($noutovarres) > 0) {

      while ($noutovarrow = mysqli_fetch_assoc($noutovarres)) {
        ...
        if ($noutomyytavissa > 0) {
          ...
          $noutolisa .= "<td>";
          $noutolisa .= ucwords(mb_strtolower($noutovarrow["nimitark"]));
          $noutolisa .= "</td>";
...
function hae_oletusasiakas($laskurow) {
  ...
  $oleasrow                 = mysqli_fetch_assoc($oleasres);

  // MUOKKAUS: isset():
  if (!isset($oleasrow)) $oleasrow = array();
  if (!isset($laskurow)) $laskurow = array();

  foreach (array("tunnus", "valkoodi", "maa", "toim_maa", "varastomaa") as $v) {
    if (!isset($oleasrow[$v])) $oleasrow[$v] = "";
  }
  foreach (array("toim_maa", "maa") as $v) {
    if (!isset($laskurow[$v])) $laskurow[$v] = "";
  }
  ...
  if ($toim_kutsu != "EXTENNAKKO" and ($verkkokauppa == "" or ($verkkokauppa != "" and $kukarow["kuka"] != "www" and $verkkokauppa_saldotsk))) {
    ...
    if ($row["tuoteperhe"] == $row["tuoteno"] and $row["sarjanumeroseuranta"] != "S") {
      ...
      // Normipupe
        ...
        $classrighx = mb_substr($classrigh, 0, -2) . " padding: 0px;' ";
    ...
    // Normaalit saldolliset tuotteet (Normi)
      ...
      if (isset($laskurow) and $laskurow["toim_maa"] != '') { // MUOKKAUS: isset()
	  ...
	  if (isset($laskurow)) { // MUOKKAUS: isset()
          ...
          if (isset($sallittu) and $sallittu === true) { // MUOKKAUS: isset()
      ...
      if ($lisatiedot != "") {
        ...
        if (mysqli_num_rows($varresult) > 0 and $hyllylisa != "") {
...
function hae_yhtiot() {
  ...
  if (mysqli_num_rows($result) > 0 and $yhtiorow["haejaselaa_konsernisaldot"] != "") {
    $yhtiot = array();
...
function hae_parametri_variaatio($row) {
  ...
  $query = "SELECT selite
            ...
            ";
  $result = pupe_query($query);
  $parametri_variaatio = mysqli_fetch_assoc($result);
  ...
  if ($kukarow['hinnat'] >= 0 and ($verkkokauppa == "" or $kukarow["kuka"] != "www")) {
    ...
    elseif ($kukarow["extranet"] != "") {
      ...
      if ($oleasrow["valkoodi"] != $yhtiorow["valkoodi"]) {
        ...
        if (mysqli_num_rows($olhires) == 1) {
          $olhirow     = mysqli_fetch_assoc($olhires);
    ...
    elseif ($asiakashinnat != '' and $kukarow["extranet"] == "" and $kukarow['hinnat'] == 0) {
      ...
      while ($hintavalrow = mysqli_fetch_assoc($hintavalresult)) {
        ...
        while ($hintarow = mysqli_fetch_assoc($hintaresult)) {
          $maa      = $hintarow['maa'];
    ...
    if ($asiakashinnat != '' and $kukarow["extranet"] == "" and $kukarow['hinnat'] == 0) {
      ...
      foreach ($ale_kaikki as $key => $val) {
        ...
        if (mb_substr($key, 3, 1) > $yhtiorow['oston_alekentat']) continue;
        $_alet .= "<br>{$val}%";
      }
      echo $_alet;
      echo "</td>";
