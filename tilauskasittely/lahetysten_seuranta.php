...
if ($tee == 'hae') {
  ...
  $nimilisa = trim($asiakas) != "" ? " AND (lasku.nimi LIKE ('%".sanitize_string($asiakas)."%') OR lasku.toim_nimi LIKE ('%".sanitize_string($asiakas)."%'))" : "";
  $postitplisa = trim($paikkakunta) != "" ? " AND (lasku.postitp LIKE ('%".sanitize_string($paikkakunta)."%') OR lasku.toim_postitp LIKE ('%".sanitize_string($paikkakunta)."%'))" : "";
  $tilauslisa = trim($tilausnumero) != "" ? " AND kerayserat.otunnus = '".sanitize_string($tilausnumero)."'" : "";
  $tuotelisa = trim($tuotenumero) != "" ? " AND tuote.tuoteno = '".sanitize_string($tuotenumero)."'" : "";
  $sscclisa = trim($sscc) != "" ? " AND (kerayserat.sscc LIKE ('%".sanitize_string($sscc)."%') OR kerayserat.sscc_ulkoinen LIKE ('%".sanitize_string($sscc)."%'))" : "";
      ...
      if (mysqli_num_rows($era_res) > 0) {
        ...
        while ($era_row = mysqli_fetch_assoc($era_res)) {
          ...
          if ((is_numeric($era_row['sscc_ulkoinen']) and (int) $era_row['sscc_ulkoinen'] > 0) or (!is_numeric($era_row['sscc_ulkoinen']) and (string) $era_row['sscc_ulkoinen'] != "")) {
            ...
            if (mb_substr($era_row['sscc_ulkoinen'], 0, 4) == "JJFI") {
            ...
            if (mb_substr($era_row['sscc_ulkoinen'], 0, 2) == "MA") {
          ...
          while ($sscc_row = mysqli_fetch_assoc($sscc_res)) {
            echo "<tr class='{$era_row['sscc']}' style='display:none'>";
            echo "<td></td>";
