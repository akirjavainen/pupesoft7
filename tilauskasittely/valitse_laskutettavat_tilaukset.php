...
if ($maksupaate_kassamyynti) {
  ...
  $ekarow = mysqli_fetch_assoc($res);

  $kaytetaan_maksupaatetta = ($maksupaate_kassamyynti and $ekarow["kateinen"] != "");
  if (!isset($kateismaksu)) $kateismaksu = null; // MUOKKAUS: isset()
...
if ($tee == 'TOIMITA' and isset($maksutapa) and $maksutapa == 'seka') {
  ...
  if (count($tunnus) > 0) {
    ...
    $laskutettavat = mb_substr($laskutettavat, 0, -1); // vika pilkku pois
  }
  ...
  if ($laskutettavaa_on) {
    ...
    if (isset($positiotunnus) and count($positiotunnus) > 0) {
      ...
      foreach ($positiotunnus as $postun) {
        ...
        $posrow = mysqli_fetch_array($rahres);
        ...
        $postarkrow = mysqli_fetch_array($postarkresult);
...
if ($tee == "VALITSE") {
  ...
  if (mysqli_num_rows($res) > 0) {
    ...
    while ($row = mysqli_fetch_assoc($res)) {
      ...
      $hyvrow = mysqli_fetch_array($hyvre);
      ...
      if ($yhtiorow["rahti_hinnoittelu"] == "" and $row["rahtivapaa"] == "") {
        ...
        if ($row["valkoodi"] != '' and trim(mb_strtoupper($row["valkoodi"])) != trim(mb_strtoupper($yhtiorow["valkoodi"]))) {
    ...
    if (!$yksi_valittu) {
      ...
      if ($kateinen == 'X') {
        ...
        while ($kassarow = mysqli_fetch_array($kassares)) {
        ...
        if (!$kaytetaan_maksupaatetta and mysqli_num_rows($maksuehtores) > 1) {
          echo "<tr><th>".t("Maksutapa")."</th><td colspan='3'>";

          echo "<select name='maksutapa'>";

          while ($maksuehtorow = mysqli_fetch_array($maksuehtores)) {
          ...
          $maksuehtorow = mysqli_fetch_array($maksuehtores);
      ...
      if ($ekarow["chn"] == "666") {
        ...
        while ($saaterow = mysqli_fetch_array($result)) {
...
<td valign='top'>$tilrow[meh]<br>$tilrow[toimitustapa]</td>
 ...
 <a href='/toimitusvahvistus/index.php?no_navi=1&summary=1&order=$tilrow[tunnukset_ruudulle]'><input type='submit' name='kuljetustilaus' value='Kuljetustilaus'></a></td>
          </td>
          </tr>";
