...
if (php_sapi_name() != 'cli' and mb_strpos($_SERVER['SCRIPT_NAME'], "keraa.php") !== FALSE) {
  ...
  if ($yhtiorow['kerays_riveittain'] == 'K' and $ajax_toiminto == 'kerattavatrivit') {
    ...
    $tunnus = (int) $tunnus;
    $hyllyalue = sanitize_string($hyllyalue);
    $hyllynro = sanitize_string($hyllynro);
    $hyllyvali = sanitize_string($hyllyvali);
    $hyllytaso = sanitize_string($hyllytaso);
    $poikkeama_kasittely = sanitize_string($poikkeama_kasittely);
...
if (isset($indexvas) and $indexvas == 1 and $tuvarasto == '') {
  ...
  if (mysqli_num_rows($keraakaikistares) > 0) {

    $keraakaikistarow = mysqli_fetch_assoc($keraakaikistares);
...
if ($yhtiorow['konsernivarasto'] != '' and $konsernivarasto_yhtiot != '') {
  ...
  if (isset($lasku_yhtio) and $lasku_yhtio != '') {
    $kukarow['yhtio'] = sanitize_string($lasku_yhtio);
...
if ($tee == 'PAKKAUKSET' and ($yhtiorow['kerayserat'] == 'P' or ($yhtiorow['kerayserat'] == 'A' and isset($kerayserat_asiakas_chk) and $kerayserat_asiakas_chk == 'A'))) {
  ...
  if (trim($pakkaukset_kaikille) == "") {
    ...
    $laskurow = mysqli_fetch_assoc($testresult);
    ...
    if ($laskurow['kerayslista'] > 0) {
      ...
      if (mysqli_num_rows($toimresult) > 0) {
        $toimrow = mysqli_fetch_assoc($toimresult);
...
if ($tee == 'P') {
  ...
  if (mysqli_num_rows($toimresult) > 0) {
    ...
    while ($toimrow = mysqli_fetch_assoc($toimresult)) {
      ...
      if ($toimrow["sarjanumeroseuranta"] == "S" or $toimrow["sarjanumeroseuranta"] == "T" or $toimrow["sarjanumeroseuranta"] == "V") {
        ...
        $sarjarow = mysqli_fetch_assoc($sarjares2);
  ...
  if ($yhtiorow['kerayserat'] == 'P' or $yhtiorow['kerayserat'] == 'A') {
    ...
    if ($yhtiorow['kerayserat'] == 'A') {
      ...
      if (mysqli_num_rows($chk_res) == 0) $ok_chk = false;
    }
    ...
    if ($ok_chk) {
      for ($y=0; $y<count($kerivi); $y++) {
        ...
        if (mysqli_num_rows($tark) == 1 and trim($keraysera_pakkaus[$kerivi[$y]]) == '') $virherivi++;
      }
...
if ($tee == 'P') {
  ...
  if (mysqli_num_rows($result) == 0) {
    ...
    $keraaja   = mysqli_fetch_assoc($result);
    $who     = $keraaja['kuka'];
    $keraamaton = 0;
    ...
    while ($prow = mysqli_fetch_assoc($pnresult)) {
    ...
    for ($i=0; $i < count($kerivi); $i++) {
      ...
      $statusrow = mysqli_fetch_assoc($ktresult);
      ...
      if ($statusrow["status"] == "keraamaton") {
        if ($kerivi[$i] > 0) {
          ...
          $otsikko = mysqli_fetch_assoc($result);
          ...
          $result = pupe_query($query1);
          $otsikkorivi = mysqli_fetch_assoc($result);
          ...
          $query1 = "SELECT *
                     ...
                     and yhtio    = '$kukarow[yhtio]'";
          $result = pupe_query($query1);
          $tilrivirow = mysqli_fetch_assoc($result);
          ...
          if (trim($maara[$apui]) != '') {
            ...
            if ($tilrivirow["var"] == 'P' and $maara[$apui] > 0) {
              ...
              if (mysqli_num_rows($puurivires) > 0) {
                $puurivirow = mysqli_fetch_assoc($puurivires);
              ...
              elseif ($maara[$apui] >= 0 and $maara[$apui] < $tilrivirow['varattu']) {
                ...
                if (isset($poikkeama_kasittely[$apui]) and $poikkeama_kasittely[$apui] != "") {
                  ...
                  if ($maara[$apui] == 0) {
                    ...
                    $vapaurow = mysqli_fetch_assoc($vapaut);
            ...
            if (isset($poikkeama_kasittely[$apui]) and $poikkeama_kasittely[$apui] != "" and $rotunnus != 0) {
              ...
              elseif ($poikkeama_kasittely[$apui] == "UT") {
                ...
                if (!isset($tilausnumerot[$tilrivirow["otunnus"]])) {
                  ...
                  $laskusplitrow = mysqli_fetch_assoc($laskusplitres);
                  ...
                  for ($islpit=1; $islpit < mysqli_num_fields($laskusplitres)-1; $islpit++) {

                    $fieldname = mysqli_field_name($laskusplitres, $islpit);
		  ...
		  // MUOKKAUS: mysqli_insert_id():
                  ...
                  $tilausnumerot[$tilrivirow["otunnus"]] = mysqli_insert_id($GLOBALS["link"]);
                  ...
                  // Ei monisteta tunnusta
                  for ($ijk = 0; $ijk < mysqli_num_fields($lisatiedot_result); $ijk++) {

                    $fieldname = mysqli_field_name($lisatiedot_result, $ijk);
	      ...
	      // MUOKKAUS: mysqli_insert_id():
              $riviresult = pupe_query($querys, $GLOBALS["link"]);
              ...
              $erarow = mysqli_fetch_assoc($sarjares2);
            ...
            if ($tilrivirow["perheid"] != 0 and trim($maara[$apui]) != ''
              ...
              while ($tilrivirow2 = mysqli_fetch_assoc($result)) {
                ...
                if (mysqli_num_rows($result2) == 1) {
                  $kerroinrow = mysqli_fetch_assoc($result2);
          ...
          if (($toim == 'VASTAANOTA_REKLAMAATIO' or $yhtiorow["kerayspoikkeama_kasittely"] == 'P') and $keraysvirhe == 0) {
            ...
            if (trim($varastorekla[$apui]) != '' and trim($vertaus_hylly[$apui]) != trim($varastorekla[$apui])) {
              ...
              if (isset($varastorekla[$apui]) and $varastorekla[$apui] != 'x' and $varastorekla[$apui] != '' and mb_strpos($varastorekla[$apui], "###") === false) {
                // tehdään uusi paikka jos valittiin paikaton lapsivarasto
                if (mb_substr($varastorekla[$apui], 0, 1) == 'V') {
                  $uusi_paikka = lisaa_tuotepaikka($tilrivirow["tuoteno"], '', '', '', '', '', '', 0, 0, mb_substr($varastorekla[$apui], 1));
                ...
                $_row_paikka = mysqli_fetch_assoc($_result_paikka);
          ...
          if ($yhtiorow['kerayserat'] == 'K' and $toim == "") {
            ...
            while ($keraysera_chk_row = mysqli_fetch_assoc($keraysera_chk_res)) {
            ...
            if ($tilrivirow["perheid"] != 0 and trim($maara[$apui]) != ''
              ...
              while ($lapset_chk_row = mysqli_fetch_assoc($lapset_chk_res)) {
          ...
          if ($toim == 'VASTAANOTA_REKLAMAATIO' and $keraysvirhe == 0) {
            ...
            $sresults = mysqli_fetch_assoc($hakures);

            if (mysqli_num_rows($hakures) == 0) {
  ...
  if ($muuttuiko == 'kylsemuuttu') {
    foreach ($poikkeamat as $poikkeamatilaus => $poikkeamatilausrivit) {
      ...
      $res = pupe_query($qry);
      $ptilarow = mysqli_fetch_assoc($res);
      $ptila = $ptilarow['tila'];
      ...
      $toimtaparow = mysqli_fetch_assoc($toimtaparesult);
      ...
      foreach ($poikkeamatilausrivit as $poikkeama) {
        ...
        $enarirow = mysqli_fetch_assoc($enariresult);
      ...
      $ei_puutteita = (mb_strpos($sellahetetyyppi, "_eipuute") === FALSE);
  ...
  if ($tee == 'P' and $real_submit == 'yes') {
    if ($keraamaton > 0) {
      ...
      if ($yhtiorow['kerayserat'] == 'K' and $toim == "") {
        ...
        while ($chk_pak_row = mysqli_fetch_assoc($chk_pak_res)) {
      ...
      while ($laskurow = mysqli_fetch_assoc($lasresult)) {
        ...
        if (mysqli_num_rows($resul) == 1) {
          $temprow = mysqli_fetch_assoc($resul);
        ...
        if ($_siirtolista) {
          ...
          $_ker_chk_row = mysqli_fetch_assoc($_keraamaton_chk_res);
      ...
      if ($toim != 'VASTAANOTA_REKLAMAATIO') {
        ...
        while ($laskurow = mysqli_fetch_assoc($lasresult)) {
            ...
            if ((is_array($komento) and count($komento) > 0) or (!is_array($komento) and $komento != "")) {
              ...
              if (is_array($komento)) {
                foreach ($komento as $paprulleko) {
                  ...
                  elseif ($paprulleko != 'email' and mb_substr($paprulleko, 0, 12) != 'asiakasemail') {
              ...
              elseif ($komento != 'email' and mb_substr($komento, 0, 12) != 'asiakasemail') {
                $lahete_tulostus_paperille++;
              }
              else {
                $lahete_tulostus_emailiin++;
          ...
          if ($valittu_oslapp_tulostin != "" and $oslapp != '' and $oslappkpl > 0) {
            ...
            $oslarow = mysqli_fetch_assoc($oslares);
            ...
            elseif ($oslarow['osoitelappu'] == 'oslap_mg' and $yhtiorow['kerayserat'] == 'K' and $toim == "") {
              ...
              $pak_num = mysqli_num_rows($pak_chk_res);

              while ($pak_chk_row = mysqli_fetch_assoc($pak_chk_res)) {
  ...
  if ($id == 0) {
    ...
    if (is_numeric($etsi) and $etsi != '') {
      if ($yhtiorow['kerayserat'] == 'K' and $toim == "") {
        ...
        $nro_chk_row = mysqli_fetch_assoc($nro_chk_res);
    ...
    if ($tumaa != '') {
      ...
      $maarow = mysqli_fetch_assoc($maare);
    ...
    if (mysqli_num_rows($result) == 1 and is_numeric($etsi) and $etsi != '') {
    ...
    elseif (mysqli_num_rows($result) > 0) {
      ...
      while ($row = mysqli_fetch_assoc($result)) {
        ...
        if ($yhtiorow['kerayserat'] == 'K' and $toim == "") {
          ...
          $lahetepvm_e = str_replace(mb_substr($h1time_e, 0, mb_strpos($h1time_e, " ")), "", $lahetepvm_e);
  ...
  if ($id != 0 and (!isset($rahtikirjaan) or $rahtikirjaan == '')) {
    ...
    if ($toim == 'MYYNTITILI') {
      ...
      $query = "SELECT
                ...
                and lasku.alatila  = '{$alatilareklamaatio}'";
      $result = pupe_query($query);
      $otsik_row = mysqli_fetch_assoc($result);
      ...
      if ($_ker_rek) {
        ...
        while ($otsik_row = mysqli_fetch_assoc($result)) {
          echo "<tr>";
          echo "<td>";
    ...
    if ($riveja > 0) {
      ...
      if ($otsik_row['kerayserat'] == "H") {
        ...
        while ($rnrow = mysqli_fetch_assoc($result)) {
      ...
      if ($yhtiorow['kerayserat'] == 'P' or ($yhtiorow['kerayserat'] == 'A' and $row_chk['kerayserat'] == 'A')) {
        ...
        if (mysqli_num_rows($pakkausres) > 0) {
          echo "<br />";
          echo "<table><tr>";
      ...
      if ($yhtiorow['kerayserat'] == 'K' and $keraajalist == "") {
        ...
        $keraaja_row = mysqli_fetch_assoc($keraaja_res);
      ...
      if ($otsik_row['pakkaamo'] > 0 and $yhtiorow['pakkaamolokerot'] != '') {
        ...
        if (mysqli_num_rows($lokero_chk_res) > 0) {
          $lokero_chk_row = mysqli_fetch_assoc($lokero_chk_res);
      ...
      $total_rivi_count = mysqli_num_rows($result);
      $lavanumero=1;
      $lava_referenssiluku=0;
      ...
      while ($row = mysqli_fetch_assoc($result)) {
        ...
        if ($yhtiorow['kerays_riveittain'] == 'K') {
          ...
          $kerattavatrivitrow = mysqli_fetch_assoc($kerattavatrivitres);
        ...
        if ($row['ei_saldoa'] != '') {
          ...
          if ((($toim == "VALMISTUS" and $yhtiorow["valmistus_kerayslistan_jarjestys"] == "S") or $yhtiorow["kerayslistan_jarjestys"] == "S") and mb_strpos($row["rivitunnukset"], ",") !== FALSE) {
          ...
          if ($toim == 'VASTAANOTA_REKLAMAATIO' or $yhtiorow["kerayspoikkeama_kasittely"] == 'P') {
            ...
            while ($rivi = mysqli_fetch_assoc($results2)) {
              $sel = '';
          ...
          if ($yhtiorow['kerays_riveittain'] == 'K') {
            ...
            $tuto_row = mysqli_fetch_assoc($tuto_results);
	  ...
	  // MUOKKAUS: BUGIKORJAUS (puuttuvat lainausmerkit):
          ...
          if ((($toim == "VALMISTUS" and $yhtiorow["valmistus_kerayslistan_jarjestys"] == "S") or $yhtiorow["kerayslistan_jarjestys"] == "S") and mb_strpos($row["rivitunnukset"], ",") !== FALSE) {
            ...
            if ($yhtiorow['kerayserat'] == 'K' and $toim == "") {
              ...
              if ($row['ohita_kerays'] != "") {
                ...
                $keraysera_res = pupe_query($query);
                $keraysera_row = mysqli_fetch_assoc($keraysera_res);
              }
              ...
              if ($alkurow["hyllyalue"] != "!!M" and
                ...
                if (($lisa_row["tyyppi"] == "O" or $lisa_row["kpl"] < 0 or $lisa_row["laskutettu"] != "" or $lisa_row["tunnus"] == $row["tunnus"]) and
                  ...
                  if (mb_strtoupper($alkurow['varastomaa']) != mb_strtoupper($yhtiorow['maa'])) {
                    $paikat .= mb_strtoupper($alkurow['varastomaa'])." ";
                  }
          ...
          if ($yhtiorow["kerayspoikkeama_kasittely"] != '') {
            ...
            if ($yhtiorow['kerays_riveittain'] == 'K' and !empty($kerattavatrivitrow['poikkeama_kasittely'])) {
              ${'selpk_'.mb_strtoupper($kerattavatrivitrow['poikkeama_kasittely'])} = 'selected';
            }
      ...
      if ($toim != 'VASTAANOTA_REKLAMAATIO' and ($otsik_row['pakkaamo'] == 0 or $yhtiorow['pakkaamolokerot'] == '')) {
        ...
        if (mysqli_num_rows($kirre) > 0 and $yhtiorow['pakkaamolokerot'] == '') {

          $prirow = mysqli_fetch_assoc($kirre);
        ...
        $var_tp_row = mysqli_fetch_assoc($var_tp_res);
	...
	echo "</tr>";
        ...
        while ($kirrow = mysqli_fetch_assoc($kirre)) {
      ...
      if ($toim != 'VASTAANOTA_REKLAMAATIO' and ($yhtiorow['karayksesta_rahtikirjasyottoon'] == '' or $otsik_row["tulostustapa"] == "X" or $otsik_row["nouto"] != "") and ($otsik_row['pakkaamo'] == 0 or $yhtiorow['pakkaamolokerot'] == '')) {
        ...
        if ($yhtiorow["oletus_oslappkpl"] != 0 and ($yhtiorow['kerayserat'] == 'P' or $yhtiorow['kerayserat'] == 'A')) {
          ...
          if ($yhtiorow['kerayserat'] == 'A') {
            ...
            if (mysqli_num_rows($asiakas_chk_res) == 0) $kaikki_ok = false;
          }
      ...
      while ($pullopantti_check_row = mysqli_fetch_assoc($pullopantti_check_res)) {
