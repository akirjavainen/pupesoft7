...
if (mb_strpos($_SERVER['SCRIPT_NAME'], "vientitilauksen_lisatiedot.php") !== FALSE) {
...
if ($tapa == "tuonti" and $tee != "") {
  ...
  if ($tee == "K") {
    ...
    $query = "SELECT sum(kollit) kollit, sum(kilot) kilot
              ...
              and yhtio        = '$kukarow[yhtio]'";
    $result = pupe_query($query);
    $rahtirow = mysqli_fetch_assoc($result);
...
elseif ($tee != "") {
  if ($tee == 'L') {
    ...
    $aktiivinen_kuljetus_kansallisuus = mb_strtoupper($aktiivinen_kuljetus_kansallisuus);
    $sisamaan_kuljetus_kansallisuus = mb_strtoupper($sisamaan_kuljetus_kansallisuus);
    $maa_maara = mb_strtoupper($maa_maara);
  ...
  if ($tee == 'K') {
    ...
    while ($row = mysqli_fetch_assoc($maat_result)) {
      $sel = '';
      if ($row["koodi"] == $laskurow["maa_maara"]) {
        $sel = 'selected';
      }
    ...
    if ($laskurow["vienti"] == "K") {
      ...
      mysqli_data_seek($maat_result, 0);
    ...
    while ($rajatoimipaikka = mysqli_fetch_assoc($aiotut_rajatoimipaikat)) {
    ...
    while ($maaratoimipaikka = mysqli_fetch_assoc($maaratoimipaikat)) {
...
elseif ($tee == '') {
  ...
  if (mysqli_num_rows($tilre) > 0) {
    ...
    for ($i = 0; $i < mysqli_num_fields($tilre) - 18; $i++) {
      echo "<th align='left'>".t(mysqli_field_name($tilre, $i))."</th>";
    }
    ...
    while ($tilrow = mysqli_fetch_assoc($tilre)) {
      ...
      $hyvrow = mysqli_fetch_assoc($hyvre);
      ...
      if (($tilrow['jaksotettu'] != 0 and $jaksotettu == $tilrow['jaksotettu']) or ($tilrow['jaksotettu'] == 0 and $ketjutus == '' and $erpcm == $tilrow["erpcm"] and $ytunnus == $tilrow["ytunnus"] and $nimi == $tilrow["nimi"] and $nimitark == $tilrow["nimitark"] and $postino == $tilrow["postino"] and $postitp == $tilrow["postitp"] and $maksuehto == $tilrow["maksuehto"] and $lisattava_era == $tilrow["lisattava_era"] and $vahennettava_era == $tilrow["vahennettava_era"])) {
        ...
        if ($lask >= 1) {
          $tunnukset = mb_substr($tunnukset, 0, -1); // Vika pilkku pois
          echo "<td class='back'>
              <form method='post'>
              <input type='hidden' name='toim' value='$toim'>
      ...
      for ($i=0; $i < mysqli_num_fields($tilre)-18; $i++) {
        $fieldname = mysqli_field_name($tilre, $i);
