...
if ($tee == "ETSILASKU") {
  ...
  if ($otunnus > 0) {
    ...
    $larow = mysqli_fetch_array($laresult);
  ...
  if (mysqli_num_rows($result) > 0) {
    ...
    for ($i=0; $i < mysqli_num_fields($result)-2; $i++) {
      $jarj = $i+1;
      echo "<th align='left'><a href='$PHP_SELF?tee=$tee&ppl=$ppl&vvl=$vvl&kkl=$kkl&ppa=$ppa&vva=$vva&kka=$kka&ytunnus=$ytunnus&asiakasid=$asiakasid&toimittajaid=$toimittajaid&jarj=$jarj'>".t(mysqli_field_name($result, $i))."</a></th>";
    }
    echo "<th>".t("Tyyppi")."</th>";
