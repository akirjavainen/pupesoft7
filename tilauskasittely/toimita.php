...
if ($id > 0) {
  ...
  if ($row["valkoodi"] != '' and trim(mb_strtoupper($row["valkoodi"])) != trim(mb_strtoupper($yhtiorow["valkoodi"])) and $row["vienti_kurssi"] != 0) {
  ...
  $toita = mysqli_fetch_assoc($tores);
  ...
  if ($toita['nouto'] != '' and $row['kateinen'] != '' and $row["chn"] != '999' and ($row["mapvm"] == "" or $row["mapvm"] == '0000-00-00')) {
    ...
    while ($kassarow = mysqli_fetch_assoc($kassares)) {
