...
if ($tee == 'yhdista') {
  ...
  if (!empty($yhdistettavat_siirtolistat)) {
    ...
    $yhdistettavat_siirtolistat = sanitize_string($yhdistettavat_siirtolistat);
...
if ($tee == 'kommentista') {
  if (!empty($id)) {
    ...
    while ($alkurow = mysqli_fetch_assoc($alkuresult)) {
  ...
  if (is_uploaded_file($_FILES['userfile']['tmp_name']) === TRUE) {
    ...
    while (!feof($file)) {
      ...
      if ($bvarasto!='' and $avarasto!='' and $tuoteno!='') {
        ...
        if (mysqli_num_rows($alkuresult) == 1) {
          $alkurow = mysqli_fetch_assoc($alkuresult);
  ...
  foreach ($tunnus as $tun) {
    ...
    elseif ($tilausrivirow["ei_saldoa"] == "") {
      ...
      if (isset($rivivarasto[$tun]) and $rivivarasto[$tun] != 'x' and $rivivarasto[$tun] != '') {
        ...
        if (mb_substr($rivivarasto[$tun], 0, 1) == 'V') {
          $uusi_paikka = lisaa_tuotepaikka($tilausrivirow["tuoteno"], '', '', '', '', '', '', 0, 0, mb_substr($rivivarasto[$tun], 1));
      ...
      if (mysqli_num_rows($result) == 0) {
        ...
        $paikkarow = mysqli_fetch_assoc($result);
...
if ($tee == 'valmis') {
    ...
    if (mysqli_num_rows($result) != 1) {
      ...
      if ($asaldo != 0 and $tilausrivirow["ei_saldoa"] == "") {
        ...
        if (isset($eancheck[$tun]) and $eancheck[$tun] != '' and (int) $kirjoitin > 0) {
          ...
          $komrow = mysqli_fetch_assoc($komres);
  ...
  if ($_jt_toimita_t) {
    ...
    $_jtoikeus = (mysqli_num_rows($jtoikeudetres));
  }
  else {
    $_jtoikeus = TRUE;
  }
  ...
  if ($_jt_toimita_sallittu and $_normisiirto) {
    ...
    if ($yhtiorow['automaattinen_jt_toimitus_siirtolista'] == 'S' and !empty($tunnus)) {
      ...
      $varastoon_result = pupe_query($query);
      $varastoon_row = mysqli_fetch_assoc($varastoon_result);
  ...
  if ($virheita == 0) {
    ...
    while ($apusummarow = mysqli_fetch_assoc($result)) {
...
if (empty($id) and $echotaanko) {
  ...
  if ($yhtiorow['toimipaikkakasittely'] == "L" and $toimipaikkares = hae_yhtion_toimipaikat($kukarow['yhtio']) and mysqli_num_rows($toimipaikkares) > 0) {
    ...
    while ($toimipaikka = mysqli_fetch_assoc($toimipaikat_result)) {
  ...
  while ($maarow = mysqli_fetch_assoc($vresult)) {
  ...
  if (isset($maa) and !empty($maa)) {
    $varasto .= " AND varastopaikat.maa = '".sanitize_string($maa)."'";
  }
...
if (!empty($id) and $echotaanko) {
  ...
  for ($y=0; $y < mysqli_num_fields($result)-1; $y++) {
    echo "<th align='left'>".t(mysqli_field_name($result, $y))."</th>";
  }
  ...
  if ($toim == "") {
    ...
    $mrow = mysqli_fetch_assoc($tresult);
  ...
  $vares = pupe_query($query);
  $varow2 = mysqli_fetch_assoc($vares);
  ...
  while ($rivirow = mysqli_fetch_assoc($result)) {
    ...
    elseif ($rivirow["ei_saldoa"] == "") {
      ...
      $presult = pupe_query($query);
      $privirow = mysqli_fetch_assoc($presult);
    }
    else {
      $privirow = array();
    }
