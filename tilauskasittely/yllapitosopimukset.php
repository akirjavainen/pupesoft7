...
if (mb_strpos($_SERVER['SCRIPT_NAME'], "yllapitosopimukset.php") !== FALSE) {
...
if ($tee == "laskuta" and count($laskutapvm) > 0) {
  ...
  foreach ($laskutapvm as $pointteri => $tapahtumapvm) {
    ...
    $soprow = mysqli_fetch_assoc($result);
    ...
    if ($ok !== FALSE) {
      ...
      if ($soprow["yllapito_kuukausihinnoittelu"] == "Y") {
        ...
        while ($kh_row = mysqli_fetch_assoc($kh_result)) {
...
if (mysqli_num_rows($result) > 0) {
  ...
  while ($row = mysqli_fetch_assoc($result)) {
    ...
    for ($pvm = $pvmalku; $pvm <= $pvmloppu; $pvm = (int) date('Ymd', mktime(0, 0, 0, $pvmloop_kk, $pvmloop_pp+1, $pvmloop_vv))) {
      ...
      $pvmloop_pp = mb_substr($pvm, 6, 2);
      $pvmloop_kk = mb_substr($pvm, 4, 2);
      $pvmloop_vv = mb_substr($pvm, 0, 4);
