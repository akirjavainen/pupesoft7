...
if ($tee == 'TULOSTA') {
  ...
  if (mysqli_num_rows($laskuresult) == 0) {
  ...
  while ($laskurow = mysqli_fetch_array($laskuresult)) {
    ...
    if (mysqli_num_rows($cresult) == 0) {
      ...
      */
      ...
      $atkrivi = 1;
      $vtietue = 2 + mysqli_num_rows($riviresult);
      atk_eratietue();
      ...
      while ($rivirow = mysqli_fetch_array($riviresult)) {
        ...
        elseif (mb_strlen($rivirow["tullikohtelu"]) != 4) {
  ...
  */
  ...
  if ($virhe > 0) {
    ...
    unlink($tarkfaili);
    unlink("{$filenimi2_ps}");
    unlink($tarkfaili . ".pdf");
    unlink("{$filenimi1_ps}");
    unlink($paperifaili . ".pdf");
    unlink($paperifaili);
    unlink($atkfaili);
    exit;
  }
