      ...
      if (mysqli_num_rows($keskenresult)==0) {
        ...
        if (mysqli_num_rows($result) > 0) {
          ...
          if ($tulostukseen_kaikki == "KYLLA") {
            ...
            $ei_pakkaamoa_row = mysqli_fetch_assoc($ei_pakkaamoa_res);
    ...
    if ($tee2 != '') {
      ...
      if ($yhtiorow["pakkaamolokerot"] != "") {
        ...
        if ($tila == 'N') {
          ...
          if (mysqli_num_rows($tilre) > 0) {
            mysqli_data_seek($tilre, 0);
            $tilrow = mysqli_fetch_array($tilre);
          }
...
if ($tee2 == '') {
  ...
  */
  ...
  if ($tumaa != '') {
    ...
    $maarow = mysqli_fetch_array($maare);
  ...
  if (mysqli_num_rows($tilre)==0 or !$valid) {
    ...
    while ($tilrow = mysqli_fetch_array($tilre)) {
      ...
      if ($tilrow["tilauksia"] > 1) {
        ...
        if ($onkologmaster_varasto or $onkosmarten_varasto) {
          ...
          if (mysqli_num_rows($keskenres) != 0) {
            $keskenrow = mysqli_fetch_assoc($keskenres);
