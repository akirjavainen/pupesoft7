...
if (!isset($tee) or $tee == '') {
  ...
  while ($kukrow = mysqli_fetch_array($kukres)) {
    ...
    $piilorow = mysqli_fetch_array($result);
    ...
    if ((mysqli_num_rows($result) > 0) or ($piilorow[0] > 0)) {

      echo "<table width='100%'>";

      // ei näytetä suotta firman nimeä, jos käyttäjä kuuluu vaan yhteen firmaan
      if (mysqli_num_rows($kukres) == 1) $kukrow["nimi"] = "";

      echo "<tr><td colspan='".mysqli_num_fields($result)."' class='back'><font class='head'>".t("Hyväksyttävät laskusi")." $kukrow[nimi]</font><hr></td></tr>";

      if ($piilorow[0] > 0)
        echo "<tr><td colspan='".mysqli_num_fields($result)."' class='back'>". sprintf(t('Sinulla on %d pysäytettyä laskua'), $piilorow[0]) . "</tr>";
  ...
  if (mysqli_num_rows($result) > 0) {
    ...
    while ($prow = mysqli_fetch_array($result)) {
      ...
      if ($yhtiorow['tyomaarays_asennuskalenteri_muistutus'] == 'K' and trim($prow['kentta02']) != '' and is_numeric($prow['kentta02']) and $prow['Tapa'] == "Asentajan kuittaus") {
        ...
        if (mysqli_num_rows($asentajien_merkkaukset_res) > 0) {
  ...
  if (mysqli_num_rows($tyoresult) > 0) {
    ...
    while ($tyorow = mysqli_fetch_array($tyoresult)) {
  ...
  if (mysqli_num_rows($tyoresult2) > 0) {
    ...
    while ($tyorow = mysqli_fetch_array($tyoresult2)) {
      // Laitetetaan taustaväri jos sellainen on syötetty
      $varilisa = ($tyorow["tyostatusvari"] != "") ? " style='background-color: {$tyorow["tyostatusvari"]};'" : "";
  ...
  if (tarkista_oikeus("alv_laskelma_uusi.php") and tarkista_oikeus("muutosite.php")) {
    ...
    for ($i = $min_row['min']; $i <= date("Ym"); $i++) {
      ...
      $alvpvm = date("Y-m-d", mktime(0, 0, 0, (mb_substr($i, 4)+1), 0, mb_substr($i, 0, 4)));
      ...
      if (mysqli_num_rows($tositelinkki_result) == 0) {
