...
while ($pos_chk_row = mysqli_fetch_assoc($pos_chk_result)) {
...
while ($iltatuotepaikatrow = mysqli_fetch_assoc($iltatuotepaikatresult)) {
  $tee = "MUUTA";
...
if ($yhtiorow['kerayserat'] == 'K') {
  ...
  while ($tuotepaikkarow = mysqli_fetch_assoc($tuotepaikat)) {
...
if ($yhtiorow['kerayserat'] == 'K') {
  ...
  while ($poistettava_tuotepaikka = mysqli_fetch_assoc($result)) {
    // Merkataan paikka poistettavaksi
    $query = "UPDATE tuotepaikat
              SET poistettava = 'D'
              WHERE yhtio = '{$kukarow['yhtio']}'
...
if (mysqli_num_rows($poistettavat_tuotepaikat) > 0) {
  ...
  while ($avoinrivi = mysqli_fetch_assoc($avoinrivi_result)) {
...
while ($tuotepaikka = mysqli_fetch_assoc($poistettavat_tuotepaikat)) {
