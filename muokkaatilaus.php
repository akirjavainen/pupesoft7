...
if (mb_strpos($_SERVER['SCRIPT_NAME'], "muokkaatilaus.php") !== FALSE) {
...
if ($toim == 'LAVAKERAYS' and $tee == 'KERAA_KAIKKI_LAVAKERAYS' and $lavakerays_keraykseen != "") {
  ...
  if (!function_exists('lavakerays_valmis')) {
    function lavakerays_valmis($tilaus) {
      ...
      $laskurow = mysqli_fetch_assoc($lasres);
  ...
  // jos kaikki on ok...
  if (mysqli_num_rows($keskenresult) == 0) {
    ...
    $query = " SELECT group_concat(tunnus) tilausnumeroita
               ...
               and alatila = 'A'";
    $result = pupe_query($query);
    $tilrow = mysqli_fetch_assoc($result);
    ...
    $keskenrow = mysqli_fetch_array($keskenresult);
...
if (isset($tee) and $tee == 'TOIMITA_ENNAKKO' and in_array($yhtiorow["ennakkotilausten_toimitus"], array('M','K'))) {
  ...
  foreach ($toimita_ennakko as $tilausnro) {
    ...
    while ($laskurow = mysqli_fetch_assoc($jtrest)) {
      ...
      while ($rivirow = mysqli_fetch_assoc($apure)) {
        ...
        if ($laskurow["varasto"] > 0 and $varastorotunnus != $laskurow["varasto"]) {
          ...
          if (mysqli_num_rows($tuotepaikka_result) == 1) {
            $tuotepaikka_row = mysqli_fetch_assoc($tuotepaikka_result);
...
if (mb_strpos($_SERVER['SCRIPT_NAME'], "muokkaatilaus.php") !== FALSE) {
  ...
  if ($toim != "MYYNTITILITOIMITA" and $toim != "EXTRANET" and $toim != "VALMISTUSMYYNTI" and $toim != "VALMISTUSMYYNTISUPER") {
    if (isset($eresult) and  mysqli_num_rows($eresult) > 0) {
...
if (mb_strpos($_SERVER['SCRIPT_NAME'], "muokkaatilaus.php") !== FALSE) {
  ...
  if ($yhtiorow['toimipaikkakasittely'] == "L" and $toimipaikkares = hae_yhtion_toimipaikat($kukarow['yhtio']) and mysqli_num_rows($toimipaikkares) > 0) {
    ...
    while ($toimipaikkarow = mysqli_fetch_assoc($toimipaikkares)) {
  ...
  if ($toim == "LAVAKERAYS") {
    ...
    foreach ($toimitustavat as $ttapa) {
      if (mb_stripos($ttapa['selite'], "HORN") === FALSE) {
        continue;
      }
  ...
  if ($toim == 'YLLAPITO' and $etsi != "" and $haku != "") {
    $haku = mb_substr($haku, 0, -2); // Poistetaan vika sulku $hausta
  ...
  if (!empty($mt_order)) {
    $hakusarake = sanitize_string(key($mt_order));
    $hakusuunta = sanitize_string($mt_order[$hakusarake]);
...
if (mb_strpos($toim, "TEHDASPALAUTUKSET") !== FALSE) {
  $tepalisa = " AND tilaustyyppi = '9' ";
}
elseif (mb_strpos($toim, "TAKUU") !== FALSE) {
...
if (mysqli_num_rows($result) != 0) {
  ...
  if (mb_strpos($_SERVER['SCRIPT_NAME'], "muokkaatilaus.php") !== FALSE and $tee_excel) {
  ...
  $ii = 0;
  for ($i = 0; $i < mysqli_num_fields($result)-$miinus; $i++) {

    if (isset($mt_order[mysqli_field_name($result, $i)]) and $mt_order[mysqli_field_name($result, $i)] == 'ASC') {
      echo "<th align='left'><a href='muokkaatilaus.php?toim=$toim&asiakastiedot=$asiakastiedot&limit=$limit&etsi=$etsi&toimipaikka=mb_substr($toimipaikka, , 1)&mt_order[".mysqli_field_name($result, $i)."]=DESC{$tturllisa}'>".t(mysqli_field_name($result, $i))."<img src='{$palvelin2}pics/lullacons/arrow-small-up-green.png' /></a></th>";
    }
    elseif (isset($mt_order[mysqli_field_name($result, $i)]) and $mt_order[mysqli_field_name($result, $i)] == 'DESC') {
      echo "<th align='left'><a href='muokkaatilaus.php?toim=$toim&asiakastiedot=$asiakastiedot&limit=$limit&etsi=$etsi&toimipaikka=$toimipaikka&mt_order[".mysqli_field_name($result, $i)."]=ASC{$tturllisa}'>".t(mysqli_field_name($result, $i))."<img src='{$palvelin2}pics/lullacons/arrow-small-down-green.png' /></a></th>";
    }
    else {
      echo "<th align='left'><a href='muokkaatilaus.php?toim=$toim&asiakastiedot=$asiakastiedot&limit=$limit&etsi=$etsi&toimipaikka=$toimipaikka&mt_order[".mysqli_field_name($result, $i)."]=ASC{$tturllisa}'>".t(mysqli_field_name($result, $i))."</a></th>";
    }
    ...
    if (isset($worksheet)) {
      ...
      if (mysqli_field_name($result, $i) == "asiakas") {
        ...
        $worksheet->write($excelrivi, $ii, ucfirst(t(mysqli_field_name($result, $i))), $format_bold);
        $ii++;
      }
    }
  }
  ...
  while ($row = mysqli_fetch_assoc($result)) {
    ...
    if ($toim == "EXTRANET" and $row["tila"] == 'N' and $row["alatila"] == 'F') {
      ...
      if (mysqli_num_rows($res_x) != 0) $piilotarivi = "kylla";
    }
    ...
    if ($piilotarivi == "") {
      ...
      if (isset($row['label']) and $row['label'] != '') {
        ...
        if (mysqli_num_rows($label_result) == 1) {
          $label_row = mysqli_fetch_assoc($label_result);
      ...
      for ($i = 0; $i < mysqli_num_fields($result)-$miinus; $i++) {
        ...
        elseif ($yhtiorow['laiterekisteri_kaytossa'] != '' and $whiletoim == "YLLAPITO" and $fieldname == 'sarjanumero') {
          ...
          $sarjanumerotrivi = mysqli_fetch_assoc($res);
        ...
        if (isset($worksheet)) {
          ...
          elseif (mysqli_field_type($result, $i) == 'real') {
      ...
      elseif ($toim != "LAVAKERAYS")  {
        ...
        if (in_array($whiletoim, array('OSTO', 'OSTOSUPER')) and $row['tila'] == 'O') {
          ...
          if (mysqli_num_rows($ostotil_tiltyyp_res) > 0) {
            ...
            mysqli_data_seek($ostotil_tiltyyp_res, 0);
	...
	$pitaako_varmistaa = ""; // MUOKKAUS: kommentoitu varmistuskysymys pois
      ...
      if (!empty($yhtiorow["saldo_kasittely"]) and $toim == '' and $naytetaanko_saldot == 'kylla') {
        ...
        while ($rivi = mysqli_fetch_assoc($_result)) {
      ...
      if ($toim != "TARJOUS") {
        ...
        if (file_exists("../toimitusvahvistus")) echo " <a href='/toimitusvahvistus/index.php?nonavi=1&summary=1&order=" . $row['tunnus'] . "'><input type='submit' name='kuljetustilaus' value='Kuljetustilaus'></a>";
      }
      echo "</td>";
  ...
  if (mb_strpos($_SERVER['SCRIPT_NAME'], "muokkaatilaus.php") !== FALSE) {
    if ($toim == "LAVAKERAYS" and !empty($toimitustapa)) {
      ...
      while ($kirrow = mysqli_fetch_array($kirre)) {
        $sel = '';
