...
if ($tee == 'P') {
  ...
  $tiliointirow   = mysqli_fetch_array($result);
...
if ($tee == 'U') {
  ...
  if ($erittely == '') {
    ...
    $nimitieto = mb_strtoupper($nimitieto);
...
if (mb_strlen($pankkitili) != 0) {
  ...
  while ($tiliointirow = mysqli_fetch_array($result)) {
    echo "<tr>";
    for ($i = 1; $i<mysqli_num_fields($result); $i++) {
      ...
      elseif (mysqli_field_name($result, $i) == 'kustp2') {
        echo "<td>";
        if ($tiliointirow[$i] != 0) { // Meillä on kustannuspaikka
          $query = "SELECT nimi
                    FROM kustannuspaikka
  ...
  if ($pankkitili != 'x') {
    if (mb_substr($erittely, 0, 1)=='o') $erittely='checked'; else $erittely='';
