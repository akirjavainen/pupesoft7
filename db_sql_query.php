...
if (isset($tee)) {
  ...
  while ($row = mysqli_fetch_array($result)) {
    ...
    while ($fields = mysqli_fetch_assoc($fieldresult)) {
  ...
  function db_piirra_rivi($fields) {
    ...
    foreach ($fields as $row) {
      ...
      elseif ($table == "tuote" and $dbtaulu == "tuotteen_toimittajat" and $sarake == "liitostunnus") {
        ...
        $rivi .= "<option value='OLETUS' $chk1>".t("Päätoimittaja")."</option>";

        while ($toimittaja = mysqli_fetch_assoc($toimittaja_result)) {
  ...
  if ($rtee == "AJA" and !empty($AJASQL) and is_array($kentat)) {
    ...
    echo "<font class='message'>", query_dump($sqlhaku)."<br>".t("Haun tulos")." ".mysqli_num_rows($result)." ".t("riviä").".</font><br><br>";
      ...
      if (include 'inc/pupeExcel.inc') {
        ...
        function tee_excel($result) {
          ...
          for ($i=0; $i < mysqli_num_fields($result); $i++) {
            $worksheet->write($excelrivi, $i, ucfirst(t(mysqli_field_name($result, $i))), $format_bold);
          }
        ...
        while ($row = mysqli_fetch_row($result)) {
          ...
          if (!empty($selecti_tt)) {
            ...
            for ($j=0; $j<mysqli_num_fields($res); $j++) {
              $fieldname = mysqli_field_name($res, $j);

              if (mysqli_field_type($res, $j) == 'real') {
  ...
  if ($table!='') {
    ...
    while ($row = mysqli_fetch_array($fieldres)) {
      $row[0] = $table.".".$row[0];
      $fields[] = $row;
    }
