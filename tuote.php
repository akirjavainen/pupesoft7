...
if (isset($ajax)) {
  ...
  if ($ajax == "varastopaikat") {
    ...
    if ($_tp_kasittely) {
      ...
      $toimipaikka_varasto_row = mysqli_fetch_assoc($toimipaikka_varasto_res);
    ...
    if (mysqli_num_rows($sresult) > 0) {
      ...
      while ($saldorow = mysqli_fetch_assoc($sresult)) {
  ...
  if ($ajax == "tapahtumat") {
    ...
    elseif (mb_strpos($historia, 'TK') !== FALSE) {
      $query = "SELECT tilikausi_alku, tilikausi_loppu FROM tilikaudet WHERE yhtio = '$kukarow[yhtio]' and tunnus = '".mb_substr($historia, 2)."'";
      $tkresult = pupe_query($query);
      $tkrow = mysqli_fetch_assoc($tkresult);
    ...
    $ale_query_concat_lisa = mb_substr($ale_query_concat_lisa, 0, -1);
  ...
  if ($ajax == "raportointi") {
    ...
    if ($raportti == "MYYNTI") {
      ...
      $lrow = mysqli_fetch_assoc($result3);
  ...
  if ($ajax == 'tuotteen_tilaukset') {
    ...
    if (mysqli_num_rows($jtresult) != 0) {
      ...
      while ($jtrow = mysqli_fetch_assoc($jtresult)) {
...
elseif (mb_strpos($lopetus, "tilaus_myynti.php") === FALSE and $kukarow["kuka"] != "" and $laskurow["tila"] != "" and $laskurow["tila"] != "K" and $toim_kutsu != "") {
...
if ($tee == 'Z') {
  ...
  $salro = mysqli_fetch_assoc($salre);
  ...
  if ($tuoterow["ei_saldoa"] == '') {
    ...
    while ($ttrowx = mysqli_fetch_assoc($ttres)) {
      $ttrow[] = $ttrowx;
    }
  }
  else {
  ...
  if ($tuoterow["tuoteno"] != "") {
    ...
    if ($tuoterow["sarjanumeroseuranta"] == "S" or $tuoterow['sarjanumeroseuranta'] == 'G') {
      ...
      if (mysqli_num_rows($sarjares) > 0) {
        ...
        $tuoterow['kehahin'] = sprintf('%.6f', ($kehahin / mysqli_num_rows($sarjares)));
      }
      else {
        $tuoterow['kehahin'] = "";
      }
    ...
    // Hinnastoon
    if (mb_strtoupper($tuoterow['hinnastoon']) == 'E') {
    ...
    $cn2 = mb_substr($tuoterow["tullinimike1"], 0, 6);
    $cn3 = mb_substr($tuoterow["tullinimike1"], 0, 4);
    ...
    $tullirow1 = mysqli_fetch_assoc($tulliresult1);
    $tullirow2 = mysqli_fetch_assoc($tulliresult2);
    $tullirow3 = mysqli_fetch_assoc($tulliresult3);

    //perusalennus
    $query  = "SELECT alennus from perusalennus where ryhma='$tuoterow[aleryhma]' and yhtio='$kukarow[yhtio]'";
    $peralresult = pupe_query($query);
    $peralrow = mysqli_fetch_assoc($peralresult);
    ...
    if (table_exists("tuotteen_orginaalit")) {
      ...
      if (mysqli_num_rows($origresult) > 0) {
        ...
        while ($origrow = mysqli_fetch_assoc($origresult)) {
          ++$i;
          if ($i == 20) {
    ...
    if ($yhtiorow["vak_kasittely"] != "" and $tuoterow["vakkoodi"] != "" and $tuoterow["vakkoodi"] != "0") {
      ...
      $vak_row = mysqli_fetch_assoc($vak_res);
    ...
    foreach ($ttrow as $tt_rivi) {
      ...
      $kurssi_chk_row = mysqli_fetch_assoc($kurssi_chk_res);
    ...
    echo "<td colspan='3'>".wordwrap(mb_substr($tullirow3['dm'], 0, 20)." - ".mb_substr($tullirow2['dm'], 0, 20)." - ".mb_substr($tullirow1['dm'], 0, 20), 70, "<br>")."</td>";
    ...
    if (count($ttrow) > 0) {
      ...
      foreach ($ttrow as $tt_rivi) {
        ...
        if (mysqli_num_rows($chk_res) > 0 and !$otsikot) {
    ...
    if (table_exists("yhteensopivuus_tuote") and file_exists("yhteensopivuus_tuote.php") and tarkista_oikeus('yhteensopivuus_tuote.php')) {
      ...
      if (mysqli_num_rows($yhtresult) > 0) {
        while ($yhtrow = mysqli_fetch_assoc($yhtresult)) {
    ...
    if ($toim != "TYOMAARAYS_ASENTAJA") {
      ...
      while ($tkrow = mysqli_fetch_assoc($tkresult)) {
        $tkchk = "";
