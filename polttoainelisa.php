...
if (isset($lisaa) and isset($polttoainelisa) and isset($toimitustapa)) {
  ...
  $toimitustapa = sanitize_string($toimitustapa);
  ...
  if ($polttoainelisa == 0) {
    ...
    while ($rahtimaksut_row = mysqli_fetch_assoc($rahtimaksut_res)) {
...
if (mysqli_num_rows($toimitustapa_res) == 0) {
  ...
  while ($toimitustapa_row = mysqli_fetch_array($toimitustapa_res)) {
