...
function hae_extranet_tarjoukset($asiakasid, $toim) {
  ...
  while ($haettu_tarjous = mysqli_fetch_assoc($result)) {
...
function hae_tarjous($valittu_tarjous_tunnus) {
  ...
  $tarjous = mysqli_fetch_assoc($result);
...
function hae_tarjouksen_tilausrivit($valittu_tarjous_tunnus) {
  ...
  if (mb_strtoupper($laskurow["maa"]) == "SE") {
    $kielilisa = "SE";
  }
  ...
  while ($tilausrivi = mysqli_fetch_assoc($result)) {
    ...
    $selite = mysqli_fetch_assoc($result2);

    $tilausrivi['parametri_ennakkoale_a'] = $selite['ennakko_pros_a'];

    if (mb_strtoupper($laskurow["valkoodi"]) != mb_strtoupper($yhtiorow['valkoodi'])) {
